  ...
  updateLogs: [
    {
      ...
      date: "2026-08-05",
      content: "仙乡领地升级为完整战略经营玩法：新增三支独立军阵、五类兵种与阵型克制、真实行军和返程倒计时、途中召回、伤兵营与治疗；据点新增双建筑槽，军府新增六项科技与四章山河目标，敌军会按现实时间动态袭扰。角色战力、宗门工程和空闲灵兽会提供有上限的统帅、产出、行军或治疗加成。旧领地存档会自动迁移，原据点、资源、兵力、士气、等级和巡境数据均保留。",
    },
    {
      title: "V3.3.43 秘境连续挑战",
      date: "2026-08-05",
      content: "处理游戏内反馈#98：灵兽森林与幽冥洞窟新增连续挑战，可在每轮胜利后自动原子拾取掉落并继续进入，最多连续100轮；遇到秘境抉择时暂停并在选择后自动继续，资源不足、战败或纳戒无法完整接收掉落时自动停止，未拾取掉落仍保留在结算页。两个秘境原有消耗、掉落、怪物数值与旧存档均不变。",
    },
    {
    ...
    {
      title: "V3.3.36 灵石坊批量折换",
      date: "2026-07-31",
      content: "处理游戏内反馈#80：洞府灵石坊的材料折灵新增兑换份数输入与一键全部功能，按钮会实时预览本次消耗材料和获得灵石；后端逻辑按实际库存自动夹紧份数并一次性扣除、结算，避免连续点击。另修正V3.3.35首次发布时因候选目录资源不完整造成的前端排版回归，已改用完整生产源码重新构建。旧存档无需迁移。",
    },
    {
    ...
    {
      title: "V3.3.30 轮回尘供给补全",
      date: "2026-07-28",
      content: "采纳玩家反馈补全轮回尘前中期来源：饕餮每日挑战胜利由45%概率改为必得1个；5转解锁的穷奇新增胜利必得1个，避免高阶凶兽产出断层；物品来源、战斗奖励提示、轮回殿缺料提示与百晓知识库同步标明饕餮、云梦泽·泽中水魄、穷奇及混沌的解锁和实际概率。旧存档与既有材料全部保留。",
    },
    {
      title: "V3.3.23 护送契约完成态",
      date: "2026-07-27",
      content: "修复瀚海护送契约完成后按钮仍保持亮起、看似可重复点击的问题；探路、护送与镇守完成后会立即显示已完成并禁用，次日自动恢复。重复奖励原已由结算逻辑拦截，存档结构与奖励数值均不变。",
    },
    {
      title: "V3.3.22 通商检索与果园扩容",
      date: "2026-07-27",
      content: "采纳两条体验反馈：瀚海通商上架物品新增名称搜索与灵植灵果、矿材素材、料理丹药、其他分类筛选，并按类别、名称和品质稳定排列；果树上限由8棵扩至12棵，既可覆盖当前8种果树各栽一棵，也保留4个自由搭配树位，旧存档无需迁移。",
    },
    {
      title: "V3.3.21 红尘连续历练",
      date: "2026-07-27",
      content: "采纳反馈优化红尘历练胜利后的操作：完成一轮并拾取掉落后，可直接在结算页继续挑战当前区域，无需返回列表重复进入；每一轮仍独立校验并扣除灵力与体力，资源不足时会正常提示，避免连续操作绕过消耗。",
    },
    {
      title: "V3.3.20 大周天与高阶修炼拓展",
      date: "2026-07-27",
      content: "采纳反馈新增金丹期后可用的「运转大周天」，单次消耗体力与灵力，按当前境界上限、灵田和功法成长获得修为与灵气；炼化灵气改为随境界扩大单次周天，避免高境界仍需反复点击固定小额；修行页补充修为、灵气主要来源总览，明确每日课业、红尘历练、秘境、灵植、聚灵阵、灵石坊、丹药与灵膳等渠道。",
    },
    {
      title: "V3.3.19 仙盟永久道具使用修复",
      date: "2026-07-27",
      content: "修复仙盟徽章、生命护符、幸运铜钱与守护符虽已有服务器权威结算、但背包漏掉使用按钮且矿洞旧入口仍调用已禁用本地结算的问题；现可在背包直接使用，也可在矿洞批量确认后由服务器锁定云档统一结算，确保物品扣除与永久属性生效原子一致。",
    },
    {
      title: "V3.3.18 连续满勤、灵鱼谱系与统一服丹",
      date: "2026-07-27",
      content: "连续满勤改为每7天循环发奖，显示总连续天数、当前轮次与轮内天数，并兼容旧领取记录且断签后重新起算；灵泉鱼池增加配对配方即时提示，图鉴升级为可按基础鱼和代数查询、查看亲本的公开谱系百科；冰魂莲与紫灵芝补入统一灵植灵气收益映射；炼丹炉服用入口统一经过物品使用服务，受保护丹药由服务器结算并防止异步重复点击。",
    },
    {
      title: "V3.3.17 仙缘加成与一键派遣",
      date: "2026-07-26",
      content: "修复狐仙仙缘能力已实际生效但商店界面漏算、漏显示狐眼折扣的问题，商店标价、购买结算与折扣提示现统一计入狐眼5%折扣；灵探说明明确为每次采矿15%概率额外获得1份当前矿石。灵牧苑新增一键派遣，可一次派出全部好感达到500且已结束休整的动物，改善手机端动物较多时逐只下拉操作的不便。",
    },
    {
    ...
    },
    {
      ...
      date: "2026-07-25",
      content:
        "补齐仙界高价值进度的服务端存档校验：仙阶必须满足飞升状态、境界范围与突破资源条件；功德、仙玉、法则碎片、仙器精魄、三元仙身、仙器、洞天、命盘、试炼与裂隙进度的异常跳增会自动回滚到服务器可信存档。历史已有高数值不直接削减，只阻止后续不合理增长，正常仙界玩法与旧存档保持兼容。",
    },
    {
      title: "V3.3.12 休息写档队列修复",
      date: "2026-07-24",
      content:
        "修复自然时钟节流时仍提前把每个画面帧标记为待保存，导致写档期间不断追加完整云存档、休息前队列无法清空并提示‘当前进度尚未写入服务器’的问题。自然走时现在仅在30秒保底计时到期时加入一次保存；玩家操作与休息仍立即写入，休息能够稳定进入第二天。",
    },
    {
      title: "V3.3.11 存档流量与日结体验修复",
      date: "2026-07-24",
      content:
        "修复生产环境无法稳定识别自然时钟变化、仍可能每秒多次上传完整云存档的问题：由时钟源明确标记自然走时并合并为低频保底存档，玩家操作、奖励和休息仍立即确认；待确认福利恢复成功后同步修正领取状态，避免按钮仍显示可领取。日结新增‘新的一天正在结算’遮罩与提示，避免大量系统更新时误以为卡死；育种图鉴新增按代数、品种名称和最佳属性总分排序，并直接展示已发现品种分数。",
    },
    {
