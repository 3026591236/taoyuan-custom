...
import {
  AFFINITY_THRESHOLDS,
  MAX_AFFINITY,
  AFFINITY_DECAY_BONDED,
  AFFINITY_DECAY_COURTING,
  MAX_OFFERS_PER_WEEK,
} from "@/types/hiddenNpc";
...
export const useHiddenNpcStore = defineStore("hiddenNpc", () => {
  const hiddenNpcStates = ref<HiddenNpcState[]>(
    HIDDEN_NPCS.map((n) => defaultState(n.id)),
  );

  // ==================== 基础查询 ====================

  const getHiddenNpcState = (npcId: string): HiddenNpcState | undefined =>
    hiddenNpcStates.value.find((s) => s.npcId === npcId);
  ...
  const getRumorNpcs = computed(() =>
    HIDDEN_NPCS.filter((n) => {
      const s = getHiddenNpcState(n.id);
      return (
        s && (s.discoveryPhase === "rumor" || s.discoveryPhase === "glimpse")
      );
    }),
  );
  ...
  const isManifestationDay = (npcId: string): boolean => {
    ...
    return (
      gameStore.season === def.manifestationDay.season &&
      gameStore.day === def.manifestationDay.day
    );
  };
  ...
  const evaluateCondition = (cond: DiscoveryCondition): boolean => {
    ...
    switch (cond.type) {
      ...
      case "skill":
        return (
          skillStore.getSkill(cond.skillType as any).level >= cond.minLevel
        );
      case "npcFriendship": {
  ...
  const addAffinity = (npcId: string, amount: number) => {
    const state = getHiddenNpcState(npcId);
    if (!state) return;
    state.affinity = Math.max(
      0,
      Math.min(MAX_AFFINITY, state.affinity + amount),
    );
  };
  ...
  /** 执行独特互动 */
  const performSpecialInteraction = (
    npcId: string,
  ): { success: boolean; message: string; affinityChange: number } => {
    const state = getHiddenNpcState(npcId);
    const def = getHiddenNpcById(npcId);
  ...
  const startCourting = (
    ...
    // 检查是否已有结缘对象
    const existingBond = hiddenNpcStates.value.find(
      (s) => s.bonded || s.courting,
    );
  ...
  const craftSpiritItem = (
    ...
    const costs =
      type === "courtship" ? def.courtshipCraftCost : def.bondCraftCost;
    const outputItemId =
      type === "courtship" ? def.courtshipItemId : def.bondItemId;
  ...
  const checkHeartEvent = (npcId: string): HeartEventDef | null => {
    ...
    for (const event of events) {
      if (
        state.affinity >= event.requiredFriendship &&
  ...
  const checkAbilityUnlocks = (): {
    ...
    for (const npc of HIDDEN_NPCS) {
      ...
      for (const ability of npc.abilities) {
        if (
          state.affinity >= ability.affinityRequired &&
    ...
    for (const state of hiddenNpcStates.value) {
      ...
      if (!state.interactedToday && !state.offeredToday) {
        if (state.bonded) {
          ...
          state.affinity = Math.max(
            0,
            state.affinity - AFFINITY_DECAY_COURTING,
          );
        }
      }
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    const newStates = HIDDEN_NPCS.filter((n) => !savedIds.has(n.id)).map((n) =>
      defaultState(n.id),
    );
