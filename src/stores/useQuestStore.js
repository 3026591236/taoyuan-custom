...
export const useQuestStore = defineStore('quest', () => {
    ...
    /** 告示栏上的可接取任务 */
    const boardQuests = ref([]);
    /** 已接取的进行中任务 */
    const activeQuests = ref([]);
    /** 累计完成任务数 */
    const completedQuestCount = ref(0);
    /** 当前可接取的特殊订单 */
    const specialOrder = ref(null);
    /** 最大同时接取任务数 */
    const MAX_ACTIVE_QUESTS = 3;
    /** 每日生成新任务到告示栏 */
    const generateDailyQuests = (season, day) => {
    ...
    const generateSpecialOrder = (season, tier) => {
    ...
    const onItemObtained = (itemId, quantity = 1) => {
    ...
    /** 检查是否有任务关注某物品 */
    const hasActiveQuestFor = (itemId) => {
    ...
    // ============================================================
    /** 当前主线任务状态 */
    const mainQuest = ref(null);
    /** 已完成的主线任务ID列表 */
    const completedMainQuests = ref([]);
    /** 好感等级层级顺序 */
    ...
    const evaluateObjective = (obj) => {
        ...
        switch (obj.type) {
            ...
            case 'skillLevel':
                if (obj.skillType) {
                    return skillStore.getSkill(obj.skillType).level >= (obj.target ?? 0);
                }
                // 无指定技能类型 = 任意技能达标
    ...
    const acceptMainQuest = () => {
        ...
        const def = getStoryQuestById(mainQuest.value.questId);
        if (!def)
            return { success: false, message: '主线任务数据异常。' };
    ...
    const deserialize = (data) => {
        ...
        specialOrder.value = data.specialOrder ?? null;
        mainQuest.value = data.mainQuest ?? null;
        completedMainQuests.value = data.completedMainQuests ?? [];
        // 加载后初始化主线任务（兼容旧存档）
        initMainQuest();
    };
    return {
