...
export const useFarmStore = defineStore('farm', () => {
    const farmSize = ref(4);
    const plots = ref(createPlots(4));
    const sprinklers = ref([]);
    const fruitTrees = ref([]);
    const greenhousePlots = ref([]);
    ...
    /** 种下育种种子 */
    const plantGeneticSeed = (plotId, genetics) => {
        const plot = plots.value[plotId];
        if (!plot || plot.state !== 'tilled')
            return false;
    ...
    const harvestPlot = (plotId) => {
        const plot = plots.value[plotId];
        if (!plot || plot.state !== 'harvestable')
            return { cropId: null, genetics: null };
        ...
        if (plot.giantCropGroup !== null) {
            ...
            for (const p of plots.value) {
                if (p.giantCropGroup === groupId)
                    p.giantCropGroup = null;
            }
        }
    ...
    // === 洒水器 ===
    /** 获取洒水器覆盖的地块ID列表 */
    const getSprinklerCoverage = (plotId, range) => {
    ...
    /** 放置洒水器 */
    const placeSprinkler = (plotId, sprinklerType) => {
        if (sprinklers.value.some(s => s.plotId === plotId))
            return false;
        const plot = plots.value[plotId];
        if (!plot)
            return false;
    ...
    const getAllWateredBySprinklers = () => {
    ...
    // === 肥料 ===
    /** 给地块施肥 */
    const applyFertilizer = (plotId, fertilizerType) => {
        const plot = plots.value[plotId];
        if (!plot)
            return false;
    ...
    const applyFertileSoil = (farmingLevel) => {
    ...
    const lightningStrike = () => {
        if (Math.random() > 0.25)
            return { hit: false, absorbed: false };
        // 避雷针吸收
    ...
    const checkGiantCrops = () => {
        ...
        for (let r = 0; r <= maxR; r++) {
            for (let c = 0; c <= maxC; c++) {
                ...
                if (topLeft.state !== 'harvestable' || !topLeft.cropId || topLeft.giantCropGroup !== null)
                    continue;
                const crop = getCropById(topLeft.cropId);
                if (!crop || !crop.giantCropEligible)
                    continue;
                let allMatch = true;
    ...
    const harvestGiantCrop = (plotId) => {
        const plot = plots.value[plotId];
        if (!plot || plot.state !== 'harvestable' || plot.giantCropGroup === null)
            return null;
        const groupId = plot.giantCropGroup;
        const cropId = plot.cropId;
        if (!cropId)
    ...
    /** 果树每日更新 */
    const dailyFruitTreeUpdate = (currentSeason) => {
        const results = [];
        // 仙缘能力
    ...
    /** 果树换季更新（仅新年时增加年龄） */
    const fruitTreeSeasonUpdate = (isNewYear) => {
        for (const tree of fruitTrees.value) {
            if (tree.mature && isNewYear)
                tree.yearAge++;
            tree.todayFruit = false;
        }
    };
    // === 野树 ===
    /** 种植野树 */
    const plantWildTree = (treeType) => {
        if (wildTrees.value.length >= MAX_WILD_TREES)
            return false;
    ...
    /** 安装采脂器 */
    const attachTapper = (treeId) => {
        const tree = wildTrees.value.find(t => t.id === treeId);
        if (!tree || !tree.mature || tree.hasTapper)
            return false;
        tree.hasTapper = true;
    ...
    /** 收取采脂产物 */
    const collectTapProduct = (treeId) => {
        const tree = wildTrees.value.find(t => t.id === treeId);
        if (!tree || !tree.tapReady)
            return null;
    ...
    const initGreenhouse = () => {
        if (greenhousePlots.value.length > 0)
            return;
    ...
    /** 温室播种 */
    const greenhousePlantCrop = (plotId, cropId) => {
    ...
    /** 温室播种育种种子 */
    const greenhousePlantGeneticSeed = (plotId, genetics) => {
        const plot = greenhousePlots.value[plotId];
        if (!plot || plot.state !== 'tilled')
            return false;
        const crop = getCropById(genetics.cropId);
    ...
    /** 温室收获 */
    const greenhouseHarvestPlot = (plotId) => {
        const plot = greenhousePlots.value[plotId];
        if (!plot || plot.state !== 'harvestable')
            return { cropId: null, genetics: null };
        const cropId = plot.cropId;
    ...
    /** 温室升级：扩展地块数量 */
    const upgradeGreenhouse = (newPlotCount) => {
        const current = greenhousePlots.value.length;
        if (newPlotCount <= current)
            return false;
    ...
    const greenhouseBatchHarvest = () => {
        ...
        for (let i = 0; i < greenhousePlots.value.length; i++) {
            ...
            if (result.cropId)
                results.push({ cropId: result.cropId, genetics: result.genetics });
        }
        return results;
    };
    const serialize = () => {
    ...
    const deserialize = (data) => {
        ...
        plots.value = data.plots.map(p => ({
            ...
            weedy: p.weedy ?? false,
            weedyDays: p.weedyDays ?? 0
        }));
        sprinklers.value = data.sprinklers ?? [];
        fruitTrees.value = (data.fruitTrees ?? []).map((t) => ({
            ...t,
            yearAge: t.yearAge ?? t.seasonAge ?? 0
        }));
        nextFruitTreeId.value =
            data.nextFruitTreeId ?? (fruitTrees.value.length > 0 ? Math.max(...fruitTrees.value.map(t => t.id)) + 1 : 0);
        wildTrees.value = (data.wildTrees ?? []).map((t) => ({
            ...t,
            chopCount: t.chopCount ?? 0
        }));
        nextWildTreeId.value =
            data.nextWildTreeId ?? (wildTrees.value.length > 0 ? Math.max(...wildTrees.value.map(t => t.id)) + 1 : 0);
        greenhousePlots.value = (data.greenhousePlots ?? []).map((p) => ({
            ...p,
            fertilizer: migrateFertilizer(p.fertilizer),
            harvestCount: p.harvestCount ?? 0,
            giantCropGroup: p.giantCropGroup ?? null,
        ...
        greenhouseLevel.value = data.greenhouseLevel ?? 0;
        lightningRods.value = data.lightningRods ?? 0;
        scarecrows.value = data.scarecrows ?? 0;
        giantCropCounter.value = data.giantCropCounter ?? 0;
    };
    return {
        farmSize,
        plots,
