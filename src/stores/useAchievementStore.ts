...
export const useAchievementStore = defineStore("achievement", () => {
  ...
  const discoverItem = (itemId: string) => {
    if (!discoveredItems.value.includes(itemId)) {
      ...
      discoveryTimes.value[itemId] =
        `第${gameStore.year}年 ${SEASON_NAMES[gameStore.season] ?? gameStore.season} 第${gameStore.day}天`;
    }
  };
  ...
  const isConditionMet = (c: AchievementCondition): boolean => {
    switch (c.type) {
      ...
      case "npcFriendship": {
        ...
        return npcStore.npcStates.every(
          (n) =>
      ...
      case "npcBestFriend": {
        const bestFriendCount = npcStore.npcStates.filter(
      ...
      case "fullShipment": {
        ...
        const shippableCount = ITEMS.filter((i) =>
          shippableCategories.includes(i.category),
        ).length;
      ...
      case "hybridsShipped": {
        ...
        const shippedHybridCount = shopStore.shippedItems.filter((id) =>
          hybridItemIds.has(id),
        ).length;
  ...
  const submitToBundle = (
    ...
    // 检查是否完成
    const allMet = bundle.requiredItems.every(
      (r) => (sub[r.itemId] ?? 0) >= r.quantity,
    );
  ...
  const shippableItemCount = ITEMS.filter((i) =>
    SHIPPABLE_CATEGORIES.includes(i.category),
  ).length;
  ...
  const perfectionPercent = computed(() => {
    ...
    const friendRate =
      npcStore.npcStates.length > 0
        ? friendlyCount / npcStore.npcStates.length
        : 0;
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    if (
      (stats.value as Record<string, unknown>).totalMonstersKilled === undefined
    ) {
      stats.value.totalMonstersKilled = 0;
    }
    if (
      (stats.value as Record<string, unknown>).totalBreedingsDone === undefined
    ) {
      stats.value.totalBreedingsDone = 0;
    }
    if (
      (stats.value as Record<string, unknown>).totalHybridsDiscovered ===
      undefined
    ) {
      stats.value.totalHybridsDiscovered = 0;
    }
    if (
      (stats.value as Record<string, unknown>).highestHybridTier === undefined
    ) {
