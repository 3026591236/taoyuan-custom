...
type Adventure = {
  ...
  hidden?: { requireFlag: string; title: string; desc: string; reward: Reward };
};
type GearAffix = {
  slot: string;
  name: string;
  ...
  desc: string;
  rarity?: "普通" | "稀有" | "绝品";
  setId?: "demon" | "spirit" | "sea";
  locked?: boolean;
  secondary?: boolean;
};
type SectProjectId = "spirit_array" | "craft_hall" | "sword_platform";
type FarmGoalId =
  "field_scale" | "greenhouse_supply" | "automation" | "spirit_crop_chain";
...
const BOSS_TIERS: BossTier[] = [
  {
    ...
    title: "镇魔先锋",
    desc: "个人贡献达到20点。",
    reward: { money: 1200, spiritStone: 6, aura: 180 },
  },
  {
    score: 60,
    title: "伏魔主力",
  ...
  },
  {
    ...
    reward: {
      ...
      aura: 900,
      items: [
        { itemId: "soul_crystal", name: "魂晶", quantity: 2 },
...
const SEASON_TASKS: SeasonTask[] = [
  {
    id: "daily_12",
    title: "月令·勤修",
    desc: "本月累计领取12个每日活跃奖励。",
    target: 12,
    reward: { money: 2200, spiritStone: 10, aura: 360 },
  },
  {
    id: "combat_40",
    title: "月令·伏魔",
...
const ADVENTURES: Adventure[] = [
  {
    ...
    stage: 1,
    title: "灵狐报恩·旧林白影",
    desc: "山路白狐衔来一枚玉简，似乎在引你去旧林深处。",
    choices: [
      {
        id: "follow",
        label: "随灵狐入林",
        result: "你帮灵狐解开旧阵，获赠魂晶；旧林深处的狐火记住了你的气息。",
        flag: "fox_trust",
        reward: {
          aura: 260,
          items: [{ itemId: "soul_crystal", name: "魂晶", quantity: 1 }],
        },
      },
      {
        id: "feed",
        label: "赠灵果结缘",
        result: "灵狐记住了你的善意，留下灵石与月华玉。",
        flag: "fox_kind",
        reward: {
          spiritStone: 8,
          items: [{ itemId: "moonlight_jade", name: "月华玉", quantity: 1 }],
  ...
  },
  {
    ...
    stage: 2,
    title: "灵狐报恩·残阵回响",
    desc: "旧林阵眼残光未灭，若曾与白狐结缘，或许能看见隐藏阵路。",
    choices: [
      {
        id: "array",
        label: "修补狐火残阵",
        result: "狐火引来灵气，宗门聚灵阵图纸更加完整。",
        flag: "array_mended",
        reward: {
          aura: 480,
          items: [{ itemId: "spirit_ink", name: "灵墨", quantity: 2 }],
    ...
    hidden: {
      requireFlag: "fox_trust",
      title: "隐藏结局·狐火护阵",
      desc: "白狐主动现身守阵，额外赠予月华玉。",
      reward: {
        aura: 300,
        items: [{ itemId: "moonlight_jade", name: "月华玉", quantity: 1 }],
      },
  ...
  },
  {
    ...
    stage: 1,
    title: "瀚海商队·沙兽围困",
    desc: "远行商队遭沙兽围困，愿以奇货相谢。",
    choices: [
      {
        id: "guard",
        label: "护送商队",
        result: "商队安全抵达，赠你灵石与凤羽。",
        flag: "sea_guard",
        reward: {
          money: 2800,
          spiritStone: 10,
          items: [{ itemId: "phoenix_plume", name: "凤羽", quantity: 1 }],
        },
      },
      {
        id: "trade",
        label: "议价换货",
        result: "你换来稀有灵墨，可用于符阵与宗门建设。",
        flag: "sea_trade",
        reward: {
          money: 1600,
          items: [{ itemId: "spirit_ink", name: "灵墨", quantity: 3 }],
        },
      },
    ],
  },
  {
    ...
    stage: 1,
    title: "古修遗府·半开石门",
    desc: "洞府石门半开，里面传来微弱剑鸣。",
    choices: [
      {
        id: "sword",
        label: "取剑痕拓片",
        result: "你悟得一缕剑意，获得星陨铁。",
        flag: "cave_sword",
        reward: {
          cultivation: 360,
          items: [{ itemId: "star_iron", name: "星陨铁", quantity: 1 }],
        },
      },
      {
        id: "array",
        label: "修补残阵",
        result: "残阵回馈灵气，护符材料入手。",
        flag: "cave_array",
        reward: {
          aura: 420,
    ...
    hidden: {
      requireFlag: "sea_guard",
      title: "隐藏结局·瀚海古图",
      desc: "商队给出的古图指向遗府暗室，你额外找到凤羽。",
      reward: {
        spiritStone: 8,
        items: [{ itemId: "phoenix_plume", name: "凤羽", quantity: 1 }],
      },
...
const FARM_GOALS: {
  ...
  {
    id: "greenhouse_supply",
    title: "温室供给",
    desc: "修复温室并保持温室地块运转，提供跨季供应。",
    target: 6,
    reward: {
      money: 4200,
      spiritStone: 12,
  ...
  {
    id: "spirit_crop_chain",
    title: "灵植供应链",
    desc: "通过灵植库存支撑宗门订单、炼丹与镇魔材料循环。",
    target: 20,
    reward: {
      money: 6800,
      spiritStone: 22,
...
const SET_BONUS: Record<string, { name: string; desc: string }> = {
  demon: {
    name: "镇魔套装",
    desc: "2件：镇魔/秘境追求增强；4件：周期贡献更稳定。",
  },
  spirit: {
    name: "聚灵套装",
    desc: "2件：灵气/修行追求增强；4件：月度成长更顺滑。",
  },
  sea: {
    name: "瀚海套装",
    desc: "2件：商队/探索追求增强；4件：隐藏奇遇更容易成型。",
  },
};
const SECONDARY_AFFIX_POOL: Record<
  string,
  {
    name: string;
    desc: string;
...
> = {
  ...
  amulet: [
    ...
    {
      name: "引雷护印",
      desc: "副词条：雷精、渡劫与护符收益提高",
      rarity: "稀有",
      setId: "demon",
    },
    {
      name: "太虚回照",
      desc: "副词条：太虚功法与月度成长增强",
      rarity: "绝品",
      setId: "spirit",
    },
  ],
...
export const useLongTermStore = defineStore("longTerm", () => {
  ...
  const sectProjects = ref<Record<SectProjectId, number>>({
    spirit_array: 1,
    craft_hall: 1,
    sword_platform: 1,
  });
  ...
  const worldBossTiers = computed(() =>
    BOSS_TIERS.map((t) => ({
      ...t,
      progress: worldBossPersonal.value,
      done: worldBossPersonal.value >= t.score,
      claimed: worldBossClaimed.value.includes(t.score),
    })),
  );
  const seasonTasks = computed(() => {
    resetMonthIfNeeded();
    return SEASON_TASKS.map((t) => ({
      ...t,
      progress: monthlyProgress.value[t.id] || 0,
      done: (monthlyProgress.value[t.id] || 0) >= t.target,
      claimed: monthlyClaimed.value.includes(t.id),
    }));
  });
  ...
  const affixPowerBonus = computed(() =>
    Object.values(gearAffixes.value)
      ...
      .reduce(
        (s, a) =>
          s + a.level + (a.rarity === "绝品" ? 3 : a.rarity === "稀有" ? 2 : 1),
        0,
      ),
  );
  const gearSetBonuses = computed(() => {
    ...
    Object.values(gearAffixes.value)
      .flat()
      .forEach((a) => {
        if (a.setId) counts[a.setId] = (counts[a.setId] || 0) + 1;
      });
    return Object.entries(counts).map(([id, count]) => ({
      id,
      count,
      name: SET_BONUS[id]?.name || id,
      desc: SET_BONUS[id]?.desc || "",
      active: count >= 2,
    }));
  });
  const rareAffixCodex = computed(() =>
    Object.values(AFFIX_POOL)
      .flat()
      .filter((a) => a.rarity !== "普通")
      .map((a) => ({ ...a, discovered: affixCodex.value.includes(a.name) })),
  );
  const adventureStory = computed(() => ({
    flags: adventureFlags.value,
    endings: adventureEndings.value,
    completed: adventureDone.value.length,
  ...
  function addMonthlyProgress(id: string, n = 1) {
    resetMonthIfNeeded();
    monthlyProgress.value[id] = (monthlyProgress.value[id] || 0) + n;
  }
  function recordDailyClaim() {
    addMonthlyProgress("daily_12", 1);
  }
  function recordCombatContribution(n = 1) {
    worldBossPersonal.value += n;
    addMonthlyProgress("combat_40", n);
  }
  function claimWorldBossTier(score: number) {
    ...
    if (worldBossPersonal.value < score)
      return { success: false, message: "镇魔贡献不足" };
    if (worldBossClaimed.value.includes(score))
      return { success: false, message: "已领取" };
    addReward(tier.reward);
    worldBossClaimed.value.push(score);
    return {
      success: true,
      message: `领取${tier.title}：${rewardText(tier.reward)}`,
    };
  }
  function contributeSectBuild(kind: "money" | "spirit" | "material") {
    let gain = 0;
    if (kind === "money") {
      if (player().money < 1200) return { success: false, message: "铜钱不足" };
      player().money -= 1200;
      gain = 30;
    }
    if (kind === "spirit") {
      if (inv().getItemCount("spirit_stone") < 8)
        return { success: false, message: "灵石不足" };
      inv().removeItem("spirit_stone", 8);
      gain = 50;
    }
    if (kind === "material") {
      if (inv().getItemCount("spirit_ink") < 1)
        return { success: false, message: "需要灵墨×1" };
      inv().removeItem("spirit_ink", 1);
      gain = 70;
    }
    sectBuildExp.value += gain;
    sectBuildContributed.value += gain;
    addMonthlyProgress("sect_160", gain);
    while (sectBuildExp.value >= sectBuildNeed.value) {
      sectBuildExp.value -= sectBuildNeed.value;
      sectBuildLevel.value += 1;
    }
    return {
      success: true,
      message: `宗门建设+${gain}，当前Lv.${sectBuildLevel.value}`,
    };
  }

  function contributeSectProject(
    id: SectProjectId,
    ...
    if (kind === "spirit") {
      if (inv().getItemCount("spirit_stone") < 10)
        return { success: false, message: "灵石不足" };
      inv().removeItem("spirit_stone", 10);
      gain = 55;
    }
    if (kind === "material") {
      if (inv().getItemCount("spirit_ink") < 1)
    ...
    return {
      success: true,
      message: `${project.name}建设+${gain}，继续投入可升级工程`,
    };
  }
  function farmGoalCards(stats: Record<string, number>) {
    return FARM_GOALS.map((g) => ({
      ...g,
      progress: Math.max(0, Number(stats[g.id] || 0) || 0),
      done: (Number(stats[g.id] || 0) || 0) >= g.target,
      claimed: farmGoalClaimed.value.includes(g.id),
    }));
  }
  function claimFarmGoal(id: FarmGoalId, stats: Record<string, number>) {
    ...
    return {
      success: true,
      message: `领取${goal.title}：${rewardText(goal.reward)}`,
    };
  }

  function claimReturnGift() {
    if (!canClaimReturnGift.value)
      return { success: false, message: "暂未触发回流礼包" };
    const reward: Reward = {
      money: 2600 + daysAway.value * 260,
      spiritStone: Math.min(28, 8 + daysAway.value * 2),
      aura: 420 + daysAway.value * 60,
      items: [{ itemId: "moonlight_jade", name: "月华玉", quantity: 1 }],
    };
    addReward(reward);
    returnGiftClaimedKey.value = dayKey.value;
    lastSeenDayKey.value = dayKey.value;
    return { success: true, message: `闭关归来礼包：${rewardText(reward)}` };
  }
  function touchLoginDay() {
    if (!lastSeenDayKey.value) lastSeenDayKey.value = dayKey.value;
  }
  function finishAdventure(choiceId: string) {
    ...
    if (
      adv.hidden &&
      adventureFlags.value[adv.hidden.requireFlag] &&
      !adventureEndings.value.includes(adv.hidden.title)
    ) {
      addReward(adv.hidden.reward);
      adventureEndings.value.push(adv.hidden.title);
      extra = `；触发${adv.hidden.title}：${adv.hidden.desc}`;
    }
    adventureDone.value.push(adv.id);
    adventureChoices.value[adv.id] = choiceId;
    adventureIndex.value += 1;
    addMonthlyProgress("adventure_4", 1);
  ...
  function claimSeasonTask(id: string) {
    ...
    if ((monthlyProgress.value[id] || 0) < task.target)
      return { success: false, message: "月令进度不足" };
    if (monthlyClaimed.value.includes(id))
      return { success: false, message: "已领取" };
    addReward(task.reward);
    monthlyClaimed.value.push(id);
    return {
      success: true,
  ...
  function rollAffix(slot: string, forceRare = false, secondary = false) {
    const pool = secondary
      ? (SECONDARY_AFFIX_POOL[slot] ?? SECONDARY_AFFIX_POOL.sword ?? [])
      : (AFFIX_POOL[slot] ?? AFFIX_POOL.sword ?? []);
  ...
  function rerollGearAffix(slot: string) {
    ...
    if (next.rarity !== "普通") {
      ...
      if (!affixCodex.value.includes(next.name))
        affixCodex.value.push(next.name);
  ...
  function rerollSecondaryAffix(slot: string) {
    ...
    if (inv().getItemCount("forge_blueprint") < costBlueprint)
      return {
        success: false,
  ...
  const serialize = () => ({
    worldBossPersonal: worldBossPersonal.value,
    worldBossClaimed: worldBossClaimed.value,
    sectBuildLevel: sectBuildLevel.value,
    sectBuildExp: sectBuildExp.value,
    sectBuildContributed: sectBuildContributed.value,
    monthlyKey: monthlyKey.value,
    monthlyProgress: monthlyProgress.value,
    monthlyClaimed: monthlyClaimed.value,
    lastSeenDayKey: lastSeenDayKey.value,
    returnGiftClaimedKey: returnGiftClaimedKey.value,
    ...
    adventureFlags: adventureFlags.value,
  ...
  const deserialize = (data: any = {}) => {
    ...
    worldBossClaimed.value = Array.isArray(data.worldBossClaimed)
      ...
      : [];
    sectBuildLevel.value = Number(data.sectBuildLevel || 1);
    sectBuildExp.value = Number(data.sectBuildExp || 0);
    sectBuildContributed.value = Number(data.sectBuildContributed || 0);
    monthlyKey.value = String(data.monthlyKey || "");
    monthlyProgress.value = data.monthlyProgress || {};
    monthlyClaimed.value = Array.isArray(data.monthlyClaimed)
      ? data.monthlyClaimed
      : [];
    lastSeenDayKey.value = String(data.lastSeenDayKey || "");
    returnGiftClaimedKey.value = String(data.returnGiftClaimedKey || "");
    adventureIndex.value = Number(data.adventureIndex || 0);
    adventureDone.value = Array.isArray(data.adventureDone)
      ? data.adventureDone
      : [];
    ...
    adventureFlags.value = data.adventureFlags || {};
    adventureEndings.value = Array.isArray(data.adventureEndings)
      ? data.adventureEndings
      : [];
    sectProjects.value = {
      spirit_array: 1,
      craft_hall: 1,
