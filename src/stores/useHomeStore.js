...
import { FARMHOUSE_UPGRADES, GREENHOUSE_UNLOCK_COST, GREENHOUSE_MATERIAL_COST, CELLAR_VALUE_CYCLE_DAYS, CELLAR_UPGRADES, CAVE_UPGRADES, getCaveUpgrade, getCaveQuality } from '@/data/buildings';
...
export const useHomeStore = defineStore('home', () => {
    ...
    /** 解锁山洞 */
    const unlockCave = () => {
        if (caveUnlocked.value)
            return false;
        caveUnlocked.value = true;
        return true;
    };
    /** 选择山洞类型 */
    const chooseCave = (choice) => {
        if (!caveUnlocked.value)
            return false;
        if (caveChoice.value !== 'none')
            return false;
    ...
    const unlockGreenhouse = () => {
        ...
        if (!playerStore.spendMoney(GREENHOUSE_UNLOCK_COST))
            return false;
        for (const mat of GREENHOUSE_MATERIAL_COST) {
            removeCombinedItem(mat.itemId, mat.quantity);
        }
    ...
    /** 酒窖放入陈酿 */
    const startAging = (itemId, quality) => {
        if (!hasCellar.value)
            return false;
        if (cellarSlots.value.length >= cellarMaxSlots.value)
            return false;
        if (!AGEABLE_ITEMS.includes(itemId))
            return false;
    ...
    /** 获取厨房加成（Level 1+） */
    const getKitchenBonus = () => {
        return farmhouseLevel.value >= 1 ? 1.2 : 1.0;
    };
    /** 获取睡眠恢复加成（Level 2+） */
    const getStaminaRecoveryBonus = () => {
    ...
    const deserialize = (data) => {
        ...
        cellarSlots.value = (data.cellarSlots ?? []).map((s) => ({
            itemId: s.itemId,
