...
export const useInventoryStore = defineStore('inventory', () => {
    ...
    /** 拥有的武器列表 */
    const ownedWeapons = ref([{ defId: 'wooden_stick', enchantmentId: null }]);
    /** 当前装备的武器索引 */
    const equippedWeaponIndex = ref(0);
    /** 拥有的戒指列表 */
    const ownedRings = ref([]);
    /** 装备的戒指索引（2个槽位，-1 = 空） */
    ...
    /** 拥有的鞋子列表 */
    const ownedShoes = ref([]);
    /** 当前装备的鞋子索引（-1 = 空） */
    const equippedShoeIndex = ref(-1);
    /** 装备方案列表 */
    const equipmentPresets = ref([]);
    /** 当前使用的方案ID */
    ...
    /** 添加物品到背包 */
    const addItem = (itemId, quantity = 1, quality = 'normal') => {
        // 校验物品是否存在
    ...
    const removeItem = (itemId, quantity = 1, quality) => {
        // 先检查总数是否足够
        const matchQuality = (i) => i.itemId === itemId && (quality === undefined || i.quality === quality);
        ...
        for (const q of quality !== undefined ? [quality] : qualityOrder) {
            for (let i = items.value.length - 1; i >= 0 && remaining > 0; i--) {
                ...
                if (slot.itemId !== itemId || slot.quality !== q)
                    continue;
    ...
    /** 查询物品数量 */
    const getItemCount = (itemId, quality) => {
        return items.value
    ...
    /** 切换物品锁定状态 */
    const toggleLock = (itemId, quality) => {
    ...
    const sortItems = () => {
        ...
        // 按分类 → 物品ID → 品质排序
        const qualityOrder = { normal: 0, fine: 1, excellent: 2, supreme: 3 };
        split.sort((a, b) => {
    ...
    const expandCapacityExtra = () => {
        capacity.value += 1;
        return true;
    };
    /** 将临时背包中的物品转移到主背包 */
    const moveFromTemp = (index) => {
        if (index < 0 || index >= tempItems.value.length)
            return false;
    ...
    /** 获取工具等级对应的体力消耗倍率 */
    const getToolStaminaMultiplier = (type) => {
        const tool = getTool(type);
        if (!tool)
            return 1;
        const multipliers = { basic: 1.0, iron: 0.8, steel: 0.6, iridium: 0.4 };
        return multipliers[tool.tier];
    };
    /** 获取工具等级对应的批量操作数量（蓄力机制） */
    const getToolBatchCount = (type) => {
        const tool = getTool(type);
        if (!tool)
            return 1;
        const counts = { basic: 1, iron: 2, steel: 4, iridium: 8 };
        return counts[tool.tier];
    };
    /** 升级工具 */
    const upgradeTool = (type) => {
        const tool = getTool(type);
        if (!tool)
            return false;
    ...
    /** 卸下戒指（指定槽位） */
    const unequipRing = (slot) => {
        if (slot === 0) {
            if (equippedRingSlot1.value < 0)
                return false;
            equippedRingSlot1.value = -1;
        }
        else {
            if (equippedRingSlot2.value < 0)
                return false;
    ...
    const sellRing = (index) => {
        ...
        // 自动卸下
        if (equippedRingSlot1.value === index)
            equippedRingSlot1.value = -1;
        if (equippedRingSlot2.value === index)
            equippedRingSlot2.value = -1;
        const playerStore = usePlayerStore();
        playerStore.earnMoney(price);
    ...
    const getEquipmentBonus = (effectType) => {
        ...
        for (const b of activeSetBonuses.value) {
            if (b.type === effectType)
                total += b.value;
        }
        return total;
    };
    /** 查询某种戒指效果的合计值（代理到 getEquipmentBonus，包含帽子/鞋子加成） */
    const getRingEffectValue = (effectType) => {
    ...
    /** 合成戒指 */
    const craftRing = (defId) => {
        const def = getRingById(defId);
        if (!def || !def.recipe)
            return { success: false, message: '该戒指无法合成。' };
        // 检查材料
    ...
    const sellHat = (index) => {
        ...
        const price = def?.sellPrice ?? 0;
        // 自动卸下
        if (equippedHatIndex.value === index)
            equippedHatIndex.value = -1;
        const playerStore = usePlayerStore();
        playerStore.earnMoney(price);
    ...
    /** 合成帽子 */
    const craftHat = (defId) => {
        const def = getHatById(defId);
        if (!def || !def.recipe)
            return { success: false, message: '该帽子无法合成。' };
        for (const mat of def.recipe) {
            if (getItemCount(mat.itemId) < mat.quantity) {
                const matName = getItemById(mat.itemId)?.name ?? mat.itemId;
                return { success: false, message: `材料不足：${matName}。` };
    ...
    const sellShoe = (index) => {
        ...
        const price = def?.sellPrice ?? 0;
        // 自动卸下
        if (equippedShoeIndex.value === index)
            equippedShoeIndex.value = -1;
        const playerStore = usePlayerStore();
        playerStore.earnMoney(price);
    ...
    /** 合成鞋子 */
    const craftShoe = (defId) => {
        const def = getShoeById(defId);
        if (!def || !def.recipe)
            return { success: false, message: '该鞋子无法合成。' };
        for (const mat of def.recipe) {
            if (getItemCount(mat.itemId) < mat.quantity) {
                const matName = getItemById(mat.itemId)?.name ?? mat.itemId;
                return { success: false, message: `材料不足：${matName}。` };
    ...
    // ============================================================
    /** 创建空方案 */
    const createEquipmentPreset = (name) => {
        if (equipmentPresets.value.length >= 5)
            return false;
    ...
    const deserialize = (data) => {
        ...
        tempItems.value = (data.tempItems ?? []).filter((i) => getItemById(i.itemId));
        ...
        // 向后兼容：旧存档可能缺少新工具
        const requiredTools = ['wateringCan', 'hoe', 'pickaxe', 'fishingRod', 'scythe', 'axe', 'pan'];
        ...
        if (data.ownedWeapons) {
            ownedWeapons.value = data.ownedWeapons;
            equippedWeaponIndex.value = data.equippedWeaponIndex ?? 0;
        }
        else {
            // 旧存档迁移：weapon: { tier: 'copper' } → ownedWeapons
            const oldWeapon = data.weapon;
            if (oldWeapon?.tier) {
                const tierMap = {
                    wood: 'wooden_stick',
                    copper: 'copper_sword',
                ...
                const defId = tierMap[oldWeapon.tier] ?? 'wooden_stick';
        ...
        pendingUpgrade.value = data.pendingUpgrade ?? null;
        // 戒指系统（向后兼容旧存档）
        ownedRings.value = data.ownedRings ?? [];
        equippedRingSlot1.value = data.equippedRingSlot1 ?? -1;
        equippedRingSlot2.value = data.equippedRingSlot2 ?? -1;
        // 修复无效索引
        if (equippedRingSlot1.value >= ownedRings.value.length)
            equippedRingSlot1.value = -1;
        if (equippedRingSlot2.value >= ownedRings.value.length)
            equippedRingSlot2.value = -1;
        // 帽子系统（向后兼容旧存档）
        ownedHats.value = data.ownedHats ?? [];
        equippedHatIndex.value = data.equippedHatIndex ?? -1;
        if (equippedHatIndex.value >= ownedHats.value.length)
            equippedHatIndex.value = -1;
        // 鞋子系统（向后兼容旧存档）
        ownedShoes.value = data.ownedShoes ?? [];
        equippedShoeIndex.value = data.equippedShoeIndex ?? -1;
        if (equippedShoeIndex.value >= ownedShoes.value.length)
            equippedShoeIndex.value = -1;
        // 装备方案（向后兼容旧存档）
        equipmentPresets.value = data.equipmentPresets ?? [];
