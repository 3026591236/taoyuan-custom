...
export const useShopStore = defineStore('shop', () => {
    ...
    /** 购买种子 */
    const buySeed = (seedId, quantity = 1) => {
        const seed = availableSeeds.value.find(s => s.seedId === seedId);
        if (!seed)
            return false;
        if (inventoryStore.isAllFull && !inventoryStore.items.some(s => s.itemId === seedId && s.quantity + quantity <= 999))
            return false;
    ...
    // === 铁匠铺 (孙铁匠) ===
    const blacksmithItems = computed(() => [
    ...
    const shopFertilizers = computed(() => FERTILIZERS.filter(f => f.shopPrice !== null).map(f => ({
        id: f.id,
        name: f.name,
        description: f.description,
        price: f.shopPrice
    })));
    const apothecaryItems = computed(() => [
    ...
    const shopBaits = computed(() => BAITS.filter(b => b.shopPrice !== null).map(b => ({
        id: b.id,
        name: b.name,
    ...
    const shopTackles = computed(() => TACKLES.filter(t => t.shopPrice !== null).map(t => ({
        id: t.id,
        name: t.name,
    ...
    // === 通用购买/出售 ===
    /** 购买通用物品 */
    const buyItem = (itemId, price, quantity = 1) => {
        if (inventoryStore.isAllFull && !inventoryStore.items.some(s => s.itemId === itemId && s.quantity + quantity <= 999))
            return false;
    ...
    /** 计算不含行情系数的基础售价 */
    const _basePrice = (itemId, quantity, quality) => {
        ...
        let bonus = 1.0;
        if (itemDef.category === 'processed' && skillStore.getSkill('farming').perk10 === 'artisan')
            bonus *= 1.25;
        if (itemDef.category === 'crop' && skillStore.getSkill('farming').perk5 === 'harvester')
            bonus *= 1.1;
        if (itemDef.category === 'animal_product' && skillStore.getSkill('farming').perk5 === 'rancher')
            bonus *= 1.2;
        if (itemDef.category === 'fish' && skillStore.getSkill('fishing').perk5 === 'fisher')
            bonus *= 1.25;
        if (itemDef.category === 'fish' && skillStore.getSkill('fishing').perk10 === 'aquaculture')
            bonus *= 1.5;
        if (itemDef.category === 'fish' && gameStore.farmMapType === 'riverland')
            bonus *= 1.1;
        if (itemDef.category === 'ore' && skillStore.getSkill('mining').perk10 === 'blacksmith')
            bonus *= 1.5;
    ...
    /** 计算物品售价（不执行出售，用于估价） */
    const calculateSellPrice = (itemId, quantity, quality) => {
        const itemDef = getItemById(itemId);
        if (!itemDef)
            return 0;
    ...
    /** 计算不含行情的基础售价（用于显示原价） */
    const calculateBaseSellPrice = (itemId, quantity, quality) => {
        return _basePrice(itemId, quantity, quality);
    };
    /** 出售物品，返回实际售价（0表示失败） */
    const sellItem = (itemId, quantity = 1, quality = 'normal') => {
        if (!inventoryStore.removeItem(itemId, quantity, quality))
            return 0;
    ...
    const refreshMerchantStock = () => {
        ...
        if (travelingStockKey.value === key)
            return;
    ...
    const buyFromTraveler = (itemId) => {
        const item = travelingStock.value.find(s => s.itemId === itemId);
        if (!item || item.quantity <= 0)
            return false;
        if (inventoryStore.isAllFull && !inventoryStore.items.some(s => s.itemId === itemId && s.quantity < 999))
            return false;
        const finalPrice = applyDiscount(item.price);
        if (!playerStore.spendMoney(finalPrice))
            return false;
    ...
    /** 添加物品到出货箱 */
    const addToShippingBox = (itemId, quantity, quality) => {
        if (!inventoryStore.removeItem(itemId, quantity, quality))
            return false;
    ...
    /** 从出货箱取回物品 */
    const removeFromShippingBox = (itemId, quantity, quality) => {
    ...
    /** 处理出货箱结算（日结时调用），返回总收入 */
    const processShippingBox = () => {
        let total = 0;
        const dayKey = `${gameStore.year}-${gameStore.seasonIndex}-${gameStore.day}`;
        const dayRecord = { ...(shippingHistory.value[dayKey] ?? {}) };
    ...
    /** 将日期转为绝对天数（用于比较距离） */
    const _toAbsoluteDay = (year, seasonIndex, day) => {
