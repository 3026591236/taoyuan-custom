...
export const useCultivationStore = defineStore('cultivation', () => {
  ...
  const combatPower = computed(() => {
    ...
    const systemPower = fieldTier.value * 120 + caveTier.value * 180 + yuanShenLevel.value * 260 + destinedArtifactLevel.value * 360 + daoGearPower.value + talismanPower.value + beastBond.value * 12 + sectContribution.value * 0.2 + (sectMerit.value || 0) * 0.6 + sectSkillPower + sectIdentityPower + pathPower + manualPower
  ...
  const daoGearPower = computed(() => DAO_GEAR.reduce((sum, gear) => sum + (daoGear.value[gear.id] || 0) * gear.powerPerLevel, 0))
  const daoGearTribulationBonus = computed(() => Math.min(0.16, DAO_GEAR.reduce((sum, gear) => sum + (daoGear.value[gear.id] || 0) * gear.tribulationPerLevel, 0)))
