...
import { HANHAI_FIXED_ITEMS, getWeeklyRotatingItems, MAX_DAILY_BETS, HANHAI_UNLOCK_COST, spinRoulette, rollDice, ROULETTE_BET_TIERS, DICE_BET_AMOUNT, CUP_BET_AMOUNT, CUP_WIN_MULTIPLIER, playCupRound, CRICKET_BET_AMOUNT, CRICKET_WIN_MULTIPLIER, fightCricket, CARD_BET_AMOUNT, CARD_WIN_MULTIPLIER, dealCards, getTexasTier, dealTexas, BUCKSHOT_BET_AMOUNT, BUCKSHOT_WIN_MULTIPLIER, BUCKSHOT_PLAYER_HP, BUCKSHOT_DEALER_HP, loadShotgun, TRADE_SHOP_UPGRADES, TRADE_EXCHANGE_ITEMS, calcTradePoints } from '@/data/hanhai';
...
export const useHanhaiStore = defineStore('hanhai', () => {
    ...
    /** 售货槽位 */
    const tradeSlots = ref([]);
    /** 本周轮换商品 */
    const weeklyRotatingStock = ref([]);
    /** 积分兑换购买记录 { itemId: count }（每周限购） */
    const weeklyExchangePurchases = ref({});
    /** 积分兑换购买记录 { itemId: count }（总限购） */
    const totalExchangePurchases = ref({});
    /** 当前店铺升级配置 */
    ...
    const unlockHanhai = () => {
        if (unlocked.value)
            return { success: false, message: '瀚海已经解锁。' };
    ...
    /** 查询某商品本周剩余可购买数量 */
    const getWeeklyRemaining = (itemId) => {
    ...
    /** 玩幸运轮盘 */
    const playRoulette = (betTier) => {
        if (!canBet.value)
            return { success: false, message: '今天的赌博次数已用完。', multiplier: 0, winnings: 0 };
        if (!ROULETTE_BET_TIERS.includes(betTier)) {
    ...
    /** 玩骰子（猜大小） */
    const playDice = (guessBig) => {
        if (!canBet.value)
            return { success: false, message: '今天的赌博次数已用完。', dice1: 0, dice2: 0, won: false, winnings: 0 };
        const playerStore = usePlayerStore();
    ...
    const playCup = (guess) => {
        if (!canBet.value)
            return { success: false, message: '今天的赌博次数已用完。', correctCup: 0, won: false, winnings: 0 };
        const playerStore = usePlayerStore();
    ...
    /** 玩斗蛐蛐 */
    const playCricketFight = (cricketId) => {
        if (!canBet.value)
    ...
    /** 玩翻牌寻宝 */
    const playCardFlip = (pick) => {
        if (!canBet.value)
            return { success: false, message: '今天的赌博次数已用完。', treasures: [], won: false, winnings: 0 };
        const playerStore = usePlayerStore();
    ...
    /** 结束瀚海扑克（结算：返还剩余筹码） */
    const endTexas = (finalChips, tierName) => {
        const playerStore = usePlayerStore();
        if (finalChips > 0) {
    ...
    /** 放入物品到通商售货槽 */
    const addTradeSlot = (itemId, quality, quantity) => {
        if (quantity <= 0)
    ...
    /** 升级通商店铺 */
    const upgradeTradeShop = () => {
        const next = nextTradeShopUpgrade.value;
        if (!next)
            return { success: false, message: '店铺已满级。' };
        const playerStore = usePlayerStore();
    ...
    const exchangeItem = (itemId) => {
        ...
        if (!exchangeDef)
            return { success: false, message: '兑换物品不存在。' };
        // 检查周限购
