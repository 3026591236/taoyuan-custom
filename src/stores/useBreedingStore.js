...
import { BASE_BREEDING_BOX, SEED_BOX_UPGRADES, SEED_BOX_UPGRADE_INCREMENT, BREEDING_DAYS, BASE_MUTATION_MAGNITUDE, GENERATIONAL_STABILITY_GAIN, MAX_STABILITY, MUTATION_JUMP_MIN, MUTATION_JUMP_MAX, MUTATION_RATE_DRIFT, MUTATION_POSITIVE_CHANCE, BREEDING_STATION_COST, MAX_BREEDING_STATIONS, generateGeneticsId, clampStat, clampMutationRate, getDefaultGenetics, getStarRating, makeSeedLabel, findPossibleHybrid, findPossibleHybridById, getSeedMakerGeneticChance, getHybridTier } from '@/data/breeding';
...
export const useBreedingStore = defineStore('breeding', () => {
    // === 状态 ===
    /** 种子箱 */
    const breedingBox = ref([]);
    /** 育种台 */
    const stations = ref([]);
    /** 已建造的育种台数量 */
    const stationCount = ref(0);
    /** 图鉴 */
    const compendium = ref([]);
    ...
    // === 种子箱操作 ===
    const addToBox = (genetics) => {
        if (breedingBox.value.length >= maxSeedBox.value)
            return false;
    ...
    const removeFromBox = (geneticsId) => {
    ...
    // === 种子制造机增强 ===
    const trySeedMakerGeneticSeed = (cropId, farmingLevel) => {
        if (breedingBox.value.length >= maxSeedBox.value)
            return false;
    ...
    // === 育种台 ===
    const craftStation = (spendMoney, removeItem) => {
        if (stationCount.value >= MAX_BREEDING_STATIONS)
            return false;
    ...
    const canCraftStation = (money, getItemCount) => {
        if (stationCount.value >= MAX_BREEDING_STATIONS)
            return false;
        if (money < BREEDING_STATION_COST.money)
            return false;
        for (const mat of BREEDING_STATION_COST.materials) {
    ...
    const canUpgradeSeedBox = (money, getItemCount) => {
    ...
    const upgradeSeedBox = (spendMoney, removeItem) => {
        const upgrade = getNextSeedBoxUpgrade();
        if (!upgrade)
            return { success: false, message: '种子箱已达到最高等级。' };
    ...
    const startBreeding = (slotIndex, seedAId, seedBId) => {
        const slot = stations.value[slotIndex];
        if (!slot || slot.parentA || slot.ready)
            return false;
        ...
        if (!seedA || !seedB) {
            // 归还已取出的种子
            if (seedA)
                addToBox(seedA.genetics);
            if (seedB)
                addToBox(seedB.genetics);
            return false;
        }
    ...
    const collectResult = (slotIndex) => {
        const slot = stations.value[slotIndex];
        if (!slot || !slot.ready || !slot.result)
            return null;
    ...
    // === 核心杂交算法 ===
    const breedSeeds = (parentA, parentB) => {
    ...
    const breedSameCrop = (a, b) => {
        ...
        if (Math.random() < avgMutationRate / 100) {
            ...
            const stats = ['sweetness', 'yield', 'resistance'];
            // Fisher-Yates 洗牌
    ...
    const breedDifferentCrop = (a, b) => {
        ...
        if (hybrid && avgSweetness >= hybrid.minSweetness && avgYield >= hybrid.minYield) {
            ...
            if (hybrid) {
                addLog(`杂交失败：亲本平均甜度${Math.round(avgSweetness)}（需≥${hybrid.minSweetness}），平均产量${Math.round(avgYield)}（需≥${hybrid.minYield}）。请先通过同种培育提升属性。`);
            }
            else {
    ...
    /** 记录杂交种被种植 */
    const recordHybridGrown = (hybridId) => {
    ...
    const deserialize = (data) => {
        breedingBox.value = (data.breedingBox ?? []).map((s) => ({
            genetics: s.genetics,
