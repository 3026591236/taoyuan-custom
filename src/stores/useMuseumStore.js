...
export const useMuseumStore = defineStore('museum', () => {
    ...
    /** 是否已捐赠 */
    const isDonated = (itemId) => {
        return donatedItems.value.includes(itemId);
    };
    /** 是否可捐赠（背包中有且未捐赠过） */
    const canDonate = (itemId) => {
        if (isDonated(itemId))
            return false;
        if (!MUSEUM_ITEMS.find(m => m.id === itemId))
            return false;
    ...
    /** 领取里程碑奖励 */
    const claimMilestone = (count) => {
        const milestone = MUSEUM_MILESTONES.find(m => m.count === count);
        if (!milestone)
            return false;
        if (donatedCount.value < count)
            return false;
        if (claimedMilestones.value.includes(count))
            return false;
