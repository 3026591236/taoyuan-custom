...
export const useProcessingStore = defineStore("processing", () => {
  ...
  const startProcessing = (
    ...
    if (
      slot.machineType === "loom" &&
      useHiddenNpcStore().isAbilityActive("gui_nv_1")
    ) {
    ...
    for (const slot of machines.value) {
      ...
      if (slot.daysProcessed >= slot.totalDays) {
        ...
        if (recipe) {
          ...
          if (
            slot.machineType === "loom" &&
            useHiddenNpcStore().isAbilityActive("gui_nv_2") &&
            Math.random() < 0.08
          ) {
          ...
          if (recipe.inputItemId === null || machineDef?.autoCollect) {
            ...
            if (voidInput && recipe.inputItemId) {
              ...
              if (slot.machineType === "seed_maker" && slot.inputItemId) {
                ...
                const farmingLevel = skillStore.farmingLevel;
                if (
                  breedingStore.trySeedMakerGeneticSeed(
                    slot.inputItemId,
                    farmingLevel,
                  )
                ) {
              ...
              // 尝试从虚空原料箱取材料开始下一轮
              const available = warehouseStore.getChestItemCount(
                voidInput.id,
                recipe.inputItemId,
              );
              if (available >= recipe.inputQuantity) {
                // 查找最低品质
                const qOrder: Quality[] = [
                  "normal",
                  "fine",
                  "excellent",
                  "supreme",
                ...
                const newQuality =
                  qOrder.find(
                    (q) =>
                      warehouseStore.getChestItemCount(
                        voidInput.id,
                        recipe.inputItemId!,
                        q,
                      ) > 0,
                  ) ?? "normal";
                warehouseStore.removeItemFromChest(
                  voidInput.id,
                  recipe.inputItemId,
                  recipe.inputQuantity,
                  newQuality,
