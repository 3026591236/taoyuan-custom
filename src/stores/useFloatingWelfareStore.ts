...
export const useFloatingWelfareStore = defineStore('floatingWelfare', () => {
  ...
  const serialize = () => ({ claimedKeys: claimedKeys.value, firstSeenDate: firstSeenDate.value })
  const deserialize = (data?: { claimedKeys?: string[]; firstSeenDate?: string }) => {
    claimedKeys.value = Array.isArray(data?.claimedKeys) ? data!.claimedKeys!.map(String).filter(Boolean) : []
    firstSeenDate.value = String(data?.firstSeenDate || '')
  }

  return { claimedKeys, firstSeenDate, ensureFirstSeenDate, isClaimed, markClaimed, serialize, deserialize }
})
