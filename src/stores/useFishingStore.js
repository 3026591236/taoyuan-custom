...
export const useFishingStore = defineStore('fishing', () => {
    ...
    /** 当前钓鱼地点 */
    const fishingLocation = ref('creek');
    /** 切换钓鱼地点 */
    ...
    /** 上次钓鱼的宝箱结果 */
    const lastTreasure = ref(null);
    /** 上次是否完美 */
    const lastPerfect = ref(false);
    /** 鱼饵/浮漂装备 */
    const equippedBait = ref(null);
    const equippedTackle = ref(null);
    const tackleDurability = ref(0);
    /** 当次钓鱼会话的鱼饵/浮漂 */
    const activeBaitDef = ref(null);
    const activeTackleDef = ref(null);
    /** 蟹笼 */
    const crabPots = ref([]);
    /** 装备鱼饵（仅标记类型，不从背包取出） */
    const equipBait = (type) => {
        const def = getBaitById(type);
        if (!def)
            return { success: false, message: '无效的鱼饵。' };
    ...
    /** 装备浮漂 */
    const equipTackle = (type) => {
        const def = getTackleById(type);
        if (!def)
            return { success: false, message: '无效的浮漂。' };
    ...
    const startFishing = () => {
        ...
        const staminaCost = Math.max(1, Math.floor(STAMINA_COST *
            rodMultiplier *
        ...
        const fishPool = baitDef?.ignoresSeason
            ? FISH.filter(f => (f.location ?? 'creek') === loc && (f.weather.includes('any') || f.weather.includes(gameStore.weather)))
            : availableFish.value;
        ...
        if (activeBaitDef.value)
            msg += ` [${activeBaitDef.value.name}]`;
        if (activeTackleDef.value)
            msg += ` [${activeTackleDef.value.name}]`;
        return { success: true, message: msg };
    };
    /** 计算小游戏参数（在 startFishing 之后调用） */
    const calculateMiniGameParams = () => {
        const fish = currentFish.value;
        const rodTier = inventoryStore.getTool('fishingRod')?.tier ?? 'basic';
        const level = skillStore.fishingLevel;
        // 基础钩子高度（鱼竿等级）
        const rodHookMap = { basic: 40, iron: 45, steel: 50, iridium: 60 };
        let hookHeight = rodHookMap[rodTier] + level * 2;
        // 基础时限（鱼竿等级）
        const rodTimeMap = { basic: 30, iron: 33, steel: 36, iridium: 40 };
        const timeLimit = rodTimeMap[rodTier];
        // 鱼速度（难度为默认，鱼种可覆盖）
        const difficultySpeedMap = { easy: 1.0, normal: 2.0, hard: 3.0, legendary: 4.0 };
        const difficultyDirMap = { easy: 0.02, normal: 0.04, hard: 0.06, legendary: 0.08 };
        ...
        if (activeTackleDef.value) {
            ...
            if (activeTackleDef.value.struggleBonus)
                hookHeight += 15;
            // 铅坠浮漂：鱼改变方向概率减半
            if (activeTackleDef.value.dangerReduction)
                fishChangeDir *= 0.5;
        }
        // 钓翁令牌效果：鱼速度 -10%
    ...
    const pickRandomFish = (pool) => {
        const fishPool = pool ?? availableFish.value;
        const cookingStore = useCookingStore();
        const fishingBuff = cookingStore.activeBuff?.type === 'fishing' || cookingStore.activeBuff?.type === 'all_skills' ? cookingStore.activeBuff.value : 0;
    ...
    const completeFishing = (rating) => {
        ...
        let quality = skillStore.rollCropQuality();
        // 水手专精：鱼品质至少为优良
        ...
        // 经验
        const difficultyExpMult = { easy: 1, normal: 1.5, hard: 2, legendary: 3 };
    ...
    const baitCrabPots = (location) => {
        ...
        if (baited === 0)
            return { success: false, message: '没有鱼饵了。' };
        return { success: true, message: `装饵${baited}个蟹笼。` };
    };
    /** 全局装饵（雇工自动调用，不限地点） */
    const baitAllCrabPots = () => {
    ...
    const crabPotsByLocation = computed(() => {
        ...
        for (const pot of crabPots.value) {
            if (!map[pot.location])
                map[pot.location] = { total: 0, baited: 0 };
            map[pot.location].total++;
            if (pot.hasBait)
                map[pot.location].baited++;
        }
        return map;
    });
