...
export const useGameStore = defineStore("game", () => {
  ...
  const rollDailyFate = () => {
    ...
    dailyFateType.value =
      DAILY_FATES[Math.floor(Math.random() * DAILY_FATES.length)]!.type;
  ...
  type DailyFateType =
    "harvest" | "combat" | "tower" | "travel" | "cultivation" | "wealth";
  const DAILY_FATES: {
    type: DailyFateType;
    name: string;
    desc: string;
  ...
  }[] = [
    {
      type: "harvest",
      name: "灵雨润田",
      desc: "今日田间灵机活跃，收获更容易得到额外地脉感应。",
      bonusText: "收获地脉感应 +20%",
    },
    {
      type: "combat",
      name: "锋芒毕露",
      desc: "今日气血沸腾，红尘历练与秘境战斗收获更多修为灵气。",
      bonusText: "战斗修为/灵气 +12%",
    },
    {
      type: "tower",
      name: "登塔吉兆",
      desc: "今日登仙塔灵压顺遂，登塔胜利奖励更丰厚。",
      bonusText: "登仙塔修为/灵气 +18%",
    },
    {
      type: "travel",
      name: "清风随行",
      desc: "今日行路轻快，跨区域移动更省时间。",
      bonusText: "移动耗时 -20%",
    },
    {
      type: "cultivation",
      name: "灵台清明",
      desc: "今日修行心境澄澈，打坐与炼化更容易增长修为。",
      bonusText: "修行效率 +10%",
    },
    {
      type: "wealth",
  ...
  const dailyFate = computed(() => {
    refreshDailyFateIfNeeded();
    return (
      DAILY_FATES.find((f) => f.type === dailyFateType.value) ?? DAILY_FATES[0]!
    );
  });
  const dailyFateBonus = (type: DailyFateType): number => {
    refreshDailyFateIfNeeded();
  ...
  const advanceTime = (
    ...
    const speedBuff =
      cookingStore.activeBuff?.type === "speed"
    ...
    if (newHour >= PASSOUT_HOUR) {
      ...
      const recoveryMsg =
        staminaRecovered > 0
          ? `路上缓过劲来，体力恢复${staminaRecovered}点。`
          : "";
      return {
        ok: true,
        passedOut: true,
        message: `${recoveryMsg}${recoveryMsg ? " " : ""}已经凌晨2点了，你撑不住倒下了……`,
      };
    }
    ...
    const recoveryMsg =
      staminaRecovered > 0 ? `休整片刻，体力恢复${staminaRecovered}点。` : "";
    ...
    if (
      ...
      return {
        ok: true,
        passedOut: false,
        message: `${recoveryMsg}${recoveryMsg ? " " : ""}已经过了午夜，你开始感到困倦……`,
      };
    }
  ...
  const travelTo = (
    ...
    const staminaCost = animalStore.hasHorse
      ? Math.max(1, Math.floor(baseStamina / 2))
      : baseStamina;
    const playerStore = usePlayerStore();
    ...
    const travelMsg =
      cost > 0
        ? `前往${targetName}，路上花了${Math.round(cost * 60)}分钟，消耗${staminaCost}点体力。`
        : "";
    return {
      ok: true,
      timeCost: cost,
