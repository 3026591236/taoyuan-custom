...
export const useInventoryStore = defineStore("inventory", () => {
  ...
  /** 拥有的武器列表 */
  const ownedWeapons = ref<OwnedWeapon[]>([
    ...
    quickUseItems.value = quickUseItems.value
      .filter(
        (q) =>
          q?.itemId &&
          items.value.some(
            (i) => i.itemId === q.itemId && i.quality === q.quality,
          ),
      )
      .filter(
        (q, idx, arr) =>
          arr.findIndex(
            (x) => x.itemId === q.itemId && x.quality === q.quality,
          ) === idx,
      )
      .slice(0, 5);
  };
  ...
  const clearQuickUseItem = (itemId?: string, quality: Quality = "normal") => {
    ...
    quickUseItems.value = quickUseItems.value.filter(
      (q) => !(q.itemId === itemId && q.quality === quality),
    );
    showFloat("已从快捷栏移除。");
  };

  const isQuickUseItem = (itemId: string, quality: Quality = "normal") =>
    quickUseItems.value.some(
      (q) => q.itemId === itemId && q.quality === quality,
    );
  ...
  const hasWeaponExact = (
    defId: string,
    enchantmentId: string | null,
  ): boolean => {
    return ownedWeapons.value.some(
      (w) => w.defId === defId && w.enchantmentId === enchantmentId,
    );
  };
  ...
  const getItemCount = (itemId: string, quality?: Quality): number => {
    return items.value
      .filter(
        (i) =>
          i.itemId === itemId &&
          (quality === undefined || i.quality === quality),
      )
      .reduce((sum, i) => sum + i.quantity, 0);
  };
  ...
  const getToolStaminaMultiplier = (type: ToolType): number => {
    ...
    const masteryBonus =
      Math.min(3, Math.max(0, tool.masteryLevel ?? 0)) * 0.05;
  ...
  const equipRing = (ringIndex: number, slot: 0 | 1): boolean => {
    ...
    if (
      otherSlot.value >= 0 &&
      otherSlot.value < ownedRings.value.length &&
      ownedRings.value[otherSlot.value]!.defId === targetDefId
    ) {
      return false;
    }
    targetSlot.value = ringIndex;
  ...
  const _getSetPieceCount = (set: (typeof EQUIPMENT_SETS)[number]): number => {
    ...
    for (const idx of [equippedRingSlot1.value, equippedRingSlot2.value]) {
      if (
        !ringMatched &&
        idx >= 0 &&
        idx < ownedRings.value.length &&
        ownedRings.value[idx]!.defId === set.pieces.ring
      ) {
        ringMatched = true;
        count++;
      }
  ...
  const saveCurrentToPreset = (id: string) => {
    ...
    preset.ringSlot1DefId =
      equippedRingSlot1.value >= 0
        ...
        : null;
    preset.ringSlot2DefId =
      equippedRingSlot2.value >= 0
        ? (ownedRings.value[equippedRingSlot2.value]?.defId ?? null)
        : null;
    preset.hatDefId =
      equippedHatIndex.value >= 0
        ? (ownedHats.value[equippedHatIndex.value]?.defId ?? null)
        : null;
    preset.shoeDefId =
      equippedShoeIndex.value >= 0
        ? (ownedShoes.value[equippedShoeIndex.value]?.defId ?? null)
        : null;
  };

  /** 应用装备方案 */
  const applyEquipmentPreset = (
    id: string,
    ...
    if (preset.ringSlot1DefId) {
      ring1Idx = ownedRings.value.findIndex(
        (r) => r.defId === preset.ringSlot1DefId,
      );
    ...
    if (preset.ringSlot2DefId) {
      if (preset.ringSlot2DefId === preset.ringSlot1DefId) {
        ...
        const idx = ownedRings.value.findIndex(
          (r) => r.defId === preset.ringSlot2DefId,
        );
    ...
    // 鞋子
    if (preset.shoeDefId) {
      const idx = ownedShoes.value.findIndex(
        (s) => s.defId === preset.shoeDefId,
      );
  ...
  const sortEquipment = () => {
    ...
    ownedWeapons.value.sort((a, b) => {
      ...
      // 附魔加成降序
      const enchA = a.enchantmentId
        ? (getEnchantmentById(a.enchantmentId)?.attackBonus ?? 0)
        : 0;
      const enchB = b.enchantmentId
        ? (getEnchantmentById(b.enchantmentId)?.attackBonus ?? 0)
        : 0;
      if (enchB !== enchA) return enchB - enchA;
      return a.defId.localeCompare(b.defId);
    });
    equippedWeaponIndex.value = equippedWeapon
      ? ownedWeapons.value.indexOf(equippedWeapon)
      : 0;

    // --- 戒指排序 ---
    const equippedRing1 =
      equippedRingSlot1.value >= 0
        ? ownedRings.value[equippedRingSlot1.value]
        : undefined;
    const equippedRing2 =
      equippedRingSlot2.value >= 0
        ? ownedRings.value[equippedRingSlot2.value]
        : undefined;
    ...
    equippedRingSlot1.value = equippedRing1
      ? ownedRings.value.indexOf(equippedRing1)
      : -1;
    equippedRingSlot2.value = equippedRing2
      ? ownedRings.value.indexOf(equippedRing2)
      : -1;

    // --- 帽子排序 ---
    const equippedHat =
      equippedHatIndex.value >= 0
        ? ownedHats.value[equippedHatIndex.value]
        : undefined;
    ...
    equippedHatIndex.value = equippedHat
      ? ownedHats.value.indexOf(equippedHat)
      : -1;

    // --- 鞋子排序 ---
    const equippedShoe =
      equippedShoeIndex.value >= 0
        ? ownedShoes.value[equippedShoeIndex.value]
        : undefined;
    ...
    equippedShoeIndex.value = equippedShoe
      ? ownedShoes.value.indexOf(equippedShoe)
      : -1;
  };
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    for (const rt of requiredTools) {
      ...
      if (!existing) {
        ...
        existing.masteryLevel = Math.min(
          3,
          Math.max(0, existing.masteryLevel ?? 0),
        );
      }
    }
    ...
    equippedRingSlot2.value =
      ((data as Record<string, unknown>).equippedRingSlot2 as
        number | undefined) ?? -1;
    // 修复无效索引
    ...
    const oldQuick = (data as Record<string, unknown>).quickUseItem as
      { itemId?: string; quality?: Quality } | null | undefined;
    quickUseItems.value = (
      Array.isArray(rawQuickItems)
        ? rawQuickItems
        : oldQuick?.itemId
          ? [oldQuick]
          : []
      ...
      .map((q) => ({
        ...
        quality: (q.quality || "normal") as Quality,
      }))
      .slice(0, 5);
  };
