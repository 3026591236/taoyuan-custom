...
const ARTIFACT_NAMES = { glimmerHoe: '流光锄', spiritKettle: '引灵壶', spiritRain: '灵雨诀' };
...
export const SPIRIT_ROOT_NAMES = {
    mixed: '杂灵根', wood: '木灵根', water: '水灵根', earth: '土灵根', fire: '火灵根', metal: '金灵根'
};
export const SPIRIT_CROP_AURA = {
    moonlight_rice: 8,
    phoenix_pepper: 10,
    snow_lotus: 12,
...
export const getSpiritCropAura = (cropId) => SPIRIT_CROP_AURA[cropId] ?? 0;
export const useCultivationStore = defineStore('cultivation', () => {
    ...
    const aura = ref(0);
    const mana = ref(30);
    const spiritRoot = ref('mixed');
    const fieldTier = ref(0);
    const totalAuraHarvested = ref(0);
    const alchemyUnlocked = ref(false);
    const artifacts = ref({ glimmerHoe: false, spiritKettle: false, spiritRain: false });
    ...
    const artifactName = (key) => ARTIFACT_NAMES[key];
    ...
    const hasCaveSlot = (type) => caveSlots.value.includes(type);
    ...
    const addAuraFromHarvest = (cropId, qty = 1) => {
        if (!unlocked.value)
            return 0;
    ...
    const craftPill = (pillId) => {
        if (!alchemyUnlocked.value && !unlockAlchemy())
            return false;
    ...
    const unlockArtifact = (key) => {
        if (!unlocked.value)
            return unlock();
        if (artifacts.value[key])
            return true;
        const cost = {
    ...
    const encounterBeast = () => {
        ...
        const chosen = roll < 0.4 ? 'fox' : roll < 0.75 ? 'crane' : 'phoenix';
        beast.value = chosen;
        beastBond.value = 10;
    ...
    const deserialize = (data) => {
        ...
        spiritRoot.value = data.spiritRoot ?? 'mixed';
        fieldTier.value = data.fieldTier ?? 0;
        totalAuraHarvested.value = data.totalAuraHarvested ?? 0;
        alchemyUnlocked.value = data.alchemyUnlocked ?? false;
        artifacts.value = { glimmerHoe: false, spiritKettle: false, spiritRain: false, ...(data.artifacts ?? {}) };
        foundationPillBlessing.value = data.foundationPillBlessing ?? 0;
        caveTier.value = data.caveTier ?? 0;
        caveSlots.value = data.caveSlots ?? [];
        beast.value = data.beast ?? null;
        beastBond.value = data.beastBond ?? 0;
        sect.value = data.sect ?? null;
        sectSkills.value = data.sectSkills ?? [0, 0, 0];
        sectContribution.value = data.sectContribution ?? 0;
    };
