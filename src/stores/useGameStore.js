...
import { DAY_START_HOUR, PASSOUT_HOUR, MIDNIGHT_HOUR, WEEKDAY_NAMES, getWeekday, formatTime, getTimePeriod, TAB_TO_LOCATION_GROUP, TRAVEL_TIME, TRAVEL_STAMINA, getLocationGroupName } from '@/data/timeConstants';
...
export const useGameStore = defineStore('game', () => {
    ...
    const tomorrowWeather = ref('sunny');
    const currentLocation = ref('farm');
    const currentLocationGroup = ref('farm');
    const isGameStarted = ref(false);
    const farmMapType = ref('standard');
    const midnightWarned = ref(false);
    const dailyLuck = ref(0);
    /** 山丘田庄：地表矿脉（日结生成，在农场面板开采后清除） */
    const surfaceOrePatch = ref(null);
    /** 溪流田庄：溪流鱼获（日结生成，在农场面板收取后清除） */
    const creekCatch = ref([]);
    ...
    const rollWeather = (s, d) => {
        const targetSeason = s ?? season.value;
        const targetDay = d ?? day.value;
        // 节日日永远晴天
        if (FESTIVAL_DAYS[targetSeason]?.includes(targetDay))
            return 'sunny';
        // 固定天气日
    ...
    /** 开始新游戏 */
    const startNewGame = (mapType = 'standard') => {
        year.value = 1;
        season.value = 'spring';
        day.value = 1;
        hour.value = DAY_START_HOUR;
