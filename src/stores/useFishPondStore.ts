...
export const useFishPondStore = defineStore("fishPond", () => {
  ...
  const capacity = computed(() =>
    pond.value.built ? POND_CAPACITY[pond.value.level] : 0,
  );
  ...
  const addFish = (fishId: string, quantity: number = 1): number => {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      const breed =
        g1Breeds.length > 0
          ? g1Breeds[Math.floor(Math.random() * g1Breeds.length)]
          : null;
      const fish: PondFish = {
        id: generateFishId(),
        fishId,
  ...
  const removeFish = (pondFishId: string): boolean => {
    ...
    if (
      pond.value.breeding &&
      (pond.value.breeding.parentA === pondFishId ||
        pond.value.breeding.parentB === pondFishId)
    ) {
  ...
  const feedFish = (): boolean => {
    ...
    pond.value.waterQuality = clamp(
      pond.value.waterQuality + FEED_WATER_RESTORE,
      0,
      100,
    );
    return true;
  ...
  const cleanPond = (): boolean => {
    ...
    pond.value.waterQuality = clamp(
      pond.value.waterQuality + PURIFIER_WATER_RESTORE,
      0,
      100,
    );
    return true;
  ...
  const _breedGenetics = (a: FishGenetics, b: FishGenetics): FishGenetics => {
    ...
    const fluctuationRange =
      GENETICS_FLUCTUATION_BASE * (1 - avgStability / 200);
    ...
    const inherit = (
      ...
      if (Math.random() < avgMutRate / 100) {
        const jump =
          POND_MUTATION_JUMP_MIN +
          Math.random() * (POND_MUTATION_JUMP_MAX - POND_MUTATION_JUMP_MIN);
  ...
  const dailyUpdate = (): PondDailyResult => {
    ...
    for (let i = 0; i < pond.value.fish.length; i++) {
      ...
      if (
        fish.sick &&
        pond.value.fedToday &&
        pond.value.waterQuality >= DISEASE_THRESHOLD
      ) {
        fish.sick = false;
        fish.sickDays = 0;
    ...
    for (let i = toRemove.length - 1; i >= 0; i--) {
      ...
      if (
        pond.value.breeding &&
        (pond.value.breeding.parentA === deadFish.id ||
          pond.value.breeding.parentB === deadFish.id)
      ) {
        pond.value.breeding = null;
      }
      pond.value.fish.splice(idx, 1);
    ...
    if (pond.value.breeding) {
      ...
      if (pond.value.breeding.daysLeft <= 0) {
        const parentA = pond.value.fish.find(
          (f) => f.id === pond.value.breeding!.parentA,
        );
        const parentB = pond.value.fish.find(
          (f) => f.id === pond.value.breeding!.parentB,
        );
        if (!parentA || !parentB) {
          ...
          if (def) {
            ...
            if (parentA.breedId && parentB.breedId) {
              const recipe = findBreedByParents(
                parentA.breedId,
                parentB.breedId,
              );
              if (recipe) {
            ...
            if (!childBreedId) {
              const parentABreed = parentA.breedId
                ? getBreedById(parentA.breedId)
                : null;
              const parentBBreed = parentB.breedId
                ? getBreedById(parentB.breedId)
                : null;
              const parentGen = Math.min(
                parentABreed?.generation ?? 1,
                parentBBreed?.generation ?? 1,
              ) as 1 | 2 | 3 | 4 | 5;
              const sameGenBreeds = getBreedsByGeneration(parentGen).filter(
                (b) => b.baseFishId === def!.fishId,
              );
              if (sameGenBreeds.length > 0) {
                const rnd =
                  sameGenBreeds[
                    Math.floor(Math.random() * sameGenBreeds.length)
                  ]!;
