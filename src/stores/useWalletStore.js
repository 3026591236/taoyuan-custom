...
export const useWalletStore = defineStore('wallet', () => {
    ...
    const unlock = (id) => {
        if (has(id))
            return false;
        if (!WALLET_ITEMS.find(w => w.id === id))
            return false;
    ...
    // === 被动效果查询 ===
    /** 商店折扣 (0.1 = 10%) */
    const getShopDiscount = () => {
        return has('merchant_seal') ? 0.1 : 0;
    };
    /** 采集品质加成档数 */
    const getForageQualityBoost = () => {
        return has('herb_guide') ? 1 : 0;
    };
    /** 挖矿体力减免 (0.15 = 15%) */
    const getMiningStaminaReduction = () => {
        return has('miners_charm') ? 0.15 : 0;
    };
    /** 钓鱼calm概率加成 */
    const getFishingCalmBonus = () => {
        return has('anglers_token') ? 0.1 : 0;
    };
    /** 烹饪恢复量加成 (0.25 = 25%) */
    const getCookingRestoreBonus = () => {
        return has('chefs_hat') ? 0.25 : 0;
    };
    /** 作物生长速度加成 (0.1 = 10%) */
    const getCropGrowthBonus = () => {
