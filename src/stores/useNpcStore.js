...
export const useNpcStore = defineStore('npc', () => {
    const npcStates = ref(NPCS.map(npc => ({
        npcId: npc.id,
        friendship: 0,
        talkedToday: false,
        giftedToday: false,
    ...
    /** 每日提示NPC是否已给过提示 */
    const tipGivenToday = ref({});
    /** 子女列表 */
    const children = ref([]);
    /** 子女ID自增计数器（避免释放后ID冲突） */
    const nextChildId = ref(0);
    /** 结婚天数计数 */
    const daysMarried = ref(0);
    /** 知己天数计数 */
    const daysZhiji = ref(0);
    /** 孕期状态（null = 无孕期） */
    const pregnancy = ref(null);
    /** 配偶是否已提议要孩子（等待玩家回应） */
    const childProposalPending = ref(false);
    /** 提议被拒绝次数（影响再次提议冷却） */
    const childProposalDeclinedCount = ref(0);
    /** 距上次拒绝/等待的天数 */
    const daysSinceProposalDecline = ref(0);
    /** 婚礼倒计时 (0=无婚礼待举行) */
    const weddingCountdown = ref(0);
    /** 婚礼对象NPC ID */
    const weddingNpcId = ref(null);
    // ============================================================
    // 雇工系统
    // ============================================================
    const hiredHelpers = ref([]);
    const MAX_HELPERS = 2;
    /** 雇工日薪 */
    const HELPER_WAGES = {
        water: 100,
    ...
    const getHireableNpcs = () => {
        return npcStates.value
            .filter(s => {
            if (s.friendship < 1000)
                return false;
            if (s.married || s.zhiji)
                return false;
            if (hiredHelpers.value.some(h => h.npcId === s.npcId))
                return false;
            return true;
        })
            .map(s => {
    ...
    /** 解雇 */
    const dismissHelper = (npcId) => {
        const idx = hiredHelpers.value.findIndex(h => h.npcId === npcId);
        if (idx < 0)
            return { success: false, message: '此人未被雇佣。' };
        const npcDef = getNpcById(npcId);
        const name = npcDef?.name ?? npcId;
    ...
    /** 每日雇工结算（useEndDay调用） */
    const processDailyHelpers = (taskFilter) => {
        ...
        for (const helper of [...hiredHelpers.value]) {
            // 按任务类型过滤
            if (taskFilter && !taskFilter.includes(helper.task))
                continue;
            ...
            switch (helper.task) {
                case 'water': {
                    ...
                    for (let i = 0; i < count; i++)
                        farmStore.waterPlot(unwatered[i].id);
                    if (count > 0)
                        messages.push(`${name}帮你浇了${count}块地。(-${helper.dailyWage}文)`);
                    else
                ...
                case 'harvest': {
                    ...
                    if (harvested > 0)
                        messages.push(`${name}帮你收了${harvested}块地的庄稼。(-${helper.dailyWage}文)`);
                    else
                ...
                case 'weed': {
                    ...
                    if (cleared > 0)
                        messages.push(`${name}清理了${cleared}处杂草和虫害。(-${helper.dailyWage}文)`);
                    else
                ...
                case 'bait': {
                    ...
                    if (baited > 0)
                        messages.push(`${name}帮你给${baited}个蟹笼装了饵。(-${helper.dailyWage}文)`);
                    else
    ...
    const getFriendshipLevel = (npcId) => {
        ...
        for (const t of FRIENDSHIP_THRESHOLDS) {
            if (state.friendship >= t.min)
                return t.level;
        }
        return 'stranger';
    };
    /** 检查NPC今天是否生日 */
    const isBirthday = (npcId) => {
        const npcDef = getNpcById(npcId);
        if (!npcDef?.birthday)
            return false;
    ...
    const checkHeartEvent = (npcId) => {
        ...
        for (const event of events) {
            // 知己事件仅知己触发
            if (event.requiresZhiji && !state.zhiji)
                continue;
            // 知己不触发恋爱告白（heart_8）
    ...
    /** 标记心事件为已触发 */
    const markHeartEventTriggered = (npcId, eventId) => {
        const state = getNpcState(npcId);
    ...
    /** 调整好感度（心事件选择结果） */
    const adjustFriendship = (npcId, amount) => {
        const state = getNpcState(npcId);
        if (state) {
    ...
    /** 替换对话中的占位符 */
    const replaceDialoguePlaceholders = (text) => {
        const playerStore = usePlayerStore();
    ...
    /** 送礼给NPC (每天1次, 每周2次) */
    const giveGift = (npcId, itemId, giftBonusMultiplier = 1, quality = 'normal') => {
        const state = getNpcState(npcId);
        if (!state)
            return null;
        ...
        // 品质加成
        const qualityMultiplier = { normal: 1.0, fine: 1.25, excellent: 1.5, supreme: 2.0 };
        // 生日加成 (4倍)
    ...
    const startDating = (npcId) => {
        ...
        if (state.dating)
            return { success: false, message: '你们已经在约会了。' };
        if (state.married)
            return { success: false, message: '你们已经结婚了。' };
    ...
    const propose = (npcId) => {
        ...
        // 需要先约会
        if (!state.dating)
            return { success: false, message: '需要先赠帕约会。' };
    ...
    const becomeZhiji = (npcId) => {
        ...
        if (state.zhiji)
            return { success: false, message: '你们已经是知己了。' };
    ...
    const dissolveZhiji = () => {
        ...
        if (!zhijiState)
            return { success: false, message: '你还没有知己。' };
        const playerStore = usePlayerStore();
    ...
    /** 放生子女 */
    const releaseChild = (childId) => {
        const child = children.value.find(c => c.id === childId);
        if (!child)
            return { success: false, message: '找不到这个孩子。' };
        const playerStore = usePlayerStore();
        if (!playerStore.spendMoney(10000)) {
            return { success: false, message: '金钱不足（需要10000文）。' };
        }
    ...
    // 孕期养成系统
    // ============================================================
    const PREGNANCY_STAGE_CONFIG = {
        early: { days: 5, label: '初期' },
        mid: { days: 5, label: '中期' },
        late: { days: 5, label: '后期' },
        ready: { days: 3, label: '待产期' }
    };
    const STAGE_ORDER = ['early', 'mid', 'late', 'ready'];
    const MEDICAL_PLANS = {
    ...
    /** 检查配偶是否应提议要孩子（每日调用） */
    const checkChildProposal = () => {
        const spouse = getSpouse();
        if (!spouse)
            return false;
        if (children.value.length >= 2)
            return false;
        if (pregnancy.value !== null)
            return false;
        if (childProposalPending.value)
            return false;
        if (daysMarried.value < 7)
            return false;
        if (spouse.friendship < 3000)
            return false;
        // 拒绝冷却：7天基础 + 每次拒绝额外7天
        if (childProposalDeclinedCount.value > 0) {
            const cooldownDays = 7 + childProposalDeclinedCount.value * 7;
            if (daysSinceProposalDecline.value < cooldownDays)
                return false;
        }
    ...
    /** 玩家回应提议 */
    const respondToChildProposal = (response) => {
        childProposalPending.value = false;
        const spouse = getSpouse();
        switch (response) {
            case 'accept':
    ...
    /** 孕期照料操作 */
    const performPregnancyCare = (action) => {
        if (!pregnancy.value)
    ...
    /** 选择接生方式（仅待产期） */
    const chooseMedicalPlan = (plan) => {
        if (!pregnancy.value)
            return { success: false, message: '没有待产。' };
    ...
    const handleDelivery = () => {
        ...
        // 根据安产分决定出生品质
        const birthQuality = pregnancy.value.careScore >= 80 ? 'healthy' : pregnancy.value.careScore < 40 ? 'premature' : 'normal';
    ...
    /** 与子女互动 */
    const interactWithChild = (childId) => {
        const child = children.value.find(c => c.id === childId);
        if (!child)
            return null;
        if (child.interactedToday)
            return null;
        if (child.stage === 'baby')
            return null;
    ...
    /** 检查NPC是否有每日提示功能 */
    const hasDailyTip = (npcId) => {
        return TIP_NPC_IDS.includes(npcId);
    };
    /** 检查NPC今天是否已给过提示 */
    const isTipGivenToday = (npcId) => {
        return tipGivenToday.value[npcId] ?? false;
    };
    /** 获取NPC的每日提示 */
    const getDailyTip = (npcId) => {
        if (!hasDailyTip(npcId))
            return null;
        if (tipGivenToday.value[npcId])
            return null;
    ...
    const deserialize = (data) => {
        const isOldScale = !data.friendshipVersion || data.friendshipVersion < 2;
        ...
        nextChildId.value =
            data.nextChildId ?? (children.value.length > 0 ? Math.max(...children.value.map((c) => c.id)) + 1 : 0);
        daysMarried.value = data.daysMarried ?? 0;
        daysZhiji.value = data.daysZhiji ?? 0;
        // 新孕期系统
        pregnancy.value = data.pregnancy ?? null;
        childProposalPending.value = data.childProposalPending ?? false;
        childProposalDeclinedCount.value = data.childProposalDeclinedCount ?? 0;
        daysSinceProposalDecline.value = data.daysSinceProposalDecline ?? 0;
        // 旧存档迁移：pendingChild → pregnancy
        if (data.pendingChild && !pregnancy.value) {
            const oldCountdown = data.childCountdown ?? 0;
            let stage = 'early';
            if (oldCountdown <= 3)
                stage = 'ready';
            else if (oldCountdown <= 8)
                stage = 'late';
            else if (oldCountdown <= 13)
                stage = 'mid';
            pregnancy.value = {
                stage,
                daysInStage: 0,
        ...
        weddingCountdown.value = data.weddingCountdown ?? 0;
        weddingNpcId.value = data.weddingNpcId ?? null;
        hiredHelpers.value = data.hiredHelpers ?? [];
    };
    return {
        npcStates,
        children,
