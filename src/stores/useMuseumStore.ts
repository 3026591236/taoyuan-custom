...
export const useMuseumStore = defineStore("museum", () => {
  ...
  const themeProgress = computed(() =>
    MUSEUM_THEME_COLLECTIONS.map((t) => ({
      ...t,
      donated: t.itemIds.filter((id) => isDonated(id)).length,
      total: t.itemIds.length,
      completed: t.itemIds.every((id) => isDonated(id)),
      claimed: claimedThemes.value.includes(t.id),
    })),
  );
  const restorationProjects = computed(() =>
    MUSEUM_RESTORATIONS.map((r) => ({
      ...r,
      restored: restoredProjects.value.includes(r.id),
      unlocked: donatedCount.value >= r.requiredDonations,
    })),
  );
  const museumLevel = computed(() =>
    Math.min(
      8,
      Math.floor(
        (fame.value +
          donatedCount.value * 8 +
          restoredProjects.value.length * 25) /
          120,
      ) + 1,
    ),
  );
  const exhibitionRate = computed(
    () =>
      EXHIBITIONS.find((e) => e.id === activeExhibition.value)?.incomeRate ?? 1,
  );
  const dailyIncome = computed(() =>
    Math.floor(
      (donatedCount.value * 18 +
        fame.value * 2.5 +
        restoredProjects.value.length * 120 +
        claimedThemes.value.length * 80) *
        exhibitionRate.value,
    ),
  );
  const exhibitionCards = computed(() =>
    EXHIBITIONS.map((e) => ({
      ...e,
      active: activeExhibition.value === e.id,
      unlocked: fame.value >= e.fameNeed,
    })),
  );
  ...
  const claimableMilestones = computed(() => {
    return MUSEUM_MILESTONES.filter(
      (m) =>
        donatedCount.value >= m.count &&
        !claimedMilestones.value.includes(m.count),
    );
  });
  ...
  const claimThemeReward = (themeId: string): boolean => {
    ...
    for (const item of theme.reward.items ?? [])
      inventoryStore.addItem(item.itemId, item.quantity);
  ...
  const canRestore = (projectId: string): boolean => {
    ...
    if (
      !project ||
      restoredProjects.value.includes(projectId) ||
      donatedCount.value < project.requiredDonations
    )
      return false;
    const playerStore = usePlayerStore();
    const inventoryStore = useInventoryStore();
    if (playerStore.money < project.cost.money) return false;
    return (project.cost.items ?? []).every(
      (item) => inventoryStore.getItemCount(item.itemId) >= item.quantity,
    );
  };
  ...
  const restoreProject = (projectId: string): boolean => {
    ...
    for (const item of project.cost.items ?? [])
      inventoryStore.removeItem(item.itemId, item.quantity);
    fame.value += project.reward.fame;
    if (project.reward.itemId)
      inventoryStore.addItem(
        project.reward.itemId,
        project.reward.quantity ?? 1,
      );
  ...
  const serialize = () => ({
    ...
    activeExhibition: activeExhibition.value,
  });
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    fame.value =
      (data as any).fame ?? Math.floor(donatedItems.value.length * 3);
    lastIncomeDay.value = (data as any).lastIncomeDay ?? "";
    activeExhibition.value = ((data as any).activeExhibition ??
      "farm_memory") as ExhibitionId;
  };
