...
import { POND_BUILD_COST, POND_UPGRADE_COSTS, POND_CAPACITY, WATER_QUALITY_DECAY_BASE, WATER_QUALITY_DECAY_HALF, WATER_QUALITY_DECAY_CROWDED, WATER_QUALITY_DECAY_HUNGRY, DISEASE_THRESHOLD, DISEASE_CHANCE_BASE, SICK_DEATH_DAYS, FEED_WATER_RESTORE, PURIFIER_WATER_RESTORE, FISH_BREEDING_DAYS, GENETICS_FLUCTUATION_BASE, POND_MUTATION_JUMP_MIN, POND_MUTATION_JUMP_MAX, getPondableFish, isPondableFish } from '@/data/fishPond';
...
export const useFishPondStore = defineStore('fishPond', () => {
    ...
    /** 已发现的品种ID集合（图鉴） */
    const discoveredBreeds = ref(new Set());
    // === 计算属性 ===
    ...
    const buildPond = () => {
        ...
        if (!playerStore.spendMoney(POND_BUILD_COST.money))
            return false;
        for (const mat of POND_BUILD_COST.materials) {
    ...
    const upgradePond = () => {
        if (!pond.value.built)
            return false;
        if (pond.value.level >= 3)
            return false;
        const nextLevel = (pond.value.level + 1);
    ...
    const addFish = (fishId, quantity = 1) => {
        if (!pond.value.built)
            return 0;
        if (!isPondableFish(fishId))
            return 0;
        ...
        for (let i = 0; i < quantity; i++) {
            if (fishCount.value >= capacity.value)
                break;
            if (!inventoryStore.removeItem(fishId, 1))
                break;
            ...
            if (breed)
                discoveredBreeds.value.add(breed.breedId);
            added++;
        }
        return added;
    };
    /** 从塘中取鱼回背包 */
    const removeFish = (pondFishId) => {
        const idx = pond.value.fish.findIndex(f => f.id === pondFishId);
        if (idx === -1)
            return false;
        const fish = pond.value.fish[idx];
        const inventoryStore = useInventoryStore();
        if (inventoryStore.isAllFull && !inventoryStore.items.some(s => s.itemId === fish.fishId && s.quantity < 999))
            return false;
    ...
    // === 喂食/清理/治疗 ===
    const feedFish = () => {
        if (!pond.value.built || pond.value.fedToday)
            return false;
        if (pond.value.fish.length === 0)
            return false;
        const inventoryStore = useInventoryStore();
        if (!inventoryStore.removeItem('fish_feed', 1))
            return false;
    ...
    const cleanPond = () => {
        if (!pond.value.built)
            return false;
        const inventoryStore = useInventoryStore();
        if (!inventoryStore.removeItem('water_purifier', 1))
            return false;
    ...
    // === 繁殖 ===
    const startBreeding = (fishIdA, fishIdB) => {
        if (!pond.value.built)
            return false;
        ...
        if (!fishA || !fishB)
            return false;
        if (fishA.fishId !== fishB.fishId)
            return false;
        if (!fishA.mature || !fishB.mature)
            return false;
        if (fishA.sick || fishB.sick)
            return false;
        pond.value.breeding = {
            parentA: fishIdA,
            parentB: fishIdB,
    ...
    // === 产出品质 ===
    const _getProductQuality = (qualityGene) => {
        const roll = Math.random() * 100;
        if (qualityGene >= 75 && roll < qualityGene - 50)
            return 'supreme';
        if (qualityGene >= 50 && roll < qualityGene - 25)
            return 'excellent';
        if (qualityGene >= 25 && roll < qualityGene)
            return 'fine';
        return 'normal';
    };
    // === 收获 ===
    ...
    const collectProducts = () => {
        if (!pond.value.built || pond.value.collectedToday)
            return [];
    ...
    const dailyUpdate = () => {
        ...
        if (density.value > 0.8)
            decay += WATER_QUALITY_DECAY_CROWDED;
        else if (density.value > 0.5)
            decay += WATER_QUALITY_DECAY_HALF;
        // 2. 未喂食额外衰减
                ...
                if (!parentA || !parentB) {
                    ...
                    if (def) {
                        ...
                        if (!childBreedId) {
                            ...
                            const parentGen = Math.min(parentABreed?.generation ?? 1, parentBBreed?.generation ?? 1);
    ...
    // === 基因星级 ===
    const getGeneticStarRating = (genetics) => {
