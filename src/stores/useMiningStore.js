...
import { getFloor, getRewardNames, getInfestedClearRewards, BOSS_MONSTERS, BOSS_MONEY_REWARDS, BOSS_ORE_REWARDS, getWeakenedBoss, MAX_MINE_FLOOR, generateSkullCavernFloor, scaleMonster, generateFloorGrid, getAdjacentIndices, getBombIndices } from '@/data';
import { getBombById } from '@/data/processing';
import { getItemById } from '@/data/items';
import { getWeaponById, getEnchantmentById, MONSTER_DROP_WEAPONS, BOSS_DROP_WEAPONS, TREASURE_DROP_WEAPONS, rollRandomEnchantment, getWeaponDisplayName, getWeaponSellPrice } from '@/data/weapons';
...
export const useMiningStore = defineStore('mining', () => {
    ...
    const cachedSkullFloorData = ref(null);
    /** 战斗状态 */
    const inCombat = ref(false);
    const combatMonster = ref(null);
    ...
    // ==================== 骷髅矿穴辅助 ====================
    /** 骷髅矿穴是否已解锁（击败60层BOSS） */
    const isSkullCavernUnlocked = () => {
        return defeatedBosses.value.includes('lava_lord');
    };
    /** 获取当前活跃楼层号 */
    const getActiveFloorNum = () => {
    ...
    const engageRevealedMonster = (index) => {
        if (!isExploring.value)
            return { success: false, message: '你不在矿洞中。', startsCombat: false };
        if (inCombat.value)
            return { success: false, message: '战斗中无法探索。', startsCombat: false };
        const tile = floorGrid.value[index];
        if (!tile || tile.state !== 'revealed')
            return { success: false, message: '无法交战。', startsCombat: false };
        if (tile.type !== 'monster' && tile.type !== 'boss')
            return { success: false, message: '该格子没有怪物。', startsCombat: false };
    ...
    const revealTile = (index) => {
        ...
        const tile = floorGrid.value[index];
        if (!tile || tile.state !== 'hidden')
            return { success: false, message: '无法翻开该格子。', startsCombat: false };
        if (!canRevealTile(index))
            return { success: false, message: '只能翻开已探索格子的相邻位置。', startsCombat: false };
        // 检查镐是否可用（未在升级中）
        ...
        const staminaCost = Math.max(1, Math.floor(2 *
            pickaxeMultiplier *
    ...
    /** 处理空格子 */
    const _handleEmptyTile = (tile, staminaCost) => {
        tile.state = 'revealed';
        return { success: true, message: `探索了一个空区域。(-${staminaCost}体力)`, startsCombat: false };
    };
    /** 处理矿石格子 */
    const _handleOreTile = (tile, staminaCost) => {
        ...
        if (gameStore.farmMapType === 'hilltop' && Math.random() < 0.5)
            quantity += 1;
        // 探矿者专精：15% 概率双倍
        if (skillStore.getSkill('mining').perk10 === 'prospector' && Math.random() < 0.15)
            quantity *= 2;
        // 戒指矿石加成
        const ringOreBonus = inventoryStore.getRingEffectValue('ore_bonus');
        if (ringOreBonus > 0)
            quantity += Math.floor(ringOreBonus);
        // 仙缘能力：灵狐眼（hu_xian_2）15%概率额外掉落矿石
        if (useHiddenNpcStore().isAbilityActive('hu_xian_2') && Math.random() < 0.15)
            quantity += 1;
    ...
    /** 处理怪物格子 */
    const _handleMonsterTile = (tile, staminaCost) => {
        const monster = tile.data?.monster;
        if (!monster) {
            tile.state = 'revealed';
            return { success: true, message: '空无一物。', startsCombat: false };
    ...
    /** 处理 BOSS 格子 */
    const _handleBossTile = (tile, staminaCost) => {
        const monster = tile.data?.monster;
        if (!monster) {
            tile.state = 'revealed';
            return { success: true, message: '空无一物。', startsCombat: false };
    ...
    /** 处理楼梯格子 */
    const _handleStairsTile = (tile, staminaCost) => {
        tile.state = 'revealed';
    ...
    /** 处理陷阱格子 */
    const _handleTrapTile = (tile, staminaCost) => {
    ...
    /** 处理宝箱格子 */
    const _handleTreasureTile = (tile, staminaCost) => {
        ...
        let msg = '发现宝箱！';
        if (items.length > 0)
            msg += `获得了${getRewardNames(items)}`;
        if (money > 0)
            msg += `${items.length > 0 ? '和' : '获得了'}${money}文`;
        if (autoSoldMoney > 0)
            msg += `（重复装备自动售出+${autoSoldMoney}文）`;
        msg += `！(-${staminaCost}体力)`;
        return { success: true, message: msg, startsCombat: false };
    };
    /** 处理蘑菇格子 */
    const _handleMushroomTile = (tile, staminaCost) => {
    ...
    // ==================== 炸弹 ====================
    /** 在格子上使用炸弹 */
    const useBombOnGrid = (bombId, centerIndex) => {
        if (!isExploring.value)
        ...
        if (!bombDef)
            return { success: false, message: '无效的炸弹。' };
        ...
        if (oreCollected > 0)
            msg += `采集了${oreCollected}份矿石`;
        if (monstersKilled > 0)
            msg += `${oreCollected > 0 ? '，' : ''}击败了${monstersKilled}只怪物`;
        if (oreCollected === 0 && monstersKilled === 0)
            msg += '翻开了一些区域';
        msg += '！';
        if (excavatorSaved)
            msg += '（挖掘者：炸弹未消耗！）';
    ...
    // ==================== 进入 / 离开 ====================
    /** 进入矿洞（可选择起始安全点楼层） */
    const enterMine = (startFromSafePoint) => {
    ...
    /** 进入骷髅矿穴（可选择起始安全点楼层） */
    const enterSkullCavern = (startFromSafePoint) => {
        if (!isSkullCavernUnlocked())
            return '需要先击败60层BOSS才能进入骷髅矿穴。';
        isExploring.value = true;
    ...
    /** 获取所有已解锁的安全点（用于楼层选择） */
    const getUnlockedSafePoints = () => {
        const points = [0]; // 0 = 从第1层开始
    ...
    /** 获取骷髅矿穴已解锁的安全点 */
    const getUnlockedSkullSafePoints = () => {
        const points = [0]; // 0 = 从第1层开始
    ...
    const combatAction = (action) => {
        ...
        if (action === 'defend') {
            ...
            const damage = Math.max(1, Math.floor(monster.attack *
                (1 - tankReduction) *
                (1 - defenseReduction) *
                sturdyReduction *
                (1 - ringDefenseBonus) *
        ...
        const sturdyReduction = enchant?.special === 'sturdy' ? 0.85 : 1.0;
        const ringDefenseBonus = inventoryStore.getRingEffectValue('defense_bonus');
        const monsterDamage = Math.max(1, Math.floor(monster.attack *
            (1 - fighterReduction) *
            (1 - defenseReduction) *
            sturdyReduction *
            (1 - ringDefenseBonus) *
    ...
    /** 处理怪物击败（普通怪和 BOSS 共用） */
    const handleMonsterDefeat = (monster, msg, _totalDamage) => {
        inCombat.value = false;
        // 经验
        const floor = getActiveFloorData();
        const wildernessXpBonus = useGameStore().farmMapType === 'wilderness' ? 1.5 : 1.0;
        ...
        const luckyBonus = (enchant?.special === 'lucky' ? 0.2 : 0) +
            ringDropBonus +
            ringLuckBonus * 0.5 +
        ...
        if (!combatIsBoss.value && floor) {
            ...
            if (monsterAutoSold > 0)
                msg += `（重复装备自动售出+${monsterAutoSold}文）`;
        }
        // BOSS 击败处理
        if (combatIsBoss.value) {
            if (isInSkullCavern.value) {
    ...
    const handleDefeat = () => {
        ...
        for (let i = 0; i < lostCount; i++) {
            ...
            if (item)
                inventoryStore.removeItem(item.itemId, item.quantity);
        }
        // 随机丢失最多3件背包物品
        const droppedItems = [];
        ...
        if (moneyLost > 0)
            playerStore.spendMoney(moneyLost);
        // HP 恢复到50%
        ...
        const parts = [`你在${location}中倒下了……`];
        parts.push('丢失了一半战利品');
        if (droppedItems.length > 0)
            parts.push(`和${droppedItems.length}件背包物品`);
        if (moneyLost > 0)
            parts.push(`及${moneyLost}文`);
        parts.push('，被送回入口。');
    ...
    // ==================== 道具使用 ====================
    /** 在战斗/探索中使用道具 */
    const useCombatItem = (itemId, quantity = 1) => {
        ...
        if (itemId === 'guild_badge') {
            ...
            if (actual <= 0)
                return { success: false, message: '没有公会徽章。' };
        ...
        if (itemId === 'life_talisman') {
            ...
            if (actual <= 0)
                return { success: false, message: '没有生命护符。' };
        ...
        if (itemId === 'lucky_coin') {
            ...
            if (actual <= 0)
                return { success: false, message: '没有幸运铜钱。' };
        ...
        if (itemId === 'defense_charm') {
            ...
            if (actual <= 0)
                return { success: false, message: '没有守护符。' };
        ...
        if (itemId === 'slayer_charm') {
            if (slayerCharmActive.value)
                return { success: false, message: '猎魔符效果已激活。' };
            if (!inventoryStore.removeItem('slayer_charm'))
                return { success: false, message: '没有猎魔符。' };
        ...
        if (itemId.startsWith('food_')) {
            ...
            if (result.success && inCombat.value)
                combatLog.value.push(result.message);
            return result;
        }
        const hpFull = playerStore.hp >= playerStore.getMaxHp();
        const staminaFull = playerStore.stamina >= playerStore.maxStamina;
    ...
    /** 在探索中使用怪物诱饵（本层怪物数量翻倍） */
    const useMonsterLure = () => {
        if (!isExploring.value)
            return { success: false, message: '不在矿洞中。' };
        if (inCombat.value)
            return { success: false, message: '战斗中无法使用怪物诱饵。' };
        if (!inventoryStore.removeItem('monster_lure'))
            return { success: false, message: '没有怪物诱饵。' };
        const floor = getActiveFloorData();
        if (!floor)
            return { success: true, message: '使用了怪物诱饵，但本层无效。' };
        // 统计现有未击败的怪物数量
    ...
    const deserialize = (data) => {
        defeatedBosses.value = data.defeatedBosses ?? [];
        // 检测旧存档（30层系统）并迁移
        const rawSafePoint = data.safePointFloor ?? 0;
        ...
        // 骷髅矿穴状态
        isInSkullCavern.value = data.isInSkullCavern ?? false;
        skullCavernFloor.value = data.skullCavernFloor ?? 0;
        skullCavernBestFloor.value = data.skullCavernBestFloor ?? 0;
        skullSafePointFloor.value = data.skullSafePointFloor ?? 0;
        // 格子状态不序列化——读档后玩家在矿洞外
        isExploring.value = false;
        floorGrid.value = [];
        // 公会徽章永久加成
        guildBadgeBonusAttack.value = data.guildBadgeBonusAttack ?? 0;
        guildBonusMaxHp.value = data.guildBonusMaxHp ?? 0;
        guildBonusDropRate.value = data.guildBonusDropRate ?? 0;
        guildBonusDefense.value = data.guildBonusDefense ?? 0;
    };
    return {
        currentFloor,
        safePointFloor,
