...
export const useMiningStore = defineStore("mining", () => {
  ...
  const _generateGrid = () => {
    ...
    const scaleFactor = isInSkullCavern.value
      ? (cachedSkullFloorData.value?.scaleFactor ?? 1)
      : 1;
    ...
    if (floor.specialType === "boss" && !isInSkullCavern.value) {
      ...
      const isFirstKill = bossId
        ? !defeatedBosses.value.includes(bossId)
        : true;
      if (!isFirstKill) {
        // 弱化版 BOSS — 需要在格子生成后替换
        // generateFloorGrid 会使用原始 BOSS，我们在这里覆盖
        const result = generateFloorGrid(
          floorForGrid,
          floorNum,
  ...
  const engageRevealedMonster = (
    ...
    if (tile.type === "boss") {
      ...
      combatLog.value = [
        `BOSS战！再次挑战${monster.name}！(HP: ${monster.hp})${isFirstKill ? "" : "（弱化版）"}`,
      ];
  ...
  const _handleOreTile = (
    ...
    // 探矿者专精：15% 概率双倍
    if (
      skillStore.getSkill("mining").perk10 === "prospector" &&
      Math.random() < 0.15
    )
      quantity *= 2;
    // 戒指矿石加成
    ...
    if (
      useHiddenNpcStore().isAbilityActive("hu_xian_2") &&
      Math.random() < 0.15
    )
      quantity += 1;
  ...
  const _handleMonsterTile = (
    ...
    combatRound.value = 0;
    combatLog.value = [
      `遭遇了${monster.name}！(HP: ${monster.hp})  (-${staminaCost}体力)`,
    ];
    combatIsBoss.value = false;
    inCombat.value = true;
  ...
  const _handleBossTile = (
    ...
    const isFirstKill = !defeatedBosses.value.includes(monster.id);
    combatLog.value = [
      `BOSS战！遭遇了${monster.name}！(HP: ${monster.hp})${isFirstKill ? "" : "（弱化版）"}  (-${staminaCost}体力)`,
    ];
    combatIsBoss.value = true;
    inCombat.value = true;
  ...
  const _handleStairsTile = (
    ...
    if (!stairsUsable.value) {
      ...
      if (floor?.specialType === "infested") {
        const remaining =
          totalMonstersOnFloor.value - monstersDefeatedCount.value;
        return {
          success: true,
  ...
  const _handleTreasureTile = (
    ...
    if (treasureRings) {
      const ringTreasureBonus =
        inventoryStore.getRingEffectValue("treasure_find");
  ...
  const useBombOnGrid = (
    ...
    const excavatorSaved =
      skillStore.getSkill("mining").perk10 === "excavator" &&
      Math.random() < 0.3;
    if (excavatorSaved) {
      ...
      switch (tile.type) {
        ...
        case "monster": {
          if (bombDef.clearsMonster && tile.data?.monster) {
            // 炸弹击杀怪物：50% 经验
            const monster = tile.data.monster;
            const wildernessXpBonus =
              useGameStore().farmMapType === "wilderness" ? 1.5 : 1.0;
            skillStore.addExp(
              "combat",
              Math.floor(monster.expReward * 0.5 * wildernessXpBonus),
            );
            // 普通掉落（概率减半）
  ...
  const combatAction = (
    ...
    if (action === "defend") {
      ...
      const defenseReduction =
        cookingStore.activeBuff?.type === "defense"
          ? cookingStore.activeBuff.value / 100
          : 0;
      const tankReduction =
        skillStore.getSkill("combat").perk10 === "tank" ? 0.7 : 0.6;
      // 坚韧附魔
      const owned = inventoryStore.getEquippedWeapon();
      const enchant = owned.enchantmentId
        ? getEnchantmentById(owned.enchantmentId)
        : null;
      const sturdyReduction = enchant?.special === "sturdy" ? 0.85 : 1.0;
      const ringDefenseBonus =
        inventoryStore.getRingEffectValue("defense_bonus");
    ...
    const bruteBonus =
      skillStore.getSkill("combat").perk10 === "brute" ? 1.25 : 1.0;
    ...
    const critRate =
      inventoryStore.getWeaponCritRate() + ringCritBonus + ringLuck * 0.5;
    const isCrit = Math.random() < critRate;
    const critMult = isCrit ? 1.5 : 1.0;

    const damageToMonster = Math.max(
      1,
      Math.floor((baseAttack - monster.defense) * bruteBonus * critMult),
    );
    ...
    const totalVampiric =
      (enchant?.special === "vampiric" ? 0.15 : 0) + ringVampiric;
    if (totalVampiric > 0) {
      const healAmount = Math.floor(
        (totalDamageDealt + extraDamage) * totalVampiric,
      );
      if (healAmount > 0) {
    ...
    // 杂技师专精：25% 概率闪避反击
    if (
      skillStore.getSkill("combat").perk10 === "acrobat" &&
      Math.random() < 0.25
    ) {
    ...
    const defenseReduction =
      cookingStore.activeBuff?.type === "defense"
        ? cookingStore.activeBuff.value / 100
        : 0;
    const fighterReduction =
      skillStore.getSkill("combat").perk5 === "fighter" ? 0.15 : 0;
    const sturdyReduction = enchant?.special === "sturdy" ? 0.85 : 1.0;
  ...
  const handleMonsterDefeat = (
    ...
    skillStore.addExp(
      "combat",
      Math.floor(monster.expReward * wildernessXpBonus * infestedXpBonus),
    );
    ...
    if (skillStore.getSkill("mining").perk10 === "mineralogist") {
      if (floor && floor.ores.length > 0) {
        const bonusOre =
          floor.ores[Math.floor(Math.random() * floor.ores.length)]!;
  ...
  const handleDefeat = (): {
    ...
    const moneyLost = Math.min(
      Math.floor(playerStore.money * DEFEAT_MONEY_PENALTY_RATE),
      DEFEAT_MONEY_PENALTY_CAP,
    );
  ...
  const goNextFloor = (): { success: boolean; message: string } => {
    ...
    if (isInSkullCavern.value) {
      ...
      if (
        skullFloor?.isSafePoint &&
        skullCavernFloor.value > skullSafePointFloor.value
      ) {
      ...
      if (
        newFloorData?.isSafePoint &&
        currentFloor.value > safePointFloor.value
      ) {
    ...
    const specialLabel = newFloor?.specialType
      ? (specialLabels[newFloor.specialType] ?? "")
      : "";
  ...
  const useCombatItem = (
    ...
    if (itemId.startsWith("food_")) {
      ...
      const foodQuality =
        qualityOrder.find((q) => inventoryStore.getItemCount(itemId, q) > 0) ??
        "normal";
    ...
    // 炼金师专精：食物恢复+50%
    const alchemistBonus =
      skillStore.getSkill("foraging").perk10 === "alchemist" ? 1.5 : 1.0;
    const parts: string[] = [];
    if (hasHpRestore) {
      const restore =
        def.healthRestore! >= 999
          ? playerStore.getMaxHp()
          : Math.floor(def.healthRestore! * alchemistBonus);
  ...
  const useMonsterLure = (): { success: boolean; message: string } => {
    ...
    // 统计现有未击败的怪物数量
    const existingMonsters = floorGrid.value.filter(
      (t) =>
        (t.type === "monster" || t.type === "boss") && t.state !== "defeated",
    ).length;

    // 找到所有隐藏的空格子
    const hiddenEmpty = floorGrid.value.filter(
      (t) => t.state === "hidden" && t.type === "empty",
    );
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    defeatedBosses.value =
      ((data as Record<string, unknown>).defeatedBosses as string[]) ?? [];

    // 检测旧存档（30层系统）并迁移
    const rawSafePoint =
      ((data as Record<string, unknown>).safePointFloor as number) ?? 0;
    ...
    if (isOldSave) {
      ...
      currentFloor.value =
        safePointFloor.value > 0 ? safePointFloor.value + 1 : 1;
    } else {
    ...
    skullCavernFloor.value =
      ((data as Record<string, unknown>).skullCavernFloor as number) ?? 0;
    skullCavernBestFloor.value =
      ((data as Record<string, unknown>).skullCavernBestFloor as number) ?? 0;
    ...
    guildBonusMaxHp.value =
      ((data as Record<string, unknown>).guildBonusMaxHp as number) ?? 0;
    guildBonusDropRate.value =
      ((data as Record<string, unknown>).guildBonusDropRate as number) ?? 0;
    guildBonusDefense.value =
      ((data as Record<string, unknown>).guildBonusDefense as number) ?? 0;
  };
