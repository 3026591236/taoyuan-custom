...
export const usePlayerStore = defineStore("player", () => {
  ...
  const getAttributeExpRequired = (key: AttributeKey): number => {
    ...
    return (
      ATTRIBUTE_EXP_BASE + (level - ATTRIBUTE_BASE_LEVEL) * ATTRIBUTE_EXP_STEP
    );
  };

  const addAttributeExp = (
    key: AttributeKey,
    amount: number,
  ): { leveledUp: boolean; newLevel: number } => {
    const attr = attributes.value[key];
    if (!attr || amount <= 0)
      return { leveledUp: false, newLevel: ATTRIBUTE_BASE_LEVEL };
    if (attr.level >= ATTRIBUTE_MAX_LEVEL)
      return { leveledUp: false, newLevel: attr.level };
  ...
  const addAttributeExpBatch = (
    ...
    for (const key of ATTRIBUTE_KEYS) {
      ...
      if (result.leveledUp)
        messages.push(`${ATTRIBUTE_NAMES[key]}提升到${result.newLevel}`);
    }
    return messages;
  };
  ...
  const attributePower = computed(() =>
    ATTRIBUTE_KEYS.reduce(
      (sum, key) =>
        sum + (attributes.value[key]?.level ?? ATTRIBUTE_BASE_LEVEL),
      0,
    ),
  );
  ...
  const attributeAttackBonus = computed(() =>
    Math.floor(
      (attributes.value.strength.level - 1) * 2.4 +
        (attributes.value.perception.level - 1) * 1.0,
    ),
  );
  ...
  const attributeDefenseBonus = computed(() =>
    Math.min(
      0.45,
      (attributes.value.physique.level - 1) * 0.008 +
        (attributes.value.agility.level - 1) * 0.006,
    ),
  );

  const attributeMaxHpBonus = computed(
    () => (attributes.value.physique.level - 1) * 10,
  );

  const attributeSpeedBonus = computed(() =>
    Math.floor((attributes.value.agility.level - 1) * 0.8),
  );

  const attributeCombatPower = computed(
    () =>
      attributePower.value * 12 +
      attributeAttackBonus.value * 8 +
      attributeMaxHpBonus.value +
      attributeSpeedBonus.value * 5 +
  ...
  const getMaxHp = (): number => {
    ...
    let bonus =
      skillStore.combatLevel * HP_PER_COMBAT_LEVEL + attributeMaxHpBonus.value;
  ...
  const consumeStamina = (amount: number): boolean => {
    ...
    const spiritShield2 =
      useHiddenNpcStore().getBondBonusByType("spirit_shield");
    const spiritSave =
      spiritShield2?.type === "spirit_shield"
        ? spiritShield2.staminaSave / 100
        : 0;
  ...
  const recoverStaminaByGameTime = (hours: number): number => {
    ...
    const recovered = Math.min(
      Math.floor(staminaRegenProgress.value / 0.25),
  ...
  const upgradeMaxStamina = (): boolean => {
    ...
    maxStamina.value =
      STAMINA_CAPS[staminaCapLevel.value]! + bonusMaxStamina.value;
    return true;
  };
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    staminaRegenProgress.value = Math.max(
      0,
      Math.min((data as any).staminaRegenProgress ?? 0, 0.24),
    );
    ...
    const expectedMax =
      (STAMINA_CAPS[staminaCapLevel.value] ?? 120) + bonusMaxStamina.value;
    ...
    for (const key of ATTRIBUTE_KEYS) {
      ...
      nextAttributes[key] = {
        level: Math.min(
          Math.max(saved?.level ?? ATTRIBUTE_BASE_LEVEL, ATTRIBUTE_BASE_LEVEL),
          ATTRIBUTE_MAX_LEVEL,
        ),
