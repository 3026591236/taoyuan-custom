...
export const useAnimalStore = defineStore("animal", () => {
  const buildings = ref<
    { type: AnimalBuildingType; built: boolean; level: number }[]
  >([
  ...
  const TRAIT_NAMES: Record<AnimalTrait, string> = {
    hardy: "健壮",
    fertile: "丰产",
    treasure: "寻宝",
    guardian: "护院",
  };
  const BLOODLINE_PRODUCT_BONUS: Record<AnimalBloodline, number> = {
    normal: 0,
    spirit: 0.15,
    rare: 0.28,
    ancient: 0.45,
  };
  const PET_DISPATCH_NAMES: Record<
    NonNullable<PetState["dispatch"]>["type"],
    string
  > = { forage: "山林寻物", guard: "看家护院", treasure: "秘径寻宝" };

  const coopBuilt = computed(
    () => buildings.value.find((b) => b.type === "coop")?.built ?? false,
  );
  const barnBuilt = computed(
    () => buildings.value.find((b) => b.type === "barn")?.built ?? false,
  );
  const stableBuilt = computed(
    () => buildings.value.find((b) => b.type === "stable")?.built ?? false,
  );
  ...
  const rollBloodline = (
    parent?: Animal,
  ): { bloodline: AnimalBloodline; trait: AnimalTrait } => {
    const parentBloodline = parent?.bloodline ?? "normal";
    const r = Math.random();
    let bloodline: AnimalBloodline = "normal";
    if (parentBloodline === "ancient")
      bloodline = r < 0.55 ? "ancient" : r < 0.85 ? "rare" : "spirit";
    else if (parentBloodline === "rare")
      bloodline =
        r < 0.22
          ? "ancient"
          : r < 0.62
            ...
            : r < 0.9
              ? "spirit"
              : "normal";
    else if (parentBloodline === "spirit")
      bloodline =
        r < 0.08
          ? "ancient"
          : r < 0.28
  ...
  const petAnimal = (animalId: string): boolean => {
    ...
    const coopmasterBonus =
      useSkillStore().getSkill("farming").perk10 === "coopmaster" ? 1.5 : 1.0;
    animal.friendship = Math.min(
      1000,
      animal.friendship + Math.floor(5 * coopmasterBonus),
    );
    return true;
  };
  ...
  const startIncubation = (
    ...
    const hasCoopmaster =
      skillStore.getSkill("farming").perk10 === "coopmaster";
  ...
  const dailyUpdate = (): {
    ...
    const gameStore = useGameStore();
    const coopmasterBonus =
      skillStore.getSkill("farming").perk10 === "coopmaster" ? 1.5 : 1.0;
    ...
    for (const animal of animals.value) {
      ...
      // 饥饿达到阈值时有概率生病
      if (
        animal.hunger >= HUNGER_SICK_THRESHOLD &&
        !animal.sick &&
      ...
      // 饥饿致死 或 久病致死
      if (
        animal.hunger >= HUNGER_DEATH_DAYS ||
        animal.sickDays >= SICK_DEATH_DAYS
      ) {
      ...
      const friendshipMultiplier =
        gameStore.farmMapType === "meadowlands" ? 1.5 : 1.0;
      // 仙缘能力：灵抚（gui_nv_3）动物好感获取+25%
      const spiritFriendshipBonus =
        1 + useHiddenNpcStore().getAbilityValue("gui_nv_3") / 100;
      ...
      if (def && def.produceDays > 0 && animal.wasFed && !animal.sick) {
        const effectiveDays =
          animal.fedWith === NOURISHING_FEED_ID
            ? Math.max(1, def.produceDays - 1)
            : def.produceDays;
        if (animal.daysSinceProduct >= effectiveDays) {
          ...
          const animalBondBonus =
            useHiddenNpcStore().getBondBonusByType("animal_blessing");
          if (
            animalBondBonus?.type === "animal_blessing" &&
            Math.random() < animalBondBonus.chance
          ) {
            const qualityOrder2: Quality[] = [
              "normal",
              "fine",
              "excellent",
              "supreme",
            ...
            if (idx2 < qualityOrder2.length - 1)
              quality = qualityOrder2[idx2 + 1]!;
          }
          products.push({ itemId: def.productId, quality });
          const bloodline = animal.bloodline ?? "normal";
          if (
            (animal.trait === "fertile" ||
              Math.random() < BLOODLINE_PRODUCT_BONUS[bloodline]) &&
            bloodline !== "normal"
          ) {
            products.push({ itemId: def.productId, quality });
          }
    ...
    if (died.length > 0) {
      animals.value = animals.value.filter(
        (a) => a.hunger < HUNGER_DEATH_DAYS && a.sickDays < SICK_DEATH_DAYS,
      );
    }
  ...
  const sellAnimal = (
    animalId: string,
  ): { success: boolean; refund: number; name: string } => {
  ...
  const getBloodlineName = (bloodline?: AnimalBloodline): string =>
    BLOODLINE_NAMES[bloodline ?? "normal"];
  const getTraitName = (trait?: AnimalTrait): string =>
    trait ? TRAIT_NAMES[trait] : "未觉醒";
  const bloodlineSummary = computed(() => {
    const counts: Record<AnimalBloodline, number> = {
      normal: 0,
      spirit: 0,
      rare: 0,
      ancient: 0,
  ...
  const startPetDispatch = (
    type: NonNullable<PetState["dispatch"]>["type"],
  ): { success: boolean; message: string } => {
    if (!pet.value) return { success: false, message: "还没有宠物。" };
    if (pet.value.dispatch)
      return { success: false, message: "宠物正在外出派遣。" };
    if (pet.value.friendship < 300)
      return { success: false, message: "宠物好感达到300后才能派遣。" };
    pet.value.dispatch = { type, daysLeft: type === "treasure" ? 2 : 1 };
    return {
      success: true,
      message: `${pet.value.name}出发执行「${PET_DISPATCH_NAMES[type]}」了。`,
    };
  };
  ...
  const dailyPetDispatchUpdate = (): { message?: string } => {
    ...
    if (mission === "forage") {
      ...
      return {
        message: `${pet.value.name}从山林叼回了${getItemById(item)?.name ?? item}×${qty}。`,
      };
    }
    if (mission === "guard") {
    ...
    return {
      message: `${pet.value.name}循着秘径找到了${getItemById(reward)?.name ?? reward}×${qty}。`,
    };
  };

  const dispatchAnimal = (
    animalId: string,
  ): { success: boolean; message: string } => {
    const animal = animals.value.find((a) => a.id === animalId);
  ...
  const deserialize = (data: any) => {
    ...
    pet.value = data.pet
      ? { ...data.pet, dispatch: data.pet.dispatch ?? null }
      : null;
