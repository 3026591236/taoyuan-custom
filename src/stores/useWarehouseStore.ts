...
export const useWarehouseStore = defineStore("warehouse", () => {
  ...
  const getVoidInputChest = (): Chest | null => {
    return (
      chests.value.find((c) => c.tier === "void" && c.voidRole === "input") ??
      null
    );
  };
  ...
  const getVoidOutputChest = (): Chest | null => {
    return (
      chests.value.find((c) => c.tier === "void" && c.voidRole === "output") ??
      null
    );
  };
  ...
  const deserialize = (data: Record<string, unknown>) => {
    ...
    if (data.items && !data.chests) {
      const oldItems = (data.items as InventoryItem[]).filter((i) =>
      ...
      if (oldItems.length > 0) {
        ...
        for (let i = 0; i < oldItems.length; i += goldCap) {
          migratedChests.push({
            ...
            tier: "gold",
            label:
              migratedChests.length === 0
                ? "旧仓库"
                : `旧仓库${migratedChests.length + 1}`,
