...
export const useBreedingStore = defineStore("breeding", () => {
  ...
  const trySeedMakerGeneticSeed = (
    ...
    const genetics: SeedGenetics = {
      ...
      sweetness: clampStat(
        base.sweetness + Math.round((Math.random() - 0.5) * 10),
      ),
      yield: clampStat(base.yield + Math.round((Math.random() - 0.5) * 10)),
      resistance: clampStat(
        base.resistance + Math.round((Math.random() - 0.5) * 10),
      ),
    };
  ...
  const collectResult = (slotIndex: number): SeedGenetics | null => {
    ...
    if (result.isHybrid && result.hybridId) {
      const existing = compendium.value.find(
        (e) => e.hybridId === result.hybridId,
      );
      if (!existing) {
  ...
  const breedSameCrop = (a: SeedGenetics, b: SeedGenetics): SeedGenetics => {
    ...
    const fluctuate = (): number => {
      return Math.round(
        (Math.random() - 0.5) * 2 * BASE_MUTATION_MAGNITUDE * fluctuationScale,
      );
    };

    let sweetness = clampStat(
      Math.round((a.sweetness + b.sweetness) / 2) + fluctuate(),
    );
    let yieldVal = clampStat(Math.round((a.yield + b.yield) / 2) + fluctuate());
    let resistance = clampStat(
      Math.round((a.resistance + b.resistance) / 2) + fluctuate(),
    );
    ...
    if (Math.random() < avgMutationRate / 100) {
      ...
      for (let i = 0; i < mutateCount; i++) {
        ...
        const jump =
          MUTATION_JUMP_MIN +
          Math.round(Math.random() * (MUTATION_JUMP_MAX - MUTATION_JUMP_MIN));
      ...
      mutationRate = clampMutationRate(
        mutationRate +
          Math.round((Math.random() - 0.5) * 2 * MUTATION_RATE_DRIFT),
      );
    ...
    const result: SeedGenetics = {
      ...
      resistance,
      stability: Math.min(
        Math.round(avgStability) + GENERATIONAL_STABILITY_GAIN,
        MAX_STABILITY,
      ),
      mutationRate,
      parentA: a.id,
  ...
  const breedDifferentCrop = (
    ...
    if (
      hybrid &&
      avgSweetness >= hybrid.minSweetness &&
      avgYield >= hybrid.minYield
    ) {
      // 匹配成功，产出杂交种
      const avgStability = (a.stability + b.stability) / 2;
      const avgMutationRate = (a.mutationRate + b.mutationRate) / 2;
      const fluctuationScale =
        (avgMutationRate / 50) * (1 - avgStability / 100);
      ...
      const result: SeedGenetics = {
        ...
        generation: Math.max(a.generation, b.generation) + 1,
        sweetness: clampStat(
          Math.round(hybrid.baseGenetics.sweetness * 0.6 + avgSweetness * 0.4) +
            fluctuate(),
        ),
        yield: clampStat(
        ...
        resistance: clampStat(
          Math.round(
            hybrid.baseGenetics.resistance * 0.6 +
              ((a.resistance + b.resistance) / 2) * 0.4,
          ) + fluctuate(),
        ),
        stability: Math.min(
          Math.round(avgStability) + GENERATIONAL_STABILITY_GAIN,
      ...
      if (slot.parentA && slot.parentB && !slot.ready) {
        ...
        if (slot.daysProcessed >= slot.totalDays) {
          ...
          if (isCrossBreed && slot.result.isHybrid) {
            addLog(
              `杂交成功：${crop?.name ?? slot.result.cropId}（${stars}星）！已记录到图鉴。`,
            );
            ...
            addLog(
              `育种完成：${crop?.name ?? slot.result.cropId}（${stars}星）。`,
            );
          }
          const achievementStore = useAchievementStore();
