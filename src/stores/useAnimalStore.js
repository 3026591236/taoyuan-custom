...
import { ANIMAL_BUILDINGS, ANIMAL_DEFS, HAY_ITEM_ID, getBuildingUpgrade, INCUBATION_MAP, PREMIUM_FEED_ID, NOURISHING_FEED_ID, VITALITY_FEED_ID } from '@/data';
...
export const useAnimalStore = defineStore('animal', () => {
    ...
    const animals = ref([]);
    /** 鸡舍孵化器状态 (同时最多1个) */
    const incubating = ref(null);
    /** 牲口棚孵化器状态 (同时最多1个, barn level ≥ 2) */
    const barnIncubating = ref(null);
    /** 宠物状态 */
    const pet = ref(null);
    /** 今天是否已放牧 */
    const grazedToday = ref(false);
    /** 已安装自动抚摸机的建筑类型 */
    const autoPetterBuildings = ref([]);
    ...
    const coopAnimals = computed(() => animals.value.filter(a => {
        const def = ANIMAL_DEFS.find(d => d.type === a.type);
        return def?.building === 'coop';
    }));
    const barnAnimals = computed(() => animals.value.filter(a => {
        const def = ANIMAL_DEFS.find(d => d.type === a.type);
    ...
    const buildBuilding = (type) => {
        ...
        // 检查铜钱
        if (!playerStore.spendMoney(def.cost))
            return false;
        // 扣除材料
        for (const mat of def.materialCost) {
    ...
    /** 购买动物 */
    const buyAnimal = (animalType, customName) => {
        const playerStore = usePlayerStore();
    ...
    const feedAll = (feedId = HAY_ITEM_ID) => {
        let fedCount = 0;
        let noFeedCount = 0;
    ...
    /** 喂食单只动物 */
    const feedAnimal = (animalId, feedId = HAY_ITEM_ID) => {
        const animal = animals.value.find(a => a.id === animalId);
        if (!animal || animal.wasFed)
            return false;
        if (!removeCombinedItem(feedId, 1))
            return false;
        animal.wasFed = true;
    ...
    const startBarnIncubation = (itemId) => {
    ...
    const dailyUpdate = () => {
        ...
        for (const animal of animals.value) {
            ...
            if (animal.wasFed && animal.wasPetted) {
                ...
                animal.friendship = Math.min(1000, animal.friendship + Math.floor(base * friendshipMultiplier * coopmasterBonus * spiritFriendshipBonus));
            }
            ...
            if (def && def.produceDays > 0 && animal.wasFed && !animal.sick) {
                ...
                if (animal.daysSinceProduct >= effectiveDays) {
                    ...
                    if (animalBondBonus?.type === 'animal_blessing' && Math.random() < animalBondBonus.chance) {
                        const qualityOrder2 = ['normal', 'fine', 'excellent', 'supreme'];
    ...
    const sellAnimal = (animalId) => {
        ...
        if (idx === -1)
            return { success: false, refund: 0, name: '' };
    ...
    /** 根据友好度决定产品品质 */
    const getAnimalProductQuality = (friendship) => {
        if (friendship >= 800)
            return 'supreme';
        if (friendship >= 500)
            return 'excellent';
    ...
    /** 检查指定建筑是否已安装自动抚摸机 */
    const hasAutoPetter = (buildingType) => {
        return autoPetterBuildings.value.includes(buildingType);
    };
    /** 安装自动抚摸机到指定建筑 */
    const installAutoPetter = (buildingType) => {
    ...
    const deserialize = (data) => {
        if (data.buildings) {
            const savedBuildings = data.buildings.map((b) => ({
                ...b,
                level: b.level && b.level > 0 ? b.level : b.built ? 1 : 0
            }));
            // 兼容旧存档：补充缺少的 stable 建筑
            const savedTypes = new Set(savedBuildings.map((b) => b.type));
        ...
        if (data.animals) {
            animals.value = data.animals.map((a) => ({
                ...a,
                hunger: a.hunger ?? 0,
                sick: a.sick ?? false,
