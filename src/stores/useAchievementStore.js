...
export const useAchievementStore = defineStore('achievement', () => {
    ...
    const discoveryTimes = ref({});
    /** 已完成的成就ID集合 */
    const completedAchievements = ref([]);
    /** 祠堂任务已提交物品 */
    const bundleSubmissions = ref({});
    /** 已完成的祠堂任务 */
    const completedBundles = ref([]);
    /** 统计计数器 */
    const stats = ref({
    ...
    const getDiscoveryTime = (itemId) => {
        return discoveryTimes.value[itemId] ?? null;
    };
    const isDiscovered = (itemId) => {
    ...
    const recordMoneyEarned = (amount) => {
        stats.value.totalMoneyEarned += amount;
    };
    const recordMineFloor = (floor) => {
    ...
    const recordSkullCavernFloor = (floor) => {
    ...
    const checkAchievements = () => {
        ...
        for (const achievement of ACHIEVEMENTS) {
            if (completedAchievements.value.includes(achievement.id))
                continue;
    ...
    // === 祠堂任务 ===
    const submitToBundle = (bundleId, itemId, quantity) => {
        if (completedBundles.value.includes(bundleId))
            return false;
        ...
        if (!req)
            return false;
        if (!inventoryStore.removeItem(itemId, quantity))
            return false;
    ...
    const getBundleProgress = (bundleId) => {
        return bundleSubmissions.value[bundleId] ?? {};
    };
    const isBundleComplete = (bundleId) => {
    ...
    const perfectionPercent = computed(() => {
        ...
        const total = achievementRate * 0.25 + shippingRate * 0.2 + bundleRate * 0.15 + collectionRate * 0.15 + skillRate * 0.15 + friendRate * 0.1;
    ...
    const deserialize = (data) => {
        ...
        if (stats.value.totalMonstersKilled === undefined) {
            stats.value.totalMonstersKilled = 0;
        }
        if (stats.value.totalBreedingsDone === undefined) {
            stats.value.totalBreedingsDone = 0;
        }
        if (stats.value.totalHybridsDiscovered === undefined) {
            stats.value.totalHybridsDiscovered = 0;
        }
        if (stats.value.highestHybridTier === undefined) {
            stats.value.highestHybridTier = 0;
        }
        // 同步已拥有装备到图鉴（修复旧存档中装备未登记到图鉴的问题）
        const inventoryStore = useInventoryStore();
        for (const w of inventoryStore.ownedWeapons)
            discoverItem(w.defId);
        for (const r of inventoryStore.ownedRings)
            discoverItem(r.defId);
        for (const h of inventoryStore.ownedHats)
            discoverItem(h.defId);
        for (const s of inventoryStore.ownedShoes)
            discoverItem(s.defId);
        // 同步背包中已有物品到图鉴
        const seen = new Set();
