...
import { PROCESSING_MACHINES, SPRINKLERS, FERTILIZERS, BAITS, TACKLES, TAPPER, CRAB_POT_CRAFT, BOMBS, getRecipesForMachine, getProcessingRecipeById } from '@/data/processing';
...
export const useProcessingStore = defineStore('processing', () => {
    ...
    // === 制造(Craft) ===
    /** 检查是否有足够材料制造某样东西 */
    const canCraft = (craftCost, craftMoney) => {
        if (playerStore.money < craftMoney)
            return false;
        return craftCost.every(c => hasCombinedItem(c.itemId, c.quantity));
    };
    /** 消耗材料 */
    const consumeCraftMaterials = (craftCost, craftMoney) => {
        if (!canCraft(craftCost, craftMoney))
            return false;
        if (!playerStore.spendMoney(craftMoney))
            return false;
    ...
    /** 制造并放置一台加工机器 */
    const craftMachine = (machineType) => {
        if (machines.value.length >= maxMachines.value)
            return false;
        const def = PROCESSING_MACHINES.find(m => m.id === machineType);
        if (!def)
            return false;
        if (!consumeCraftMaterials(def.craftCost, def.craftMoney))
            return false;
        machines.value.push({
            machineType,
            recipeId: null,
    ...
    /** 制造洒水器（返回物品ID放入背包） */
    const craftSprinkler = (sprinklerId) => {
    ...
    /** 制造肥料 */
    const craftFertilizer = (fertilizerId) => {
    ...
    /** 制造采脂器 */
    const craftTapper = () => {
        if (!consumeCraftMaterials(TAPPER.craftCost, TAPPER.craftMoney))
            return false;
    ...
    /** 制造蟹笼 */
    const craftCrabPot = () => {
        if (!consumeCraftMaterials(CRAB_POT_CRAFT.craftCost, CRAB_POT_CRAFT.craftMoney))
            return false;
        inventoryStore.addItem(CRAB_POT_CRAFT.id);
        return true;
    };
    /** 制造炸弹 */
    const craftBomb = (bombId) => {
    ...
    const startProcessing = (slotIndex, recipeId, specifiedQuality) => {
        ...
        if (recipe.inputItemId !== null) {
            if (specifiedQuality !== undefined) {
                ...
                if (!removeCombinedItem(recipe.inputItemId, recipe.inputQuantity, specifiedQuality))
                    return false;
            }
            else {
                quality = getLowestQuality(recipe.inputItemId);
                if (!removeCombinedItem(recipe.inputItemId, recipe.inputQuantity))
                    return false;
            }
        }
    ...
    /** 收取加工产物 */
    const collectProduct = (slotIndex) => {
        const slot = machines.value[slotIndex];
        if (!slot || !slot.ready || !slot.recipeId)
            return null;
    ...
    /** 取消加工（退回原料，机器回到空闲状态） */
    const cancelProcessing = (slotIndex) => {
        const slot = machines.value[slotIndex];
    ...
    /** 获取某台机器可用的加工配方列表 */
    const getAvailableRecipes = (machineType) => {
                    ...
                    if (recipe.inputItemId === null || machineDef?.autoCollect) {
                        ...
                        if (voidInput && recipe.inputItemId) {
                            ...
                            if (available >= recipe.inputQuantity) {
                                // 查找最低品质
                                const qOrder = ['normal', 'fine', 'excellent', 'supreme'];
    ...
    const upgradeWorkshop = () => {
        ...
        if (!upgrade)
            return { success: false, message: '工坊已达到最高等级。' };
    ...
    const deserialize = (data) => {
        ...
        workshopLevel.value = data.workshopLevel ?? 0;
        collapsedGroups.value = new Set(data.collapsedGroups ?? []);
    };
    return {
        machines,
        machineCount,
