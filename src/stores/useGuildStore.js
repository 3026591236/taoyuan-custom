...
export const useGuildStore = defineStore('guild', () => {
    /** 按怪物ID记录击杀数 */
    const monsterKills = ref({});
    /** 已领取奖励的讨伐目标monsterId集合 */
    const claimedGoals = ref([]);
    /** 已遭遇过的怪物ID集合（用于图鉴） */
    const encounteredMonsters = ref([]);
    /** 贡献点（可消费货币） */
    ...
    /** 记录遭遇（进入战斗时调用，不管是否击杀） */
    const recordEncounter = (monsterId) => {
        if (!encounteredMonsters.value.includes(monsterId)) {
            encounteredMonsters.value.push(monsterId);
        }
    };
    ...
    /** 领取讨伐奖励 */
    const claimGoal = (monsterId) => {
        const goal = MONSTER_GOALS.find(g => g.monsterId === monsterId);
        if (!goal)
            return false;
        if ((monsterKills.value[monsterId] ?? 0) < goal.killTarget)
            return false;
        if (claimedGoals.value.includes(monsterId))
            return false;
    ...
    const checkLevelUp = () => {
        while (guildLevel.value < GUILD_LEVELS.length) {
            ...
            if (!next || guildExp.value < next.expRequired)
                break;
            guildLevel.value++;
    ...
    /** 捐献物品 */
    const donateItem = (itemId, quantity) => {
    ...
    const buyShopItem = (itemId) => {
        ...
        if (item.dailyLimit) {
            ensureDailyReset();
            if ((dailyPurchases.value[itemId] ?? 0) >= item.dailyLimit)
                return false;
        }
        // 每周限购检查
        if (item.weeklyLimit) {
        ...
        // 永久总限购检查
        if (item.totalLimit) {
            if ((totalPurchases.value[itemId] ?? 0) >= item.totalLimit)
                return false;
        }
        const playerStore = usePlayerStore();
        const inventoryStore = useInventoryStore();
        ...
        if (item.contributionCost) {
            if (contributionPoints.value < item.contributionCost)
                return false;
            contributionPoints.value -= item.contributionCost;
        }
        else {
            if (playerStore.money < item.price)
                return false;
    ...
    const deserialize = (data) => {
        ...
        dailyPurchases.value = data.dailyPurchases ?? {};
        lastResetDay.value = data.lastResetDay ?? -1;
        weeklyPurchases.value = data.weeklyPurchases ?? {};
        lastResetWeek.value = data.lastResetWeek ?? -1;
        ...
        if (isOldSave && claimedGoals.value.length > 0) {
            ...
            contributionPoints.value = data.contributionPoints ?? 0;
            guildExp.value = data.guildExp ?? 0;
            guildLevel.value = data.guildLevel ?? 0;
        }
    };
    return {
        monsterKills,
