...
const QUALITY_MULTIPLIER = { normal: 1, fine: 1.25, excellent: 1.5, supreme: 2 };
const QUALITY_LABEL = { normal: '', fine: '优良', excellent: '精品', supreme: '极品' };
export const useCookingStore = defineStore('cooking', () => {
    ...
    /** 当天生效的食物增益 */
    const activeBuff = ref(null);
    /** 已解锁的食谱定义 */
    const recipes = computed(() => unlockedRecipes.value.map(id => getRecipeById(id)).filter((r) => r !== undefined));
    /** 检查是否有足够材料 */
    const canCook = (recipeId) => {
        const recipe = getRecipeById(recipeId);
        if (!recipe)
            return false;
        if (!unlockedRecipes.value.includes(recipeId))
            return false;
        // 检查技能等级门槛
    ...
    /** 预览烹饪品质（取所有材料最低品质） */
    const previewCookQuality = (recipeId) => {
        const recipe = getRecipeById(recipeId);
        if (!recipe)
            return 'normal';
        let minIdx = 3;
    ...
    /** 烹饪 */
    const cook = (recipeId, quantity = 1) => {
        const recipe = getRecipeById(recipeId);
        if (!recipe)
            return { success: false, message: '食谱不存在。' };
        if (!unlockedRecipes.value.includes(recipeId))
            return { success: false, message: '尚未解锁此食谱。' };
        // 计算最多能做几份
    ...
    /** 食用烹饪品 */
    const eat = (recipeId, quality = 'normal') => {
        ...
        const recipe = getRecipeById(recipeId);
        if (!recipe)
            return { success: false, message: '食谱数据丢失。' };
        // 品质加成
        ...
        const staminaRestore = Math.floor(recipe.effect.staminaRestore * qualityBonus * alchemistBonus * chefBonus * kitchenBonus * moonRabbitBonus);
        ...
        if (recipe.effect.healthRestore) {
            const healthRestore = Math.floor(recipe.effect.healthRestore * qualityBonus * alchemistBonus * chefBonus * kitchenBonus * moonRabbitBonus);
