...
export const useHomeStore = defineStore("home", () => {
  ...
  const nextCellarUpgrade = computed(() => {
    return (
      CELLAR_UPGRADES.find((u) => u.level === cellarLevel.value + 1) ?? null
    );
  });
  ...
  const dailyCaveUpdate = (): {
    ...
    if (caveChoice.value === "mushroom") {
      if (Math.random() < def.mushroomChance) {
        // 按权重随机选择蘑菇产物
        const totalWeight = def.mushroomPool.reduce(
          (sum, p) => sum + p.weight,
          0,
        );
