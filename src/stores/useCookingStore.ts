...
export const useCookingStore = defineStore("cooking", () => {
  ...
  const canCook = (recipeId: string): boolean => {
    ...
    return recipe.ingredients.every(
      (ing) => getCombinedItemCount(ing.itemId) >= ing.quantity,
    );
  };
  ...
  const cook = (
    ...
    const qualityTag = QUALITY_LABEL[resultQuality]
      ? `【${QUALITY_LABEL[resultQuality]}】`
      : "";
  ...
  const eat = (
    ...
    const moonRabbitBonus = useHiddenNpcStore().isAbilityActive("yue_tu_2")
      ? 1.5
      : 1.0;
    const staminaRestore = Math.floor(
      recipe.effect.staminaRestore *
        qualityBonus *
        alchemistBonus *
        chefBonus *
        kitchenBonus *
    ...
    const qualityTag = QUALITY_LABEL[quality]
