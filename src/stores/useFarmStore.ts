...
export const useFarmStore = defineStore("farm", () => {
  ...
  const tilledPlots = computed(() =>
    plots.value.filter((p) => p.state !== "wasteland"),
  );
    ...
    for (const plot of plots.value) {
      ...
      if (plot.watered) {
        ...
        const fertDef = plot.fertilizer
          ...
          : null;
        const speedup =
          (fertDef?.growthSpeedup ?? 0) + walletGrowth + spiritGrowth;
        plot.growthDays += 1;
        ...
        const resistanceFactor = plot.seedGenetics
          ? 1 - plot.seedGenetics.resistance / 100
          : 1;
      ...
      if (sprinklerWatered.has(plot.id)) {
        ...
        const retainFert = plot.fertilizer
          ? getFertilizerById(plot.fertilizer)
          : null;
        if (
          retainFert?.retainChance &&
          Math.random() < retainFert.retainChance
        ) {
          // 保湿土保持浇水
        } else {
          plot.watered = false;
      ...
      if (
        ...
        const weedChance =
          scarecrows.value > 0 ? baseWeedChance * 0.6 : baseWeedChance;
  ...
  const onSeasonChange = (
    newSeason: Season,
  ): { witheredCount: number; reclaimedCount: number } => {
    let witheredCount = 0;
    ...
    const preExistingTilled = new Set(
      plots.value
        .filter((p) => p.state === "tilled" && !p.cropId)
        .map((p) => p.id),
    );
  ...
  const lightningStrike = (): {
    ...
    const croppedPlots = plots.value.filter(
      (p) =>
        (p.state === "planted" ||
          p.state === "growing" ||
          p.state === "harvestable") &&
  ...
  const checkGiantCrops = (): { cropId: string; cropName: string }[] => {
    ...
    for (let r = 0; r <= maxR; r++) {
      for (let c = 0; c <= maxC; c++) {
        ...
        if (
          topLeft.state !== "harvestable" ||
          !topLeft.cropId ||
          topLeft.giantCropGroup !== null
        )
          continue;
  ...
  /** 果树每日更新 */
  const dailyFruitTreeUpdate = (
    currentSeason: Season,
  ): { fruits: { fruitId: string; quality: Quality }[] } => {
  ...
  const initGreenhouse = (): void => {
    ...
    greenhousePlots.value = Array.from(
      { length: GREENHOUSE_PLOT_COUNT },
      (_, i) => ({
        id: i,
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    nextFruitTreeId.value =
      ...
      (fruitTrees.value.length > 0
        ? Math.max(...fruitTrees.value.map((t) => t.id)) + 1
        : 0);
    ...
    nextWildTreeId.value =
      (data as any).nextWildTreeId ??
      (wildTrees.value.length > 0
        ? Math.max(...wildTrees.value.map((t) => t.id)) + 1
        : 0);
    greenhousePlots.value = ((data as any).greenhousePlots ?? []).map(
      (p: any) => ({
        ...p,
        fertilizer: migrateFertilizer(p.fertilizer),
