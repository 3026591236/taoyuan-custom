...
export const IMMORTAL_EXPEDITIONS: Array<{ id: ImmortalExpeditionId; name: string; desc: string; needNode: ImmortalCaveId; jade: number; essence: number; merit: number; rule: number }> = [
  ...
  { id: 'star_mining', name: '星砂采炼', desc: '借星炉余火采集星砂，供仙器淬炼。', needNode: 'spirit_spring', jade: 10, essence: 7, merit: 8, rule: 2 },
  { id: 'rift_watch', name: '裂隙镇守', desc: '派遣洞天仙卫观测混沌波动，积累镇压先机。', needNode: 'law_tablet', jade: 5, essence: 5, merit: 22, rule: 4 }
]
...
export const useAscensionStore = defineStore('ascension', () => {
  ...
  const riftBossHp = ref<Record<ChaosRiftId, number>>({ void_tide: 900, fallen_star: 1300, demon_gate: 1800, law_maze: 2400 })
  const riftBossShield = ref<Record<ChaosRiftId, number>>({ void_tide: 180, fallen_star: 260, demon_gate: 360, law_maze: 480 })
  const riftBossTurns = ref<Record<ChaosRiftId, number>>({ void_tide: 0, fallen_star: 0, demon_gate: 0, law_maze: 0 })
  ...
  const immortalMaxHp = computed(() => Math.floor(780 + immortalRealmStage.value * 260 + immortalBodyLevel.value * 175 + immortalSoulLevel.value * 65 + gearPower.value * 1.8 + cavePower.value * .7))
  const immortalMaxQi = computed(() => Math.floor(100 + immortalRealmStage.value * 26 + immortalSoulLevel.value * 18 + immortalBoneLevel.value * 8 + fatePlatePower.value * .18))
  const immortalAttack = computed(() => Math.floor(58 + immortalRealmStage.value * 24 + immortalBoneLevel.value * 19 + gearPower.value * .42 + fatePlatePower.value * .28 + cavePower.value * .16))
  const immortalDefense = computed(() => Math.floor(32 + immortalRealmStage.value * 17 + immortalBodyLevel.value * 14 + gearPower.value * .25 + cavePower.value * .12))
  const immortalCritRate = computed(() => Math.min(65, 8 + Math.floor((gearLevels.value.weapon || 0) * 2.4) + Math.floor(pkStreak.value * 1.5) + (immortalLineage.value === 'sword_dao' ? 8 : 0)))
  const immortalDamageReduction = computed(() => Math.min(58, 5 + Math.floor((gearLevels.value.armor || 0) * 2.2) + Math.floor(immortalBodyLevel.value * 1.8) + (immortalLineage.value === 'harvest_dao' ? 6 : 0)))
  const immortalHpRate = computed(() => immortalMaxHp.value ? Math.max(0, Math.min(100, Math.round(immortalHp.value / immortalMaxHp.value * 100))) : 100)
  const immortalQiRate = computed(() => immortalMaxQi.value ? Math.max(0, Math.min(100, Math.round(immortalQi.value / immortalMaxQi.value * 100))) : 100)
  const restoreImmortalCombat = (full = false) => { immortalHp.value = Math.min(immortalMaxHp.value, full ? immortalMaxHp.value : Math.max(immortalHp.value, Math.floor(immortalMaxHp.value * .7))); immortalQi.value = Math.min(immortalMaxQi.value, full ? immortalMaxQi.value : Math.max(immortalQi.value, Math.floor(immortalMaxQi.value * .65))) }
  ...
  const caveResonanceLevel = computed(() => Math.min(...IMMORTAL_CAVE_NODES.map(node => caveLevels.value[node.id] || 0)))
  const caveResonance = computed(() => caveResonanceLevel.value >= 3 ? { name: '五府·周天大阵', yieldRate: 1.35, decayReduce: 5 } : caveResonanceLevel.value >= 2 ? { name: '五府·灵脉相连', yieldRate: 1.18, decayReduce: 3 } : caveResonanceLevel.value >= 1 ? { name: '五府·初启共鸣', yieldRate: 1.08, decayReduce: 1 } : { name: '洞天未成阵', yieldRate: 1, decayReduce: 0 })
  const caveExpeditionReward = (id: ImmortalExpeditionId) => { const mission = IMMORTAL_EXPEDITIONS.find(item => item.id === id)!; const lv = caveLevels.value[mission.needNode] || 0; const rate = (1 + Math.max(0, lv - 1) * .15) * caveHeavenStabilityRate.value * caveResonance.value.yieldRate; return { merit: Math.max(1, Math.floor(mission.merit * rate)), jade: Math.max(1, Math.floor(mission.jade * rate)), rule: Math.max(1, Math.floor(mission.rule * rate)), essence: Math.max(1, Math.floor(mission.essence * rate)) } }
  ...
  const breakthroughImmortalRealm = (): boolean => {
    ...
    immortalBreakthroughFx.value = { name: next.name, icon: next.icon, stage: immortalRealmStage.value }
    immortalBreakthroughPhase.value = 'charge'
    if (immortalBreakthroughTimer) clearTimeout(immortalBreakthroughTimer)
    window.setTimeout(() => { if (immortalBreakthroughFx.value) immortalBreakthroughPhase.value = 'tribulation' }, 850)
    window.setTimeout(() => { if (immortalBreakthroughFx.value) immortalBreakthroughPhase.value = 'ascend' }, 1750)
    window.setTimeout(() => { if (immortalBreakthroughFx.value) immortalBreakthroughPhase.value = 'reveal' }, 2600)
    immortalBreakthroughTimer = window.setTimeout(() => { immortalBreakthroughFx.value = null }, 4300)
    visualPulse.value++
    lastBattleText.value = `${next.icon} 仙阶突破至「${next.name}」：${next.desc} 仙战力底蕴 +${next.powerBonus}。`
    addLog(lastBattleText.value)
    return true
  }
  const clearImmortalBreakthroughFx = () => { if (immortalBreakthroughTimer) clearTimeout(immortalBreakthroughTimer); immortalBreakthroughFx.value = null; immortalBreakthroughPhase.value = 'charge' }
  ...
  const caveHeavenYield = computed(() => {
    const stable = Math.max(.5, Math.min(1.15, caveHeavenStability.value / 100)) * caveResonance.value.yieldRate
    const lv = (id: ImmortalCaveId) => caveLevels.value[id] || 0
    return { merit: Math.floor((4 + lv('merit_pool') * 5 + lv('mortal_anchor') * 2) * stable), jade: Math.floor((1 + lv('star_platform') + lv('spirit_spring') * 2) * stable), rule: Math.floor((lv('law_tablet') * 2 + lv('star_platform')) * stable), essence: Math.floor((lv('spirit_spring') + lv('law_tablet') * .5) * stable), dew: Math.max(0, Math.floor(lv('spirit_spring') * stable)) }
  })
  const caveHeavenHarvestedToday = computed(() => caveHeavenHarvestKey.value === todayKey())
  const claimCaveHeavenYield = (): boolean => {
    if (!ascended.value || caveHeavenHarvestedToday.value) { if (ascended.value) addLog('洞天今日灵机已收取，明日再来。'); return false }
  ...
  const deserialize = (data: any) => {
    ...
    caveHeavenMaintenanceKey.value = String(data?.caveHeavenMaintenanceKey ?? '')
    ...
    riftBossHp.value = { void_tide: 900, fallen_star: 1300, demon_gate: 1800, law_maze: 2400, ...(data?.riftBossHp ?? {}) }
    riftBossShield.value = { void_tide: 180, fallen_star: 260, demon_gate: 360, law_maze: 480, ...(data?.riftBossShield ?? {}) }
    riftBossTurns.value = { void_tide: 0, fallen_star: 0, demon_gate: 0, law_maze: 0, ...(data?.riftBossTurns ?? {}) }
