...
/** 解密并解析存档数据 */
export const parseSaveData = (raw) => {
...
export const useSaveStore = defineStore('save', () => {
    ...
    const loadFromSlot = (slot) => {
        try {
            ...
            if (data.skill)
                skillStore.deserialize(data.skill);
            if (data.npc)
                npcStore.deserialize(data.npc);
            if (data.mining)
                miningStore.deserialize(data.mining);
            if (data.cooking)
                cookingStore.deserialize(data.cooking);
            if (data.processing)
                processingStore.deserialize(data.processing);
            if (data.achievement)
                achievementStore.deserialize(data.achievement);
            if (data.animal)
                animalStore.deserialize(data.animal);
            if (data.home)
                homeStore.deserialize(data.home);
            if (data.fishing)
                fishingStore.deserialize(data.fishing);
            if (data.wallet)
                walletStore.deserialize(data.wallet);
            if (data.quest)
                questStore.deserialize(data.quest);
            if (data.shop)
                shopStore.deserialize(data.shop);
            if (data.settings)
                settingsStore.deserialize(data.settings);
            if (data.warehouse)
                warehouseStore.deserialize(data.warehouse);
            if (data.breeding)
                breedingStore.deserialize(data.breeding);
            if (data.museum)
                museumStore.deserialize(data.museum);
            if (data.guild)
                guildStore.deserialize(data.guild);
            if (data.secretNote)
                secretNoteStore.deserialize(data.secretNote);
            if (data.hanhai)
                hanhaiStore.deserialize(data.hanhai);
            if (data.fishPond)
                fishPondStore.deserialize(data.fishPond);
            if (data.tutorial)
                tutorialStore.deserialize(data.tutorial);
            if (data.hiddenNpc)
                hiddenNpcStore.deserialize(data.hiddenNpc);
            if (data.cultivation)
                cultivationStore.deserialize(data.cultivation);
            activeSlot.value = slot;
            return true;
        }
        catch {
    ...
    const exportSave = (slot) => {
        try {
            ...
            const name = info?.exists
                ? `桃源乡_存档${slot + 1}_第${info.year}年${SEASON_NAMES[info.season] ?? info.season}第${info.day}天`
                : `桃源乡_存档${slot + 1}`;
    ...
    /** 从文件导入存档到指定槽位 */
    const importSave = (slot, fileContent) => {
        if (slot < 0 || slot >= MAX_SLOTS)
            return false;
        try {
            // 验证文件内容可解密
