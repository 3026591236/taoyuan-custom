...
export const useSecretNoteStore = defineStore("secretNote", () => {
  ...
  const tryCollectNote = (): number | null => {
    ...
    const uncollected = SECRET_NOTES.filter(
      (n) => !collectedNotes.value.includes(n.id),
    );
  ...
  const useNote = (noteId: number): { success: boolean; message: string } => {
    if (!isCollected(noteId))
      return { success: false, message: "尚未获得此笔记。" };
    if (isUsed(noteId))
      return { success: false, message: "已经使用过此笔记。" };

    const noteDef = SECRET_NOTES.find((n) => n.id === noteId);
    if (!noteDef || !noteDef.usable)
      return { success: false, message: "此笔记不可使用。" };
