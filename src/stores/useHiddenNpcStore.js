...
export const useHiddenNpcStore = defineStore('hiddenNpc', () => {
    const hiddenNpcStates = ref(HIDDEN_NPCS.map(n => defaultState(n.id)));
    // ==================== 基础查询 ====================
    const getHiddenNpcState = (npcId) => hiddenNpcStates.value.find(s => s.npcId === npcId);
    const getAffinityLevel = (npcId) => {
        const state = getHiddenNpcState(npcId);
        if (!state)
            return 'wary';
        for (const t of AFFINITY_THRESHOLDS) {
            if (state.affinity >= t.min)
                return t.level;
        }
        return 'wary';
    };
    const getRevealedNpcs = computed(() => HIDDEN_NPCS.filter(n => {
        const s = getHiddenNpcState(n.id);
        return s && s.discoveryPhase === 'revealed';
    }));
    const getRumorNpcs = computed(() => HIDDEN_NPCS.filter(n => {
        const s = getHiddenNpcState(n.id);
    ...
    /** 是否为显灵日 */
    const isManifestationDay = (npcId) => {
    ...
    const evaluateCondition = (cond) => {
        ...
        switch (cond.type) {
            ...
            case 'skill':
                return skillStore.getSkill(cond.skillType).level >= cond.minLevel;
            case 'npcFriendship': {
    ...
    const checkDiscoveryConditions = () => {
        ...
        for (const npc of HIDDEN_NPCS) {
            ...
            if (!state || state.discoveryPhase === 'revealed')
                continue;
            // 发现链中按顺序找到下一个未完成的步骤
    ...
    // ==================== 互动系统 ====================
    /** 增减缘分 */
    const addAffinity = (npcId, amount) => {
        const state = getHiddenNpcState(npcId);
        if (!state)
            return;
    ...
    /** 供奉物品 */
    const performOffering = (npcId, itemId, quality) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def)
            return { success: false, message: '找不到此仙灵。', affinityChange: 0 };
        if (state.discoveryPhase !== 'revealed')
            return { success: false, message: '尚未与此仙灵建立联系。', affinityChange: 0 };
        if (state.offeredToday)
            return { success: false, message: '今日已供奉过了。', affinityChange: 0 };
        if (state.offersThisWeek >= MAX_OFFERS_PER_WEEK)
            return { success: false, message: '本周供奉次数已满。', affinityChange: 0 };
        const inventoryStore = useInventoryStore();
        ...
        if (def.resonantOfferings.includes(itemId))
            base = OFFERING_RESONANT;
        else if (def.pleasedOfferings.includes(itemId))
            base = OFFERING_PLEASED;
        else if (def.repelledOfferings.includes(itemId))
            base = OFFERING_REPELLED;
        let multiplier = QUALITY_MULTIPLIER[quality];
        if (isManifestationDay(npcId))
            multiplier *= MANIFESTATION_BONUS;
        const change = Math.round(base * multiplier);
        addAffinity(npcId, change);
        state.offeredToday = true;
        state.offersThisWeek++;
        let reaction = '……';
        if (base === OFFERING_RESONANT)
            reaction = `${def.name}感到灵犀相通。`;
        else if (base === OFFERING_PLEASED)
            reaction = `${def.name}表示认可。`;
        else if (base === OFFERING_REPELLED)
            reaction = `${def.name}皱起了眉头。`;
        else
            reaction = `${def.name}接受了供奉。`;
        return { success: true, message: reaction, affinityChange: change };
    };
    /** 执行独特互动 */
    const performSpecialInteraction = (npcId) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def)
            return { success: false, message: '找不到此仙灵。', affinityChange: 0 };
        ...
        if (state.interactedToday)
            return { success: false, message: '今日已互动过了。', affinityChange: 0 };
        ...
        switch (def.interactionType) {
            case 'meditation': {
                // 参悟：缘分增加 = 总技能等级 × 3
                const totalLevels = skillStore.getSkill('farming').level +
    ...
    // ==================== 求缘与结缘 ====================
    const startCourting = (npcId) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def)
            return { success: false, message: '找不到此仙灵。' };
        if (!def.bondable)
            return { success: false, message: '此仙灵无法结缘。' };
        if (state.courting)
            return { success: false, message: '已在求缘中。' };
        if (state.bonded)
            return { success: false, message: '已结缘。' };
        if (state.affinity < def.courtshipThreshold)
            return { success: false, message: `缘分不足（需要${def.courtshipThreshold}）。` };
        // 检查是否已有结缘对象
    ...
    const formBond = (npcId) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def)
            return { success: false, message: '找不到此仙灵。' };
        if (!state.courting)
            return { success: false, message: '需要先求缘。' };
        if (state.bonded)
            return { success: false, message: '已结缘。' };
        if (state.affinity < def.bondThreshold)
            return { success: false, message: `缘分不足（需要${def.bondThreshold}）。` };
        const inventoryStore = useInventoryStore();
    ...
    const dissolveBond = (npcId) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def)
            return { success: false, message: '找不到此仙灵。' };
        if (!state.bonded && !state.courting)
            return { success: false, message: '无缘分羁绊可解除。' };
        const playerStore = usePlayerStore();
        if (playerStore.money < 10000)
            return { success: false, message: '需要10000文。' };
    ...
    // ==================== 仙灵物品制作 ====================
    const craftSpiritItem = (npcId, type) => {
        const def = getHiddenNpcById(npcId);
        if (!def)
            return { success: false, message: '找不到此仙灵。' };
    ...
    const checkHeartEvent = (npcId) => {
        const state = getHiddenNpcState(npcId);
        const def = getHiddenNpcById(npcId);
        if (!state || !def || state.discoveryPhase !== 'revealed')
            return null;
    ...
    /** 查询指定能力是否已激活 */
    const isAbilityActive = (abilityId) => {
        return getActiveAbilities.value.some(a => a.id === abilityId);
    };
    /** 获取指定能力的被动值（未激活则返回0） */
    const getAbilityValue = (abilityId) => {
    ...
    const deserialize = (data) => {
        const savedStates = (data.hiddenNpcStates ?? []).map((s) => ({
