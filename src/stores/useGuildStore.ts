...
export const useGuildStore = defineStore("guild", () => {
  ...
  const guildProjects = ref<Record<GuildProjectId, number>>({
    monster_watch: 0,
    supply_line: 0,
    sect_drill: 0,
  });
  type SectOfficeId = "outer_hall" | "alchemy_hall" | "patrol_hall";
  const sectOffices = ref<Record<SectOfficeId, number>>({
    outer_hall: 0,
    alchemy_hall: 0,
    patrol_hall: 0,
  });
  ...
  const completedGoalCount = computed(() => {
    return MONSTER_GOALS.filter(
      (g) => (monsterKills.value[g.monsterId] ?? 0) >= g.killTarget,
    ).length;
  });
  ...
  const claimableGoals = computed(() => {
    return MONSTER_GOALS.filter(
      (g) =>
        (monsterKills.value[g.monsterId] ?? 0) >= g.killTarget &&
        !claimedGoals.value.includes(g.monsterId),
    );
  });
  ...
  const donateItem = (
    itemId: string,
    quantity: number,
  ): { success: boolean; pointsGained: number } => {
  ...
  const guildProjectCards = computed(() =>
    GUILD_PROJECTS.map((p) => ({
      ...p,
      level: guildProjects.value[p.id] ?? 0,
      completed: (guildProjects.value[p.id] ?? 0) >= p.max,
    })),
  );
  const guildProjectBonusText = computed(
    () =>
      `协作工程：攻击+${guildProjects.value.monster_watch * 2}，生命+${guildProjects.value.supply_line * 8}，周常贡献+${guildProjects.value.sect_drill * 2}%`,
  );
  const sectOfficeCards = computed(() =>
    SECT_OFFICES.map((o) => ({
      ...o,
      level: sectOffices.value[o.id] ?? 0,
      completed: (sectOffices.value[o.id] ?? 0) >= o.max,
    })),
  );
  const sectOfficeBonusText = computed(
    () =>
      `宗门经营：捐献收益+${sectOffices.value.outer_hall * 2}%，丹药补给+${sectOffices.value.alchemy_hall}级，巡山战备+${sectOffices.value.patrol_hall}级`,
  );
  const upgradeSectOffice = (
    id: SectOfficeId,
  ): { success: boolean; message: string } => {
    const office = SECT_OFFICES.find((o) => o.id === id);
    if (!office) return { success: false, message: "宗门机构不存在。" };
    if ((sectOffices.value[id] ?? 0) >= office.max)
      return { success: false, message: "该机构已满级。" };
    const inv = useInventoryStore();
    const player = usePlayerStore();
    if (inv.getItemCount(office.itemId) < office.quantity)
      return {
        success: false,
        message: `${office.itemName}不足，需要${office.quantity}。`,
      };
    if (!player.spendMoney(office.cost))
      return { success: false, message: `铜钱不足，需要${office.cost}文。` };
  ...
  const contributeGuildProject = (
    id: GuildProjectId,
  ): { success: boolean; message: string } => {
    const project = GUILD_PROJECTS.find((p) => p.id === id);
    if (!project) return { success: false, message: "工程不存在。" };
    if ((guildProjects.value[id] ?? 0) >= project.max)
      return { success: false, message: "工程已满级。" };
    const inv = useInventoryStore();
    if (inv.getItemCount(project.itemId) < project.quantity)
      return {
        success: false,
        message: `${project.itemName}不足，需要${project.quantity}。`,
      };
  ...
  const weeklyExpeditions = computed(() => {
    ensureWeeklyReset();
    const totalKills = Object.values(monsterKills.value).reduce(
      (s, n) => s + (Number(n) || 0),
      0,
    );
    return WEEKLY_EXPEDITIONS.map((e) => {
      const progress =
        e.id === "raid"
          ? totalKills
          : e.id === "donate"
            ? guildLevel.value >= 2 || contributionPoints.value >= 300
              ? 1
              : 0
            : contributionPoints.value >= 500
              ? 1
              : 0;
      return {
        ...e,
        progress,
        done: progress >= e.target,
        claimed: weeklyExpeditionClaimed.value.includes(e.id),
      };
    });
  });

  const claimWeeklyExpedition = (
    id: string,
  ): { success: boolean; message: string } => {
    ensureWeeklyReset();
    ...
    inventory.addItem(
      "spirit_stone",
      Math.max(3, Math.floor(task.rewardPoints / 20)),
    );
  ...
  const buyShopItem = (itemId: string): boolean => {
    ...
    if (item.weeklyLimit) {
      ensureWeeklyReset();
      if ((weeklyPurchases.value[itemId] ?? 0) >= item.weeklyLimit)
        return false;
    }
  ...
  const serialize = () => ({
    ...
    guildProjects: { ...guildProjects.value },
    sectOffices: { ...sectOffices.value },
  });
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    dailyPurchases.value =
      ((data as Record<string, unknown>).dailyPurchases as Record<
        string,
        number
      >) ?? {};
    lastResetDay.value =
      ((data as Record<string, unknown>).lastResetDay as number) ?? -1;
    weeklyPurchases.value =
      ((data as Record<string, unknown>).weeklyPurchases as Record<
        string,
        number
      >) ?? {};
    lastResetWeek.value =
      ((data as Record<string, unknown>).lastResetWeek as number) ?? -1;
    totalPurchases.value =
      ((data as Record<string, unknown>).totalPurchases as Record<
        string,
        number
      >) ?? {};
    weeklyExpeditionClaimed.value =
      ((data as Record<string, unknown>).weeklyExpeditionClaimed as string[]) ??
      [];
    guildProjects.value = {
      monster_watch: 0,
      supply_line: 0,
      sect_drill: 0,
      ...((data as any).guildProjects ?? {}),
    };
    sectOffices.value = {
      outer_hall: 0,
      alchemy_hall: 0,
      patrol_hall: 0,
      ...((data as any).sectOffices ?? {}),
    };
    ...
    if (isOldSave && claimedGoals.value.length > 0) {
      ...
      guildLevel.value =
        ((data as Record<string, unknown>).guildLevel as number) ?? 0;
    }
  };
