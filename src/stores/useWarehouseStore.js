...
export const useWarehouseStore = defineStore('warehouse', () => {
    ...
    // ---- 箱子管理 ----
    /** 创建箱子 */
    const addChest = (tier, label) => {
        if (chests.value.length >= maxChests.value)
            return false;
    ...
    /** 删除空箱子 */
    const removeChest = (chestId) => {
        const idx = chests.value.findIndex(c => c.id === chestId);
        if (idx === -1)
            return false;
        if (chests.value[idx].items.length > 0)
            return false;
        chests.value.splice(idx, 1);
        return true;
    };
    /** 重命名箱子 */
    const renameChest = (chestId, label) => {
    ...
    /** 获取箱子引用 */
    const getChest = (chestId) => {
        return chests.value.find(c => c.id === chestId);
    };
    /** 获取箱子容量 */
    const getChestCapacity = (chestId) => {
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest)
            return 0;
    ...
    // ---- 物品操作 ----
    /** 直接往箱子加物品（内部/自动路由用） */
    const addItemToChest = (chestId, itemId, quantity = 1, quality = 'normal') => {
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest)
            return false;
    ...
    /** 直接从箱子移除物品 */
    const removeItemFromChest = (chestId, itemId, quantity = 1, quality) => {
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest)
            return false;
    ...
    /** 查询箱子内物品数量 */
    const getChestItemCount = (chestId, itemId, quality) => {
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest)
            return 0;
        return chest.items
    ...
    const depositToChest = (chestId, itemId, quantity, quality) => {
        ...
        if (actual <= 0)
            return 0;
        if (!inv.removeItem(itemId, actual, quality))
            return 0;
        addItemToChest(chestId, itemId, actual, quality);
        return actual;
    };
    /** 从箱子取出到背包 */
    const withdrawFromChest = (chestId, itemId, quantity, quality) => {
        const inv = useInventoryStore();
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest)
            return false;
    ...
    // ---- 仓库扩容 ----
    /** 扩容仓库（增加箱子槽位） */
    const expandMaxChests = () => {
        if (maxChests.value >= MAX_CHESTS_CAP)
            return false;
        maxChests.value += 1;
        return true;
    };
    // ---- 虚空箱管理 ----
    /** 设置虚空箱角色（同角色互斥） */
    const setVoidRole = (chestId, role) => {
        const chest = chests.value.find(c => c.id === chestId);
        if (!chest || chest.tier !== 'void')
            return false;
        // 清除同角色的其他箱子
        if (role !== 'none') {
    ...
    // ---- 箱子排序 ----
    /** 移动箱子位置（上移/下移） */
    const moveChest = (chestId, direction) => {
        const idx = chests.value.findIndex(c => c.id === chestId);
        if (idx === -1)
            return false;
        const targetIdx = direction === 'up' ? idx - 1 : idx + 1;
        if (targetIdx < 0 || targetIdx >= chests.value.length)
            return false;
    ...
    const sortChest = (chestId) => {
        const chest = getChest(chestId);
        if (!chest || chest.items.length === 0)
            return;
        // 合并同类栈
        const merged = [];
    ...
    const deserialize = (data) => {
        ...
        maxChests.value = data.maxChests ?? INITIAL_MAX_CHESTS;
        // 旧存档迁移：有 items 无 chests
        if (data.items && !data.chests) {
            const oldItems = data.items.filter(i => getItemById(i.itemId));
            if (oldItems.length > 0) {
                // 金箱容量36，超出时分多个箱子
                const goldCap = CHEST_DEFS.gold.capacity;
                const migratedChests = [];
        ...
        if (!unlocked.value && chests.value.length > 0)
            unlocked.value = true;
    };
    return {
        unlocked,
