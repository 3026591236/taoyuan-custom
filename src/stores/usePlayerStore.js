...
export const usePlayerStore = defineStore('player', () => {
    ...
    const getMaxHp = () => {
        ...
        if (perk5 === 'fighter')
            bonus += FIGHTER_HP_BONUS;
        if (perk10 === 'warrior')
            bonus += WARRIOR_HP_BONUS;
    ...
    const consumeStamina = (amount) => {
        ...
        if (stamina.value < effectiveAmount)
            return false;
    ...
    /** 提升体力上限 */
    const upgradeMaxStamina = () => {
        if (staminaCapLevel.value >= STAMINA_CAPS.length - 1)
            return false;
    ...
    /** 增加额外体力上限加成（仙翁金丹等） */
    const addBonusMaxStamina = (amount) => {
    ...
    const deserialize = (data) => {
        const hasIdentity = data.playerName != null;
        playerName.value = data.playerName ?? '未命名';
        ...
        hp.value = data.hp ?? BASE_MAX_HP;
        baseMaxHp.value = data.baseMaxHp ?? BASE_MAX_HP;
    };
    return {
        playerName,
        gender,
