  ...
  <div>
    ...
    <div class="flex space-x-1 mb-3">
      <Button
        ...
        :class="{ '!bg-accent !text-bg': tab === 'goals' }"
        @click="tab = 'goals'"
        >讨伐</Button
      >
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'donate' }"
        @click="tab = 'donate'"
        >捐献</Button
      >
      <Button
      ...
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'weekly' }"
        @click="tab = 'weekly'"
        >周常</Button
      >
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'bestiary' }"
        @click="tab = 'bestiary'"
        >图鉴</Button
      >
    </div>
        ...
        >
          <div class="flex items-center justify-between mb-1">
            <div class="flex items-center space-x-1.5">
              <CircleCheck
                v-if="isGoalClaimed(goal.monsterId)"
                :size="12"
              ...
              <Circle
                ...
                class="shrink-0"
                :class="
                  getKillCount(goal.monsterId) >= goal.killTarget
                    ? 'text-accent'
                    : 'text-muted'
                "
              />
              <span
                class="text-xs"
                :class="
                  isGoalClaimed(goal.monsterId) ? 'text-success' : 'text-text'
                "
                >{{ goal.monsterName }}</span
              >
            </div>
            <span class="text-xs text-muted"
              >{{ getKillCount(goal.monsterId) }}/{{ goal.killTarget }}</span
            >
          </div>
          <!-- 进度条 -->
            ...
            <div
              ...
              :style="{
                width:
                  Math.min(
                    100,
                    (getKillCount(goal.monsterId) / goal.killTarget) * 100,
                  ) + '%',
              }"
            />
          </div>
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="
              isGoalClaimed(selectedGoal.monsterId)
                ? 'text-success'
                : 'text-accent'
            "
          >
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs"
                >{{ getKillCount(selectedGoal.monsterId) }}/{{
                  selectedGoal.killTarget
                }}</span
              >
            </div>
            ...
            <p class="text-xs text-accent">
              {{ selectedGoal.reward.money }}文{{
                selectedGoal.reward.items
                  ? " + " +
                    selectedGoal.reward.items
                      .map((i) => `${getDropName(i.itemId)}×${i.quantity}`)
                      .join("、")
                  : ""
              }}
          ...
          <div
            v-if="isGoalClaimed(selectedGoal.monsterId)"
          ...
          <Button
            :icon="Gift"
            v-else-if="
              getKillCount(selectedGoal.monsterId) >= selectedGoal.killTarget
            "
        ...
        >
          <div class="flex-1">
            ...
            <p class="text-xs text-muted">
              持有 {{ item.count }} · 每个 {{ item.points }} 贡献点
            </p>
          </div>
          <span
            class="text-xs ml-2"
            :class="item.count > 0 ? 'text-accent' : 'text-muted'"
            >{{ item.count * item.points }}点</span
          >
        </div>
      </div>
    </div>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div class="flex space-x-1">
              <Button
                class="flex-1 justify-center"
                :disabled="donateQuantity <= 1"
                @click="donateQuantity = 1"
                >最少</Button
              >
              <Button
                class="flex-1 justify-center"
                :disabled="donateQuantity >= donateModalItem.count"
                @click="setDonateQuantityToMax"
              >
                最多
              </Button>
            </div>
            <div class="flex items-center justify-between mt-1.5">
              ...
              <span class="text-xs text-accent"
                >{{ donateQuantity * donateModalItem.points }} 贡献点</span
              >
            </div>
          </div>
          ...
          <div v-if="!donateConfirmed" class="flex flex-col space-y-1">
            <Button
              class="btn text-xs w-full justify-center !bg-accent !text-bg"
              :icon="HandHeart"
              @click="donateConfirmed = true"
            >
              确认捐献
            </Button>
          </div>
          <div v-else class="flex flex-col space-y-1">
            <p class="text-xs text-center text-danger mb-1">
              确定捐献 {{ donateQuantity }} 个{{ donateModalItem.name }}？
            </p>
            <div class="flex space-x-2">
              <Button
                class="flex-1 btn text-xs justify-center"
                @click="donateConfirmed = false"
                >取消</Button
              >
              <Button
      ...
      >
        <div>
          <p
            class="text-sm"
            :class="
              guildStore.isShopItemUnlocked(item.itemId) ? '' : 'text-muted'
            "
          >
            {{ item.name }}
          </p>
          ...
          <p
            v-if="item.materials && guildStore.isShopItemUnlocked(item.itemId)"
          ...
          <p
            v-if="
              ...
              !guildStore.isShopItemUnlocked(item.itemId)
            "
          ...
          >
            今日剩余:
            {{ guildStore.getDailyRemaining(item.itemId, item.dailyLimit) }}/{{
              item.dailyLimit
            }}
          </p>
          <p
            v-if="
              item.weeklyLimit && guildStore.isShopItemUnlocked(item.itemId)
            "
            class="text-xs text-muted mt-0.5"
          >
            本周剩余:
            {{
              guildStore.getWeeklyRemaining(item.itemId, item.weeklyLimit)
            }}/{{ item.weeklyLimit }}
          </p>
          <p
            v-if="item.totalLimit && guildStore.isShopItemUnlocked(item.itemId)"
            class="text-xs text-muted mt-0.5"
          >
            总限购:
            {{ guildStore.getTotalRemaining(item.itemId, item.totalLimit) }}/{{
              item.totalLimit
            }}
          </p>
        </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="
                  shopModalItem.contributionCost
                    ? 'text-success'
                    : 'text-accent'
                "
              >
                {{
                  shopModalItem.contributionCost
                    ? `${shopModalItem.contributionCost} 贡献点`
                    : `${shopModalItem.price}文`
                }}
              </span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                inventoryStore.getItemCount(shopModalItem.itemId)
              }}</span>
            </div>
            <div
              v-if="shopModalItem.contributionCost"
            ...
            >
              ...
              <span
                ...
                :class="
                  ...
                  (shopModalItem.contributionCost ?? 0) * shopBuyQty
                    ? 'text-text'
                    : 'text-danger'
                "
              >
            ...
            <div v-else class="flex items-center justify-between mt-0.5">
              ...
              <span
                class="text-xs"
                :class="
                  playerStore.money >= shopModalItem.price * shopBuyQty
                    ? 'text-text'
                    : 'text-danger'
                "
              >
            ...
            <template v-if="shopModalItem.materials">
              ...
              <div
                v-for="mat in shopModalItem.materials"
                :key="mat.itemId"
                class="flex items-center justify-between mt-0.5"
              >
                <span class="text-xs"
                  >{{ getMaterialName(mat.itemId) }} ×{{
                    mat.quantity * shopBuyQty
                  }}</span
                >
                <span
                  class="text-xs"
                  :class="
                    inventoryStore.getItemCount(mat.itemId) >=
                    mat.quantity * shopBuyQty
                      ? 'text-success'
                      : 'text-danger'
                  "
            ...
            <div
              v-if="shopModalItem.dailyLimit"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">今日剩余</span>
              <span class="text-xs">
                {{
                  guildStore.getDailyRemaining(
                    shopModalItem.itemId,
                    shopModalItem.dailyLimit,
                  )
                }}/{{ shopModalItem.dailyLimit }}
              </span>
            </div>
            ...
            <div
              v-if="shopModalItem.totalLimit"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">总限购剩余</span>
              <span class="text-xs">
                {{
                  guildStore.getTotalRemaining(
                    shopModalItem.itemId,
                    shopModalItem.totalLimit,
                  )
                }}/{{ shopModalItem.totalLimit }}
              </span>
            </div>
          ...
          >
            <div class="flex items-center justify-between mb-1.5">
              ...
              <div class="flex items-center space-x-1">
                <Button
                  class="h-6 px-1.5 py-0.5 text-xs justify-center"
                  :disabled="shopBuyQty <= 1"
                  @click="addShopBuyQty(-1)"
                  >-</Button
                >
                <input
                  type="number"
                ...
                <Button
                  class="h-6 px-1.5 py-0.5 text-xs justify-center"
                  :disabled="shopBuyQty >= maxShopBuyQty"
                  @click="addShopBuyQty(1)"
                >
                  +
                </Button>
            ...
            <div class="flex space-x-1">
              <Button
                class="flex-1 justify-center"
                :disabled="shopBuyQty <= 1"
                @click="setShopBuyQty(1)"
                >最少</Button
              >
              <Button
                class="flex-1 justify-center"
                :disabled="shopBuyQty >= maxShopBuyQty"
                @click="setShopBuyQty(maxShopBuyQty)"
              >
                最多
              </Button>
            </div>
            <div class="flex items-center justify-between mt-1.5">
              ...
              >
                {{ shopBuyTotalCost
                }}{{ shopModalItem.contributionCost ? "贡献点" : "文" }}
              </span>
            </div>
          </div>
          ...
          <p
            v-if="
              shopModalItem.unlockGuildLevel &&
              !guildStore.isShopItemUnlocked(shopModalItem.itemId)
            "
    ...
    <div v-if="tab === 'weekly'" class="flex flex-col space-y-2">
      <div class="border border-accent/20 rounded-xs p-3 bg-accent/5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div
            v-for="project in guildStore.guildProjectCards"
            :key="project.id"
            class="border border-accent/10 rounded-xs p-2"
          >
            <div class="flex items-center justify-between">
              <span class="text-xs text-accent">{{ project.name }}</span
              ><span class="text-[10px] text-muted"
                >Lv.{{ project.level }}/{{ project.max }}</span
              >
            </div>
            ...
            <p class="text-[10px] text-warning">
              消耗：{{ project.itemName }}×{{ project.quantity }}
            </p>
            <Button
              class="w-full justify-center mt-1"
              :disabled="project.completed"
              @click="guildStore.contributeGuildProject(project.id)"
              >{{ project.completed ? "已满级" : "投入物资" }}</Button
            >
          </div>
        </div>
      </div>
      ...
      <div
        v-for="task in guildStore.weeklyExpeditions"
        :key="task.id"
      ...
      >
        <div class="flex items-center justify-between mb-1">
          ...
          <span class="text-[10px] text-muted"
            >{{ task.progress }}/{{ task.target }}</span
          >
        </div>
    ...
    <div v-if="tab === 'bestiary'">
      <div
        v-if="guildStore.encounteredMonsters.length === 0"
      ...
      <template v-else>
        <p class="text-xs text-muted mb-2">
          已发现 {{ guildStore.encounteredMonsters.length }}/{{
            allMonsters.length
          }}
        </p>
          ...
          <div v-for="group in monsterGroups" :key="group.label">
            ...
            <div class="grid grid-cols-3 md:grid-cols-5 gap-1 mr-1">
              <div
                ...
                "
              >
                <template v-if="guildStore.isEncountered(monster.id)">{{
                  monster.name
                }}</template>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                guildStore.getKillCount(selectedMonster.id)
              }}</span>
            </div>
          </div>

          <div
            v-if="selectedMonster.drops.length > 0"
            class="border border-accent/10 rounded-xs p-2"
          >
            <p class="text-xs text-muted mb-1">掉落物</p>
            <div
              v-for="drop in selectedMonster.drops"
              :key="drop.itemId"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs">{{ getDropName(drop.itemId) }}</span>
              <span class="text-xs text-muted"
                >{{ Math.round(drop.chance * 100) }}%</span
              >
            </div>
          </div>

          <div
            v-if="getEquipDrops(selectedMonster).length > 0"
            class="border border-accent/10 rounded-xs p-2 mt-2"
          >
            <p class="text-xs text-muted mb-1">装备掉落</p>
            <div
              v-for="(drop, idx) in getEquipDrops(selectedMonster)"
              :key="idx"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs">
                {{ drop.name }}
                <span v-if="drop.firstKill" class="text-[10px] text-accent"
                  >（首杀）</span
                >
              </span>
              <span v-if="drop.chance !== null" class="text-xs text-muted"
                >{{ Math.round(drop.chance * 100) }}%</span
              >
    ...
    <div class="mt-3 border border-accent/20 rounded-xs p-2">
        ...
        <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
          <div
            ...
            :style="{
              width:
                Math.round(
                  (guildStore.completedGoalCount / MONSTER_GOALS.length) * 100,
                ) + '%',
            }"
          />
        </div>
      ...
      <div class="grid grid-cols-2 gap-x-3 gap-y-0.5">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs"
            >{{ guildStore.completedGoalCount }}/{{
              MONSTER_GOALS.length
            }}</span
          >
        </div>
        <div class="flex items-center justify-between">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs"
            >{{ guildStore.encounteredMonsters.length }}/{{
              allMonsters.length
            }}</span
          >
        </div>
...
const maxShopBuyQty = computed(() => {
  ...
  if (item.contributionCost) {
    max = Math.min(
      max,
      Math.floor(guildStore.contributionPoints / item.contributionCost),
    );
  ...
  if (item.materials) {
    for (const mat of item.materials) {
      max = Math.min(
        max,
        Math.floor(inventoryStore.getItemCount(mat.itemId) / mat.quantity),
      );
    }
  }
  if (item.dailyLimit)
...
const onDonateInput = (e: Event) => {
  ...
  donateQuantity.value = Math.max(
    1,
    Math.min(donateModalItem.value.count, val),
  );
};
...
const setDonateQuantityToMax = () => {
  if (!donateModalItem.value) return;
  donateQuantity.value = donateModalItem.value.count;
};

const handleClaimWeekly = (id: string) => {
  const r = guildStore.claimWeeklyExpedition(id);
  addLog(r.message);
...
const executeDonate = () => {
  if (!donateModalItem.value) return;
  const result = guildStore.donateItem(
    donateModalItem.value.itemId,
    donateQuantity.value,
  );
  if (result.success) {
    addLog(
      `捐献了${donateModalItem.value.name}×${donateQuantity.value}，获得 ${result.pointsGained} 贡献点。`,
    );
  }
...
const canBuyItem = (item: GuildShopItemDef): boolean => {
  ...
  if (
    item.dailyLimit &&
    guildStore.getDailyRemaining(item.itemId, item.dailyLimit) <= 0
  )
    return false;
  if (
    item.weeklyLimit &&
    guildStore.getWeeklyRemaining(item.itemId, item.weeklyLimit) <= 0
  )
    return false;
  if (
    item.totalLimit &&
    guildStore.getTotalRemaining(item.itemId, item.totalLimit) <= 0
  )
    return false;
  if (item.materials) {
    for (const mat of item.materials) {
...
const hasAnyKills = computed(() =>
  Object.values(guildStore.monsterKills).some((v) => v > 0),
);
...
const getEquipDrops = (
  monster: MonsterDef,
): { name: string; chance: number | null; firstKill: boolean }[] => {
