<template>
  <div>
    <div class="flex items-center justify-between mb-1">
      ...
      <span v-if="hanhaiStore.unlocked" class="text-xs text-success"
        >已开通</span
      >
    ...
    >
      ...
      <p v-if="bossDefeated && !canUnlock" class="text-xs text-danger">
    ...
    <template v-else>
      ...
      <div class="flex space-x-1 mb-3">
        ...
        <Button
          class="flex-1 justify-center"
          :class="{ '!bg-accent !text-bg': activeTab === 'trade' }"
        ...
        <Button
          class="flex-1 justify-center"
          :class="{ '!bg-accent !text-bg': activeTab === 'caravan' }"
          @click="activeTab = 'caravan'"
        >
          商路
        </Button>
        <Button
      ...
      <template v-if="activeTab === 'shop'">
          ...
          >
            ...
            <div class="flex flex-col items-end ml-2 shrink-0">
              ...
              <span
                ...
                class="text-[10px]"
                :class="
                  hanhaiStore.getWeeklyRemaining(item.itemId) > 0
                    ? 'text-muted'
                    : 'text-danger'
                "
              >
                限购 {{ hanhaiStore.getWeeklyRemaining(item.itemId) }}/{{
                  item.weeklyLimit
                }}
              </span>
            </div>
        ...
        <!-- 藏宝图寻宝 -->
        <Button
          v-if="treasureMapCount > 0"
          :icon="Map"
          :icon-size="12"
          class="w-full justify-center mt-2"
          @click="handleUseTreasureMap"
        >
      ...
      <template v-if="activeTab === 'trade'">
        ...
        <div class="border border-accent/20 rounded-xs p-2 mb-3">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-accent">{{
              hanhaiStore.tradePoints
            }}</span>
          </div>
          <div class="flex items-center justify-between mt-0.5">
            <span class="text-xs text-muted">店铺等级</span>
            <span class="text-xs"
              >{{ hanhaiStore.tradeShopConfig.name }}（Lv.{{
                hanhaiStore.tradeShopLevel
              }}）</span
            >
          </div>
        </div>
        ...
        <div class="mb-3">
          <div class="flex items-center justify-between mb-1">
            ...
            <span class="text-xs text-muted"
              >{{ hanhaiStore.tradeSlots.length }}/{{
                hanhaiStore.tradeShopConfig.maxSlots
              }}</span
            >
          </div>
          <!-- 已有槽位 -->
          <div
            v-for="(slot, idx) in hanhaiStore.tradeSlots"
            :key="idx"
            class="border border-accent/10 rounded-xs px-2 py-1.5 mb-1"
          >
            <div class="flex items-center justify-between">
              <span class="text-xs"
                >{{ getItemName(slot.itemId) }}×{{ slot.quantity }}</span
              >
              <span class="text-xs text-muted"
                >{{ slot.daysRemaining }}天后售出</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
        ...
        <!-- 店铺升级 -->
        <div
          v-if="hanhaiStore.nextTradeShopUpgrade"
          class="border border-accent/20 rounded-xs p-2 mb-3"
        >
          <p class="text-xs text-accent mb-1">店铺升级</p>
          <p class="text-xs text-muted mb-1">
            下一级：{{ hanhaiStore.nextTradeShopUpgrade.name }}（槽位{{
              hanhaiStore.nextTradeShopUpgrade.maxSlots
            }}·{{ hanhaiStore.nextTradeShopUpgrade.sellDays }}天）
          </p>
          <p class="text-xs text-muted mb-1">
            费用：{{ hanhaiStore.nextTradeShopUpgrade.cost }}文
            <template
              v-for="mat in hanhaiStore.nextTradeShopUpgrade.materialCost"
              :key="mat.itemId"
            >
              + {{ getItemName(mat.itemId) }}×{{ mat.quantity }}
            </template>
          </p>
          <Button class="w-full justify-center" @click="handleUpgradeTrade"
            >升级</Button
          >
        </div>
        ...
        <div class="mb-2">
          ...
          <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
            ...
            >
              ...
              <div class="flex flex-col items-end ml-2 shrink-0">
                <span class="text-xs text-accent"
                  >{{ exItem.pointsCost }}积分</span
                >
                <span v-if="exItem.weeklyLimit" class="text-[10px] text-muted"
                  >周限{{ getExchangeWeeklyRemaining(exItem) }}</span
                >
                <span
                  v-if="exItem.totalLimit"
                  class="text-[10px]"
                  :class="
                    getExchangeTotalRemaining(exItem) > 0
                      ? 'text-muted'
                      : 'text-danger'
                  "
                >
      ...
      <template v-if="activeTab === 'caravan'">
        <div class="border border-accent/20 rounded-xs p-2 mb-3">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-accent"
              >{{ hanhaiStore.activeCaravanCount }}/{{
                hanhaiStore.maxCaravanRuns
              }}</span
            >
          </div>
          <div v-if="hanhaiStore.caravanRuns.length" class="mt-2 space-y-1">
            <div
              v-for="run in hanhaiStore.caravanRuns"
              :key="run.routeId + run.daysLeft"
              class="border border-accent/10 rounded-xs px-2 py-1"
            >
              <div class="flex items-center justify-between">
                <span class="text-xs">{{ getRouteName(run.routeId) }}</span
                ><span class="text-[10px] text-muted"
                  >剩余{{ run.daysLeft }}天</span
                >
              </div>
              <p class="text-[10px] text-muted">
                风险：{{ run.risk }} · {{ run.protected ? "护送中" : "未护送" }}
              </p>
            </div>
          </div>
        </div>
        <div class="space-y-2 mb-3">
          <div
            v-for="route in hanhaiStore.caravanRoutes"
            :key="route.id"
            class="border border-accent/20 rounded-xs p-2"
          >
            <div class="flex items-center justify-between mb-1">
              <span class="text-xs text-accent">{{ route.name }}</span
              ><span class="text-[10px] text-muted"
                >{{ route.days }}天 · 风险{{
                  Math.round(route.risk * 100)
                }}%</span
              >
            </div>
            <p class="text-[10px] text-muted mb-1">{{ route.desc }}</p>
            <p class="text-[10px] text-muted mb-2">
              费用{{ route.cost }}文 · 预计商誉+{{ route.rewardPoints }} ·
              铜钱+{{ route.rewardMoney }}
            </p>
            <div class="grid grid-cols-2 gap-1">
              <Button
                ...
                :disabled="
                  hanhaiStore.activeCaravanCount >=
                    hanhaiStore.maxCaravanRuns || playerStore.money < route.cost
                "
                @click="handleStartCaravan(route.id, false)"
                >启程</Button
              >
              <Button
                class="justify-center"
                :disabled="
                  hanhaiStore.activeCaravanCount >=
                    hanhaiStore.maxCaravanRuns ||
                  playerStore.money < route.cost + 600 ||
                  spiritStoneCount < 5
                "
                @click="handleStartCaravan(route.id, true)"
                >护送启程</Button
              >
            </div>
          </div>
      ...
      <template v-if="activeTab === 'casino'">
        <div class="border border-accent/20 rounded-xs p-2 mb-3">
          <div class="flex items-center justify-between">
            ...
            <span
              class="text-xs"
              :class="hanhaiStore.canBet ? 'text-accent' : 'text-danger'"
            >
        ...
        <div v-else class="flex flex-col space-y-2">
          ...
          <div class="border border-accent/20 rounded-xs p-2">
            ...
            <p class="text-xs text-muted mb-2">
              投注{{ DICE_BET_AMOUNT }}文，猜对大小赢2倍
            </p>
            <div class="flex space-x-1">
              <Button
                class="flex-1 justify-center"
                :disabled="playerStore.money < DICE_BET_AMOUNT"
                @click="handleDice(false)"
              >
                猜小 (2-6)
              </Button>
              <Button
          ...
          <div class="border border-accent/20 rounded-xs p-2">
            ...
            <p class="text-xs text-muted mb-2">
              投注{{ CUP_BET_AMOUNT }}文，3选1猜中赢{{ CUP_WIN_MULTIPLIER }}倍
            </p>
            <div class="flex space-x-1">
              <Button
                v-for="i in 3"
          ...
          <div class="border border-accent/20 rounded-xs p-2">
            ...
            <p class="text-xs text-muted mb-2">
              投注{{ CRICKET_BET_AMOUNT }}文，选蛐蛐上场对战，赢{{
          ...
          <div class="border border-accent/20 rounded-xs p-2">
            ...
            <p class="text-xs text-muted mb-2">
              投注{{ CARD_BET_AMOUNT }}文，{{ CARD_TOTAL }}张牌中{{
                CARD_TREASURE_COUNT
              }}张有宝
            </p>
            <div class="flex space-x-1">
              ...
              >
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-muted">
                    入场{{ t.entryFee }}文 + 抽水{{ t.rake }}文 · 盲注{{
                      t.blind
                    }}
                    · {{ t.rounds }}手
                  </p>
                ...
                <Button
                  class="ml-2 shrink-0"
                  :disabled="playerStore.money < t.minMoney"
                  @click="handleTexas(t.id)"
                >
                  {{
                    playerStore.money < t.minMoney
                      ? `需${t.minMoney}文`
                      : "入场"
                  }}
                </Button>
          ...
          <div class="border border-accent/20 rounded-xs p-2">
            ...
            <p class="text-xs text-muted mb-2">
              投注{{ BUCKSHOT_BET_AMOUNT }}文，与庄家轮流开枪，胜者得{{
                BUCKSHOT_WIN_MULTIPLIER
              }}倍
            </p>
            <Button
              class="w-full justify-center"
              :disabled="playerStore.money < BUCKSHOT_BET_AMOUNT"
              @click="handleBuckshot"
              >挑战</Button
            >
          </div>
        </div>
      ...
      <div class="mt-3 border border-accent/20 rounded-xs p-2">
        <div class="grid grid-cols-2 gap-x-3 gap-y-0.5">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs"
              >{{ MAX_DAILY_BETS - hanhaiStore.betsRemaining }}/{{
                MAX_DAILY_BETS
              }}</span
            >
          </div>
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div
              v-if="shopModalItem.weeklyLimit"
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  hanhaiStore.getWeeklyRemaining(shopModalItem.itemId) > 0
                    ? ''
                    : 'text-danger'
                "
              >
                剩余
                {{ hanhaiStore.getWeeklyRemaining(shopModalItem.itemId) }}/{{
                  shopModalItem.weeklyLimit
                }}
              </span>
            </div>
          </div>
          ...
          <Button
            ...
            :disabled="
              playerStore.money < shopModalItem.price ||
              hanhaiStore.getWeeklyRemaining(shopModalItem.itemId) <= 0
            "
            ...
            <div
              ...
              :class="
                rouletteHighlight === i
                  ...
                  : 'border-accent/10 text-muted/40'
              "
            >
              {{ outcome.label }}
          ...
          <template v-if="roulettePhase === 'done' && rouletteAnimResult">
            ...
            >
              <p
                ...
                :class="
                  rouletteAnimResult.multiplier > 0
                    ? 'text-success'
                    : 'text-danger'
                "
              >
              ...
              <p
                v-if="rouletteAnimResult.multiplier > 0"
                class="text-xs text-success"
              >
                +{{ rouletteAnimResult.winnings }}文
              </p>
      ...
      >
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex justify-center space-x-4 mb-3">
            <div
              v-for="(val, di) in diceDisplay"
              :key="di"
              class="dice-face"
              :class="{ 'dice-rolling': dicePhase === 'rolling' }"
            >
              <div
                v-for="pos in 9"
                :key="pos"
          ...
          <p v-if="dicePhase !== 'rolling'" class="text-xs text-center mb-3">
            <span class="text-muted"
              >{{ diceDisplay[0] }} + {{ diceDisplay[1] }} =</span
            >
          ...
          <template v-if="dicePhase === 'done' && diceAnimResult">
            ...
            >
              <p
                class="text-sm"
                :class="diceAnimResult.won ? 'text-success' : 'text-danger'"
              >
              ...
              >
                {{
                  diceAnimResult.won
                    ? "+" + diceAnimResult.winnings + "文"
                    : "-" + DICE_BET_AMOUNT + "文"
                }}
              </p>
            </div>
            <Button class="w-full justify-center" @click="showDiceModal = false"
              >确定</Button
            >
          </template>
        </div>
        ...
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex justify-center space-x-3 mb-3">
            <div
              ...
              :class="{
                'cup-highlight':
                  cupPhase === 'shuffling' && cupShuffleIndex === i - 1,
                'cup-correct':
                  cupPhase === 'done' &&
                  cupAnimResult &&
                  cupAnimResult.correctCup === i - 1,
                'cup-picked':
                  cupPhase === 'done' &&
                  cupGuess === i - 1 &&
                  cupAnimResult &&
            ...
            >
              <div
                class="cup-body"
                :class="{ 'cup-shake': cupPhase === 'shuffling' }"
              >
              ...
              <p
                ...
                :class="
                  cupPhase === 'done' && cupAnimResult?.correctCup === i - 1
                    ? 'text-accent'
                    : 'text-muted/40'
                "
              >
          ...
          <template v-if="cupPhase === 'done' && cupAnimResult">
            ...
            >
              <p
                class="text-sm"
                :class="cupAnimResult.won ? 'text-success' : 'text-danger'"
              >
              ...
              >
                {{
                  cupAnimResult.won
                    ? "+" + cupAnimResult.winnings + "文"
                    : "-" + CUP_BET_AMOUNT + "文"
                }}
              </p>
            </div>
            <Button class="w-full justify-center" @click="showCupModal = false"
              >确定</Button
            >
          </template>
        </div>
          ...
          <div class="flex items-center justify-between space-x-2 mb-3">
            ...
            <div class="flex-1 text-center">
              ...
              <div class="border border-accent/20 rounded-xs p-2">
                <div
                  class="cricket-icon"
                  :class="{ 'cricket-fight': cricketPhase === 'fighting' }"
                >
              ...
              <div class="border border-danger/20 rounded-xs p-2">
                ...
                >
                  {{
                    cricketPhase === "fighting"
                      ? "?"
                      : cricketAnimResult?.opponentPower
                  }}
                </p>
              </div>
            </div>
        ...
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex justify-center space-x-2 mb-3">
            <div
              ...
              :class="{
                'card-flipping':
                  cardPhase === 'flipping' && cardFlipIndex === i - 1,
                'card-treasure':
                  cardPhase === 'done' &&
                  cardAnimResult &&
                  cardAnimResult.treasures.includes(i - 1),
                'card-empty':
                  cardPhase === 'done' &&
                  cardAnimResult &&
                  !cardAnimResult.treasures.includes(i - 1),
            ...
            >
              <template v-if="cardPhase === 'done' && cardAnimResult">
                <Gem
                  v-if="cardAnimResult.treasures.includes(i - 1)"
                  :size="16"
                  class="text-success"
                />
              ...
              <p
                class="text-xs mt-0.5"
                :class="cardPick === i - 1 ? 'text-accent' : 'text-muted/30'"
              >
                {{ i }}
              </p>
            </div>
          ...
          <template v-if="cardPhase === 'done' && cardAnimResult">
            ...
            >
              <p
                class="text-sm"
                :class="cardAnimResult.won ? 'text-success' : 'text-danger'"
              >
              ...
              >
                {{
                  cardAnimResult.won
                    ? "+" + cardAnimResult.winnings + "文"
                    : "-" + CARD_BET_AMOUNT + "文"
                }}
              </p>
            </div>
            <Button class="w-full justify-center" @click="showCardModal = false"
              >确定</Button
            >
          </template>
        </div>
    ...
    <Transition name="panel-fade">
      <div
        v-if="showTexasModal && texasSetup"
        class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4"
      >
    ...
    <Transition name="panel-fade">
      <div
        v-if="showBuckshotModal && buckshotSetup"
        class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4"
      >
        <BuckshotRouletteGame
          :setup="buckshotSetup"
          @complete="handleBuckshotComplete"
        />
      </div>
    </Transition>
          ...
          <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
            ...
            >
              <div class="flex-1 min-w-0">
                ...
                <span
                  v-if="inv.quality !== 'normal'"
                  class="text-[10px] ml-1"
                  :class="qualityColor(inv.quality)"
                >
              ...
              <span class="text-[10px] text-accent shrink-0"
                >~{{ calcPreviewPoints(inv) }}积分</span
              >
            </div>
          </div>
          <p
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="tradeSelectedItem = null"
          >
            <X :size="14" />
          </button>
          <p class="text-sm text-accent mb-2">
            上架：{{ tradeSelectedItem.name }}
          </p>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="qualityColor(tradeSelectedItem.quality)"
                >{{ qualityLabel(tradeSelectedItem.quality) }}</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">持有</span>
            ...
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs"
                >{{ hanhaiStore.tradeShopConfig.sellDays }}天</span
              >
            </div>
          </div>
          ...
          <Button
            class="w-full justify-center !bg-accent !text-bg"
            @click="handleConfirmTradeSlot"
            >上架 ×{{ tradeQuantity }}</Button
          >
        </div>
      </div>
    </Transition>
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div
              v-if="exchangeModalItem.weeklyLimit"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">本周剩余</span>
              <span class="text-xs">{{
                getExchangeWeeklyRemaining(exchangeModalItem)
              }}</span>
            </div>
            <div
              v-if="exchangeModalItem.totalLimit"
              class="flex items-center justify-between mt-0.5"
            >
              ...
              <span
                class="text-xs"
                :class="
                  getExchangeTotalRemaining(exchangeModalItem) > 0
                    ? ''
                    : 'text-danger'
                "
              >
...
const bossDefeated = computed(() =>
  miningStore.defeatedBosses.includes("abyss_dragon"),
);
const canUnlock = computed(
  () => bossDefeated.value && playerStore.money >= HANHAI_UNLOCK_COST,
);
const handleUnlock = () => {
...
const rouletteAnimResult = ref<{ multiplier: number; winnings: number } | null>(
  null,
);
...
const diceSum = computed(
  () => (diceDisplay.value[0] ?? 0) + (diceDisplay.value[1] ?? 0),
);
...
const treasureMapCount = computed(() =>
  inventoryStore.getItemCount("hanhai_map"),
);
const spiritStoneCount = computed(() =>
  inventoryStore.getItemCount("spirit_stone"),
);
const getRouteName = (id: string) =>
  hanhaiStore.caravanRoutes.find((r) => r.id === id)?.name ?? id;
const handleStartCaravan = (
  id: "short" | "long" | "forbidden",
  protect: boolean,
) => {
  const r = hanhaiStore.startCaravanRoute(id, protect);
  addLog(r.message);
  if (r.success) showFloat("商队启程", "success");
};
const handleEscort = (id: "escort" | "scout" | "guard") => {
  const r = hanhaiStore.finishEscortContract(id);
  addLog(r.message);
  showFloat(r.message, r.success ? "success" : "danger");
};
...
const handleRoulette = (betTier: number) => {
  ...
  const targetIndex = ROULETTE_OUTCOMES.findIndex(
    (o) => o.multiplier === result.multiplier,
  );
...
const tradePreviewPoints = computed(() => {
  ...
  const base = calcTradePoints(
    tradeSelectedItem.value.sellPrice * tradeQuantity.value,
    tradeSelectedItem.value.quality,
  );
  return Math.ceil(base * (1 + walletStore.getTradeBonus()));
});
...
const handleConfirmTradeSlot = () => {
  ...
  const result = hanhaiStore.addTradeSlot(
    tradeSelectedItem.value.id,
    tradeSelectedItem.value.quality,
    tradeQuantity.value,
  );
  if (result.success) {
...
const getExchangeWeeklyRemaining = (item: TradeExchangeItemDef): number => {
  ...
  return Math.max(
    0,
    item.weeklyLimit - (hanhaiStore.weeklyExchangePurchases[item.itemId] ?? 0),
  );
};
...
const getExchangeTotalRemaining = (item: TradeExchangeItemDef): number => {
  ...
  return Math.max(
    0,
    item.totalLimit - (hanhaiStore.totalExchangePurchases[item.itemId] ?? 0),
  );
};
