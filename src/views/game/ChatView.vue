  ...
  <div class="space-y-3">
    ...
    <template v-else>
      ...
      <div class="flex gap-1 mb-2">
        <button v-for="ch in channels" :key="ch.key" class="btn flex-1 justify-center text-xs" :class="{ '!bg-accent !text-bg': activeChannel === ch.key }" @click="switchChannel(ch.key)">
          {{ ch.label }}
        </button>
      </div>
      ...
      <!-- 消息列表 -->
      <div ref="chatListRef" class="border border-accent/15 rounded-xs p-2 bg-bg/40 max-h-72 overflow-y-auto space-y-1" @scroll="onScroll">
        <div v-if="loading && messages.length === 0" class="text-xs text-muted text-center py-4">加载中...</div>
        <div v-else-if="messages.length === 0" class="text-xs text-muted text-center py-4">还没有消息，来说点什么吧！</div>
        <template v-for="msg in messages" :key="msg.id">
          <div class="text-xs leading-relaxed py-0.5" :class="msg.isMine ? 'text-right' : ''">
            <template v-if="!msg.isMine">
              <span class="text-accent/80 font-bold cursor-pointer hover:text-accent" @click="handleNameClick(msg)">{{ msg.fromPlayerName || msg.fromUsername }}</span>
              <span v-if="msg.fromRealmName" class="text-[10px] text-muted/60 ml-0.5">[{{ msg.fromRealmName }}]</span>
              <span class="text-muted mx-0.5">:</span>
            </template>
            <span :class="msg.isMine ? 'text-accent/90' : 'text-text/85'">{{ msg.content }}</span>
            <template v-if="msg.isMine">
              <span class="text-muted mx-0.5">:</span>
              <span class="text-accent/80 font-bold">我</span>
            </template>
            <span class="text-[8px] text-muted/40 ml-1">{{ formatTime(msg.createdAt) }}</span>
          </div>
        </template>
      </div>
      ...
      <div class="flex gap-1.5">
        ...
        />
        <button class="btn text-xs shrink-0" :disabled="!inputText.trim() || sending" @click="sendMessage">
          {{ sending ? '...' : '发送' }}
        </button>
      </div>

      <p class="text-[10px] text-muted/40 text-center">文明聊天 · 每条200字以内 · 每10秒5条</p>
    </template>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const fetchMessages = async () => {
    ...
    loading.value = true
    try {
      const params = new URLSearchParams({ channel: activeChannel.value, limit: '50' })
