  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-1">
      ...
      <span class="text-xs text-muted"
        >第{{ gameStore.year }}年 {{ SEASON_NAMES[gameStore.season] }}</span
      >
    </div>
    ...
    <div class="border border-accent/20 rounded-xs p-2 mb-3">
      <div class="flex gap-3">
        ...
        <div class="shrink-0 flex items-center justify-center">
          <div
            class="pixel-avatar-card"
            :class="[
              `avatar-${playerStore.gender}`,
              ascensionStore.ascended
                ? 'avatar-immortal'
          ...
          >
            ...
            <div
              ...
              class="pixel-avatar-immortal-ring"
            ></div>
            <div
              v-if="ascensionStore.ascended"
              class="pixel-avatar-star star-a"
            >
              ✦
            </div>
            <div
              v-if="ascensionStore.ascended"
              class="pixel-avatar-star star-b"
            >
              ✧
            </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          >
            <span class="text-xs text-accent">{{
              cultivationStore.realmName
            }}</span>
            <span class="text-xs text-muted">{{
              cultivationStore.spiritRootName
            }}</span>
          </div>
          <div
            v-if="ascensionStore.ascended"
            class="grid grid-cols-3 gap-1 mb-1 text-center"
          >
            <div
              v-for="part in ascensionStore.bodyProfile"
              :key="part.name"
              class="border border-amber-200/20 rounded-xs px-1 py-0.5 bg-amber-200/5"
            >
          ...
          >
            ...
            <span class="text-xs text-accent font-bold">{{
              cultivationStore.combatPower.toLocaleString()
            }}</span>
          </div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
                <div
                  ...
                  :class="
                    playerStore.staminaPercent > 35 ? 'bg-success' : 'bg-danger'
                  "
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
                <div
                  class="h-full rounded-xs transition-all"
                  :class="
                    playerStore.getHpPercent() > 25 ? 'bg-success' : 'bg-danger'
                  "
                  :style="{ width: playerStore.getHpPercent() + '%' }"
                />
              </div>
              <span class="text-xs whitespace-nowrap"
                >{{ playerStore.hp }}/{{ playerStore.getMaxHp() }}</span
              >
            </div>
            <!-- 铜钱 -->
      ...
      <div class="mt-2 pt-2 border-t border-accent/10">
        <div class="flex items-center justify-between mb-1.5">
          ...
          <span class="text-[10px] text-accent"
            >战力 {{ playerStore.attributeCombatPower }} · 总评
            {{ playerStore.attributePower }}</span
          >
        </div>
        <div class="grid grid-cols-2 gap-1.5">
          ...
          >
            ...
            <div
              class="h-1 bg-bg rounded-xs overflow-hidden border border-accent/10"
            >
              <div
        ...
        <div class="grid grid-cols-4 gap-1 mt-1.5 text-[10px] text-muted">
          ...
          <span
            >减伤
            {{ Math.round(playerStore.attributeDefenseBonus * 100) }}%</span
          >
        </div>
      </div>
      ...
      >
        <div class="grid grid-cols-2 gap-x-3 gap-y-0.5">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-accent"
              >{{ cultivationStore.mana }}/{{ cultivationStore.maxMana }}</span
            >
          </div>
          <div class="flex items-center justify-between">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-accent">{{
              cultivationStore.fieldTierName
            }}</span>
          </div>
          <div class="flex items-center justify-between">
            ...
            <span
              class="text-xs"
              :class="
                cultivationStore.alchemyUnlocked
                  ? 'text-accent'
                  : 'text-muted/40'
              "
              >{{
                cultivationStore.alchemyUnlocked ? "已安置" : "未安置"
              }}</span
            >
          </div>
          <div class="flex items-center justify-between">
        ...
        <div class="mt-1.5 flex items-center space-x-2">
          ...
          <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
            <div
              ...
              :style="{ width: breakthroughPercent + '%' }"
            />
          </div>
          <span
        ...
        <button
          v-if="cultivationStore.canBreakthrough"
    ...
    <div class="border border-accent/20 rounded-xs p-2 mb-3">
      ...
      <div class="grid grid-cols-3 gap-1 mb-1">
        ...
        >
          ...
          <p
            ...
            :class="equippedRing1 ? 'text-accent' : 'text-muted/40'"
          >
        ...
        >
          ...
          <p
            class="text-xs truncate"
            :class="equippedRing2 ? 'text-accent' : 'text-muted/40'"
          >
      ...
      <div class="grid grid-cols-2 gap-1">
        ...
        >
          ...
          <p
            class="text-xs truncate"
            :class="equippedHatName ? 'text-accent' : 'text-muted/40'"
          >
            {{ equippedHatName ?? "空" }}
          </p>
        </div>
        ...
        >
          ...
          <p
            class="text-xs truncate"
            :class="equippedShoeName ? 'text-accent' : 'text-muted/40'"
          >
            {{ equippedShoeName ?? "空" }}
          </p>
        </div>
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-1.5">
        ...
        >
          <div class="flex items-center justify-between gap-1">
            ...
            <span
              class="text-[10px]"
              :class="gear.level > 0 ? 'text-success' : 'text-muted'"
              >{{ gear.level > 0 ? `${gear.level}阶` : "未凝练" }}</span
            >
          </div>
          <p class="text-[10px] text-muted leading-relaxed">{{ gear.desc }}</p>
          <p class="text-[10px] text-muted">
            战力 +{{ gear.power }} · 渡劫 +{{ gear.tribulation }}%
          </p>
          <p class="text-[10px] text-muted">
            淬炼：{{ gear.material.name }}×{{ gear.material.quantity }} /
            灵石×{{ gear.spiritStoneCost }}
          </p>
          <button
            class="btn w-full justify-center text-xs"
            :disabled="!gear.canForge"
            @click="handleForgeDaoGear(gear.id)"
          >
            {{
              gear.level >= gear.maxLevel
                ? "已圆满"
                : gear.level > 0
                  ? "继续淬炼"
                  : "凝练"
      ...
      <p class="text-[10px] text-muted mt-1.5">
        当前修仙装备总战力 +{{ cultivationStore.daoGearPower }}，渡劫稳定 +{{
          Math.round(cultivationStore.daoGearTribulationBonus * 100)
        }}%。
      </p>
      <p class="text-[10px] text-muted mt-1 leading-relaxed">
        三类成长边界：修仙装备由炼器淬炼，偏战斗/渡劫；本命法宝是单件核心法宝，偏长期战力；农具法宝化只强化生产与灵气循环。
      </p>
    </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <template v-if="activeSlot === 'weapon'">
            ...
            <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
              <div
                ...
                :class="
                  index === inventoryStore.equippedWeaponIndex
                    ? 'border-accent/30'
                    : 'border-accent/10'
                "
              ...
              >
                <div class="min-w-0">
                  ...
                  <p class="text-[10px] text-muted truncate">
                    攻{{ getWeaponStats(weapon).attack }} · 暴击{{
                      Math.round(getWeaponStats(weapon).critRate * 100)
                    }}%
                    <template v-if="weapon.enchantmentId"
                      >· {{ getEnchantName(weapon.enchantmentId) }}</template
                    >
                  </p>
                </div>
                <span
                  v-if="index === inventoryStore.equippedWeaponIndex"
          ...
          <!-- 戒指弹窗 -->
          <template
            v-else-if="activeSlot === 'ring1' || activeSlot === 'ring2'"
          >
            <p class="text-sm text-accent mb-2">
              选择{{ activeSlot === "ring1" ? "戒指1" : "戒指2" }}
            </p>
            <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
              <!-- 卸下按钮 -->
              <div
                v-if="
                  (activeSlot === 'ring1'
                    ? inventoryStore.equippedRingSlot1
                    : inventoryStore.equippedRingSlot2) >= 0
                "
              ...
              <template v-if="inventoryStore.ownedRings.length > 0">
                ...
                >
                  <div class="min-w-0">
                    <span
                      class="text-xs"
                      :class="isRingInCurrentSlot(idx) ? 'text-accent' : ''"
                      >{{ ring.name }}</span
                    >
                    <p class="text-[10px] text-muted truncate">
                      {{ ring.effectText }}
                  ...
                  <span
                    v-if="isRingInCurrentSlot(idx)"
                    class="text-[10px] text-accent shrink-0 ml-1"
                    >当前</span
                  >
                  <span
                    v-else-if="isRingInOtherSlot(idx)"
              ...
              <p v-else class="text-xs text-muted/40 text-center py-2">
                暂无戒指
              </p>
            </div>
          </template>
            ...
            <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
              ...
              <template v-if="inventoryStore.ownedHats.length > 0">
                <div
                  ...
                  class="flex items-center justify-between border rounded-xs px-2 py-1.5 cursor-pointer hover:bg-accent/5 mr-1"
                  :class="
                    hat.index === inventoryStore.equippedHatIndex
                      ? 'border-accent/30'
                      : 'border-accent/10'
                  "
                ...
                >
                  ...
                  <span
                    v-if="hat.index === inventoryStore.equippedHatIndex"
                    class="text-[10px] text-accent shrink-0 ml-1"
                    >当前</span
                  >
                </div>
            ...
            <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
              ...
              <template v-if="inventoryStore.ownedShoes.length > 0">
                <div
                  ...
                  class="flex items-center justify-between border rounded-xs px-2 py-1.5 cursor-pointer hover:bg-accent/5 mr-1"
                  :class="
                    shoe.index === inventoryStore.equippedShoeIndex
                      ? 'border-accent/30'
                      : 'border-accent/10'
                  "
                ...
                >
                  ...
                  <span
                    v-if="shoe.index === inventoryStore.equippedShoeIndex"
                    class="text-[10px] text-accent shrink-0 ml-1"
                    >当前</span
                  >
                </div>
      ...
      <div class="flex flex-col space-y-1">
        ...
        >
          ...
          <span class="text-[10px] text-muted"
            >-{{
              Math.round(
                (1 - inventoryStore.getToolStaminaMultiplier(tool.type)) * 100,
              )
            }}%体力</span
          >
        </div>
      ...
      <div class="flex flex-col space-y-0.5">
        ...
        >
          ...
          <div class="flex items-center space-x-1.5">
            ...
            <span v-if="skill.perk5" class="text-[10px] text-success">{{
              PERK_NAMES[skill.perk5]
            }}</span>
            <span v-if="skill.perk10" class="text-[10px] text-success">{{
    ...
    <div v-if="spouseInfo" class="border border-accent/20 rounded-xs p-2">
      ...
      <div class="flex flex-col space-y-0.5">
        ...
        <!-- 成长诊断 -->
        <div
          v-if="growthDiagnosis.length"
          class="mt-2 pt-2 border-t border-accent/10"
        >
          <p class="text-xs text-accent mb-1">成长诊断</p>
          <div class="flex flex-col space-y-0.5">
            <p
              v-for="(tip, i) in growthDiagnosis"
              :key="i"
...
const growthDiagnosis = computed(() => {
  const tips: string[] = [];
  if (cultivationStore.unlocked && cultivationStore.canBreakthrough)
    tips.push("⚡ 可突破！前往修仙");
  if (playerStore.stamina <= 5) tips.push("💤 体力不足，可休息或吃食物");
  if (
    inventoryStore.getItemCount("spirit_stone") < 5 &&
    cultivationStore.unlocked
  )
    tips.push("💎 灵石不足，建议红尘历练");
...
const attributeList = computed(() =>
  (Object.keys(ATTRIBUTE_NAMES) as AttributeKey[]).map((key) => {
    ...
    return {
      ...
      percent:
        required > 0
          ? Math.min(100, Math.round((attr.exp / required) * 100))
          : 100,
...
const equippedArtifactCount = computed(() => {
  ...
  return (
    (a.glimmerHoe ? 1 : 0) + (a.spiritKettle ? 1 : 0) + (a.spiritRain ? 1 : 0)
  );
});
...
const activeSlot = ref<"weapon" | "ring1" | "ring2" | "hat" | "shoe" | null>(
  null,
);
...
const equippedWeaponName = computed(() => {
  const weapon =
    inventoryStore.ownedWeapons[inventoryStore.equippedWeaponIndex];
  if (!weapon) return "无";
...
const equippedRing1 = computed(() =>
  getRingInfo(inventoryStore.equippedRingSlot1),
);
const equippedRing2 = computed(() =>
  getRingInfo(inventoryStore.equippedRingSlot2),
);
...
const handleUnequipRingFromPopup = () => {
  ...
  const idx =
    slot === 0
      ? inventoryStore.equippedRingSlot1
      : inventoryStore.equippedRingSlot2;
  const def =
    idx >= 0 ? getRingById(inventoryStore.ownedRings[idx]!.defId) : null;
...
const handleUnequipHatFromPopup = () => {
  ...
  const def =
    idx >= 0 ? getHatById(inventoryStore.ownedHats[idx]!.defId) : null;
...
const handleUnequipShoeFromPopup = () => {
  ...
  const def =
    idx >= 0 ? getShoeById(inventoryStore.ownedShoes[idx]!.defId) : null;
...
// === 被动 ===
const unlockedWalletItems = computed(() =>
  WALLET_ITEMS.filter((w) => walletStore.has(w.id)),
);
...
.pixel-avatar-aura {
  position: absolute;
  inset: 10px 6px 18px;
  border: 2px solid rgba(120, 245, 220, 0.38);
  box-shadow:
...
.avatar-female .px-belt {
  background: #efca75;
}
.avatar-mortal .px-tool {
  box-shadow: 0 -5px 0 #b0a081;
}
...
.pixel-avatar-card::after {
  ...
  top: 6px;
  bottom: 6px;
  border: 1px solid rgba(224, 178, 94, 0.22);
  pointer-events: none;
}
.pixel-avatar {
  top: 5px;
...
.avatar-immortal {
  ...
  background:
    radial-gradient(
      circle at 50% 22%,
      rgba(255, 246, 188, 0.22),
      transparent 34%
    ),
    linear-gradient(180deg, rgba(25, 31, 64, 0.96), rgba(56, 36, 79, 0.98));
  box-shadow:
    inset 0 0 0 2px rgba(255, 255, 255, 0.08),
    0 0 18px rgba(255, 226, 138, 0.24);
}
.avatar-immortal .pixel-avatar-aura {
  border-color: rgba(255, 230, 151, 0.72);
  box-shadow:
    0 0 18px rgba(255, 223, 126, 0.48),
    inset 0 0 18px rgba(121, 214, 255, 0.18);
}
.avatar-immortal .px-robe {
  background: linear-gradient(
    90deg,
    #fff4b6 0 28%,
    #8ddfff 28% 55%,
  ...
  box-shadow:
    inset -3px -3px 0 rgba(80, 45, 120, 0.28),
    0 0 8px rgba(255, 240, 170, 0.42);
}
.avatar-immortal .px-sleeve {
  background: #8ddfff;
}
.avatar-immortal .px-belt {
  background: #fff0a6;
  box-shadow: 0 0 6px rgba(255, 226, 138, 0.7);
}
.avatar-immortal .px-tool {
  background: #f7f1ff;
  box-shadow:
    0 -6px 0 #8de7ff,
    0 -12px 0 #fff7bd,
...
.pixel-avatar-immortal-ring {
  position: absolute;
  left: 10px;
  right: 10px;
  top: 18px;
  height: 48px;
  border: 2px double rgba(255, 235, 154, 0.72);
  border-radius: 50%;
  box-shadow: 0 0 16px rgba(255, 226, 138, 0.45);
  animation: immortal-pulse 1.8s ease-in-out infinite alternate;
}
.pixel-avatar-star {
  position: absolute;
  color: #fff2a8;
  text-shadow: 0 0 8px #fff2a8;
  font-size: 12px;
  animation: immortal-float 1.8s ease-in-out infinite alternate;
}
.star-a {
  left: 8px;
  top: 18px;
