  ...
  <div>
    ...
    <div class="flex space-x-1 mb-3">
      <Button
        ...
        :class="{ '!bg-accent !text-bg': tab === 'breeding' }"
        @click="tab = 'breeding'"
        >育种台</Button
      >
      <Button
    ...
    <template v-if="tab === 'breeding'">
      ...
      <div class="mb-3">
        <div class="flex items-center justify-between mb-1.5">
          <span class="text-xs text-muted"
            >育种台 {{ breedingStore.stationCount }}/{{
        ...
        <!-- 无育种台空状态 -->
        <div
          v-if="breedingStore.stationCount === 0"
        ...
        <div v-else class="flex flex-col space-y-1.5">
          <div
            v-for="(slot, idx) in breedingStore.stations"
            :key="idx"
            ...
            <template v-if="!slot.parentA && !slot.ready">
              <div class="flex items-center justify-between">
                ...
                <Button
                  :icon="Dna"
                  :icon-size="12"
                  :disabled="breedingStore.boxCount < 2"
                  @click="openBreedingSelect(idx)"
                  >育种</Button
                >
              </div>
            </template>
      ...
      <div>
        ...
        >
          <button
            ...
            :class="
              selectedSeedIds.includes(seed.genetics.id)
          ...
          >
            ...
            <p
              ...
              :class="seedStarColor(seed.genetics)"
            >
              <Star
    ...
    <template v-if="tab === 'compendium'">
      ...
      <div class="border border-accent/10 rounded-xs mb-2">
        ...
        >
          ...
          <ChevronDown
            :size="12"
            :class="{ 'transform rotate-180': showRules }"
          />
        </button>
      ...
      <div class="mt-3 border border-accent/20 rounded-xs p-2">
        <div class="flex items-center space-x-2 text-xs mb-1.5">
          ...
          <span class="text-xs text-accent whitespace-nowrap"
            >{{ totalDiscovered }}/{{ HYBRID_DEFS.length }}</span
          >
        </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-3">
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="
                  playerStore.money >= BREEDING_STATION_COST.money
                    ? 'text-accent'
                    : 'text-danger'
                "
              >
    ...
    <Transition name="panel-fade">
      <div
        v-if="detailSeed"
        class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
        @click.self="detailSeed = null"
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="detailSeed = null"
          >
            <X :size="14" />
          </button>

          <p class="text-sm text-accent mb-2">
            {{ getCropName(detailSeed.genetics.cropId) }} · G{{
              detailSeed.genetics.generation
            }}
          </p>
          <p
            class="text-xs mb-2 flex items-center space-x-1"
            :class="seedStarColor(detailSeed.genetics)"
          >
            <span class="flex items-center space-x-px">
              <Star
                v-for="n in getStarRating(detailSeed.genetics)"
                :key="n"
                :size="10"
              />
            </span>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs"
                >{{
                  TIER_LABELS[getHybridTier(activeHybrid.id)] ?? "一"
                }}代</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                getCropName(activeHybrid.parentCropA)
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">亲本B</span>
              <span class="text-xs">{{
                getCropName(activeHybrid.parentCropB)
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">甜度要求</span>
              <span class="text-xs text-accent"
                >≥{{ activeHybrid.minSweetness }}</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">产量要求</span>
              <span class="text-xs text-accent"
                >≥{{ activeHybrid.minYield }}</span
              >
            </div>
          </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                activeHybrid.baseGenetics.sweetness
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
            ...
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                activeHybrid.baseGenetics.resistance
              }}</span>
            </div>
            <div
              v-if="getCompendiumEntry(activeHybrid.id)"
              class="flex items-center justify-between mt-1 pt-1 border-t border-accent/10"
            >
              <span class="text-xs text-muted">种植次数</span>
              <span class="text-xs">{{
                getCompendiumEntry(activeHybrid.id)?.timesGrown ?? 0
              }}</span>
            </div>
          </div>
        </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="showSeedBoxUpgradeModal = false"
          >
            <X :size="14" />
          </button>
          ...
          <template v-if="nextSeedBoxUpgrade">
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">
                升级至 Lv.{{ breedingStore.seedBoxLevel + 1 }}
              </p>
              <div class="flex items-center justify-between">
                <span class="text-xs text-muted">容量上限</span>
                <span class="text-xs text-text">
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">所需材料</p>
              <div
                v-for="mat in nextSeedBoxUpgrade.materials"
                :key="mat.itemId"
                class="flex items-center justify-between"
              >
                <span class="text-xs text-muted">{{
              ...
              <div class="flex items-center justify-between mt-0.5">
                ...
                <span
                  class="text-xs"
                  :class="
                    playerStore.money >= nextSeedBoxUpgrade.cost
                      ? ''
                      : 'text-danger'
                  "
                >
            ...
            <div v-else class="flex space-x-1">
              <Button
                class="flex-1 justify-center"
                @click="showSeedBoxUpgradeConfirm = false"
                >取消</Button
              >
              <Button
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div
            ...
            :class="
              crossBreedHint.type === 'recipe' && crossBreedHint.canSucceed
                ? 'border-success/30'
                : 'border-accent/10'
            "
          >
            ...
            <template v-else-if="crossBreedHint.type === 'recipe'">
              ...
              <div class="flex items-center justify-between">
                ...
                <span
                  class="text-xs"
                  :class="
                    crossBreedHint.sweetOk ? 'text-success' : 'text-danger'
                  "
                >
              ...
              <div class="flex items-center justify-between mt-0.5">
                ...
                <span
                  class="text-xs"
                  :class="
                    crossBreedHint.yieldOk ? 'text-success' : 'text-danger'
                  "
                >
              ...
              <p
                v-if="!crossBreedHint.canSucceed"
                class="text-xs text-danger mt-1"
              >
                属性未达标，杂交将失败。请先同种培育提升属性。
              </p>
...
const crossBreedHint = computed(() => {
  ...
  const seedA = breedingStore.breedingBox.find(
    (s) => s.genetics.id === selectedSeedIds.value[0],
  );
  const seedB = breedingStore.breedingBox.find(
    (s) => s.genetics.id === selectedSeedIds.value[1],
  );
...
const canCraftStation = computed(() => {
  return breedingStore.canCraftStation(playerStore.money, (id: string) =>
    getCombinedItemCount(id),
  );
});
...
const nextSeedBoxUpgrade = computed(() =>
  breedingStore.getNextSeedBoxUpgrade(),
);

const canUpgradeSeedBox = computed(() => {
  return breedingStore.canUpgradeSeedBox(playerStore.money, (id: string) =>
    getCombinedItemCount(id),
  );
});
