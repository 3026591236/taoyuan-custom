    ...
    <div class="territory-layout">
      <main class="map-shell">
        ...
        <div ref="mapViewport" class="map-viewport" @pointerdown="startPan" @pointermove="pan" @pointerup="endPan" @pointercancel="endPan" @wheel.prevent="onWheel">
          <div class="strategic-map" :style="mapTransform">
            <div class="mountain mountain-a" /><div class="mountain mountain-b" /><div class="mountain mountain-c" /><div class="mountain mountain-d" />
            <div class="river river-a" /><div class="river river-b" /><div class="river river-c" />
            <svg class="supply-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              <line v-for="route in routes" :key="route.id" :x1="route.from.x" :y1="route.from.y" :x2="route.to.x" :y2="route.to.y" :class="{ active: route.active }" />
            </svg>
            <button v-for="node in store.nodes" :key="node.id" class="territory-node" :class="[node.status, { selected: node.id === store.selectedId, capital: node.kind === 'capital', reachable: isReachable(node) }]" :style="{ left: `${node.x}%`, top: `${node.y}%` }" @click.stop="select(node.id)">
        ...
        <div class="map-legend"><span><i class="legend owned" />已控制 {{ store.ownedNodes.length }}</span><span><i class="legend reachable" />可推进 {{ store.reachableNodes.length }}</span><span><i class="legend neutral" />未接通 {{ store.nodes.filter(n => n.status === 'neutral').length - store.reachableNodes.length }}</span><span><i class="legend contested" />告急 {{ store.nodes.filter(n => n.status === 'contested').length }}</span></div>
      </main>
      ...
      <aside class="territory-sidebar">
        ...
        <div class="selected-card" v-if="selected">
          ...
          <div v-if="selected.status === 'neutral'" class="route-note" :class="{ blocked: !isReachable(selected) }"><MapPinned :size="14" /> {{ isReachable(selected) ? '已接入己方供给线，可调度占领。' : '未接入己方供给线，需先控制相邻据点。' }}</div>
...
const isReachable = (node: { id: string; links: string[] }) => node.id === "capital" || node.links.some((id) => store.nodes.find((entry) => entry.id === id)?.status === "owned");
const routes = computed(() => {
  const seen = new Set<string>();
  return store.nodes.flatMap((from) => from.links.flatMap((targetId) => {
    const to = store.nodes.find((node) => node.id === targetId);
    const id = [from.id, targetId].sort().join(":");
    if (!to || seen.has(id)) return [];
    seen.add(id);
    return [{ id, from, to, active: from.status === "owned" && to.status === "owned" }];
  }));
});
