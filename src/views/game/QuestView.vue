<template>
  <div>
    ...
    <div class="border border-accent/30 rounded-xs p-3 mb-3 bg-accent/5">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-[10px] text-muted"
          >已完成 {{ questStore.journeySummary.done }}/{{
            questStore.journeySummary.total
          }}
          ...
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-1.5">
                <span
                  class="text-[10px] px-1 border border-accent/20 rounded-xs text-muted"
                  >{{ journeyTypeName(task.type, task.day) }}</span
                >
                <span
                  v-if="JOURNEY_LOCATION_HINTS[task.metric]"
                  class="text-[10px] text-accent/70"
                  >{{ JOURNEY_LOCATION_HINTS[task.metric] }}</span
                >
              ...
              <p class="text-[10px] text-accent/90 mt-1">
                奖励：{{ journeyRewardText(task.reward) }}
              </p>
              <div class="mt-1 flex items-center gap-2">
                <div
                  class="flex-1 h-1 bg-bg rounded-xs border border-accent/10 overflow-hidden"
                >
                  <div
                ...
                <span class="text-[10px] text-muted whitespace-nowrap"
                  >{{ Math.min(task.progress, task.target) }}/{{
                    task.target
                  }}</span
                >
              </div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-3">
      ...
      >
        <div class="min-w-0">
          <p class="text-xs text-accent truncate">
            第{{ mainQuestDef.chapter }}章 · {{ mainQuestDef.title }}
          </p>
        ...
        <span
          ...
          :class="
            questStore.canSubmitMainQuest()
              ? 'text-success'
              : questStore.mainQuest?.accepted
                ? 'text-accent'
                : 'text-muted'
          "
        >
      ...
      <div
        v-else-if="questStore.completedMainQuests.length >= 50"
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-3">
      ...
      <div
        v-if="questStore.boardQuests.length === 0"
        class="flex flex-col items-center justify-center py-4 text-muted"
      >
        <Calendar :size="24" />
    ...
    >
      ...
      >
        ...
        <span class="text-xs text-accent whitespace-nowrap ml-2"
          >{{ questStore.specialOrder.moneyReward }}文</span
        >
      </div>
    </div>
      ...
      <div v-else class="flex flex-col space-y-1.5">
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <span
              class="text-xs whitespace-nowrap ml-2"
              :class="canSubmit(quest) ? 'text-success' : 'text-muted'"
            >
              {{ canSubmit(quest) ? "可提交" : `剩${quest.daysRemaining}天` }}
            </span>
          </div>
          <div
            v-if="quest.type !== 'delivery'"
            ...
            <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
              <div
                ...
                :style="{
                  width:
                    Math.floor(
                      (getEffectiveProgress(quest) / quest.targetQuantity) *
                        100,
                    ) + '%',
                }"
              />
    ...
    <div class="border border-accent/10 rounded-xs p-2 text-center">
      <p class="text-xs text-muted">
        累计完成委托 {{ questStore.completedQuestCount }} 个 · 主线进度
        {{ questStore.completedMainQuests.length }}/50
      </p>
    </div>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <template v-if="questModal.type === 'main' && mainQuestDef">
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              ...
              <div
                v-for="(obj, i) in mainQuestDef.objectives"
                :key="i"
                class="flex items-center space-x-1"
              >
                <CircleCheck
                  v-if="mainQuestProgress[i]"
                  :size="12"
                ...
                <span
                  class="text-xs"
                  :class="mainQuestProgress[i] ? 'text-success' : ''"
                  >{{ obj.label }}</span
                >
              </div>
            </div>
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              ...
              <p class="text-xs">
                ...
                <template v-if="mainQuestDef.friendshipReward?.length"
                  >+ 好感</template
                >
                <template v-if="mainQuestDef.itemReward?.length">
                  +
                  {{
                    mainQuestDef.itemReward
                      .map((i) => `${getItemName(i.itemId)}×${i.quantity}`)
                      .join(", ")
                  }}
                </template>
              </p>
          ...
          <template v-if="questModal.type === 'board' && selectedBoardQuest">
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              <p class="text-xs text-muted mb-1">奖励</p>
              <p class="text-xs">
                {{ selectedBoardQuest.moneyReward }}文 + 好感{{
                  selectedBoardQuest.friendshipReward
                }}
              </p>
            </div>
            <Button
              ...
              :icon-size="12"
              :disabled="
                questStore.activeQuests.length >= questStore.MAX_ACTIVE_QUESTS
              "
          ...
          >
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">目标</p>
              <p class="text-xs">
                {{ targetSummary(questStore.specialOrder) }}
              </p>
            </div>
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">限时</p>
              <p class="text-xs">
                {{ questStore.specialOrder.daysRemaining }} 天
              </p>
            </div>
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              <p class="text-xs text-muted mb-1">奖励</p>
              <p class="text-xs">
                {{ questStore.specialOrder.moneyReward }}文 + 好感{{
                  questStore.specialOrder.friendshipReward
                }}
                <template v-if="questStore.specialOrder.itemReward?.length">
                  +
                  {{
                    questStore.specialOrder.itemReward
                      .map((i) => `${getItemName(i.itemId)}×${i.quantity}`)
                      .join(", ")
                  }}
                </template>
          ...
          <template v-if="questModal.type === 'active' && selectedActiveQuest">
            <p class="text-accent text-sm mb-2">
              {{
                selectedActiveQuest.orderTag ||
                (selectedActiveQuest.type === "special_order"
                  ? "特殊订单"
                  : "委托")
              }}
            </p>
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              ...
              <div
                v-if="selectedActiveQuest.type !== 'delivery'"
                ...
                >
                  <div
                    ...
                    :style="{
                      width:
                        Math.floor(
                          (getEffectiveProgress(selectedActiveQuest) /
                            selectedActiveQuest.targetQuantity) *
                            100,
                        ) + '%',
                    }"
                  />
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              ...
              <p class="text-xs">
                ...
                <template v-if="selectedActiveQuest.itemReward?.length">
                  +
                  {{
                    selectedActiveQuest.itemReward
                      .map((i) => `${getItemName(i.itemId)}×${i.quantity}`)
                      .join(", ")
                  }}
                </template>
...
const visibleJourneyTasks = computed(() => {
  return questStore.journeyTasks
    .filter((task) => task.type === "guide" || task.done || !task.claimed)
    .slice(0, 15);
});
...
const journeyRewardText = (reward: {
  money?: number;
  aura?: number;
  attributeExp?: Record<string, number>;
}) => {
  const parts: string[] = [];
  if (reward.money) parts.push(`${reward.money}文`);
  if (reward.aura) parts.push(`灵气${reward.aura}`);
  if (reward.attributeExp && Object.keys(reward.attributeExp).length > 0)
    parts.push("资质经验");
...
type QuestModalState =
  | { type: "main" }
  | { type: "board"; questId: string }
  | { type: "special" }
  | { type: "active"; questId: string };
...
const canSubmit = (quest: QuestInstance): boolean => {
  if (quest.type === "delivery") {
    return (
      inventoryStore.getItemCount(quest.targetItemId) >= quest.targetQuantity
    );
  }
