<template>
  <div>
    ...
    <div class="flex space-x-1.5 mb-3">
      ...
      >
        加工区
        <span class="text-[10px] ml-0.5 opacity-70"
          >{{ processingStore.machineCount }}/{{
            processingStore.maxMachines
          }}</span
        >
      </Button>
      <Button
    ...
    >
      ...
      <!-- 空状态 -->
      <div
        v-if="processingStore.machines.length === 0"
      ...
      <div v-else class="flex flex-col space-y-2">
        ...
        >
          ...
          >
            ...
            <span class="text-[10px] text-muted">{{
              processingStore.collapsedGroups.has(group.machineType) ? "▸" : "▾"
            }}</span>
          </div>

          <!-- 展开的机器明细 -->
          <div
            v-if="!processingStore.collapsedGroups.has(group.machineType)"
            ...
            >
              ...
              <div v-if="!slot.recipeId">
                ...
                <template v-if="slot.machineType === 'seed_maker'">
                  <div
                    v-if="
                      getSeedMakerQualityRecipes(slot.machineType).length > 0
                    "
                  ...
                  >
                    ...
                    >
                      ...
                      <span class="text-muted"
                        >({{ qr.count }}/{{ qr.recipe.inputQuantity }})</span
                      >
                    </Button>
                  </div>
                ...
                <template v-else>
                  <div
                    v-if="getFilteredRecipes(slot.machineType).length > 0"
                    class="grid space-y-1"
                  >
                    <Button
              ...
              <div v-else-if="!slot.ready">
                <div class="flex items-center justify-between text-xs mb-1">
                  <span class="text-muted">{{
                    getRecipeName(slot.recipeId)
                  }}</span>
                  <span class="text-muted"
                    >{{ slot.daysProcessed }}/{{ slot.totalDays }}天</span
                  >
                </div>
                <div
                ...
                >
                  <div
                    ...
                    :style="{
                      width:
                        Math.floor(
                          (slot.daysProcessed / slot.totalDays) * 100,
                        ) + '%',
                    }"
                  />
                </div>
                <Button
                  ...
                  :icon="X"
                  :icon-size="10"
                  @click="handleCancelProcessing(originalIndex)"
                >
                  取消加工
                </Button>
              </div>
    ...
    >
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-xs text-muted"
          >机器 {{ processingStore.machineCount }}/{{
            processingStore.maxMachines
          }}</span
        >
      </div>
        ...
        <div class="flex flex-col space-y-1 max-h-60 overflow-y-auto">
          ...
          >
            <div class="text-xs truncate mr-2">
              {{ item.name }}
              <span v-if="item.badge" class="text-muted ml-1"
                >[{{ item.badge }}]</span
              >
            </div>
            <span
          ...
          <template v-if="nextUpgrade">
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">
                升级至 Lv.{{ processingStore.workshopLevel + 1 }}
              </p>
              <div class="flex items-center justify-between">
                <span class="text-xs text-muted">机器上限</span>
                <span class="text-xs text-text"
                  >{{ processingStore.maxMachines }} →
                  {{ processingStore.maxMachines + 5 }}</span
                >
              </div>
            </div>
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              ...
              <div
                v-for="mat in nextUpgrade.materials"
                :key="mat.itemId"
                class="flex items-center justify-between"
              >
                <span class="text-xs text-muted">{{
                  getItemById(mat.itemId)?.name
                }}</span>
                <span
                  class="text-xs"
                  :class="
              ...
              <div class="flex items-center justify-between mt-0.5">
                ...
                <span
                  class="text-xs"
                  :class="
                    playerStore.money >= nextUpgrade.cost ? '' : 'text-danger'
                  "
                  >{{ nextUpgrade.cost }}文</span
                >
              </div>
            </div>
    ...
    <Transition name="panel-fade">
      <div
        v-if="craftModal"
        class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
        @click.self="craftModal = null"
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="craftModal = null"
          >
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <p v-if="craftModal.badge" class="text-xs text-muted mt-0.5">
              当前：{{ craftModal.badge }}
            </p>
          </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <p class="text-xs text-muted mb-1">所需材料</p>
            <div
              v-for="mat in craftModal.materials"
              :key="mat.itemId"
              class="flex items-center justify-between"
            >
              <span class="text-xs text-muted">{{
              ...
              <span
                ...
                "
              >
                {{ getCombinedItemCount(mat.itemId) }}/{{
                  mat.quantity * displayQty
                }}
              </span>
            </div>
            <div
              v-if="craftModal.cost > 0"
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  playerStore.money >= craftModal.cost * displayQty
                    ? ''
                    : 'text-danger'
                "
              >
          ...
          <!-- 批量数量控制 -->
          <div
            v-if="craftModal.batchable && maxCraftable > 1"
          ...
          >
            ...
            <div class="flex space-x-1">
              <Button
                class="flex-1 justify-center"
                :disabled="craftQuantity <= 1"
                @click="setCraftQuantity(1)"
                >最少</Button
              >
              <Button
                class="flex-1 justify-center"
                :disabled="craftQuantity >= maxCraftable"
                @click="setCraftQuantity(maxCraftable)"
              >
                最多
              </Button>
            </div>
            ...
            >
              ...
              <span class="text-xs text-accent"
                >{{ craftModal.cost * craftQuantity }}文</span
              >
            </div>
          </div>
          ...
          >
            {{
              craftModal.batchable && craftQuantity > 1
                ? `制造 ×${craftQuantity}`
                : "制造"
            }}
          </Button>
...
const machineGroups = computed((): MachineGroup[] => {
  ...
  const typeOrder = new Map(
    PROCESSING_MACHINES.map((m, i) => [m.id as MachineType, i]),
  );
  ...
  return [...groupMap.values()].sort(
    (a, b) =>
      (typeOrder.get(a.machineType) ?? 99) -
      (typeOrder.get(b.machineType) ?? 99),
  );
});
...
const maxCraftable = computed(() => {
  ...
  for (const m of item.materials) {
    max = Math.min(
      max,
      Math.floor(getCombinedItemCount(m.itemId) / m.quantity),
    );
  }
  if (item.cost > 0) {
...
const displayQty = computed(() =>
  craftModal.value?.batchable ? craftQuantity.value : 1,
);
...
const canCraftJadeRing = computed(() =>
  processingStore.canCraft(JADE_RING_COST, JADE_RING_MONEY),
);
...
const allSkillsAbove8 = computed(() =>
  ["farming", "foraging", "fishing", "mining"].every(
    (s) => skillStore.getSkill(s as any).level >= 8,
  ),
);
const canCraftStaminaFruit = computed(
  () =>
    ...
    processingStore.canCraft(STAMINA_FRUIT_COST, STAMINA_FRUIT_MONEY),
);

const craftCategories = computed(
  (): { label: string; items: CraftableItem[] }[] => [
    {
      label: "加工机器",
      items: PROCESSING_MACHINES.map((m) => ({
        ...
        canCraft: () =>
          processingStore.canCraft(m.craftCost, m.craftMoney) &&
          processingStore.machineCount < processingStore.maxMachines,
        badge: `已有${getMachineCountByType(m.id)}`,
        batchable: true,
        maxBatch: () =>
          processingStore.maxMachines - processingStore.machineCount,
      })),
    },
    {
      label: "农场设施",
      items: [
        ...
        })),
        {
          ...
          canCraft: () =>
            processingStore.canCraft(TAPPER.craftCost, TAPPER.craftMoney),
          batchable: true,
        },
        {
          id: "lightning_rod",
          ...
          canCraft: () =>
            processingStore.canCraft(
              LIGHTNING_ROD.craftCost,
              LIGHTNING_ROD.craftMoney,
            ),
        ...
        },
        {
          ...
          canCraft: () =>
            processingStore.canCraft(SCARECROW.craftCost, SCARECROW.craftMoney),
    ...
    },
    {
      ...
      items: [
        ...
        })),
        {
          ...
          canCraft: () =>
            processingStore.canCraft(
              CRAB_POT_CRAFT.craftCost,
              CRAB_POT_CRAFT.craftMoney,
            ),
          batchable: true,
        },
      ],
    },
    {
      ...
      items: [
        ...BOMBS.map((b) => ({
          ...
          canCraft: () =>
            (b.id !== "mega_bomb" || hasCombinedItem("mega_bomb_recipe")) &&
      ...
      ? [
          {
            ...
            items: CHEST_TIER_ORDER.map((tier) => {
              ...
              return {
                ...
                canCraft: () =>
                  warehouseStore.chests.length < warehouseStore.maxChests &&
                  processingStore.canCraft(def.craftCost, def.craftMoney),
                badge: `${warehouseStore.chests.length}/${warehouseStore.maxChests}`,
                batchable: true,
                maxBatch: () =>
                  warehouseStore.maxChests - warehouseStore.chests.length,
              };
            }),
          },
        ]
...
const handleCraftFromModal = () => {
  ...
  const qty = craftModal.value.batchable
    ? Math.min(craftQuantity.value, maxCraftable.value)
    : 1;
