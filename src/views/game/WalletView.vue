    ...
    <div class="flex flex-col space-y-1.5">
      ...
      >
        <div class="flex items-center justify-between">
          <span
            class="text-sm"
            :class="walletStore.has(item.id) ? 'text-accent' : 'text-muted'"
          >
            {{ item.name }}
          </span>
          <CircleCheck
            v-if="walletStore.has(item.id)"
            :size="14"
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="
              walletStore.has(selectedItem.id) ? 'text-accent' : 'text-muted'
            "
          >
            {{ selectedItem.name }}
          </p>
          ...
          <div
            ...
            :class="
              walletStore.has(selectedItem.id)
                ? 'border-success/30'
                : 'border-accent/10'
            "
          >
            <div class="flex items-center justify-center space-x-1">
              <CircleCheck
                v-if="walletStore.has(selectedItem.id)"
                :size="12"
                class="text-success"
              />
...
const unlockedCount = computed(
  () => WALLET_ITEMS.filter((i) => walletStore.has(i.id)).length,
);
</script>
