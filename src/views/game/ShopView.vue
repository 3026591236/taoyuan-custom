<template>
  <div>
    ...
    <Button
      ...
      :icon="ChevronLeft"
      @click="shopStore.currentShopId = null"
    >
      {{
    ...
    <div class="flex flex-col md:flex-row space-x-0 md:space-x-4 md:space-y-6">
      ...
      <div class="flex-1" :class="{ 'hidden md:block': mobileTab === 'sell' }">
        ...
        <template v-if="!shopStore.currentShopId">
          ...
          <div class="flex flex-col space-y-2">
            <div
              ...
              :class="
                isOpen(shop)
                  ? 'border-accent/30 cursor-pointer hover:bg-accent/5'
            ...
            >
              <div>
                ...
                <span v-if="!isOpen(shop)" class="text-danger text-xs ml-2">{{
                  closedReason(shop)
                }}</span>
              </div>
        ...
        <template v-else-if="shopStore.currentShopId === 'cultivation-market'">
          ...
          <div class="border border-accent/20 rounded-xs p-2 mb-3 bg-bg/40">
            ...
            <div class="grid grid-cols-3 md:grid-cols-7 gap-1 text-[10px]">
              <div
                v-for="sig in shopStore.marketSignalCards"
                :key="sig.category"
                class="border border-accent/10 rounded-xs p-1 text-center"
              >
                <p class="text-muted">
                  {{ MARKET_CATEGORY_NAMES[sig.category] }}
                </p>
          ...
          <div class="border border-accent/20 rounded-xs p-2 mb-3 bg-bg/40">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
              <div
                v-for="lot in shopStore.marketAuctionCards"
                :key="lot.id"
              ...
              >
                ...
                <p class="text-[10px] text-muted mt-1">
                  拍品：{{
                    lot.reward.map((i) => `${i.name}×${i.quantity}`).join("、")
                  }}
                </p>
          ...
          <h4
            id="cultivation-market"
            ref="cultivationMarketRef"
            class="text-accent text-sm mb-2 mt-3 scroll-mt-4"
          >
            ...
            >
              <div>
                <p class="text-sm">
                  {{ item.name }}
                  <span
                    v-if="CULTIVATION_USAGE[item.itemId]"
                    class="text-[10px] text-success"
                    >【{{ CULTIVATION_USAGE[item.itemId] }}】</span
                  >
                </p>
          ...
          <div class="flex flex-col space-y-2">
            ...
            >
              <div>
                ...
                <p class="text-muted text-xs">
                  {{ seed.season.map((s) => SEASON_NAMES[s]).join("/") }}季 ·
                  {{ seed.growthDays }}天{{
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(seed.price) }}文</span
              >
            </div>
            <div
              v-if="shopStore.availableSeeds.length === 0"
          ...
          <div class="flex flex-col space-y-2">
            ...
            <div
              v-if="
                ...
                warehouseStore.maxChests < warehouseStore.MAX_CHESTS_CAP
              "
              class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-2 cursor-pointer hover:bg-accent/5"
              @click="
                openBuyModal(
            ...
            >
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(warehouseExpandPrice) }}文</span
              >
            </div>
            ...
            >
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(farmExpandInfo.price) }}文</span
              >
            </div>
            ...
            >
              <div>
                ...
                <p class="text-muted text-xs">
                  28天成熟 · {{ seasonName(tree.fruitSeason) }}季产{{
                    tree.fruitName
                  }}
                </p>
              </div>
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(tree.saplingPrice) }}文</span
              >
            </div>
            ...
            <div
              ...
              @click="
                openBatchBuyModal(
                  ...
                  (count) =>
                    handleBatchBuyItem('hay', HAY_PRICE, '干草', count),
            ...
            <div
              ...
              @click="
                openBatchBuyModal(
                  ...
                  (count) =>
                    handleBatchBuyItem('wood', WOOD_PRICE, '木材', count),
            ...
            >
              <div>
                <p class="text-sm">
                  {{ ring.name }}
                  <span
                    v-if="inventoryStore.hasRing(ring.id)"
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ ring.recipeMoney }}文</span
              >
            </div>
            <div
              v-if="craftableRings.length === 0"
              class="flex flex-col items-center justify-center py-4 text-muted"
            >
            ...
            >
              <div>
                <p class="text-sm">
                  {{ hat.name }}
                  <span
                    v-if="inventoryStore.hasHat(hat.id)"
                    class="text-success text-xs ml-1"
                    >已拥有</span
                  >
                </p>
            ...
            >
              <div>
                <p class="text-sm">
                  {{ shoe.name }}
                  <span
                    v-if="inventoryStore.hasShoe(shoe.id)"
                    class="text-success text-xs ml-1"
                    >已拥有</span
                  >
                </p>
            ...
            >
              <div>
                <p class="text-sm">
                  {{ w.name }}
                  <span
                    v-if="inventoryStore.hasWeapon(w.id)"
                    class="text-success text-xs ml-1"
                    >已拥有</span
                  >
                </p>
                <p class="text-muted text-xs">
                  {{ WEAPON_TYPE_NAMES[w.type] }} · 攻击{{ w.attack }}
                </p>
              </div>
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(w.shopPrice!) }}文</span
              >
            </div>
          </div>
        </template>
        ...
        <template v-else-if="shopStore.currentShopId === 'yaopu'">
          ...
          <h4
            id="cultivation-market"
            ref="cultivationMarketRef"
            class="text-accent text-sm mb-2 mt-4 scroll-mt-4"
          >
            <Sparkles :size="14" class="inline" />
            修仙市集
          </h4>
        ...
        <template v-else-if="shopStore.currentShopId === 'chouduanzhuang'">
          ...
          <div class="flex flex-col space-y-2">
            ...
            >
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(hat.shopPrice!) }}文</span
              >
            </div>
          </div>
          ...
          <div class="flex flex-col space-y-2">
            ...
            >
              ...
              <span class="text-xs text-accent whitespace-nowrap"
                >{{ discounted(shoe.shopPrice!) }}文</span
              >
            </div>
          </div>
        </template>
      ...
      <div class="flex-1" :class="{ 'hidden md:block': mobileTab === 'buy' }">
        ...
        <!-- 售价加成提示 -->
        <p v-if="hasSellBonus" class="text-success text-xs mb-2">
          戒指加成中：所有售价 +{{ sellBonusPercent }}%
        </p>
        ...
        <div class="border border-accent/30 rounded-xs p-2 mb-3">
          ...
          <div class="grid grid-cols-4">
            <span
              v-for="m in todayMarket"
              :key="m.category"
              class="text-[10px] whitespace-nowrap mt-2"
            >
              <span class="text-muted">{{
                MARKET_CATEGORY_NAMES[m.category]
              }}</span>
              <span v-if="m.trend === 'stable'" class="text-muted/40 ml-0.5"
                >—</span
              >
        ...
        <div class="flex flex-col space-y-2">
          <div
            ...
            class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-2 cursor-pointer hover:bg-accent/5"
            @click="
              openSellModal(item.itemId, item.quality, item.originalIndex)
            "
          >
            <div>
              <span class="text-sm" :class="qualityTextClass(item.quality)">{{
                item.def?.name
              }}</span>
            ...
            <div class="flex items-center space-x-1">
              <span class="text-xs text-accent whitespace-nowrap"
                >{{
                  shopStore.calculateSellPrice(item.itemId, 1, item.quality)
                }}文</span
              >
              <span
                v-if="
                  getItemTrend(item.itemId) === 'rising' ||
                  getItemTrend(item.itemId) === 'boom'
                "
                class="text-[10px] text-success"
              >
              ...
              <span
                v-else-if="
                  getItemTrend(item.itemId) === 'falling' ||
                  getItemTrend(item.itemId) === 'crash'
                "
                class="text-[10px]"
                :class="
                  getItemTrend(item.itemId) === 'crash'
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            ...
            @click="showSellFilterModal = false"
          >
            <X :size="14" />
          </button>
          ...
          <div class="flex space-x-1.5">
            <Button class="flex-1 justify-center" @click="handleClearSellFilter"
              >全部显示</Button
            >
            <Button
              class="flex-1 justify-center !bg-accent !text-bg"
              @click="handleSaveSellFilter"
              >保存</Button
            >
          </div>
        </div>
      ...
      >
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex space-x-1.5">
            <Button
              class="flex-1 justify-center"
              @click="showSellAllConfirm = false"
              >取消</Button
            >
            <Button
      ...
      >
        ...
        <div v-if="buyModalData" class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <p
              v-for="(line, i) in buyModalData.extraLines"
              :key="i"
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              <span class="text-xs text-muted">{{
                buyModalData.batchBuy ? "单价" : "价格"
              }}</span>
              <span class="text-xs text-accent">{{
                formatBuyPrice(buyModalData.price, buyModalData.currency)
              }}</span>
            </div>
            <div
              v-if="buyModalData.itemId"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">持有</span>
              <span class="text-xs">{{
                inventoryStore.getItemCount(buyModalData.itemId)
              }}</span>
            </div>
          </div>
          ...
          >
            ...
            <div class="flex space-x-1">
              ...
              <Button
                class="flex-1 justify-center"
                :disabled="buyQuantity >= maxBuyQuantity"
                @click="setBuyQuantity(maxBuyQuantity)"
              >
                最多
              </Button>
            </div>
            <div class="flex items-center justify-between mt-1.5">
              ...
              <span class="text-xs text-accent">{{
                formatBuyPrice(buyTotalPrice, buyModalData.currency)
              }}</span>
            </div>
          </div>
        ...
        <!-- 出售弹窗 -->
        <div
          v-else-if="sellModalData && sellModalItem && sellModalDef"
        ...
        >
          ...
          <p
            ...
            :class="qualityTextClass(sellModalItem.quality, 'text-accent')"
          >
            {{ sellModalDef.name }}
          </p>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div
              v-if="sellModalItem.quality !== 'normal'"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">品质</span>
              <span
                class="text-xs"
                :class="qualityTextClass(sellModalItem.quality)"
                >{{ QUALITY_NAMES[sellModalItem.quality] }}</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs flex items-center space-x-1">
                <span
                  v-if="
                    getItemTrend(sellModalData!.itemId) &&
                    getItemTrend(sellModalData!.itemId) !== 'stable'
                  "
                  class="line-through text-muted/40"
                >
                  {{
                    shopStore.calculateBaseSellPrice(
                      sellModalData!.itemId,
                      1,
                      sellModalData!.quality,
                    )
                  }}文
                </span>
            ...
            >
              ...
              <span
                class="text-xs"
                :class="trendColor(getItemTrend(sellModalData!.itemId)!)"
              >
                {{ TREND_NAMES[getItemTrend(sellModalData!.itemId)!] }} ×{{
                  getItemMultiplier(sellModalData!.itemId)
                }}
              </span>
            </div>
            <div
          ...
          >
            ...
            <div class="flex space-x-1">
              ...
              <Button
                class="flex-1 justify-center"
                :disabled="sellQuantity >= maxSellQuantity"
                @click="setSellQuantity(maxSellQuantity)"
              >
                最多
              </Button>
            </div>
          ...
          <div class="flex flex-col space-y-1.5">
            <Button
              class="w-full justify-center"
              :icon="Coins"
              @click="handleModalSell(sellQuantity)"
              >出售 ×{{ sellQuantity }}</Button
            >
          </div>
        </div>
      </div>
...
const scrollToCultivationMarket = async () => {
  ...
  shopStore.currentShopId = "cultivation-market";
  await nextTick();
  cultivationMarketRef.value?.scrollIntoView({
    behavior: "smooth",
    block: "start",
  });
...
const getItemMultiplier = (itemId: string): number => {
  ...
  if (!def) return 1;
  return (
    todayMarket.value.find((m) => m.category === def.category)?.multiplier ?? 1
  );
};
...
const sellModalItem = computed(() => {
  ...
  return (
    inventoryStore.items.find(
      (i) => i.itemId === data.itemId && i.quality === data.quality,
    ) ?? null
  );
});
...
const getMaxBuyable = (unitPrice: number, stockLimit?: number): number => {
  const affordable =
    unitPrice > 0 ? Math.floor(playerStore.money / unitPrice) : 0;
...
const openWeaponModal = (w: WeaponDef) => {
  ...
  if (w.shopMaterials.length > 0) {
    lines.push(
      "需要材料：" +
        w.shopMaterials
          .map(
            (m) => `${getItemById(m.itemId)?.name ?? m.itemId}×${m.quantity}`,
          )
          .join("、"),
    );
  }
  openBuyModal(
    ...
    () =>
      ...
      playerStore.money >= discounted(w.shopPrice!) &&
      hasWeaponMaterials(w),
    lines,
  );
};
  ...
  const lines = [
    "效果：" +
      ring.effects
        .map(
          (eff) =>
            RING_EFFECT_LABELS[eff.type] +
            (eff.value > 0 && eff.value < 1
    ...
    "材料：" +
      (ring.recipe
        ?.map((m) => `${getItemById(m.itemId)?.name ?? m.itemId}×${m.quantity}`)
        .join("、") ?? "") +
      ` + ${ring.recipeMoney}文`,
  ];
  openBuyModal(
...
const handleModalSell = (count: number) => {
  ...
  // 物品消耗完则关闭弹窗，否则修正出售数量
  const remaining = inventoryStore.items.find(
    (i) => i.itemId === modal.itemId && i.quality === modal.quality,
  );
  if (!remaining) {
...
const hasSellBonus = computed(
  () => inventoryStore.getRingEffectValue("sell_price_bonus") > 0,
);
const sellBonusPercent = computed(() =>
  Math.round(inventoryStore.getRingEffectValue("sell_price_bonus") * 100),
);
...
const canBuyMarketItem = (
  item: ShopItemEntry,
  countRef: { value: number } | number = 1,
) => {
...
const handleBatchBuyMarketItem = (item: ShopItemEntry, count: number) => {
  ...
  if (shopStore.buyItem(item.itemId, item.price, count, currency)) {
    sfxBuy();
    showFloat(
      `-${unit * count}${currency === "spirit_stone" ? "灵石" : "文"}`,
      "danger",
    );
    addLog(
      `购买了${count}个${item.name}。(-${unit * count}${currency === "spirit_stone" ? "灵石" : "文"})`,
    );
  } else {
    addLog(
...
const handleBatchBuyFromTraveler = (
  itemId: string,
  name: string,
  originalPrice: number,
  count: number,
...
const handleBuyWeapon = (w: WeaponDef) => {
  ...
  const matStr =
    w.shopMaterials.length > 0
      ? " + " +
        w.shopMaterials
          .map((m) => `${getItemById(m.itemId)?.name}×${m.quantity}`)
          .join(" + ")
      : "";
...
const openHatShopModal = (hat: HatDef) => {
  ...
  openBuyModal(
    ...
    () =>
      ...
      playerStore.money >= discounted(hat.shopPrice!),
    lines,
  );
};
...
const openShoeShopModal = (shoe: ShoeDef) => {
  ...
  openBuyModal(
    ...
    () =>
      ...
      playerStore.money >= discounted(shoe.shopPrice!),
    lines,
  );
};
...
const sellableItems = computed(() => {
  const allowed =
    sellFilter.value.length > 0 ? new Set(sellFilter.value) : null;
  return inventoryStore.items
    ...
    .filter(
      (item) =>
        item.def &&
        !item.locked &&
        (!allowed || allowed.has(item.def!.category)),
    );
});
</script>
  ...
  setup(props) {
    return () =>
      h("div", { class: "flex items-center space-x-2 mb-3" }, [
        h("h3", { class: "text-accent text-sm" }, [
          `${props.name} · ${props.npc}`,
        ]),
      ]);
  },
});
