  ...
  <div>
    ...
    <template v-else>
      ...
      <template v-if="currentTab === 'pond'">
            ...
            <div class="flex items-center space-x-2">
              <span class="text-xs text-muted"
                >{{ fishPondStore.fishCount }}/{{
                  fishPondStore.capacity
                }}</span
              >
              <Button
                v-if="fishPondStore.pond.level < 3"
                :icon="ArrowUp"
                :icon-size="12"
                @click="pondModal = 'upgrade'"
                >升级</Button
              >
            </div>
          </div>
          ...
          <div class="border border-accent/20 rounded-xs px-3 py-2">
            <div class="flex items-center space-x-2 mb-1.5">
              ...
              <span
                ...
                :class="waterQualityTextColor"
                >{{ fishPondStore.pond.waterQuality }}%</span
              >
            </div>
            <!-- 操作按钮 -->
            <div class="flex flex-wrap space-x-1">
              <Button
                ...
                :icon-size="12"
                :disabled="
                  fishPondStore.pond.fedToday ||
                  fishPondStore.pond.fish.length === 0
                "
                @click="handleFeed"
              >
                {{ fishPondStore.pond.fedToday ? "已喂食" : "喂食" }}
              </Button>
              <Button :icon="Sparkles" :icon-size="12" @click="handleClean"
                >改良水质</Button
              >
              <Button
                v-if="fishPondStore.sickFish.length > 0"
                :icon="HeartPulse"
                :icon-size="12"
                @click="handleTreat"
              >
          ...
          <div v-else class="flex flex-col space-y-1.5 max-h-80 overflow-auto">
            <div
              ...
              :class="
                fish.sick
                  ? 'border-danger/30'
                  : selectedBreedingFish?.id === fish.id
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-1.5">
                  ...
                  <span v-if="fish.sick" class="text-[10px] text-danger"
                    >[病]</span
                  >
                  <span v-if="!fish.mature" class="text-[10px] text-muted"
                    >[幼]</span
                  >
                </div>
                <div class="flex items-center space-x-2">
                  ...
                  >
                    <Star
                      v-for="n in fishPondStore.getGeneticStarRating(
                        fish.genetics,
                      )"
                      :key="n"
                      :size="10"
        ...
        <div class="mb-3">
          <Divider label="放入鱼苗" />
          <div
            v-if="pondableFishInBag.length > 0"
          ...
          >
            ...
            >
              ...
              <Button :icon-size="12" @click="handleAddFish(item.itemId)"
                >放入</Button
              >
            </div>
          </div>
        ...
        <div class="mb-3">
          ...
          <!-- 繁殖中 -->
          <div
            v-if="fishPondStore.pond.breeding"
          ...
          >
            <div class="flex items-center justify-between mb-1">
              ...
              <span class="text-xs text-muted"
                >{{ fishPondStore.pond.breeding.daysLeft }}/{{
                  breedingTotalDays
                }}天</span
              >
            </div>
          ...
          <!-- 已选择一条 -->
          <div
            v-else-if="selectedBreedingFish"
            class="border border-accent/20 rounded-xs px-3 py-2"
          >
            <div class="flex items-center justify-between mb-1">
              <div class="flex items-center space-x-1.5">
      ...
      <template v-if="currentTab === 'compendium'">
        ...
        <p class="text-xs text-muted mb-2">
          已发现 {{ discoveredCountByGen(compendiumGen) }}/{{
            BREED_COUNTS[compendiumGen]
          }}
        </p>

        <!-- 提示 -->
        <div
          v-if="compendiumGen > 1"
        ...
        <div class="mt-3 border border-accent/20 rounded-xs p-2">
          <div class="flex items-center space-x-2 text-xs mb-1.5">
            ...
            <span class="text-xs text-accent whitespace-nowrap"
              >{{ fishPondStore.discoveredBreeds.size }}/{{
                totalBreedCount
              }}</span
            >
          </div>
          <div class="grid grid-cols-2 gap-x-3 gap-y-0.5">
            ...
            >
              ...
              <span class="text-xs"
                >{{ discoveredCountByGen(g) }}/{{ BREED_COUNTS[g] }}</span
              >
            </div>
          </div>
        </div>
    ...
    <Transition name="panel-fade">
      <div
        v-if="pondModal"
        class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
        @click.self="pondModal = null"
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="pondModal = null"
          >
            <X :size="14" />
          </button>
...
const getPondableFishName = (fishId: string): string =>
  getPondableFish(fishId)?.name ?? fishId;

const totalBreedCount = 400;

const isDiscovered = (breedId: string): boolean =>
  fishPondStore.discoveredBreeds.has(breedId);

const discoveredCountByGen = (gen: number): number => {
  const breeds = getBreedsByGeneration(gen as 1 | 2 | 3 | 4 | 5);
  return breeds.filter((b) => fishPondStore.discoveredBreeds.has(b.breedId))
    .length;
};
...
const completionPercent = computed(() => {
  return Math.floor(
    (fishPondStore.discoveredBreeds.size / totalBreedCount) * 100,
  );
});
...
const breedingProgress = computed(() => {
  ...
  return (
    ((breedingTotalDays - fishPondStore.pond.breeding.daysLeft) /
      breedingTotalDays) *
    100
  );
});
...
const upgradeNextLevel = computed(
  () => Math.min(fishPondStore.pond.level + 1, 3) as 2 | 3,
);
...
const modalCurrentLevel = computed(() =>
  pondModal.value === "build" ? 1 : fishPondStore.pond.level,
);

const modalCurrentCapacity = computed(() =>
  pondModal.value === "build" ? POND_CAPACITY[1] : fishPondStore.capacity,
);

const modalTargetLevel = computed(() => upgradeNextLevel.value);

const modalTargetCapacity = computed(
  () => POND_CAPACITY[upgradeNextLevel.value],
);

const modalMoney = computed(() =>
  pondModal.value === "build"
    ? POND_BUILD_COST.money
    : POND_UPGRADE_COSTS[upgradeNextLevel.value].money,
);

const modalMaterials = computed(() => {
  const mats =
    pondModal.value === "build"
      ? POND_BUILD_COST.materials
      : POND_UPGRADE_COSTS[upgradeNextLevel.value].materials;
  return mats.map((m) => ({
    itemId: m.itemId,
