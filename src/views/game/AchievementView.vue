<template>
  <div>
    ...
    <div class="flex space-x-1 mb-3">
      ...
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'bundles' }"
        @click="tab = 'bundles'"
        >祠堂</Button
      >
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'shipping' }"
        @click="tab = 'shipping'"
        >出货</Button
      >
      <Button
        class="flex-1 justify-center"
        :class="{ '!bg-accent !text-bg': tab === 'notes' }"
        @click="tab = 'notes'"
        >笔记</Button
      >
    </div>
    ...
    <template v-if="tab === 'collection'">
      <div class="flex items-center justify-between mb-1.5">
        <p class="text-xs text-muted">
          ...
          <span v-if="selectedCategory" class="text-accent ml-0.5"
            >· {{ CATEGORY_NAMES[selectedCategory] ?? selectedCategory }}</span
          >
        </p>
        <button
        ...
        <button
          ...
          :class="
            selectedCategory === cat.key
              ...
              : 'border-accent/15 text-muted/60 hover:text-muted'
          "
          @click="
            selectedCategory = selectedCategory === cat.key ? null : cat.key
          "
        >
          {{ cat.label }}({{ cat.count }})
        </button>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="getCategoryColor(activeCollectionItem.category)"
          >
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs">{{
                CATEGORY_NAMES[activeCollectionItem.category] ??
                activeCollectionItem.category
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ activeCollectionItem.sellPrice }}文</span
              >
            </div>
            <div
              v-if="
                activeCollectionItem.edible &&
                activeCollectionItem.staminaRestore
              "
            ...
            >
              ...
              <span class="text-xs text-success">
                ...
                <template v-if="activeCollectionItem.healthRestore"
                  >/ +{{ activeCollectionItem.healthRestore }}HP</template
                >
              </span>
            </div>
            <!-- 武器属性 -->
            <template v-if="activeWeaponDef">
              <div class="flex items-center justify-between mt-0.5">
                ...
                <span class="text-xs">{{
                  WEAPON_TYPE_NAMES[activeWeaponDef.type]
                }}</span>
              </div>
              <div class="flex items-center justify-between mt-0.5">
                <span class="text-xs text-muted">攻击力</span>
                <span class="text-xs text-danger">{{
                  activeWeaponDef.attack
                }}</span>
              </div>
              <div class="flex items-center justify-between mt-0.5">
                <span class="text-xs text-muted">暴击率</span>
                <span class="text-xs"
                  >{{ Math.round(activeWeaponDef.critRate * 100) }}%</span
                >
              </div>
              <div
                v-if="activeWeaponDef.fixedEnchantment"
                class="flex items-center justify-between mt-0.5"
              >
                <span class="text-xs text-muted">固定附魔</span>
                <span class="text-xs text-quality-supreme">{{
                  ENCHANTMENTS[activeWeaponDef.fixedEnchantment]?.name
                }}</span>
              </div>
            </template>
            <!-- 戒指/帽子/鞋子效果 -->
            <template v-if="activeEquipEffects.length > 0">
              <div
                v-for="(eff, i) in activeEquipEffects"
                :key="i"
                class="flex items-center justify-between mt-0.5"
              >
                <span class="text-xs text-muted">{{
            ...
            <div
              v-if="achievementStore.getDiscoveryTime(activeCollectionItem.id)"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">发现于</span>
              <span class="text-xs text-muted">{{
                achievementStore.getDiscoveryTime(activeCollectionItem.id)
              }}</span>
            </div>
          </div>
        </div>
    ...
    <template v-if="tab === 'achievements'">
      ...
      >
        <div
          ...
          :class="
            isCompleted(a.id)
              ? 'border-accent/20 cursor-pointer hover:bg-accent/5 text-success'
              : 'border-accent/10 text-muted/30'
          "
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="flex items-center space-x-1.5 mb-2">
            <CircleCheck
              v-if="isCompleted(activeAchievement.id)"
              :size="14"
            ...
            <span
              class="text-sm"
              :class="
                isCompleted(activeAchievement.id) ? 'text-success' : 'text-text'
              "
            >
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between mb-1">
              ...
              <span
                class="text-xs"
                :class="
                  isCompleted(activeAchievement.id)
                    ? 'text-success'
                    : 'text-text'
                "
              >
            ...
            <div class="h-1.5 bg-bg rounded-xs border border-accent/10">
              <div
                ...
                :class="
                  isCompleted(activeAchievement.id) ? 'bg-success' : 'bg-accent'
                "
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div class="flex flex-wrap space-x-3">
              <span
                v-if="activeAchievement.reward.money"
                class="text-xs text-accent"
                >{{ activeAchievement.reward.money }}文</span
              >
              <span
                v-for="ri in activeAchievement.reward.items ?? []"
                :key="ri.itemId"
    ...
    <template v-if="tab === 'bundles'">
      <div class="flex flex-col space-y-1.5 max-h-72 overflow-y-auto">
        <div
          ...
          :class="
            achievementStore.isBundleComplete(bundle.id)
              ? 'border-success/30'
              : 'border-accent/20'
          "
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-1.5">
              <CircleCheck
                v-if="achievementStore.isBundleComplete(bundle.id)"
                :size="12"
                class="text-success shrink-0"
              />
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="
              achievementStore.isBundleComplete(activeBundle.id)
                ? 'text-success'
                : 'text-accent'
            "
          >
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div
              v-for="req in activeBundle.requiredItems"
              :key="req.itemId"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">{{
              ...
              <span
                ...
                "
              >
                {{ getSubmittedCount(activeBundle.id, req.itemId) }}/{{
                  req.quantity
                }}
              </span>
            </div>
          ...
          <!-- 提交按钮 -->
          <div
            v-if="!achievementStore.isBundleComplete(activeBundle.id)"
            class="flex flex-col space-y-1"
          >
            <Button
              v-for="req in activeBundle.requiredItems.filter(
                (r) =>
                  getSubmittedCount(activeBundle!.id, r.itemId) < r.quantity,
              )"
    ...
    <template v-if="tab === 'shipping'">
      <p class="text-xs text-muted mb-2">
        出货记录 {{ shopStore.shippedItems.length }}/{{ shippableItems.length }}
      </p>
        ...
        >
          ...
          <div class="grid grid-cols-3 md:grid-cols-5 gap-1">
            <div
              ...
              @click="
                shopStore.shippedItems.includes(item.id) &&
                (activeShippingId = item.id)
              "
            >
              <template v-if="shopStore.shippedItems.includes(item.id)">{{
                item.name
              }}</template>
              <Lock v-else :size="12" class="mx-auto text-muted/30" />
            </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="getCategoryColor(activeShippingItem.category)"
          >
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between">
              <span class="text-xs text-muted">分类</span>
              <span class="text-xs">{{
                CATEGORY_NAMES[activeShippingItem.category] ??
                activeShippingItem.category
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ activeShippingItem.sellPrice }}文</span
              >
            </div>
            <div
              v-if="
                activeShippingItem.edible && activeShippingItem.staminaRestore
              "
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">恢复</span>
              <span class="text-xs text-success">
                ...
                <template v-if="activeShippingItem.healthRestore"
                  >/ +{{ activeShippingItem.healthRestore }}HP</template
                >
              </span>
            </div>
          </div>
    ...
    <template v-if="tab === 'notes'">
      <div
        v-if="secretNoteStore.collectedCount === 0"
      ...
      <template v-else>
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs text-accent"
            >{{ secretNoteStore.collectedCount }}/{{
              secretNoteStore.totalNotes
            }}</span
          >
        </div>
        <div
          ...
          <div
            ...
            :class="
              secretNoteStore.isCollected(note.id)
                ? 'border-accent/20 cursor-pointer hover:bg-accent/5 ' +
                  noteTypeColor(note.type)
                : 'border-accent/10 text-muted/30'
            "
            @click="
              secretNoteStore.isCollected(note.id) ? (activeNote = note) : null
            "
          >
            <template v-if="secretNoteStore.isCollected(note.id)"
              >#{{ note.id }} {{ note.title }}</template
            >
            <template v-else>
              #{{ note.id }}
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div
            v-if="activeNote.usable && !secretNoteStore.isUsed(activeNote.id)"
            class="mt-2"
          >
            <Button
              class="w-full justify-center !bg-accent !text-bg"
              @click="handleUseNote(activeNote.id)"
              >使用笔记</Button
            >
          </div>
          <div
            v-else-if="
              activeNote.usable && secretNoteStore.isUsed(activeNote.id)
            "
    ...
    <div class="mt-3 border border-accent/20 rounded-xs p-2">
      <div class="flex items-center space-x-2 text-xs mb-1.5">
        ...
        <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
          <div
            ...
            :style="{ width: achievementStore.perfectionPercent + '%' }"
          />
        </div>
      ...
      <div class="grid grid-cols-2 gap-x-3 gap-y-0.5">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs"
            >{{ achievementStore.stats.totalMoneyEarned }}文</span
          >
        </div>
        <div class="flex items-center justify-between">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs">{{
            achievementStore.stats.totalBreedingsDone
          }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">杂交发现</span>
          <span class="text-xs">{{
            achievementStore.stats.totalHybridsDiscovered
          }}</span>
        </div>
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs">
            {{
              achievementStore.stats.highestHybridTier > 0
                ? achievementStore.stats.highestHybridTier + "代"
                : "-"
            }}
          </span>
        </div>
...
const filteredDiscoveredCount = computed(() => {
  return filteredItems.value.filter((i) => achievementStore.isDiscovered(i.id))
    .length;
});
...
const totalRows = computed(() =>
  Math.ceil(filteredItems.value.length / collectionCols.value),
);

const visibleRange = computed(() => {
  const start = Math.max(
    0,
    Math.floor(collectionScrollTop.value / ROW_H) - VBUFFER,
  );
  const end = Math.min(
    totalRows.value,
    Math.ceil((collectionScrollTop.value + containerH.value) / ROW_H) + VBUFFER,
  );
  return { start, end };
});
...
const visibleItems = computed(() => {
  ...
  return filteredItems.value.slice(
    start * collectionCols.value,
    end * collectionCols.value,
  );
});

const topPad = computed(() => visibleRange.value.start * ROW_H);
const bottomPad = computed(() =>
  Math.max(0, (totalRows.value - visibleRange.value.end) * ROW_H),
);
...
const noteTypeColor = (type: string): string =>
  NOTE_TYPE_COLORS[type] ?? "text-accent";
...
const activeWeaponDef = computed(() => {
  if (
    !activeCollectionItem.value ||
    activeCollectionItem.value.category !== "weapon"
  )
    return null;
...
/** 格式化效果值 */
const FLAT_VALUE_EFFECTS = new Set([
  "attack_bonus",
  "max_hp_bonus",
  "ore_bonus",
]);
...
const shippableItems = computed(() =>
  ITEMS.filter((i) => SHIPPABLE_CATEGORIES.includes(i.category)),
);
...
const itemsByCategory = computed(() => {
  ...
  for (const item of shippableItems.value) {
    const cat =
      item.category === "crop" && hybridItemIds.has(item.id)
        ? "hybrid"
        : item.category;
...
const getProgressPercent = (a: (typeof ACHIEVEMENTS)[number]): number => {
  ...
  switch (c.type) {
    ...
    case "npcFriendship": {
      ...
      current = npcStore.npcStates.filter(
        (n) =>
          (LEVEL_RANK[npcStore.getFriendshipLevel(n.npcId)] ?? 0) >=
          requiredRank,
      ).length;
    ...
    case "npcBestFriend":
      current = npcStore.npcStates.filter(
        (n) => npcStore.getFriendshipLevel(n.npcId) === "bestFriend",
      ).length;
      target = c.count;
      break;
    case "npcAllFriendly":
...
const getProgressText = (a: (typeof ACHIEVEMENTS)[number]): string => {
  ...
  switch (c.type) {
    ...
    case "npcFriendship": {
      ...
      const requiredRank = LEVEL_RANK[c.level] ?? 0;
      const metCount = npcStore.npcStates.filter(
        (n) =>
          (LEVEL_RANK[npcStore.getFriendshipLevel(n.npcId)] ?? 0) >=
          requiredRank,
      ).length;
    ...
    case "npcBestFriend": {
      const bestCount = npcStore.npcStates.filter(
        (n) => npcStore.getFriendshipLevel(n.npcId) === "bestFriend",
      ).length;
