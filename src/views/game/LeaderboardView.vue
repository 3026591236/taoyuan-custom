<template>
  <div class="space-y-3">
    ...
    <div v-else class="space-y-1">
      <div v-for="(entry, idx) in entries" :key="idx" class="leaderboard-row border border-accent/15 rounded-xs p-2 flex items-center gap-2" :class="rankRowClass(idx)">
        <span class="rank-badge text-lg w-8 text-center shrink-0" :class="rankBadgeClass(idx)">{{ idx < 3 ? ['🥇','🥈','🥉'][idx] : idx + 1 }}</span>
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <span class="rank-name text-sm truncate" :class="rankNameClass(idx)" :data-name="entry.playerName">{{ entry.playerName }}</span>
            <span v-if="idx < 10" class="rank-effect-tag text-[10px]" :class="rankTagClass(idx)">{{ rankEffectLabel(idx) }}</span>
...
<style scoped>
  ...
  .rank-row-gold {
    ...
    background: radial-gradient(circle at 12% 20%, rgba(255, 210, 90, 0.18), transparent 34%), rgba(255, 193, 7, 0.06);
    box-shadow: 0 0 18px rgba(255, 190, 70, 0.16), inset 0 0 14px rgba(255, 231, 135, 0.06);
  }

  .rank-row-silver {
    border-color: rgba(190, 225, 255, 0.46);
    background: radial-gradient(circle at 12% 20%, rgba(190, 225, 255, 0.16), transparent 34%), rgba(125, 190, 255, 0.05);
    box-shadow: 0 0 16px rgba(160, 220, 255, 0.13), inset 0 0 14px rgba(220, 245, 255, 0.05);
  }

  .rank-row-bronze {
    border-color: rgba(255, 142, 86, 0.42);
    background: radial-gradient(circle at 12% 20%, rgba(255, 132, 72, 0.15), transparent 34%), rgba(255, 104, 54, 0.05);
    box-shadow: 0 0 14px rgba(255, 120, 70, 0.12), inset 0 0 14px rgba(255, 180, 120, 0.05);
  }

  .rank-row-top10 {
    border-color: rgba(120, 220, 255, 0.26);
    background: linear-gradient(90deg, rgba(60, 180, 255, 0.04), rgba(200, 120, 255, 0.05), rgba(60, 180, 255, 0.04));
  }
  ...
  .rank-row-silver::before { animation-delay: .35s; }
  .rank-row-bronze::before { animation-delay: .7s; }
  .rank-row-top10::before { animation-delay: 1s; opacity: .65; }
  ...
  .rank-name-gold {
    color: #ffe08a;
    text-shadow: 0 0 6px rgba(255, 201, 74, .95), 0 0 14px rgba(255, 116, 38, .55), 0 1px 0 #7a3600;
    animation: goldPulse 1.9s ease-in-out infinite;
  }

  .rank-name-silver {
    color: #e9fbff;
    text-shadow: 0 0 6px rgba(200, 245, 255, .9), 0 0 14px rgba(112, 180, 255, .55), 0 1px 0 #24406a;
    animation: silverFloat 2.4s ease-in-out infinite;
  }

  .rank-name-bronze {
    color: #ffb07a;
    text-shadow: 0 0 6px rgba(255, 138, 82, .9), 0 0 13px rgba(255, 67, 38, .45), 0 1px 0 #65240f;
  ...
  .rank-name-top10 {
    ...
    color: transparent;
    text-shadow: 0 0 8px rgba(95, 220, 255, .35);
    animation: starGradient 3s linear infinite;
  }

  .rank-badge { position: relative; z-index: 1; }
  .rank-badge-gold { filter: drop-shadow(0 0 6px rgba(255, 210, 70, .9)); animation: badgePop 1.8s ease-in-out infinite; }
  .rank-badge-silver { filter: drop-shadow(0 0 6px rgba(190, 235, 255, .75)); animation: badgePop 2.2s ease-in-out infinite; }
  .rank-badge-bronze { filter: drop-shadow(0 0 6px rgba(255, 135, 75, .75)); animation: badgePop 2s ease-in-out infinite; }
  .rank-badge-top10 { color: #7ee7ff; text-shadow: 0 0 8px rgba(95, 220, 255, .55); }
  ...
  .rank-tag-gold { color: #ffd45a; background: rgba(255, 205, 80, .08); box-shadow: 0 0 8px rgba(255, 205, 80, .22); }
  .rank-tag-silver { color: #ccefff; background: rgba(180, 225, 255, .08); box-shadow: 0 0 8px rgba(180, 225, 255, .2); }
  .rank-tag-bronze { color: #ffa06d; background: rgba(255, 132, 82, .08); box-shadow: 0 0 8px rgba(255, 132, 82, .18); }
  .rank-tag-top10 { color: #9beeff; background: rgba(100, 220, 255, .06); }

  @keyframes rankSweep { 0% { transform: translateX(-130%); } 52%, 100% { transform: translateX(130%); } }
  @keyframes goldPulse { 0%, 100% { filter: brightness(1); } 50% { filter: brightness(1.35); } }
  @keyframes silverFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-1px); } }
  @keyframes bronzeSpark { 0%, 100% { filter: saturate(1); } 50% { filter: saturate(1.45) brightness(1.18); } }
  @keyframes starGradient { 0% { background-position: 0% 50%; } 100% { background-position: 240% 50%; } }
