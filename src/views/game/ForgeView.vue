<template>
  <div class="space-y-3">
    ...
    <template v-else>
      <div
        class="border border-accent/20 rounded-xs p-3 bg-panel/30 text-xs text-muted leading-relaxed space-y-1"
      >
        <p class="text-accent text-sm">修仙装备淬炼</p>
        <p>
          炼器页现在直接服务角色页的「灵剑 / 法衣 / 云靴 /
          护符」。这里负责淬炼、词条洗练、灵韧维护与15转后的装备升星；灵韧过低会削弱战力和渡劫稳定，升星则消耗星陨铁、炼器图纸、法宝碎片等高阶材料。
        </p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div
          v-for="gear in gearCards"
          :key="gear.id"
        ...
        >
          <div class="flex items-start justify-between gap-2">
            ...
            <span
              ...
              >{{
                gear.level > 0 ? `${gear.level}阶 · ${gear.star}星` : "未凝练"
              }}</span
            >
          </div>
          ...
          <div class="grid grid-cols-3 gap-1 text-[10px]">
            ...
            <div
              class="stat-chip"
              :class="gear.durability < 70 ? 'text-caution' : 'text-success'"
            >
              灵韧 {{ gear.durability }}%
            </div>
          </div>
          ...
          <div class="text-[10px] leading-relaxed">
            ...
            <span :class="gear.hasMaterial ? 'text-accent' : 'text-danger'"
              >{{ gear.material.name }}×{{ gear.material.quantity }}（{{
                gear.materialOwned
              }}）</span
            >
            <span class="mx-1 text-muted">/</span>
            <span :class="gear.hasSpiritStone ? 'text-accent' : 'text-danger'"
              >灵石×{{ gear.spiritStoneCost }}（{{ spiritStoneCount }}）</span
            >
          </div>
          ...
          <div class="grid grid-cols-2 gap-1">
            <button
              ...
              :disabled="!gear.canForge || forging"
              @click="forgeGear(gear.id)"
            >
              {{
            ...
            <button
              class="btn w-full justify-center text-xs"
              :disabled="gear.level <= 0 || gear.durability >= 100"
              @click="maintainGear(gear.id)"
            >
              维护灵韧
            </button>
          </div>
          <p v-if="gear.maintenance" class="text-[10px] text-muted">
            维护：{{ gear.material.name }}×{{ gear.maintenance.materialQty }} /
            灵石×{{ gear.maintenance.spiritStone }} / 铜钱{{
              gear.maintenance.money
            }}
          </p>

          <div
            class="border border-accent/10 rounded-xs p-2 text-[10px] space-y-1 star-forge-box"
          >
            <p class="text-accent">
              装备升星
              <span class="text-muted">{{
                cultivationStore.daoGearStarUnlocked ? "已解锁" : "15转解锁"
              }}</span>
            </p>
            <p class="text-muted">
              当前 {{ gear.star }}/5 星；需先淬炼至{{
                gear.maxLevel
              }}阶。升星消耗星陨铁、法宝碎片、灵石，高星额外消耗炼器图纸。
            </p>
            <p v-if="gear.starCost" class="text-warning">
              下星：星陨铁×{{ gear.starCost.starIron }} / 法宝碎片×{{
                gear.starCost.artifactShard
              }}
              / 炼器图纸×{{ gear.starCost.blueprint }} / 灵石×{{
                gear.starCost.spiritStone
              }}
              / 铜钱{{ gear.starCost.money }}
            </p>
            <button
              class="btn w-full justify-center text-xs"
              :disabled="!gear.canStar"
              @click="starGear(gear.id)"
            >
              {{ gear.star >= 5 ? "升星圆满" : "装备升星" }}
            </button>
          </div>
      ...
      <div
        class="border border-accent/15 rounded-xs p-3 text-[10px] text-muted leading-relaxed space-y-2"
      >
        <p class="text-accent text-xs">套装词条与稀有图鉴</p>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div
            v-for="set in longTerm.gearSetBonuses"
            :key="set.id"
            class="stat-chip"
            :class="set.active ? 'text-success' : ''"
          >
            {{ set.name }} {{ set.count }}/4 · {{ set.desc }}
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-1">
          <div
            v-for="entry in longTerm.rareAffixCodex"
            :key="entry.name"
            class="stat-chip"
            :class="
              entry.discovered ? affixRarityClass(entry.rarity) : 'text-muted'
            "
          >
            {{ entry.discovered ? "已发现" : "未发现" }} · {{ entry.rarity }}
      ...
      <div
        class="border border-accent/15 rounded-xs p-3 text-[10px] text-muted leading-relaxed"
      >
        <p class="text-accent text-xs mb-1">材料路线</p>
        <p>
          法宝碎片来自青丘旧林、凶兽、登仙塔和秘境抉择；魂晶来自幽冥洞窟/登仙塔；风羽来自昆仑外境；雷精来自昆仑外境、宗门订单和雷云观想相关循环。炼器图纸暂不参与四件装备淬炼，可在洞府折灵换灵石。
        </p>
      </div>
    </template>
...
const affixRarityClass = (rarity?: string) =>
  rarity === "绝品"
    ? "text-amber-300"
    : rarity === "稀有"
      ? "text-purple-300"
  ...
  DAO_GEAR.map((gear) => {
    ...
    return {
      ...
      durability: Math.floor(
        cultivationStore.daoGearDurability[gear.id] ?? 100,
      ),
      maintenance: cultivationStore.daoGearMaintenanceCost(gear.id),
      star: cultivationStore.daoGearStarLevel(gear.id),
      starCost: cultivationStore.daoGearStarCost(gear.id),
      canStar: cultivationStore.canStarDaoGear(gear.id),
      power: Math.floor(
        (level * gear.powerPerLevel +
          cultivationStore.daoGearStarLevel(gear.id) *
            gear.powerPerLevel *
            0.55) *
          cultivationStore.daoGearDurabilityRate(gear.id),
      ),
      tribulation: Math.round(
        (level * gear.tribulationPerLevel +
          cultivationStore.daoGearStarLevel(gear.id) *
...
const starGear = (id: DaoGearId) => {
  cultivationStore.starDaoGear(id);
};
