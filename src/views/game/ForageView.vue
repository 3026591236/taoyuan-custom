<template>
  <div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div class="flex flex-wrap space-x-3 mt-2">
        <span
          v-if="weatherMod !== 1"
          class="text-[10px]"
          :class="weatherMod > 1 ? 'text-success' : 'text-danger'"
        >
          {{ weatherModLabel }}
        </span>
        <span v-if="hasHerbalistPerk" class="text-[10px] text-success"
          >药师：概率+20%</span
        >
        <span v-if="hasLumberjackPerk" class="text-[10px] text-success">
          {{
            foragingSkill.perk10 === "forester"
              ? "伐木工：必得木材"
              : "樵夫：25%额外木材"
          }}
        </span>
        <span
          v-if="foragingSkill.perk10 === 'tracker'"
          class="text-[10px] text-success"
          >追踪者：额外+1物品</span
        >
        <span v-if="cookingLuckBuff > 0" class="text-[10px] text-success"
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div v-if="lastResults.length > 0" class="flex flex-col space-y-1">
        ...
        >
          <span
            class="text-xs"
            :class="r.quality ? QUALITY_COLORS[r.quality] : ''"
            >{{ r.label }}</span
          >
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="
              selectedResult.quality
                ? QUALITY_COLORS[selectedResult.quality]
                : 'text-accent'
            "
          >
            {{ selectedResultDef.name }}
            <span v-if="selectedResult.quantity > 1" class="text-muted"
              >×{{ selectedResult.quantity }}</span
            >
          </p>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs">{{
                CATEGORY_NAMES[selectedResultDef.category] ??
                selectedResultDef.category
              }}</span>
            </div>
            <div
              v-if="
                selectedResult.quality && selectedResult.quality !== 'normal'
              "
            ...
            >
              ...
              <span
                class="text-xs"
                :class="QUALITY_COLORS[selectedResult.quality]"
                >{{ QUALITY_NAMES[selectedResult.quality] }}</span
              >
            </div>
            <div
              v-if="selectedResultDef.sellPrice > 0"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ selectedResultDef.sellPrice }}文</span
              >
            </div>
            <div
              v-if="selectedResultDef.edible"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">食用效果</span>
              <span class="text-xs text-success">
                {{
                  selectedResultDef.staminaRestore
                    ? `体力+${selectedResultDef.staminaRestore}`
                    : ""
                }}
                {{
                  selectedResultDef.healthRestore
                    ? `HP+${selectedResultDef.healthRestore}`
                    : ""
                }}
              </span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs">{{
                getItemSource(selectedResult.itemId!)
              }}</span>
            </div>
          </div>
        </div>
    ...
    <Transition name="panel-fade">
      <div
        v-if="encounter && encounter.type === 'friendly'"
        class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
      >
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            >
              ...
              <span class="text-[10px] text-muted"
                >+{{ encounter.animal.collectExp }}采集经验</span
              >
            </div>
            <div
              class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-1.5 cursor-pointer hover:bg-accent/5"
            ...
            >
              ...
              <span class="text-[10px] text-muted"
                >+{{ encounter.animal.chaseExp }}采集经验</span
              >
            </div>
            <div
    ...
    <Transition name="panel-fade">
      <div
        v-if="inForestCombat && forestCombatMonster"
        class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
      >
        <div class="game-panel max-w-xs w-full">
          <p class="text-sm text-danger mb-2">
            遭遇{{ forestCombatMonster.name }}！
          </p>
          ...
          >
            ...
            <div class="border border-danger/20 rounded-xs p-2">
              ...
              <p class="text-[10px] text-muted">
                {{ forestCombatMonsterHp }}/{{ forestCombatMonster.hp }}
              </p>
            </div>
          </div>
          ...
          <div class="text-xs space-y-0.5 max-h-28 overflow-y-auto">
            <p
              v-for="(msg, i) in forestCombatLog"
              :key="i"
              :class="
                i < forestCombatLog.length - 1 ? 'text-muted' : 'text-text'
              "
            >
              {{ msg }}
            </p>
...
const forageTime = computed(() => {
  ...
  const skillReduction =
    skillStore.getSkill("foraging").level * SKILL_TIME_REDUCTION_PER_LEVEL;
  return (
    Math.max(
      MIN_ACTION_MINUTES,
      Math.round((baseMin - saving) * (1 - skillReduction)),
    ) / 60
  );
});

const forageTimeLabel = computed(
  () => `${Math.round(forageTime.value * 60)}分钟`,
);

const weatherMod = computed(
...
const weatherModLabel = computed(
  () => WEATHER_MOD_LABELS[gameStore.weather] ?? "",
);

const hasHerbalistPerk = computed(
  () => foragingSkill.value.perk5 === "herbalist",
);
const hasLumberjackPerk = computed(
  () =>
    foragingSkill.value.perk5 === "lumberjack" ||
    foragingSkill.value.perk10 === "forester",
);
const isForestFarm = computed(() => gameStore.farmMapType === "forest");
const cookingLuckBuff = computed(() =>
  cookingStore.activeBuff?.type === "luck" ? cookingStore.activeBuff.value : 0,
);
...
const handleForestCombat = (action: CombatAction) => {
    ...
    const damage = Math.max(
      ...
      Math.floor(
        monster.attack *
          ...
          (1 - cookingDefBuff) *
          (1 - ringDefenseBonus) *
          (1 - miningStore.guildBonusDefense),
      ),
    );
  ...
  const playerDmg = Math.max(
    1,
    Math.floor(baseAttack * critMultiplier * bruteBonus) - monster.defense,
  );
  forestCombatMonsterHp.value -= playerDmg;
  let atkMsg = isCrit
    ? `暴击！对${monster.name}造成${playerDmg}点伤害！`
    : `对${monster.name}造成${playerDmg}点伤害。`;
  ...
  const fighterReduction =
    skillStore.getSkill("combat").perk5 === "fighter" ? 0.85 : 1.0;
  const ringDefenseBonus = inventoryStore.getRingEffectValue("defense_bonus");
...
const handleForestDefeat = () => {
  ...
  // 惩罚：损失金钱
  const moneyLoss = Math.min(
    Math.floor(playerStore.money * FOREST_DEFEAT_MONEY_PENALTY_RATE),
    FOREST_DEFEAT_MONEY_PENALTY_CAP,
  );
