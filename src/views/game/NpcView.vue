<template>
  <div>
    ...
    <div v-if="activeTab === 'villager'">
      ...
      <section class="border border-accent/20 rounded-xs p-2 mb-3 bg-bg/40">
        <div class="flex items-center justify-between mb-2">
          <p class="text-xs text-accent">NPC 主动来信</p>
          <span class="text-[10px] text-muted">每日可回复，消耗物资换人情</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div
            v-for="letter in npcStore.npcLetterCards"
            :key="letter.id"
          ...
          >
            ...
            <p class="text-[10px] text-warning mt-1">
              需要：{{ letter.itemName }}×{{ letter.quantity }}
            </p>
            <button
              class="mini-btn mt-2"
              :disabled="letter.claimed"
              @click="handleNpcLetter(letter.id)"
            >
              {{ letter.claimed ? "已回复" : "回复" }}
            </button>
          </div>
        </div>
      </section>
      ...
      <section class="border border-accent/20 rounded-xs p-2 mb-3 bg-bg/40">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div
            v-for="fb in npcStore.worldFeedbackCards"
            :key="fb.id"
            class="border border-accent/10 rounded-xs p-2"
            :class="fb.claimed ? 'opacity-60' : fb.done ? 'bg-accent/5' : ''"
          >
            ...
            <button
              class="mini-btn mt-2"
              :disabled="!fb.done || fb.claimed"
              @click="handleWorldFeedback(fb.id)"
            >
              {{ fb.claimed ? "本周已回响" : fb.done ? "领取回响" : "未达成" }}
            </button>
          </div>
        </div>
      </section>
        ...
        >
          ...
          <div class="md:hidden">
            <p
              ...
              :class="levelColor(npcStore.getFriendshipLevel(npc.id))"
            >
              {{ npc.name }}
            </p>
            <p
              class="text-[10px] flex items-center justify-center"
              :class="heartCount(npc.id) > 0 ? 'text-danger' : 'text-muted/30'"
            >
              {{ heartCount(npc.id) }}
              <Heart
                :size="10"
                :fill="heartCount(npc.id) > 0 ? 'currentColor' : 'none'"
              />
              <span class="text-muted/50 ml-0.5">{{
            ...
            >
              <MessageCircle
                :size="10"
                :class="
                  npcStore.getNpcState(npc.id)?.talkedToday
                    ? 'text-muted/20'
                    : 'text-success'
                "
              />
              ...
              <Heart
                v-if="npcStore.getNpcState(npc.id)?.married"
                :size="10"
                class="text-danger"
              />
              <Heart
                v-else-if="npcStore.getNpcState(npc.id)?.dating"
                :size="10"
                class="text-danger/50"
              />
              <Heart
                v-else-if="npcStore.getNpcState(npc.id)?.zhiji"
                :size="10"
                class="text-accent"
              />
              <Heart
                v-else-if="npc.marriageable"
                :size="10"
                class="text-muted/30"
              />
              <Cake
                v-if="npcStore.isBirthday(npc.id)"
                :size="10"
                class="text-danger"
              />
            </div>
            ...
            <div class="flex items-center justify-between mt-0.5">
              <div class="flex items-center space-x-px">
                <Heart
                  ...
                  class="flex-shrink-0"
                  :class="
                    (npcStore.getNpcState(npc.id)?.friendship ?? 0) >= h * 250
                      ? 'text-danger'
                      : 'text-muted/30'
                  "
                  :fill="
    ...
    <div v-if="activeTab === 'spirit'">
          ...
          >
            ...
            <div class="md:hidden">
              ...
              <p
                class="text-[10px] flex items-center justify-center"
                :class="
                  hiddenHeartCount(npc.id) > 0 ? 'text-accent' : 'text-muted/30'
                "
              >
                {{ hiddenHeartCount(npc.id) }}
                <Diamond
                  :size="10"
                  :fill="hiddenHeartCount(npc.id) > 0 ? 'currentColor' : 'none'"
                />
                <span class="text-muted/50 ml-0.5">{{
                  hiddenNpcStore.getHiddenNpcState(npc.id)?.affinity ?? 0
                }}</span>
              </p>
            </div>
            <!-- 桌面端：显示更多信息 -->
              ...
              <div class="flex items-center justify-between mt-0.5">
                <div class="flex items-center space-x-px">
                  <Diamond
                    ...
                    class="flex-shrink-0"
                    :class="
                      (hiddenNpcStore.getHiddenNpcState(npc.id)?.affinity ??
                        0) >=
                      d * 250
                        ? 'text-accent'
                        : 'text-muted/20'
      ...
      <div
        v-if="rumorHiddenNpcs.length > 0"
        :class="{ 'mt-4': revealedHiddenNpcs.length > 0 }"
      >
        ...
        <div class="flex flex-col space-y-1">
          ...
          >
            <span
              v-if="
                hiddenNpcStore.getHiddenNpcState(npc.id)?.discoveryPhase ===
                'rumor'
              "
            >
    ...
    <Transition name="panel-fade">
      <HiddenNpcModal
        v-if="selectedHiddenNpc"
        :npc-id="selectedHiddenNpc"
        @close="selectedHiddenNpc = null"
      />
    </Transition>
      ...
      >
        <div class="game-panel max-w-lg w-full max-h-[80vh] overflow-y-auto">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between mb-1">
              <div class="flex items-center space-x-px">
                <Heart
                  ...
                  class="flex-shrink-0"
                  :class="
                    (selectedNpcState?.friendship ?? 0) >= h * 250
                      ? 'text-danger'
                      : 'text-muted/20'
                  "
                  :fill="
              ...
              <span
                class="text-xs"
                :class="levelColor(npcStore.getFriendshipLevel(selectedNpc!))"
              >
            ...
            <div class="flex items-center space-x-1.5 flex-wrap">
              <span
                ...
                :class="
                  selectedNpcState?.talkedToday
                    ? 'text-muted/40 border-muted/10'
              ...
              >
                <Cake :size="10" />
                <span
                  >{{ SEASON_NAMES_MAP[selectedNpcDef.birthday.season]
                  }}{{ selectedNpcDef.birthday.day }}日</span
                >
              </span>
              <span
                v-if="npcStore.isBirthday(selectedNpc!)"
          ...
          <div
            v-if="
              selectedNpcState &&
              selectedNpcState.triggeredHeartEvents.length > 0
            "
            class="mb-3"
          >
          ...
          <div class="mb-3 flex space-y-2 flex-wrap">
            <Button
              class="w-full"
              :icon="MessageCircle"
              :disabled="selectedNpcState?.talkedToday"
              @click="handleTalk"
            >
            ...
            <Button
              ...
              :icon="Lightbulb"
              :disabled="
                !!(selectedNpc && npcStore.isTipGivenToday(selectedNpc))
              "
            ...
            >
              {{
                selectedNpc && npcStore.isTipGivenToday(selectedNpc)
                  ? "今天已提示"
                  : TIP_NPC_LABELS[selectedNpc as TipNpcId]
              }}
            </Button>
            <!-- 离婚按钮 -->
            <Button
          ...
          <p
            v-if="
              npcStore.weddingCountdown > 0 &&
              npcStore.weddingNpcId === selectedNpc
            "
          ...
          >
            ...
            >
              <p
                v-if="npcStore.npcStates.some((s) => s.married)"
              ...
              <template v-else>
                <div class="flex flex-col space-y-0.5 mb-1.5">
                  <span
                    ...
                    :class="
                      (selectedNpcState?.friendship ?? 0) >= 2000
                        ? 'text-success'
                        : 'text-muted/50'
                    "
                  >
                    <CircleCheck
                      v-if="(selectedNpcState?.friendship ?? 0) >= 2000"
                      :size="10"
                    />
                    <Circle v-else :size="10" />
                    <span>好感≥2000（8心）</span>
                    <span class="text-muted/40"
                      >— 当前{{ selectedNpcState?.friendship ?? 0 }}</span
                    >
                  </span>
                  <span
                    class="text-[10px] flex items-center space-x-1"
                    :class="
                      inventoryStore.hasItem('silk_ribbon')
                        ? 'text-success'
                        : 'text-muted/50'
                    "
                  >
                    <CircleCheck
                      v-if="inventoryStore.hasItem('silk_ribbon')"
                      :size="10"
                    />
                    <Circle v-else :size="10" />
                    <span>持有丝帕</span>
                ...
                <Button
                  class="w-full text-danger border-danger/40"
                  :icon="Heart"
                  :disabled="!canStartDating"
                  @click="handleStartDating"
                >
                  赠帕（开始约会）
                </Button>
              </template>
            ...
            <template v-else-if="selectedNpcState?.dating">
              ...
              <div class="flex flex-col space-y-0.5 mb-1.5">
                <span
                  ...
                  :class="
                    (selectedNpcState?.friendship ?? 0) >= 2500
                      ? 'text-success'
                      : 'text-muted/50'
                  "
                >
                  <CircleCheck
                    v-if="(selectedNpcState?.friendship ?? 0) >= 2500"
                    :size="10"
                  />
                  <Circle v-else :size="10" />
                  好感≥2500（10心）
                ...
                <span
                  class="text-[10px] flex items-center space-x-0.5"
                  :class="
                    inventoryStore.hasItem('jade_ring')
                      ? 'text-success'
                      : 'text-muted/50'
                  "
                >
                  <CircleCheck
                    v-if="inventoryStore.hasItem('jade_ring')"
                    :size="10"
                  />
                  <Circle v-else :size="10" />
                  持有翡翠戒指
          ...
          >
            ...
            <template v-if="selectedNpcState?.zhiji">
              ...
              <Button
                class="w-full text-danger border-danger/40"
                @click="showZhijiDissolveConfirm = true"
                >断缘</Button
              >
            </template>
            ...
            <template v-else>
              <div class="flex flex-col space-y-0.5 mb-1.5">
                ...
                <span
                  class="text-[10px] flex items-center space-x-0.5"
                  :class="
                    inventoryStore.hasItem('zhiji_jade')
                      ? 'text-success'
                      : 'text-muted/50'
                  "
                >
                  <CircleCheck
                    v-if="inventoryStore.hasItem('zhiji_jade')"
                    :size="10"
                  />
                  <Circle v-else :size="10" />
                  持有知己玉佩
              ...
              <Button
                ...
                :icon="Heart"
                :disabled="!canBecomeZhiji"
                @click="handleBecomeZhiji"
              >
                赠玉（结为知己）
              </Button>
            </template>
          ...
          <!-- 断缘确认 -->
          <div
            v-if="showZhijiDissolveConfirm"
            class="game-panel mb-3 border-accent/40"
          >
            <p class="text-xs text-danger mb-2">
              确定要与{{ selectedNpcDef?.name }}断缘吗？（花费10000文）
            </p>
            <div class="flex space-x-2">
              <Button class="text-danger" @click="handleDissolveZhiji"
                >确认</Button
              >
          ...
          <!-- 离婚确认 -->
          <div
            v-if="showDivorceConfirm"
            class="game-panel mb-3 border-danger/40"
          >
            <p class="text-xs text-danger mb-2">
              确定要与{{ selectedNpcDef?.name }}和离吗？（花费30000文）
            </p>
            <div class="flex space-x-2">
              ...
              <div class="flex flex-col space-y-1 max-h-40 overflow-y-auto">
                ...
                >
                  <span class="flex items-center space-x-1">
                    <span
                      class="text-xs"
                      :class="qualityTextClass(item.quality)"
                    >
            ...
            >
              <div class="game-panel max-w-xs w-full relative">
                ...
                <p
                  ...
                  :class="
                    qualityTextClass(activeGiftItem.quality, 'text-accent')
                  "
                >
                  {{ activeGiftDef.name }}
                </p>
                ...
                <div class="border border-accent/10 rounded-xs p-2 mb-2">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs"
                      >&times;{{ activeGiftItem.quantity }}</span
                    >
                  </div>
                  <div
                    v-if="activeGiftItem.quality !== 'normal'"
                    class="flex items-center justify-between mt-0.5"
                  >
                    <span class="text-xs text-muted">品质</span>
                    <span
                      class="text-xs"
                      :class="qualityTextClass(activeGiftItem.quality)"
                    >
                ...
                <div class="flex flex-col space-y-1.5">
                  <Button
                    ...
                    @click="
                      handleGift(
                        activeGiftItem!.itemId,
                        activeGiftItem!.quality,
                      )
                    "
                  >
...
const selectedNpcDef = computed(() =>
  selectedNpc.value ? getNpcById(selectedNpc.value) : null,
);
const selectedNpcState = computed(() =>
  selectedNpc.value ? npcStore.getNpcState(selectedNpc.value) : null,
);
...
const giftableItems = computed(() => {
  ...
  return [...filtered].sort(
    (a, b) =>
      GIFT_PREF_ORDER[getGiftPreference(a.itemId)] -
      GIFT_PREF_ORDER[getGiftPreference(b.itemId)],
  );
});
...
const canBecomeZhiji = computed(() => {
  ...
  if (selectedNpcDef.value.gender !== playerStore.gender) return false;
  if (
    selectedNpcState.value?.zhiji ||
    selectedNpcState.value?.dating ||
    selectedNpcState.value?.married
  )
    return false;
...
const handleTalk = () => {
  ...
  if (result) {
    ...
    addLog(
      `与${selectedNpcDef.value?.name}聊天。(+${result.friendshipGain}好感)`,
    );
...
const handleGift = (itemId: string, quality: Quality = "normal") => {
  ...
  if (result) {
    ...
    if (result.gain > 0) {
      addLog(
        `送给${npcName}${itemName}，${npcName}觉得${result.reaction}。(+${result.gain}好感)`,
      );
    } else if (result.gain < 0) {
      addLog(
        `送给${npcName}${itemName}，${npcName}${result.reaction}这个……(${result.gain}好感)`,
      );
    } else {
