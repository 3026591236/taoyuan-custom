  ...
  <div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      <p class="text-sm text-accent mb-2">
        ...
        {{
          homeStore.caveUnlocked && homeStore.caveChoice !== "none"
            ? homeStore.caveName
            : "山洞"
        }}
      </p>
        ...
        <div class="border border-accent/10 rounded-xs p-2 mb-2 space-y-0.5">
          <div class="flex items-center justify-between">
            ...
            <span class="text-[10px] text-accent"
              >{{ homeStore.caveName }}（Lv.{{ homeStore.caveLevel }}）</span
            >
          </div>
          <div class="flex items-center justify-between">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-[10px]" :class="caveQualityClass">{{
              caveQualityLabel
            }}</span>
          </div>
          <div class="flex items-center justify-between">
          ...
          <div
            v-if="currentCaveDef && currentCaveDef.doubleChance > 0"
            class="flex items-center justify-between"
          >
            <span class="text-[10px] text-muted">双倍概率</span>
            <span class="text-[10px]"
              >{{ Math.round(currentCaveDef.doubleChance * 100) }}%</span
            >
          </div>
        </div>
        <!-- 山洞升级 -->
        <div
          v-if="homeStore.nextCaveUpgrade"
          class="border border-accent/10 rounded-xs p-2"
        >
          <p class="text-[10px] text-muted mb-1">
            升级至 {{ homeStore.nextCaveUpgrade.name }}
          </p>
          <div class="space-y-0.5 mb-1.5">
            <div
              v-for="mat in homeStore.nextCaveUpgrade.materialCost"
              :key="mat.itemId"
              class="flex items-center justify-between"
            >
            ...
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-[10px]"
                :class="
                  playerStore.money >= homeStore.nextCaveUpgrade.cost
                    ? ''
                    : 'text-danger'
                "
              >
        ...
        <div class="grid grid-cols-3 gap-2 text-center">
          <div class="border border-accent/10 rounded-xs py-1">
            ...
            <p class="text-xs text-accent">
              {{ farmStore.greenhousePlots.length }}
            </p>
          </div>
          <div class="border border-accent/10 rounded-xs py-1">
          ...
          <div class="border border-accent/10 rounded-xs py-1">
            ...
            <p
              class="text-xs"
              :class="
                greenhouseHarvestableCount > 0 ? 'text-success' : 'text-muted'
              "
            >
    ...
    <div class="border border-accent/20 rounded-xs p-3">
      ...
      <div v-if="!warehouseStore.unlocked">
        ...
        >
          ...
          <span class="text-xs text-accent whitespace-nowrap"
            >{{ warehouseStore.UNLOCK_COST }}文</span
          >
        </div>
      </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-1.5">
                <span
                  ...
                  :class="
                    chest.tier === 'void'
                      ? 'text-quality-supreme'
                      : 'text-accent'
                  "
                >
                  {{ CHEST_DEFS[chest.tier].name }}
                </span>
                <span v-if="renamingChestId !== chest.id" class="text-xs">{{
                  chest.label
                }}</span>
                <input
                  v-else
              ...
              <div class="flex items-center space-x-1.5">
                <span class="text-[10px] text-muted"
                  >{{ chest.items.length }}/{{
                    CHEST_DEFS[chest.tier].capacity
                  }}</span
                >
                <button
                ...
                <button
                  v-if="
                    warehouseStore.chests.length > 1 &&
                    chestIdx < warehouseStore.chests.length - 1
                  "
                  class="text-muted hover:text-accent"
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2 space-y-1">
            ...
            <div
              v-for="mat in GREENHOUSE_MATERIAL_COST"
              :key="mat.itemId"
              class="flex items-center justify-between"
            >
            ...
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="
                  playerStore.money >= GREENHOUSE_UNLOCK_COST
                    ? ''
                    : 'text-danger'
                "
              >
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="showWarehouseUnlockModal = false"
          >
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2 space-y-1">
            <div
              v-for="mat in WAREHOUSE_UNLOCK_MATERIALS"
              :key="mat.itemId"
              class="flex items-center justify-between"
            >
              <span class="text-xs text-muted">{{
            ...
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="
                  playerStore.money >= warehouseStore.UNLOCK_COST
                    ? ''
                    : 'text-danger'
                "
              >
        ...
        <div v-if="currentOpenChest" class="game-panel max-w-sm w-full">
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center space-x-1.5">
              ...
              <span class="text-[10px] text-muted">
                {{ currentOpenChest.items.length }}/{{
                  CHEST_DEFS[currentOpenChest.tier].capacity
                }}
              </span>
            </div>
            <Button
          ...
          <!-- 存入按钮 -->
          <Button
            v-if="depositableItems.length > 0"
            class="w-full"
            :icon="ArrowDown"
            :icon-size="12"
            @click="showChestDepositModal = true"
          >
            存入物品
          </Button>
        </div>
        ...
        <div class="game-panel max-w-sm w-full">
          <div class="flex items-center justify-between mb-2">
            ...
            <Button
              ...
              :icon="X"
              :icon-size="12"
              @click="showChestDepositModal = false"
            />
          </div>
            ...
            >
              ...
              >
                ...
                <span v-if="item.quality !== 'normal'" class="text-[10px]"
                  >({{ QUALITY_LABEL[item.quality] }})</span
                >
              </span>
        ...
        <div class="game-panel max-w-xs w-full">
          <div class="flex items-center justify-between mb-2">
            ...
            <Button
              ...
              :icon="X"
              :icon-size="12"
              @click="chestQtyModal = null"
            />
          </div>
          <p
            class="text-xs mb-2"
            :class="qualityTextClass(chestQtyModal.quality)"
          >
            {{ getItemName(chestQtyModal.itemId) }}
            <span v-if="chestQtyModal.quality !== 'normal'" class="text-[10px]"
              >({{ QUALITY_LABEL[chestQtyModal.quality] }})</span
            >
          </p>
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between mb-1.5">
              ...
              <div class="flex items-center space-x-1">
                ...
                <Button
                  class="h-6 px-1.5 py-0.5 text-xs justify-center"
                  :disabled="chestQty >= chestQtyModal.max"
                  @click="addChestQty(1)"
                >
                  +
                </Button>
            ...
            <div class="flex space-x-1">
              ...
              <Button
                class="flex-1 justify-center"
                :disabled="chestQty >= chestQtyModal.max"
                @click="setChestQty(chestQtyModal!.max)"
              >
                最多
              </Button>
            </div>
          ...
          <Button
            ...
            @click="confirmChestQty"
          >
            {{ chestQtyModal.mode === "withdraw" ? "取出" : "存入" }} &times;{{
              chestQty
            }}
          </Button>
        </div>
          ...
          <template v-if="dismantleChestInfo">
            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              ...
              <div class="flex flex-wrap gap-x-3 gap-y-0.5">
                <span
                  v-for="mat in dismantleChestInfo.refund"
                  :key="mat.itemId"
                ...
                <span
                  v-if="dismantleChestInfo.refund.length === 0"
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="chestItemDetail = null"
          >
            <X :size="14" />
          </button>
          <p
            class="text-sm mb-2"
            :class="qualityTextClass(chestItemDetail.quality, 'text-accent')"
          >
            {{ chestItemDef.name }}
          </p>
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div
              v-if="chestItemDetail.quality !== 'normal'"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">品质</span>
              <span
                class="text-xs"
                :class="qualityTextClass(chestItemDetail.quality)"
                >{{ QUALITY_LABEL[chestItemDetail.quality] }}</span
              >
            </div>
            <div
              v-if="chestItemDef.sellPrice"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ chestItemDef.sellPrice }}文</span
              >
            </div>
            <div
              v-if="chestItemDef.staminaRestore"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">恢复</span>
              <span class="text-xs text-success">
                +{{ chestItemDef.staminaRestore }}体力
                <template v-if="chestItemDef.healthRestore"
                  >/ +{{ chestItemDef.healthRestore }}HP</template
                >
              </span>
            </div>
          </div>
        ...
        <div class="game-panel max-w-sm w-full">
          <div class="flex items-center justify-between mb-2">
            ...
            <Button
              ...
              :icon="X"
              :icon-size="12"
              @click="showAddChestModal = false"
            />
          </div>
          <div class="flex flex-col space-y-1.5">
            <div
              v-for="tier in CHEST_TIER_ORDER"
              :key="tier"
            ...
            >
              <div class="flex items-center justify-between mb-1">
                <div class="flex items-center space-x-1.5">
                  <span
                    ...
                    :class="
                      tier === 'void' ? 'text-quality-supreme' : 'text-accent'
                    "
                  >
                    {{ CHEST_DEFS[tier].name }}
                  </span>
                  <span class="text-[10px] text-muted"
                    >{{ CHEST_DEFS[tier].capacity }}格</span
                  >
                </div>
                <Button
                  class="py-0 px-1.5"
              ...
              <div class="flex flex-wrap gap-x-3 gap-y-0.5">
                ...
                <span
                  class="text-[10px]"
                  :class="
                    playerStore.money >= CHEST_DEFS[tier].craftMoney
                      ? 'text-muted'
                      : 'text-danger'
                  "
                >
...
const currentCaveDef = computed(
  () => getCaveUpgrade(homeStore.caveLevel) ?? null,
);

const caveQualityLabel = computed(() => QUALITY_LABEL[homeStore.caveQuality]);

const caveQualityClass = computed(() =>
  qualityTextClass(homeStore.caveQuality),
);
...
const canUnlockGreenhouse = computed(() => {
  ...
  return GREENHOUSE_MATERIAL_COST.every(
    (mat) => getCombinedItemCount(mat.itemId) >= mat.quantity,
  );
});
...
const greenhousePlantedCount = computed(
  () =>
    farmStore.greenhousePlots.filter(
      (plot) =>
        plot.state === "planted" ||
        plot.state === "growing" ||
...
const greenhouseHarvestableCount = computed(
  () =>
    farmStore.greenhousePlots.filter((plot) => plot.state === "harvestable")
      .length,
);
...
const canUnlockWarehouse = computed(() => {
  ...
  return WAREHOUSE_UNLOCK_MATERIALS.every(
    (mat) => getCombinedItemCount(mat.itemId) >= mat.quantity,
  );
});
...
const duplicateDepositItems = computed(() => {
  ...
  const chestItemIds = new Set(
    currentOpenChest.value.items.map((i) => i.itemId),
  );
...
const canCraftChest = (tier: ChestTier): boolean => {
  ...
  return processingStore.canCraft(
    CHEST_DEFS[tier].craftCost,
    CHEST_DEFS[tier].craftMoney,
  );
};
...
const handleCraftChest = (tier: ChestTier) => {
  ...
  if (
    !processingStore.consumeCraftMaterials(
      CHEST_DEFS[tier].craftCost,
      CHEST_DEFS[tier].craftMoney,
    )
  )
  ...
  addLog(
    `制作了${CHEST_DEFS[tier].name}！（-${CHEST_DEFS[tier].craftMoney}文）`,
  );
...
const executeChestDeposit = (
  ...
  if (
    depositableItems.value.length === 0 ||
