<template>
  <div>
      ...
      <div class="grid grid-cols-3 gap-1">
        ...
        >
          <span
            class="text-xs"
            :class="
              fishingStore.fishingLocation === loc.id ? 'text-accent' : ''
            "
          >
            {{ loc.name }}
          </span>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div class="flex flex-col space-y-1">
        ...
        >
          ...
          <span
            class="text-xs"
            :class="fishingStore.equippedBait ? 'text-accent' : 'text-muted'"
          >
            <template v-if="fishingStore.equippedBait">
              {{ getBaitName(fishingStore.equippedBait) }}
              <span class="text-muted"
                >(&times;{{
                  inventoryStore.getItemCount(fishingStore.equippedBait)
                }})</span
              >
            </template>
        ...
        <div
          ...
          :class="
            canEquipTackle ? 'cursor-pointer hover:bg-accent/5' : 'opacity-50'
          "
        ...
        >
          ...
          <span
            class="text-xs"
            :class="fishingStore.equippedTackle ? 'text-accent' : 'text-muted'"
          >
            <template v-if="fishingStore.equippedTackle">
              {{ getTackleName(fishingStore.equippedTackle) }}
              <span class="text-muted"
                >({{ fishingStore.tackleDurability }})</span
              >
            </template>
            <template v-else>{{
              canEquipTackle ? "未装备" : "需铁竿以上"
            }}</template>
          </span>
        </div>
      </div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div
        v-if="fishingStore.availableFish.length > 0"
      ...
      >
        ...
        >
          <span class="text-xs" :class="DIFFICULTY_COLORS[f.difficulty]">{{
            f.name
          }}</span>
        ...
        >
          <div class="flex items-center justify-between mb-1">
            ...
            <div class="flex space-x-1">
              <Button class="py-0 px-1" @click="handleBaitCrabPots(loc.id)"
                >装饵</Button
              >
              <Button class="py-0 px-1" @click="handleRemoveCrabPot(loc.id)"
                >回收</Button
              >
            </div>
          </div>
          <p class="text-[10px] text-muted">
            {{ loc.total }}个 · {{ loc.baited }}个已装饵
          </p>
        </div>
      </div>
      <div
        v-else-if="!hasCrabPotInBag"
    ...
    <div class="border border-accent/20 rounded-xs p-3">
      ...
      <div v-if="canPan">
        ...
        >
          ...
          <span class="text-xs text-muted"
            >消耗体力 · {{ Math.round(panTime * 60) }}分钟</span
          >
        </div>
        <div
          v-if="panResult"
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <!-- 当前装备 -->
          <div
            v-if="fishingStore.equippedBait"
          ...
          <div
            v-else-if="!fishingStore.equippedBait"
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <!-- 当前装备 -->
          <div
            v-if="fishingStore.equippedTackle"
            class="border border-accent/10 rounded-xs p-2 mb-2"
          >
            <p class="text-[10px] text-muted mb-1">当前装备</p>
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center space-x-2">
                <span class="text-[10px] text-muted"
                  >耐久 {{ fishingStore.tackleDurability }}</span
                >
                <Button class="py-0 px-1" @click="handleUnequipTackle"
                  >卸下</Button
                >
              </div>
            </div>
          ...
          <div
            v-else-if="!fishingStore.equippedTackle"
            class="flex flex-col items-center justify-center py-4 text-muted"
          >
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-xs"
                :class="catchResult.success ? 'text-success' : 'text-danger'"
              >
                {{ catchResult.success ? "成功捕获" : "鱼跑了" }}
              </span>
            </div>
            <div
              v-if="catchResult.success && catchResult.quantity"
            ...
            <div
              v-if="catchResult.success && catchResult.quality"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">品质</span>
              <span
                class="text-xs"
                :class="QUALITY_COLORS[catchResult.quality]"
                >{{ QUALITY_NAMES[catchResult.quality] }}</span
              >
            </div>
            <div
              v-if="catchResult.difficulty"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">难度</span>
              <span
                class="text-xs"
                :class="DIFFICULTY_COLORS[catchResult.difficulty]"
                >{{ DIFFICULTY_NAMES[catchResult.difficulty] }}</span
              >
            </div>
            <div
              v-if="catchResult.sellPrice"
              class="flex items-center justify-between mt-0.5"
            >
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ catchResult.sellPrice }}文</span
              >
            </div>
          </div>

          <p
            v-if="catchResult.message.includes('宝箱')"
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="DIFFICULTY_COLORS[selectedFish.difficulty]"
          >
            {{ selectedFish.name }}
          </p>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">售价</span>
              <span class="text-xs text-accent"
                >{{ selectedFish.sellPrice }}文</span
              >
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">季节</span>
              <span class="text-xs">{{
                selectedFish.season.map((s) => SEASON_LABEL[s] ?? s).join("、")
              }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
...
const fishTime = computed(() => {
  ...
  const skillReduction =
    skillStore.getSkill("fishing").level * SKILL_TIME_REDUCTION_PER_LEVEL;
  return (
    Math.max(
      MIN_ACTION_MINUTES,
...
const currentLocationName = computed(() => {
  return (
    FISHING_LOCATIONS.find((l) => l.id === fishingStore.fishingLocation)
      ?.name ?? "溪流"
  );
});
...
const hasCrabPotInBag = computed(
  () => inventoryStore.getItemCount("crab_pot") > 0,
);
...
const canPan = computed(
  () =>
    gameStore.isRainy && PAN_LOCATIONS.includes(fishingStore.fishingLocation),
);
