  ...
  <div>
    ...
    >
      ...
      <span class="text-xs text-muted whitespace-nowrap ml-2"
        >剩{{ inventoryStore.pendingUpgrade.daysRemaining }}天</span
      >
    </div>
    ...
    <div class="flex flex-col space-y-1.5">
      <div
        ...
        :class="
          isUpgrading(tool.type) ? 'border-accent/30' : 'border-accent/20'
        "
      ...
      >
        <div class="min-w-0">
          <span
            class="text-sm"
            :class="isUpgrading(tool.type) ? 'text-accent' : ''"
            >{{ TOOL_NAMES[tool.type] }}</span
          >
          <p class="text-xs text-muted">{{ TIER_NAMES[tool.tier] }}</p>
        </div>
        <span
          v-if="isUpgrading(tool.type)"
          class="text-xs text-accent whitespace-nowrap ml-2"
          >锻造中</span
        >
        <span
          v-else-if="getUpgradeCost(tool.type, tool.tier)"
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <p
            class="text-sm mb-2"
            :class="isUpgrading(selectedTool) ? 'text-accent' : 'text-text'"
          >
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs">{{
                TIER_NAMES[selectedToolObj!.tier]
              }}</span>
            </div>
            ...
            >
              ...
              <div class="flex-1 h-1 bg-bg rounded-xs border border-accent/10">
                <div
                  ...
                  :style="{
                    width:
                      ((2 - inventoryStore.pendingUpgrade!.daysRemaining) / 2) *
                        100 +
                      '%',
                  }"
                />
              ...
              <span class="text-xs text-muted whitespace-nowrap"
                >{{
                  2 - inventoryStore.pendingUpgrade!.daysRemaining
                }}/2天</span
              >
            </div>
          </div>
          ...
          <template v-if="!isUpgrading(selectedTool) && selectedUpgradeCost">
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              <p class="text-xs text-muted mb-1">
                升级至 {{ TIER_NAMES[selectedUpgradeCost.toTier] }}
              </p>
              <div class="flex items-center justify-between">
                ...
                <span
                  class="text-xs"
                  :class="
                    playerStore.money >= selectedUpgradeCost.money
                      ? ''
                      : 'text-danger'
                  "
                >
              ...
              <div
                v-for="mat in selectedUpgradeCost.materials"
                :key="mat.itemId"
                class="flex items-center justify-between mt-0.5"
              >
                <span class="text-xs text-muted">{{
                  getItemById(mat.itemId)?.name ?? mat.itemId
                }}</span>
                <span
                  class="text-xs"
                  :class="
              ...
              <template v-if="selectedFriendshipReq">
                <div class="flex items-center justify-between mt-0.5">
                  ...
                  <span
                    ...
                    :class="
                      meetsLevel(
                        npcStore.getFriendshipLevel('xiao_man'),
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs">
                  ...
                  <span class="text-success">{{
                    staminaText(selectedUpgradeCost.toTier)
                  }}</span>
                </span>
              </div>
              <template v-if="selectedTool === 'fishingRod'">
                <div class="flex items-center justify-between mt-0.5">
                  ...
                  <span class="text-xs">
                    {{ ROD_HOOK[selectedToolObj!.tier] }} →
                    <span class="text-success">{{
                      ROD_HOOK[selectedUpgradeCost.toTier]
                    }}</span>
                  </span>
                </div>
                <div class="flex items-center justify-between mt-0.5">
                  ...
                  <span class="text-xs">
                    ...
                    <span class="text-success"
                      >{{ ROD_TIME[selectedUpgradeCost.toTier] }}秒</span
                    >
                  </span>
                </div>
              </template>
            ...
            <p
              v-if="getUpgradeBlockReason(selectedTool)"
            ...
            <div
              ...
              class="mt-3 rounded border border-muted p-3 bg-bg/40"
            >
...
const meetsLevel = (
  ...
  required: FriendshipLevel,
): boolean => LEVEL_ORDER.indexOf(current) >= LEVEL_ORDER.indexOf(required);
...
const selectedUpgradeCost = computed(() => {
  ...
  return (
    getUpgradeCost(selectedToolObj.value.type, selectedToolObj.value.tier) ??
    null
  );
});
...
const canUpgrade = (type: ToolType): boolean => {
  ...
  if (
    requiredLevel &&
    !meetsLevel(npcStore.getFriendshipLevel("xiao_man"), requiredLevel)
  )
    return false;
...
const handleUpgradeAndClose = (type: ToolType) => {
  ...
  addLog(
    `你把${TOOL_NAMES[type]}和材料交给了小满，${cost.money}文。2天后可以取回升级后的工具。`,
  );
...
const formatMasteryMaterials = (type: ToolType): string => {
  ...
  return cost
    ? cost.materials
        .map((mat) => `${getItemName(mat.itemId)}×${mat.quantity}`)
        .join("、")
    : "";
};
