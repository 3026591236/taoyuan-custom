<template>
  <div>
    ...
    >
      <div class="flex items-center justify-between mb-1">
        ...
        <span
          ...
          >最深 第{{ miningStore.skullCavernBestFloor }}层</span
        >
        <span v-else class="text-xs text-muted/40">未探索</span>
      </div>
      <p class="text-xs text-muted">
        无限层 · 每10层安全点 · 铱矿来源 · 怪物随深度增强
      </p>
      <p
        v-if="miningStore.skullSafePointFloor > 0"
        class="text-xs text-muted mt-0.5"
      >
        安全点：第{{ miningStore.skullSafePointFloor }}层
      </p>
    </div>
      ...
      <div class="flex flex-col space-y-1">
        ...
        >
          ...
          <span class="text-xs text-muted"
            >{{ weaponTypeName }} · {{ critRateDisplay }}</span
          >
        </div>
        <div
          v-if="weaponEnchantName"
          class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5"
        >
        ...
        >
          ...
          <div class="flex items-center space-x-2">
            ...
            <span
              class="text-xs"
              :class="playerStore.getIsLowHp() ? 'text-danger' : 'text-muted'"
            >
    ...
    <div
      ...
      @click="
        hasElevator ? (showElevatorModal = true) : handleEnterMine(undefined)
      "
    >
    ...
    <!-- 已击败BOSS -->
    <div
      v-if="miningStore.defeatedBosses.length > 0"
        ...
        <div class="game-panel max-w-xs w-full">
          ...
          <p class="text-xs text-muted mb-2">
            安全点：{{
              miningStore.safePointFloor > 0
                ? `第${miningStore.safePointFloor}层`
                : "入口"
            }}
          </p>
          <div class="flex flex-col space-y-1.5">
            <div
              ...
              :class="
                zone.isCurrentZone ? 'border-accent/40' : 'border-accent/10'
              "
            >
              <div class="flex justify-between items-center text-xs mb-1">
                ...
                >
                  ...
                  <span class="text-muted ml-1"
                    >{{ zone.start }}-{{ zone.end }}层</span
                  >
                </span>
                <span
                  v-if="zone.bossDefeated"
                  class="text-success flex items-center"
                >
                  <Check :size="12" class="mr-0.5" />
                  {{ zone.bossName }}
                </span>
                <span v-else-if="zone.reached" class="text-danger/70">{{
                  zone.bossName
                }}</span>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <!-- 电梯楼层（按区域分组网格） -->
          <div
            v-if="elevatorZones.length > 0"
            class="max-h-48 overflow-y-auto mb-2"
          >
            <div
              v-for="zone in elevatorZones"
              :key="zone.name"
      ...
      >
        <div class="game-panel max-w-sm w-full">
          ...
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm text-accent">
              第{{ activeFloorNum }}层
              <span v-if="!miningStore.isInSkullCavern" class="text-muted">{{
                zoneName
              }}</span>
              <span
                v-if="currentFloorSpecial === 'mushroom'"
                class="text-success ml-1"
                >蘑菇洞穴</span
              >
              <span
                v-if="currentFloorSpecial === 'treasure'"
                class="text-accent ml-1"
                >宝箱层</span
              >
              <span
                v-if="currentFloorSpecial === 'infested'"
                class="text-danger ml-1"
                >感染层</span
              >
              <span
                v-if="currentFloorSpecial === 'dark'"
                class="text-muted ml-1"
                >暗河层</span
              >
              <span
                v-if="currentFloorSpecial === 'boss'"
                class="text-danger ml-1"
                >BOSS层</span
              >
            </p>
          ...
          >
            <p>
              ...
              {{ weaponDisplayName }}（{{ weaponTypeName }} · 攻击
              {{ weaponAttack }} · 暴击 {{ critRateDisplay }}）
            </p>
            <p v-if="weaponEnchantName" class="text-success">
              附魔：{{ weaponEnchantName }}
            </p>
          </div>

          <!-- 感染层提示 -->
          <p
            v-if="currentFloorSpecial === 'infested' && remainingMonsters > 0"
          ...
          <div class="flex flex-col space-y-1 mb-3">
            ...
            >
              ...
              <span class="text-xs text-muted"
                >&times;{{ inventoryStore.getItemCount("monster_lure") }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="text-xs text-muted"
                >{{ availableCombatItems.length }}种</span
              >
            </div>
            <div
            ...
            >
              ...
              <span v-if="!miningStore.stairsUsable" class="text-xs text-muted"
                >楼梯不可用</span
              >
            </div>
            <div
          ...
          <div class="text-xs text-muted space-y-0.5 max-h-24 overflow-y-auto">
            <p
              v-for="(msg, i) in recentLog"
              :key="i"
              :class="{ 'text-text': i === recentLog.length - 1 }"
            >
              {{ msg }}
            </p>
          </div>
      ...
      >
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex items-center justify-between mb-2">
            <p
              class="text-sm"
              :class="miningStore.combatIsBoss ? 'text-danger' : 'text-accent'"
            >
          ...
          <div class="grid grid-cols-[1fr_auto_1fr] gap-1.5 mb-3 items-center">
            ...
            >
              <p class="text-xs text-center text-danger mb-1.5 truncate">
                ...
                <span v-if="miningStore.combatIsBoss" class="text-[10px]"
                  >[BOSS]</span
                >
              </p>
              <div class="bg-bg rounded-xs h-1.5 mb-1">
              ...
              <p class="text-[10px] text-muted">
                {{ miningStore.combatMonsterHp }}/{{
                  miningStore.combatMonster?.hp
                }}
              </p>
              <span
                v-if="monsterFloat"
          ...
          <div class="mb-3 space-y-1">
            ...
            <div class="grid grid-cols-3 gap-1">
              ...
              >
                <span
                  class="text-xs"
                  :class="
                    miningStore.combatIsBoss ? 'text-muted' : 'text-danger'
                  "
                >
            ...
            >
              ...
              <span
                v-if="inventoryStore.activePresetId"
                class="text-[10px] text-muted"
              >
                {{
                  inventoryStore.equipmentPresets.find(
                    (p) => p.id === inventoryStore.activePresetId,
                  )?.name ?? ""
                }}
              </span>
            </div>
          ...
          <div class="text-xs space-y-0.5 max-h-28 overflow-y-auto">
            <p
              ...
              :key="i"
              :class="
                i < miningStore.combatLog.length - 1
                  ? 'text-muted'
                  : 'text-text'
              "
            >
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <!-- 批量数量选择（仅永久增益类道具） -->
          <div
            v-if="pendingCanBatch && pendingItem.count > 1"
          ...
          <div class="flex space-x-1.5">
            <Button class="flex-1 justify-center" @click="pendingItemId = null"
              >取消</Button
            >
            <Button
              class="flex-1 justify-center !bg-accent !text-bg"
              @click="handleConfirmUseItem"
            >
              确认使用{{
                pendingCanBatch && pendingUseQty > 1 ? ` ×${pendingUseQty}` : ""
              }}
            </Button>
          </div>
        </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="showPresetListModal = false"
          >
            <X :size="14" />
          </button>
          <p class="text-sm text-accent mb-2">
          ...
          >
            <div
              ...
              class="border rounded-xs p-2"
              :class="
                inventoryStore.activePresetId === preset.id
                  ? 'border-accent/40'
                  : 'border-accent/10'
              "
            >
              <div class="flex items-center justify-between mb-1">
                ...
                <span
                  v-if="inventoryStore.activePresetId === preset.id"
              ...
              <div class="grid grid-cols-2 gap-1">
                ...
                <Button
                  class="py-0 px-1.5 text-[10px]"
                  @click="viewPresetDetail(preset.id)"
                  >查看</Button
                >
              </div>
            </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="showPresetDetailModal = false"
          >
            <X :size="14" />
          </button>
          ...
          <div class="flex flex-col space-y-1">
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  detailPreset.weaponDefId ? 'text-accent' : 'text-muted/40'
                "
              >
                {{
                  detailPreset.weaponDefId
                    ? (getWeaponById(detailPreset.weaponDefId)?.name ?? "未知")
                    : "无"
                }}
              </span>
            </div>
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  detailPreset.ringSlot1DefId ? 'text-accent' : 'text-muted/40'
                "
              >
                {{
                  detailPreset.ringSlot1DefId
                    ? (getRingById(detailPreset.ringSlot1DefId)?.name ?? "未知")
                    : "无"
                }}
              </span>
            </div>
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  detailPreset.ringSlot2DefId ? 'text-accent' : 'text-muted/40'
                "
              >
                {{
                  detailPreset.ringSlot2DefId
                    ? (getRingById(detailPreset.ringSlot2DefId)?.name ?? "未知")
                    : "无"
                }}
              </span>
            </div>
            ...
            >
              ...
              <span
                class="text-xs"
                :class="detailPreset.hatDefId ? 'text-accent' : 'text-muted/40'"
              >
                {{
                  detailPreset.hatDefId
                    ? (getHatById(detailPreset.hatDefId)?.name ?? "未知")
                    : "无"
                }}
              </span>
            </div>
            ...
            >
              ...
              <span
                class="text-xs"
                :class="
                  detailPreset.shoeDefId ? 'text-accent' : 'text-muted/40'
                "
              >
                {{
                  detailPreset.shoeDefId
                    ? (getShoeById(detailPreset.shoeDefId)?.name ?? "未知")
                    : "无"
                }}
              </span>
            </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          <button
            class="absolute top-2 right-2 text-muted hover:text-text"
            @click="showEquipPropertyModal = false"
          >
            <X :size="14" />
          </button>
          ...
          <div
            v-if="equipPropertyInfo.effects.length > 0"
...
/** 道具使用确认 */
const BATCH_USABLE_ITEMS = new Set([
  "guild_badge",
  "life_talisman",
  "lucky_coin",
  "defense_charm",
...
const pendingItem = computed(() => {
  ...
  return (
    availableCombatItems.value.find((i) => i.itemId === pendingItemId.value) ??
    null
  );
});
const pendingCanBatch = computed(
  () =>
    pendingItemId.value !== null && BATCH_USABLE_ITEMS.has(pendingItemId.value),
);
...
const parseDamage = (
  msg: string,
): { dealt: number; taken: number; isCrit: boolean } => {
...
const activeFloorNum = computed(() => {
  return miningStore.isInSkullCavern
    ? miningStore.skullCavernFloor
    : miningStore.currentFloor;
});
...
const availableCombatItems = computed(() => {
  ...
  for (const invItem of inventoryStore.items) {
    ...
    if (def.healthRestore)
      parts.push(
        def.healthRestore >= 999 ? "HP全满" : `HP+${def.healthRestore}`,
      );
...
/** 是否有怪物诱饵 */
const hasMonsterLure = computed(
  () => inventoryStore.getItemCount("monster_lure") > 0,
);
...
const mineZones = computed(() => {
  ...
  return zones.map((z) => {
    ...
    const bossDefeated = boss
      ? miningStore.defeatedBosses.includes(boss.id)
      : false;
    const progress = Math.min(
      100,
      Math.max(0, ((sp - (z.start - 1)) / 20) * 100),
    );
...
const elevatorZones = computed(() => {
  const allSafePoints = miningStore
    .getUnlockedSafePoints()
    .filter((sp) => sp < miningStore.safePointFloor);
  const zones = [
...
const skullElevatorFloors = computed(() => {
  return miningStore
    ...
    .filter((sp) => sp < miningStore.skullSafePointFloor);
});
...
const getTileClass = (tile: MineTile): string => {
  ...
  switch (tile.type) {
    ...
    case "boss":
      return tile.state === "defeated"
        ? "bg-bg border-accent/10"
        : "bg-danger/30 border-danger/50 cursor-pointer";
    case "stairs":
...
const handleCombat = (action: CombatAction) => {
  ...
  if (action === "attack" && dealt > 0) {
    triggerAnim(
      "monster",
      isCrit ? "anim-shake-heavy" : "anim-shake",
      isCrit ? 400 : 300,
    );
  ...
  if (taken > 0) {
    triggerAnim(
      "player",
      isCrit ? "anim-shake-heavy anim-flash-red" : "anim-flash-red",
      400,
    );
