  ...
  <div class="immortal-page" :class="`art-${ascensionStore.lastArtId}`">
    ...
    <div v-if="isTab('home')" class="immortal-hero relative z-10 mb-4">
      ...
      <div class="flex-1 min-w-0 immortal-profile">
        <div class="profile-crown"><span>云阙仙录 · 正册</span><i></i><span>飞升者档案</span></div>
        <p class="text-xl text-accent">{{ ascensionStore.immortalTitle || '初入仙门' }} <em>· {{ ascensionStore.immortalRank }}</em></p>
        ...
        <div class="character-combat-deck mt-3">
          <div class="combat-power"><span>仙界战力</span><b>{{ ascensionStore.immortalPower }}</b><small>IMMORTAL POWER</small></div>
          <div class="combat-vitals"><div class="vital-row"><span><i class="vital-mark hp-mark"></i>仙躯气血</span><b>{{ ascensionStore.immortalHp }} <small>/ {{ ascensionStore.immortalMaxHp }}</small></b></div><div class="vital-bar hp"><i :style="{ width: `${ascensionStore.immortalHpRate}%` }"></i></div><div class="vital-row"><span><i class="vital-mark qi-mark"></i>本源仙力</span><b>{{ ascensionStore.immortalQi }} <small>/ {{ ascensionStore.immortalMaxQi }}</small></b></div><div class="vital-bar qi"><i :style="{ width: `${ascensionStore.immortalQiRate}%` }"></i></div></div>
          <div class="combat-stats"><div><span>仙攻</span><b>{{ ascensionStore.immortalAttack }}</b></div><div><span>仙防</span><b>{{ ascensionStore.immortalDefense }}</b></div><div><span>暴击</span><b>{{ ascensionStore.immortalCritRate }}%</b></div><div><span>减伤</span><b>{{ ascensionStore.immortalDamageReduction }}%</b></div></div>
        </div>
        <div class="profile-insight-grid mt-2">
          <div><span>道统</span><b>{{ ascensionStore.lineageInfo.name }}</b></div><div><span>本命仙术</span><b>{{ currentArt.name }}</b></div><div><span>仙器共鸣</span><b>+{{ ascensionStore.gearPower }}</b></div><div><span>洞天稳定</span><b :class="ascensionStore.caveHeavenStability < 80 ? 'text-danger' : 'text-success'">{{ ascensionStore.caveHeavenStability }}%</b></div><div><span>斗法战绩</span><b>{{ ascensionStore.pkRecord }}</b></div><div><span>裂隙镇压</span><b>{{ ascensionStore.riftScore }} 次</b></div>
        </div>
      </div>
    </div>
    ...
    <div v-if="isTab('home')" class="border border-amber-200/20 rounded-xs p-3 mb-4 bg-black/10 relative z-10">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-[10px] text-muted">仙体、仙骨、仙魂共同决定仙界根基</span>
      </div>
    ...
    <div v-if="isTab('home')" class="immortal-record relative z-10 mb-4">
      <div><p class="text-xs text-accent">云阙战报</p><p class="text-[10px] text-muted">最近一次仙界行动</p></div><p class="text-[11px] text-slate-200 leading-relaxed">{{ ascensionStore.lastBattleText || '仙门初开，万象待定。前往试炼、裂隙或洞天，书写你的仙界战绩。' }}</p>
    </div>

    <div v-if="isTab('edict')" class="celestial-edict-board relative z-10 mb-4">
      <div class="edict-head"><div><p class="text-xs text-accent">云阙天诏</p><p class="text-[10px] text-muted">每日与每周目标，完成后领取仙界成长资源</p></div><span>天庭玉册</span></div>
      <div class="grid grid-cols-2 gap-2">
        <div v-for="edict in ascensionStore.celestialEdicts" :key="edict.id" class="edict-card" :class="edict.state.claimed ? 'claimed' : ''">
          <div class="flex justify-between gap-1"><p class="text-[11px] text-accent">{{ edict.name }}</p><span class="text-[9px] text-muted">{{ edict.period === 'daily' ? '每日' : '每周' }}</span></div>
          <p class="text-[10px] text-muted leading-relaxed">{{ edict.desc }}</p>
          <div class="edict-progress"><i :style="{ width: `${Math.min(100, edict.state.value / edict.target * 100)}%` }"></i></div>
          <div class="flex justify-between items-center gap-1"><span class="text-[10px] text-warning">{{ edict.state.value }}/{{ edict.target }} · 器魄+{{ edict.reward.essence }}</span><button class="btn-mini" :disabled="edict.state.claimed || edict.state.value < edict.target" @click="ascensionStore.claimCelestialEdict(edict.id)">{{ edict.state.claimed ? '已领' : edict.state.value >= edict.target ? '领诏赏' : '进行中' }}</button></div>
        </div>
      </div>
    </div>
    ...
    <div v-if="isTab('arts')" class="mb-4 relative z-10">
      ...
      <div class="grid grid-cols-2 gap-2">
        <button v-for="art in IMMORTAL_ARTS" :key="art.id" class="art-card" :class="ascensionStore.lastArtId === art.id ? 'active' : ''" @click="ascensionStore.castImmortalArt(art.id)">
          <span class="text-xl">{{ art.icon }}</span><span class="text-xs text-accent">{{ art.name }}</span><span class="text-[10px] text-muted">{{ art.element }} · {{ art.effect }}</span><span class="text-[10px] text-warning">{{ art.id === 'purple_thunder_seal' ? '破盾强效' : art.id === 'solar_flame' ? '越战越强' : art.id === 'starfall_sword' ? '连胜爆发' : '仙体护持' }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="isTab('rift')" class="relative z-10 mb-4">
      ...
      <div class="grid grid-cols-2 gap-2">
        <button v-for="rift in CHAOS_RIFTS" :key="rift.id" class="rift-card visual-rift-card rift-boss-card" :class="ascensionStore.riftBossInfo(rift.id).turns >= 4 ? 'boss-fury' : ''" @click="ascensionStore.challengeChaosRift(rift.id)">
          ...
          <span class="text-sm text-accent relative z-[1]">{{ rift.name }} · {{ rift.bossName }} <b v-if="ascensionStore.riftBossInfo(rift.id).turns >= 4" class="fury-tag">狂暴</b></span>
          <span class="text-[10px] text-muted leading-relaxed">{{ rift.desc }}</span>
          <div class="rift-phase-row"><b>{{ ascensionStore.riftBossInfo(rift.id).dungeonRank }}</b><span>阶段：{{ ascensionStore.riftBossInfo(rift.id).phaseName }}</span><span>弱点：{{ IMMORTAL_ARTS.find(a => a.id === rift.weakness)?.name }}</span></div>
          ...
          <span class="text-[10px]" :class="ascensionStore.immortalPower >= rift.needPower ? 'text-success' : 'text-danger'">建议仙战力 {{ rift.needPower }}｜当前 {{ ascensionStore.immortalPower }}｜已镇压 {{ ascensionStore.riftClears[rift.id] || 0 }} 层</span>
          <span class="text-[10px] text-warning">三阶段首领会重组护盾/狂暴反击，击杀掉落专属材料与仙器精魄</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="isTab('cave')" class="relative z-10 mb-4">
      ...
      <div class="cave-resonance"><span>阵眼共鸣 · {{ ascensionStore.caveResonance.name }}</span><b>全府均衡 Lv.{{ ascensionStore.caveResonanceLevel }} · 灵机与派遣 ×{{ ascensionStore.caveResonance.yieldRate.toFixed(2) }} · 每日失稳 -{{ ascensionStore.caveResonance.decayReduce }}</b></div>
      <div class="cave-yield-panel"><div><p>洞天灵机</p><span>功德 +{{ ascensionStore.caveHeavenYield.merit }}　仙玉 +{{ ascensionStore.caveHeavenYield.jade }}　法则 +{{ ascensionStore.caveHeavenYield.rule }}　器魄 +{{ ascensionStore.caveHeavenYield.essence }}<template v-if="ascensionStore.caveHeavenYield.dew">　仙露 +{{ ascensionStore.caveHeavenYield.dew }}</template></span></div><button class="btn-mini" :disabled="ascensionStore.caveHeavenHarvestedToday" @click="ascensionStore.claimCaveHeavenYield()">{{ ascensionStore.caveHeavenHarvestedToday ? '今日已收取' : '收取灵机' }}</button></div>
      ...
      <div v-if="ascensionStore.caveHeavenFx" class="cave-cinematic" :class="`cave-fx-${ascensionStore.caveHeavenFx.type}`"><div class="cave-fx-cloud cf1"></div><div class="cave-fx-cloud cf2"></div><div class="cave-fx-ring cr1"></div><div class="cave-fx-ring cr2"></div><i v-for="n in 14" :key="n" class="cave-fx-spark" :style="{ '--i': n }"></i><div class="cave-fx-content"><em>{{ ascensionStore.caveHeavenFx.icon }}</em><h4>{{ ascensionStore.caveHeavenFx.title }}</h4><p>{{ ascensionStore.caveHeavenFx.text }}</p></div></div>
    </div>

    <div v-if="isTab('realm')" class="realm-sanctum relative z-10 mb-4" :class="`realm-stage-${ascensionStore.immortalRealmStage}`">
      <div class="realm-sky-art" aria-hidden="true"><span class="realm-moon"></span><span class="realm-star rs1">✦</span><span class="realm-star rs2">✧</span><span class="realm-star rs3">✦</span><span class="realm-cloud rc1"></span><span class="realm-cloud rc2"></span></div>
      <div class="realm-sanctum-content">
        <p class="realm-kicker">云阙仙阶 · 破境台</p><div class="realm-icon">{{ ascensionStore.immortalRealmInfo.icon }}</div>
        <h4>{{ ascensionStore.immortalRealmInfo.name }}</h4><p class="realm-desc">{{ ascensionStore.immortalRealmInfo.desc }}</p>
        <div v-if="ascensionStore.nextImmortalRealm" class="realm-next"><span>下一仙阶 · {{ ascensionStore.nextImmortalRealm.name }}</span><b>功德 {{ ascensionStore.nextImmortalRealm.meritCost }}　仙玉 {{ ascensionStore.nextImmortalRealm.jadeCost }}　法则 {{ ascensionStore.nextImmortalRealm.ruleCost }}</b></div>
        <p v-else class="realm-peak">仙阶已至当前顶峰，后续可通过洞天、仙器与赛季继续成长。</p>
        <button class="realm-break-btn" :disabled="!ascensionStore.nextImmortalRealm" @click="triggerRealmBreakthrough"><span>凝神破境</span><small>仙阶战力底蕴 +{{ ascensionStore.nextImmortalRealm?.powerBonus || 0 }}</small></button>
      </div>
      <div v-if="ascensionStore.immortalBreakthroughFx" class="breakthrough-cinematic" :class="[`fx-stage-${ascensionStore.immortalBreakthroughFx.stage}`, `fx-phase-${ascensionStore.immortalBreakthroughPhase}`]">
        <div class="fx-vignette"></div><div class="fx-aurora"></div><div class="fx-heaven-mark"></div><div class="fx-rune r1">☷</div><div class="fx-rune r2">☵</div><div class="fx-ring r1"></div><div class="fx-ring r2"></div>
        <div class="fx-lightning l1"></div><div class="fx-lightning l2"></div><div class="fx-lightning l3"></div><div class="fx-sword-light"></div><div class="fx-sword-light s2"></div>
        <i v-for="n in 18" :key="n" class="fx-particle" :style="{ '--i': n }"></i><div class="fx-content"><em>{{ ascensionStore.immortalBreakthroughPhase === 'charge' ? '法阵凝聚' : ascensionStore.immortalBreakthroughPhase === 'tribulation' ? '天劫临身' : ascensionStore.immortalBreakthroughPhase === 'ascend' ? '仙光破界' : '大道显名' }}</em><p>{{ ascensionStore.immortalBreakthroughFx.icon }}</p><h4>破境 · {{ ascensionStore.immortalBreakthroughFx.name }}</h4><span>仙光入体　法则重铸</span></div>
      </div>
    </div>
    ...
    <div v-if="isTab('cave')" class="relative z-10 mb-4 expedition-panel">
      ...
      <div class="grid grid-cols-3 gap-2">
        <button v-for="mission in IMMORTAL_EXPEDITIONS" :key="mission.id" class="expedition-card" :class="ascensionStore.expeditionClaimed[mission.id] === new Date().toISOString().slice(0, 10) ? 'done' : ''" @click="ascensionStore.dispatchImmortalExpedition(mission.id)">
          <p class="text-xs text-accent">{{ mission.name }}</p><p class="text-[10px] text-muted leading-relaxed">{{ mission.desc }}</p><p class="text-[10px] text-warning">需 {{ IMMORTAL_CAVE_NODES.find(n => n.id === mission.needNode)?.name }} Lv.1</p><p class="text-[10px] text-success">功德+{{ ascensionStore.caveExpeditionReward(mission.id).merit }} · 仙玉+{{ ascensionStore.caveExpeditionReward(mission.id).jade }} · 器魄+{{ ascensionStore.caveExpeditionReward(mission.id).essence }}</p><p class="text-[10px]" :class="ascensionStore.expeditionClaimed[mission.id] === new Date().toISOString().slice(0, 10) ? 'text-success' : 'text-muted'">{{ ascensionStore.expeditionClaimed[mission.id] === new Date().toISOString().slice(0, 10) ? '今日已归' : '派遣仙舟' }}</p>
        </button>
      </div>
    </div>
...
const triggerRealmBreakthrough = () => { ascensionStore.breakthroughImmortalRealm() }
...
.rift-boss-card{min-height:214px}.rift-phase-row{position:relative;z-index:1;display:flex;flex-wrap:wrap;gap:4px;width:100%;font-size:9px}.rift-phase-row b,.rift-phase-row span{border:1px solid rgba(255,226,138,.22);border-radius:999px;padding:1px 5px;background:rgba(0,0,0,.34);color:rgba(230,246,255,.78)}.rift-phase-row b{color:#ffe28a}.boss-bars{position:relative;z-index:1;width:100%;margin:2px 0 3px}.boss-bars>span{display:block;height:5px;overflow:hidden;border:1px solid rgba(255,180,180,.35);border-radius:999px;background:rgba(0,0,0,.5)}.boss-bars i{display:block;height:100%;border-radius:inherit;transition:width .45s ease}.hp-bar{background:linear-gradient(90deg,#ff476a,#ffbe58);box-shadow:0 0 9px #ff3f69}.boss-bars small{display:block;margin:2px 0;color:rgba(255,255,255,.68);font-size:9px}.boss-bars .shield-bar{border-color:rgba(102,222,255,.42)}.shield-bar i{background:linear-gradient(90deg,#66dfff,#a78bfa);box-shadow:0 0 9px #65dfff}.fury-tag{margin-left:4px;padding:1px 4px;border:1px solid rgba(255,85,85,.7);border-radius:4px;color:#ff9b9b;font-size:9px;animation:furyPulse .7s infinite alternate}.boss-fury{border-color:rgba(255,72,92,.7)!important;box-shadow:0 0 22px rgba(255,55,80,.27)!important}.expedition-panel{padding:10px;border:1px solid rgba(102,222,255,.2);border-radius:7px;background:linear-gradient(135deg,rgba(65,170,255,.07),rgba(255,226,138,.04))}.expedition-card{display:flex;min-height:134px;flex-direction:column;align-items:flex-start;text-align:left;padding:8px;border:1px solid rgba(162,220,255,.18);border-radius:5px;background:rgba(3,10,28,.46);transition:.18s}.expedition-card:hover{transform:translateY(-2px);border-color:rgba(255,226,138,.62);box-shadow:0 0 16px rgba(126,225,255,.12)}.expedition-card.done{opacity:.62;border-color:rgba(105,232,162,.42)}@keyframes furyPulse{from{box-shadow:0 0 4px rgba(255,55,80,.35)}to{box-shadow:0 0 14px rgba(255,55,80,.9)}}
.celestial-edict-board{padding:10px;border:1px solid rgba(255,226,138,.32);border-radius:8px;background:linear-gradient(135deg,rgba(53,29,78,.68),rgba(8,27,56,.62));box-shadow:inset 0 0 34px rgba(255,226,138,.07),0 0 18px rgba(130,207,255,.08)}.edict-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px}.edict-head>span{padding:3px 7px;border:1px solid rgba(255,226,138,.38);border-radius:999px;color:#ffe28a;font-size:10px;letter-spacing:.12em}.edict-card{display:flex;min-height:104px;flex-direction:column;gap:4px;padding:7px;border:1px solid rgba(158,216,255,.19);border-radius:5px;background:rgba(0,0,0,.22)}.edict-card.claimed{opacity:.64;border-color:rgba(105,232,162,.46)}.edict-progress{height:4px;overflow:hidden;border-radius:99px;background:rgba(0,0,0,.45)}.edict-progress i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#6ce0ff,#ffe28a);box-shadow:0 0 8px rgba(108,224,255,.7);transition:width .35s}
.immortal-hero{min-height:275px;padding:18px;background:linear-gradient(120deg,rgba(255,239,179,.1),rgba(79,153,255,.08) 45%,rgba(31,12,65,.36))}.hero-avatar.immortal-portrait-stage{width:220px;height:254px;flex:none}.immortal-profile{align-self:stretch;display:flex;flex-direction:column;justify-content:center}.profile-crown{display:flex;align-items:center;gap:7px;margin-bottom:6px;color:rgba(255,228,143,.7);font-size:9px;letter-spacing:.13em}.profile-crown i{flex:1;height:1px;background:linear-gradient(90deg,rgba(255,226,138,.6),transparent)}.immortal-profile em{font-style:normal;font-size:12px;color:rgba(163,225,255,.78)}.stat-power{background:linear-gradient(145deg,rgba(255,220,123,.2),rgba(108,213,255,.12));border-color:rgba(255,226,138,.48)!important}.profile-insight-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}.profile-insight-grid>div{min-height:43px;padding:5px 6px;border:1px solid rgba(150,219,255,.16);border-radius:3px;background:rgba(2,8,27,.35)}.profile-insight-grid span{display:block;color:rgba(255,255,255,.47);font-size:9px}.profile-insight-grid b{display:block;overflow:hidden;color:#c9ecff;font-size:10px;white-space:nowrap;text-overflow:ellipsis}.immortal-record{display:grid;grid-template-columns:128px 1fr;gap:11px;padding:11px;border:1px solid rgba(144,221,255,.25);border-radius:5px;background:linear-gradient(90deg,rgba(73,164,255,.1),rgba(255,226,138,.05));box-shadow:inset 0 0 22px rgba(111,220,255,.04)}
.combat-core-grid{display:grid;grid-template-columns:110px minmax(170px,1fr) 150px;gap:7px}.combat-power{display:flex;min-height:88px;flex-direction:column;justify-content:center;align-items:center;border:1px solid rgba(255,225,129,.55);border-radius:5px;background:radial-gradient(circle,rgba(255,226,138,.2),rgba(18,36,77,.38));box-shadow:inset 0 0 18px rgba(255,226,138,.1)}.combat-power span,.combat-power small{color:rgba(255,255,255,.58);font-size:9px}.combat-power b{color:#ffe28a;font-size:22px;line-height:1.2;text-shadow:0 0 13px rgba(255,214,105,.5)}.combat-vitals{padding:7px 8px;border:1px solid rgba(135,219,255,.3);border-radius:5px;background:rgba(3,12,37,.38)}.vital-row{display:flex;justify-content:space-between;margin:1px 0;color:rgba(255,255,255,.62);font-size:10px}.vital-row b{color:#effcff;font-weight:600}.vital-bar{height:8px;overflow:hidden;margin-bottom:7px;border:1px solid rgba(255,255,255,.18);border-radius:99px;background:rgba(0,0,0,.48)}.vital-bar i{display:block;height:100%;border-radius:inherit;transition:width .35s}.vital-bar.hp i{background:linear-gradient(90deg,#e93655,#ffad62);box-shadow:0 0 9px #ff4967}.vital-bar.qi i{background:linear-gradient(90deg,#47cfff,#a58bff);box-shadow:0 0 9px #63daff}.combat-stats{display:grid;grid-template-columns:1fr 1fr;gap:4px}.combat-stats>div{display:flex;flex-direction:column;justify-content:center;align-items:center;border:1px solid rgba(151,216,255,.18);border-radius:4px;background:rgba(4,14,38,.3)}.combat-stats span{color:rgba(255,255,255,.48);font-size:9px}.combat-stats b{color:#c7edff;font-size:12px}.resource-row .stat{padding:4px}.resource-row .stat b{font-size:11px}
/* V2.8.2 大厅重排：上方仅仙身展示，全部角色数据统一落在立绘下方 */
.immortal-hero{position:relative;display:flex!important;flex-direction:column;align-items:center;gap:0;min-height:0;padding:24px 20px 18px;overflow:hidden;border:1px solid rgba(255,226,138,.32);border-radius:12px;background:radial-gradient(ellipse at 50% 21%,rgba(108,212,255,.22),transparent 31%),radial-gradient(ellipse at 50% 6%,rgba(255,221,133,.19),transparent 35%),linear-gradient(148deg,rgba(8,16,46,.97),rgba(20,14,59,.92) 58%,rgba(5,13,38,.96));box-shadow:0 16px 40px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.1)}
.immortal-hero::before{content:'';position:absolute;left:50%;top:42px;width:450px;height:450px;border:1px solid rgba(126,229,255,.16);border-radius:50%;transform:translateX(-50%);box-shadow:0 0 55px rgba(98,211,255,.15)}
.hero-avatar.immortal-portrait-stage{z-index:1;width:272px;height:350px;align-self:center;border-radius:132px 132px 18px 18px;border-color:rgba(255,230,151,.62);box-shadow:0 0 52px rgba(255,221,129,.2),0 16px 38px rgba(0,0,0,.4),inset 0 0 34px rgba(132,223,255,.16)}
.immortal-artwork{border-radius:128px 128px 15px 15px;object-position:center 39%}.immortal-profile{position:relative;z-index:1;width:min(760px,100%);padding-top:17px;text-align:center}.profile-crown{margin:0 auto 6px;justify-content:center}.immortal-profile>p.text-xl{font-size:26px;line-height:1.25;letter-spacing:.05em;text-shadow:0 0 18px rgba(255,222,130,.32)}
.character-combat-deck{display:grid;grid-template-columns:140px minmax(200px,1fr) 180px;gap:9px;min-height:105px;margin-top:15px;padding:8px;border-top:1px solid rgba(255,229,154,.22);border-bottom:1px solid rgba(137,218,255,.17);text-align:left;background:linear-gradient(90deg,rgba(4,10,31,.28),rgba(120,206,255,.07),rgba(4,10,31,.18))}.combat-power{min-height:86px;border:0;border-right:1px solid rgba(255,226,138,.27);border-radius:0;background:transparent;box-shadow:none}.combat-power b{font-size:28px;letter-spacing:.03em}.combat-power small{color:rgba(255,226,138,.44);letter-spacing:.12em;font-size:7px}.combat-vitals{padding:6px 10px;border:0;border-radius:0;background:transparent}.vital-row{margin:1px 0 3px;font-size:10px}.vital-row span{display:flex;align-items:center;gap:5px;color:rgba(220,242,255,.72)}.vital-row b{font-size:12px}.vital-row b small{font-size:9px;opacity:.58}.vital-mark{width:6px;height:6px;border-radius:50%;display:inline-block}.hp-mark{background:#ff6576;box-shadow:0 0 8px #ff4f67}.qi-mark{background:#67dfff;box-shadow:0 0 8px #55d4ff}.vital-bar{height:7px;margin-bottom:9px;border:0;background:rgba(0,0,0,.46)}.combat-stats{gap:0;border-left:1px solid rgba(139,217,255,.17)}.combat-stats>div{border:0;border-radius:0;background:transparent}.combat-stats>div:nth-child(odd){border-right:1px solid rgba(139,217,255,.13)}.combat-stats>div:nth-child(-n+2){border-bottom:1px solid rgba(139,217,255,.13)}.combat-stats span{font-size:9px}.combat-stats b{font-size:13px;color:#d8f3ff}.profile-insight-grid{grid-template-columns:repeat(6,minmax(0,1fr));gap:0;margin-top:10px!important;border:1px solid rgba(154,222,255,.16);border-radius:5px;overflow:hidden;text-align:left}.profile-insight-grid>div{min-height:47px;padding:7px 8px;border:0!important;border-right:1px solid rgba(154,222,255,.12);border-radius:0;background:rgba(3,10,29,.24)}.profile-insight-grid>div:last-child{border-right:0!important}.profile-insight-grid b{margin-top:2px;font-size:10px}.immortal-record{grid-template-columns:150px 1fr;padding:13px 15px;border-left:3px solid rgba(255,226,138,.65);border-radius:0 7px 7px 0}
...
.immortal-dossier{overflow:hidden;border-top:1px solid rgba(255,226,138,.24);border-bottom:1px solid rgba(148,218,255,.18);border-radius:0;background:linear-gradient(90deg,transparent,rgba(102,197,255,.07),transparent);box-shadow:none}.dossier-resource-row{display:grid;grid-template-columns:repeat(4,1fr);padding:10px 6px 9px}.dossier-resource-row span{display:flex;align-items:baseline;justify-content:center;gap:7px;border-right:1px solid rgba(148,218,255,.15)}.dossier-resource-row span:last-child{border-right:0}.dossier-resource-row i{font-style:normal;color:rgba(216,239,255,.54);font-size:10px}.dossier-resource-row b{color:#e6f8ff;font-size:16px;font-weight:600}.dossier-summary-row{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid rgba(148,218,255,.12);background:rgba(2,8,27,.2)}.dossier-summary-row>div{display:flex;align-items:center;gap:12px;padding:9px 14px}.dossier-summary-row>div+div{border-left:1px solid rgba(148,218,255,.12)}.dossier-summary-row span{flex:none;color:rgba(255,226,151,.66);font-size:10px}.dossier-summary-row b{color:rgba(226,244,255,.8);font-size:11px;font-weight:400;white-space:nowrap}.dossier-summary-row em{font-style:normal;color:#ffe5a1;font-size:12px;font-weight:600}@media (max-width:520px){.dossier-resource-row{padding:8px 2px}.dossier-resource-row span{gap:3px;flex-direction:column;align-items:center}.dossier-resource-row b{font-size:14px}.dossier-summary-row{grid-template-columns:1fr}.dossier-summary-row>div+div{border-left:0;border-top:1px solid rgba(148,218,255,.12)}.dossier-summary-row>div{justify-content:space-between;padding:8px 10px}.dossier-summary-row b{font-size:10px}.dossier-summary-row em{font-size:11px}}.realm-sanctum{position:relative;overflow:hidden;min-height:390px;border:1px solid rgba(255,225,135,.37);border-radius:12px;background:linear-gradient(155deg,#080f36,#17135a 62%,#060d30);box-shadow:0 16px 42px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.1)}.realm-sky-art{position:absolute;inset:0;background:radial-gradient(circle at 50% 20%,rgba(166,220,255,.32),transparent 20%),radial-gradient(circle at 50% 74%,rgba(255,208,100,.18),transparent 38%)}.realm-moon{position:absolute;left:50%;top:24px;width:136px;height:136px;border-radius:50%;transform:translateX(-50%);background:radial-gradient(circle at 35% 35%,#fff, #bfeaff 31%,rgba(115,180,255,.2) 61%,transparent 69%);box-shadow:0 0 50px rgba(137,213,255,.65)}.realm-star{position:absolute;color:#fff4c3;text-shadow:0 0 12px #7cddff;animation:realmTwinkle 2s ease-in-out infinite}.rs1{top:34px;left:24%}.rs2{top:90px;right:22%;animation-delay:.7s}.rs3{top:175px;left:16%;animation-delay:1.2s}.realm-cloud{position:absolute;bottom:46px;width:230px;height:68px;border-radius:50%;background:radial-gradient(ellipse,rgba(155,218,255,.24),rgba(93,134,255,.04) 58%,transparent 70%);filter:blur(4px);animation:realmDrift 7s ease-in-out infinite}.rc1{left:-30px}.rc2{right:-45px;bottom:80px;animation-delay:-3s}.realm-sanctum-content{position:relative;z-index:1;display:flex;min-height:390px;flex-direction:column;align-items:center;justify-content:center;padding:39px 22px 25px;text-align:center}.realm-kicker{color:rgba(255,229,150,.72);font-size:10px;letter-spacing:.3em}.realm-icon{display:grid;width:66px;height:66px;place-items:center;margin:8px 0;border:1px solid rgba(255,234,165,.58);border-radius:50%;background:rgba(11,19,72,.45);font-size:31px;box-shadow:0 0 22px rgba(255,220,120,.28),inset 0 0 20px rgba(143,217,255,.22)}.realm-sanctum h4{color:#fff0bf;font-size:27px;letter-spacing:.14em;text-shadow:0 0 16px rgba(255,217,104,.5)}.realm-desc{max-width:440px;margin-top:6px;color:rgba(220,240,255,.73);font-size:11px}.realm-next{display:flex;flex-direction:column;gap:4px;margin-top:18px;color:rgba(221,242,255,.64);font-size:10px}.realm-next b{color:#ffe6a1;font-size:11px;font-weight:500}.realm-peak{margin-top:17px;color:#8ff3c0;font-size:11px}.realm-break-btn{display:flex;flex-direction:column;align-items:center;min-width:176px;margin-top:18px;padding:9px 25px;border:1px solid rgba(255,232,151,.7);border-radius:999px;background:linear-gradient(110deg,rgba(61,93,200,.72),rgba(161,103,228,.74),rgba(255,186,80,.64));color:#fff7da;box-shadow:0 0 24px rgba(255,211,104,.3),inset 0 1px 0 rgba(255,255,255,.42);transition:.2s}.realm-break-btn:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 0 34px rgba(255,219,110,.54)}.realm-break-btn:disabled{opacity:.45}.realm-break-btn span{font-size:14px;letter-spacing:.13em}.realm-break-btn small{margin-top:2px;color:rgba(255,255,255,.7);font-size:9px}.breakthrough-cinematic{position:absolute;z-index:5;inset:0;display:grid;overflow:hidden;place-items:center;background:radial-gradient(circle,rgba(255,247,205,.72),rgba(81,146,255,.58) 25%,rgba(31,17,100,.9) 55%,#040721);isolation:isolate}.breakthrough-cinematic:after{position:absolute;inset:-20%;z-index:1;background:repeating-conic-gradient(from 0deg,transparent 0 12deg,rgba(255,239,165,.16) 13deg 14deg,transparent 15deg 30deg);content:"";animation:fxHalo 5s linear infinite}.fx-vignette{position:absolute;inset:0;background:radial-gradient(circle,transparent 28%,rgba(1,3,20,.58) 100%);z-index:4}.fx-aurora{position:absolute;inset:-30%;background:conic-gradient(from 0deg,transparent,rgba(85,220,255,.4),transparent,rgba(255,191,81,.37),transparent);filter:blur(18px);animation:fxHalo 3.6s linear infinite}.fx-content{position:relative;z-index:5;display:flex;flex-direction:column;align-items:center;text-align:center}.fx-content em{margin-bottom:5px;color:#ddf7ff;font-size:10px;font-style:normal;letter-spacing:.38em;text-shadow:0 0 12px #82ddff}.fx-content p{margin:0;font-size:64px;filter:drop-shadow(0 0 18px #fff);animation:fxAvatar 1.2s ease-in-out infinite}.fx-content h4{margin:3px 0;color:#fff4c4;font-size:28px;letter-spacing:.2em;text-shadow:0 0 15px #ffce67}.fx-content span{color:#dff8ff;font-size:11px;letter-spacing:.25em}.fx-heaven-mark,.fx-ring{position:absolute;border:2px solid rgba(255,239,158,.75);border-radius:50%;box-shadow:0 0 28px rgba(255,229,120,.65)}.fx-heaven-mark{z-index:2;width:116px;height:116px}.fx-ring{z-index:2;width:320px;height:320px;border-color:rgba(120,223,255,.48);animation:fxSpin 2.2s linear infinite}.fx-ring.r2{width:454px;height:454px;border-style:dashed;animation-direction:reverse}.fx-rune{position:absolute;z-index:3;color:#fff2ad;font-size:25px;text-shadow:0 0 13px #9ceaff}.fx-rune.r1{transform:translate(-130px,-92px);animation:fxRune 1.5s ease-in-out infinite}.fx-rune.r2{transform:translate(120px,94px);animation:fxRune 1.5s ease-in-out .7s infinite}.fx-lightning{position:absolute;z-index:4;top:-15%;width:13px;height:120%;background:linear-gradient(90deg,transparent,#efffff,#9ae6ff,transparent);filter:blur(1px);transform:rotate(19deg);box-shadow:0 0 22px #b9f3ff;opacity:0}.fx-lightning.l2{transform:rotate(-25deg)}.fx-lightning.l3{transform:rotate(4deg);height:95%}.fx-sword-light{position:absolute;z-index:3;width:3px;height:145%;background:#fff9cb;box-shadow:0 0 23px 9px #8fe5ff;transform:rotate(42deg);opacity:0}.fx-sword-light.s2{transform:rotate(-38deg)}.fx-particle{position:absolute;z-index:3;width:5px;height:5px;border-radius:50%;background:#fff1aa;box-shadow:0 0 9px 3px #82e6ff;opacity:0;transform:rotate(calc(var(--i) * 20deg)) translateY(-30px)}.fx-phase-charge .fx-heaven-mark{animation:fxCharge 1.1s ease-out infinite}.fx-phase-charge .fx-ring{animation-duration:1.3s}.fx-phase-charge .fx-content{animation:fxBreath .7s ease-in-out infinite}.fx-phase-tribulation{animation:fxShake .11s linear 9}.fx-phase-tribulation .fx-lightning{opacity:1;animation:fxLightning .33s steps(2) infinite}.fx-phase-tribulation .fx-heaven-mark{animation:fxExpand .7s ease-out infinite}.fx-phase-ascend .fx-sword-light{opacity:1;animation:fxSword .65s ease-out infinite}.fx-phase-ascend .fx-particle{animation:fxParticle 1s ease-out infinite;animation-delay:calc(var(--i) * -55ms)}.fx-phase-reveal .fx-content{animation:fxReveal .8s ease-out both}.fx-stage-2{--stage:#9fe9ff}.fx-stage-3{--stage:#bfa8ff}.fx-stage-4{--stage:#ffbd6d}.fx-stage-5{--stage:#ffef9d}.fx-stage-2,.fx-stage-3,.fx-stage-4,.fx-stage-5{background:radial-gradient(circle,var(--stage),rgba(62,36,137,.67) 26%,rgba(12,10,53,.94) 61%,#040721)}@keyframes fxHalo{to{transform:rotate(360deg)}}@keyframes fxSpin{to{transform:rotate(360deg)}}@keyframes fxCharge{50%{transform:scale(.72);box-shadow:0 0 60px 16px #ffe89a}to{transform:scale(1.7);opacity:0}}@keyframes fxLightning{50%{opacity:.12}}@keyframes fxSword{to{transform:rotate(42deg) translateX(285px);opacity:0}}@keyframes fxParticle{to{transform:rotate(calc(var(--i) * 20deg)) translateY(-255px) scale(.15);opacity:0}}@keyframes fxShake{25%{transform:translate(3px,-2px)}75%{transform:translate(-3px,2px)}}@keyframes fxAvatar{50%{transform:scale(1.13);filter:drop-shadow(0 0 29px #fff)}}@keyframes fxRune{50%{opacity:.35;transform:translate(-130px,-112px) rotate(30deg)}}@keyframes fxBreath{50%{transform:scale(1.035)}}@keyframes fxReveal{from{transform:scale(1.35);filter:brightness(2)}to{transform:scale(1);filter:brightness(1)}}@media(max-width:520px){.fx-ring.r2{width:360px;height:360px}.fx-ring{width:285px;height:285px}.fx-content h4{font-size:23px}}.cave-cinematic{position:absolute;z-index:12;inset:0;display:grid;overflow:hidden;place-items:center;border-radius:10px;background:radial-gradient(circle,rgba(224,255,225,.7),rgba(61,164,177,.56) 28%,rgba(6,30,68,.94) 72%);isolation:isolate;animation:caveFxFade 2.6s ease-out forwards}.cave-cinematic:before{position:absolute;inset:-30%;background:repeating-conic-gradient(transparent 0 17deg,rgba(225,255,201,.22) 18deg 19deg,transparent 20deg 35deg);content:"";animation:caveFxSpin 4s linear infinite}.cave-fx-content{position:relative;z-index:3;text-align:center}.cave-fx-content em{display:block;font-size:58px;font-style:normal;filter:drop-shadow(0 0 18px #fff);animation:caveFxPulse .8s ease-in-out infinite}.cave-fx-content h4{margin:2px 0;color:#f4ffcf;font-size:23px;letter-spacing:.14em;text-shadow:0 0 13px #76f4df}.cave-fx-content p{color:#e1fbff;font-size:10px;letter-spacing:.15em}.cave-fx-ring{position:absolute;z-index:2;width:190px;height:190px;border:2px solid rgba(236,255,184,.7);border-radius:50%;box-shadow:0 0 28px #79e8d5;animation:caveFxRing 1.2s ease-out infinite}.cave-fx-ring.cr2{width:340px;height:340px;border-style:dashed;animation-direction:reverse}.cave-fx-cloud{position:absolute;width:280px;height:90px;border-radius:50%;background:radial-gradient(ellipse,rgba(174,255,226,.42),transparent 67%);filter:blur(5px);animation:caveFxCloud 2s ease-in-out infinite}.cf1{bottom:18%;left:-50px}.cf2{top:17%;right:-70px;animation-delay:-1s}.cave-fx-spark{position:absolute;z-index:2;width:5px;height:5px;border-radius:50%;background:#f8ffd1;box-shadow:0 0 10px 3px #78f6e5;transform:rotate(calc(var(--i) * 25deg)) translateY(-65px);animation:caveFxSpark 1.1s ease-out infinite;animation-delay:calc(var(--i) * -70ms)}.cave-fx-upgrade{background:radial-gradient(circle,rgba(255,240,164,.75),rgba(177,112,45,.56) 30%,rgba(49,21,71,.95) 72%)}.cave-fx-maintain{background:radial-gradient(circle,rgba(171,255,224,.75),rgba(35,160,138,.57) 32%,rgba(4,35,65,.95) 72%)}.cave-fx-expedition{background:radial-gradient(circle,rgba(190,232,255,.74),rgba(49,107,190,.58) 32%,rgba(7,18,72,.96) 72%)}.cave-fx-expedition .cave-fx-content{animation:caveFxSail 1.1s ease-in-out infinite}@keyframes caveFxFade{0%,73%{opacity:1}100%{opacity:0}}@keyframes caveFxSpin{to{transform:rotate(360deg)}}@keyframes caveFxRing{to{transform:scale(2.35);opacity:0}}@keyframes caveFxPulse{50%{transform:scale(1.14)}}@keyframes caveFxCloud{50%{transform:translateX(36px)}}@keyframes caveFxSpark{to{transform:rotate(calc(var(--i) * 25deg)) translateY(-240px);opacity:0}}@keyframes caveFxSail{50%{transform:translateX(9px) rotate(2deg)}}.cave-resonance{display:flex;justify-content:space-between;gap:10px;margin:8px 0;padding:8px 10px;border-left:2px solid #9fe7ff;background:rgba(76,190,255,.08);font-size:10px}.cave-resonance span{color:#ffe3a1}.cave-resonance b{color:rgba(220,246,255,.72);font-weight:400;text-align:right}.cave-yield-panel{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:9px 0;padding:10px 12px;border:1px solid rgba(119,221,255,.26);border-radius:6px;background:linear-gradient(90deg,rgba(69,188,255,.12),rgba(255,223,130,.07))}.cave-yield-panel p{color:#ffe8a7;font-size:11px}.cave-yield-panel span{display:block;margin-top:3px;color:rgba(221,245,255,.68);font-size:10px}.cave-yield-panel .btn-mini{flex:none}@media(max-width:450px){.cave-resonance{flex-direction:column}.cave-resonance b{text-align:left}.cave-yield-panel{align-items:flex-start}.cave-yield-panel span{line-height:1.6}}
</style>
