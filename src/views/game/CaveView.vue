<template>
  <div class="space-y-3">
    ...
    <div
      v-if="cultivation.caveTier === 0"
      class="game-panel p-3 text-center space-y-2"
    >
      <p class="text-xs text-muted leading-relaxed">
        花费 8000 文 + 200
        灵气，在山壁开凿一处洞府。洞府内可安置丹房、灵圃、静室、百草园、聚灵阵，大幅提升修行效率。
      </p>
      <Button
        class="w-full justify-center"
        :disabled="!cultivation.unlocked"
        @click="cultivation.openCave"
        >开辟洞府（8000文 + 200灵气）</Button
      >
    </div>
    <div v-else class="space-y-2">
      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="stat-card">
          <span>洞府</span><b>{{ cultivation.caveTierName }}</b>
        </div>
        <div class="stat-card">
          <span>槽位</span
          ><b
            >{{ cultivation.caveSlots.length }}/{{
              cultivation.caveMaxSlots
            }}</b
          >
        </div>
        <div class="stat-card">
          <span>灵气恢复</span><b>+{{ cultivation.caveAuraRegen }}/次</b>
        </div>
        <div class="stat-card">
          <span>灵脉稳定</span
          ><b
            :class="
              cultivation.caveStability < 80 ? 'text-caution' : 'text-success'
            "
            >{{ cultivation.caveStability }}%</b
          >
        </div>
        <div class="stat-card col-span-2">
          <span>设施</span
          ><b>{{ cultivation.caveSlotNames.join("、") || "空" }}</b>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-2">
        <Button class="w-full justify-between" @click="cultivation.upgradeCave"
          ><span>扩建洞府</span
          ><span class="text-muted text-xs">消耗灵气</span></Button
        >
        <Button
          class="w-full justify-between"
          :disabled="!cultivation.caveNeedsMaintenance"
          @click="cultivation.maintainCave"
          ><span>维护灵脉</span
          ><span class="text-muted text-xs"
            >{{ cultivation.caveMaintenanceCost.money }}文</span
          ></Button
        >
      </div>
      <p class="text-[10px] text-muted leading-relaxed">
        洞府稳定影响灵气恢复。长期不维护会缓慢下降；维护消耗铜钱、灵气与灵石，形成中后期资源消耗闭环。
      </p>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
        <div
          v-for="slot in caveSlotOptions"
          :key="slot.type"
        ...
        >
          ...
          <Button
            class="w-full justify-center"
            :disabled="
              !cultivation.hasCaveSlot(slot.type) &&
              cultivation.caveSlots.length >= cultivation.caveMaxSlots
            "
            @click="
              cultivation.hasCaveSlot(slot.type)
                ? cultivation.removeCaveSlot(slot.type)
                : cultivation.placeCaveSlot(slot.type)
            "
          >
    ...
    <div
      v-if="!cultivation.hasCaveSlot('herbgarden')"
      class="game-panel p-3 text-center space-y-2"
    >
      <p class="text-xs text-muted leading-relaxed">
        需先在洞府中安置百草园设施，才可在此管理药材。
      </p>
    </div>
    <div v-else class="space-y-2">
      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="stat-card">
          <span>百草园</span><b>Lv.{{ cultivation.herbGardenLevel }}</b>
        </div>
        <div class="stat-card">
          <span>现实日收益</span><b>{{ cultivation.herbDailyYield }}株</b>
        </div>
        <div class="stat-card col-span-2">
          <span>可领取</span
          ><b>{{ cultivation.herbClaimDays }}天（最多累计7天）</b>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-2">
        <Button
          class="justify-center"
          :disabled="cultivation.herbClaimDays <= 0"
          @click="cultivation.claimDailyHerbs"
          >领取现实日药材</Button
        >
        <Button
          class="justify-center"
          :disabled="cultivation.herbGardenLevel >= 10"
          @click="cultivation.upgradeHerbGarden"
        >
      ...
      <div class="grid grid-cols-3 md:grid-cols-4 gap-1">
        <div
          ...
          :key="herb.id"
          class="border border-accent/10 rounded-xs p-2 text-center text-xs"
        >
    ...
    <div
      v-if="!cultivation.hasCaveSlot('spiritArray')"
      class="game-panel p-3 text-center space-y-2"
    >
      <p class="text-xs text-muted leading-relaxed">
        需先在洞府中安置聚灵阵，才可凝聚五行元气。
      </p>
    </div>
    <div v-else class="space-y-2">
      <div class="grid grid-cols-2 gap-2 text-xs mb-2">
        ...
        <div class="stat-card">
          <span>现实日元气</span
          ><b>{{ cultivation.spiritArrayElementYield }}/种</b>
        </div>
        <div class="stat-card">
          <span>现实日灵石</span><b>{{ cultivation.spiritArrayStoneYield }}</b>
        </div>
        <div class="stat-card">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p class="text-[10px] text-muted">
              今日 {{ cultivation.spiritStoneRefineCountToday }}/{{
                cultivation.spiritStoneRefineLimit
              }}
              次，剩余 {{ cultivation.spiritStoneRefineRemaining }} 次
            </p>
          </div>
          <div class="text-xs text-muted">
            灵石×{{ cultivation.spiritStoneRefineCost }} → 灵气+{{
              cultivation.spiritStoneRefineAuraGain
            }}
          </div>
        </div>
        <Button
          class="w-full justify-center"
          :disabled="
            itemCount('spirit_stone') < cultivation.spiritStoneRefineCost ||
            cultivation.spiritStoneRefineRemaining <= 0
          "
          @click="cultivation.refineSpiritStoneToAura"
        >
          灵石炼气（持有灵石 {{ itemCount("spirit_stone") }}）
        </Button>
        <p class="text-[10px] text-muted leading-relaxed">
          每日固定10次。首次约1000灵石→10000灵气，后续同日成本快速递增、收益逐步递减，定位为消耗大量多余灵石、突破前补缺口，避免无限刷灵气。
        </p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div
          v-for="recipe in SPIRIT_STONE_EXCHANGES"
          :key="recipe.id"
          class="border border-accent/15 rounded-xs p-2 text-xs space-y-1"
        >
