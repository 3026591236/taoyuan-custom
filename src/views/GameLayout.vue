  ...
  >
    ...
    <Transition name="panel-fade">
      <div v-if="showMailModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" @click.self="showMailModal = false">
        <div class="game-panel w-full max-w-2xl max-h-[82vh] overflow-y-auto space-y-3">
          <div class="flex items-center justify-between gap-2">
            <h2 class="text-accent text-lg">系统邮件</h2>
            <Button @click="showMailModal = false">关闭</Button>
          </div>
          <p class="text-xs text-muted">领取奖励后会立即写入当前游戏，并触发一次自动存档。</p>
          <div v-if="mailLoading" class="text-sm text-muted">邮件加载中…</div>
          <div v-else-if="mails.length === 0" class="text-sm text-muted">暂无邮件。</div>
          <div v-for="mail in mails" :key="mail.id" class="border border-accent/20 rounded-xs p-3 space-y-2">
            <div class="flex flex-wrap justify-between gap-2">
              <div>
                <div class="text-accent">{{ mail.title }}</div>
                <div class="text-xs text-muted">{{ formatMailTime(mail.createdAt) }} / {{ mail.from || '系统' }}</div>
              </div>
              <span class="text-xs" :class="mail.claimed ? 'text-muted' : 'text-accent'">{{ mail.claimed ? '已领取' : '未领取' }}</span>
            </div>
...
<script setup lang="ts">
  ...
  const dailyCheckin = async () => {
    ...
    const t = accountToken()
    if (!t) { addLog('请先在首页登录账号，再进行每日签到。'); return }
