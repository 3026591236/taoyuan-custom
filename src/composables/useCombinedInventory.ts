...
const QUALITY_CONSUME_ORDER: Quality[] = [
  "normal",
  "fine",
  "excellent",
  "supreme",
...
/** 合计纳戒 + 仓库所有储物匣中某物品数量 */
export const getCombinedItemCount = (
  itemId: string,
  quality?: Quality,
): number => {
...
/** 纳戒+仓库所有储物匣是否合计拥有足够数量 */
export const hasCombinedItem = (
  itemId: string,
  quantity: number = 1,
): boolean => getCombinedItemCount(itemId) >= quantity;

/** 优先从纳戒消耗，不足部分从仓库储物匣消耗（虚空原料箱优先） */
export const removeCombinedItem = (
  itemId: string,
  quantity: number = 1,
  quality?: Quality,
): boolean => {
  ...
  for (const q of qualities) {
    ...
    const ordered = voidInput
      ? [voidInput, ...wh.chests.filter((c) => c.id !== voidInput.id)]
      : [...wh.chests];
