...
/** 检查好感度是否达到指定等级 */
const meetsLevel = (current, required) => {
...
const checkAchievementRecipes = () => {
    ...
    const checks = [
        ...
        {
            condition: ['chen_bo', 'liu_niang', 'a_shi', 'qiu_yue', 'lin_lao', 'xiao_man'].filter(id => meetsLevel(npcStore.getFriendshipLevel(id), 'friendly')).length >= 3,
            recipeId: 'social_tea',
            message: '社交达人'
        },
...
// ==================== 晨间随机事件 ====================
/** 应用晨间效果 */
const applyMorningEffect = (effect) => {
    if (!effect)
        return;
    const playerStore = usePlayerStore();
    const inventoryStore = useInventoryStore();
...
export const handleEndDay = () => {
    ...
    if (playerStore.stamina < 20)
        tutorialStore.setFlag('staminaWasLow');
    // 恢复模式
    let recoveryMode;
    ...
    if (pestResult.newInfestations > 0) {
        addLog(`虫害来袭！${pestResult.newInfestations}块地遭到了虫害侵袭。${farmStore.scarecrows > 0 ? '稻草人降低了虫害风险。' : '放置稻草人可以降低虫害概率。'}`);
    }
    ...
    if (pestResult.newWeeds > 0) {
        addLog(`杂草蔓延！${pestResult.newWeeds}块地长出了杂草。${farmStore.scarecrows > 0 ? '稻草人抑制了杂草蔓延。' : '放置稻草人可以减少杂草。'}`);
    }
    ...
    for (const msg of helperFeedResult.messages)
        addLog(msg);
        ...
        switch (zhiji.npcId) {
            ...
            case 'da_niu':
                if (Math.random() < 0.3 + bonusChance2) {
                    ...
                    if (result2.fedCount > 0)
                        addLog(`${zhijiName}帮你喂了所有牲畜。`);
                }
                break;
            case 'mo_bai':
    ...
    for (const msg of helperMorningResult.messages)
        addLog(msg);
    // 晨间工作：配偶浇水/做饭/收获
    if (spouse) {
        ...
        if (Math.random() < 0.5 + bonusChance + highBond) {
            ...
            if (count > 0)
                addLog(`${spouseName}一早帮你浇了${count}块地。`);
        }
        // 做饭：30% + bonus（好感>=2000）
    ...
    for (const msg of helperBaitResult.messages)
        addLog(msg);
    // 蟹笼收获
    ...
    for (const p of caveProducts) {
        ...
        const qualityLabel = p.quality === 'normal' ? '' : p.quality === 'fine' ? '（优质）' : p.quality === 'excellent' ? '（精品）' : '（极品）';
    ...
    if (pregResult.born) {
        const qMsg = pregResult.born.quality === 'healthy' ? '健健康康的！' : pregResult.born.quality === 'premature' ? '虽然早产了一些，但平安无事。' : '';
        addLog(`${pregResult.born.name}出生了！恭喜！${qMsg}`);
    }
    if (pregResult.stageChanged) {
        const stageLabels = { early: '初期', mid: '中期', late: '后期', ready: '待产期' };
    ...
    const specialOrderDays = { 7: 1, 14: 2, 21: 3, 28: 4 };
    ...
    for (const msg of bondMessages.messages)
        addLog(msg);
    let summary;
    ...
    if (tutorialStore.enabled && gameStore.year === 1) {
        const conditions = {
            ...
            tilledNoPlanted: () => farmStore.plots.some(p => p.state === 'tilled') &&
                !farmStore.plots.some(p => p.state === 'planted' || p.state === 'growing' || p.state === 'harvestable'),
            plantedUnwatered: () => farmStore.plots.some(p => (p.state === 'planted' || p.state === 'growing') && !p.watered) && !gameStore.isRainy,
            ...
            hasCropNoSprinkler: () => farmStore.plots.some(p => p.state === 'growing') && farmStore.sprinklers.length === 0 && gameStore.day >= 11,
            neverHadAnimal: () => animalStore.animals.length === 0 && gameStore.day >= 15
        };
        for (const tip of MORNING_TIPS) {
            if (tutorialStore.isTipShown(tip.id))
                continue;
    ...
    if (gameStore.farmMapType === 'riverland') {
        const seasonFish = FISH.filter(f => (f.location ?? 'creek') === 'creek' && f.season.includes(gameStore.season));
        if (seasonFish.length > 0) {
            ...
            for (let i = 0; i < catchCount; i++) {
                ...
                const quality = Math.random() < 0.15 ? 'fine' : 'normal';
