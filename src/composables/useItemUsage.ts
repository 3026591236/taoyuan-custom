...
export const isQuickUsableItem = (itemId: string): boolean =>
  SPECIAL_USABLE_ITEM_IDS.has(itemId) || CULTIVATION_PILL_IDS.has(itemId);
...
const staminaPillUsageKey = () =>
  `taoyuan_stamina_pill_usage_real_${realDateKey()}`;
const timeStasisPillUsageKey = () =>
  `taoyuan_time_stasis_pill_usage_real_${realDateKey()}`;
...
export const useItemUsage = () => {
  ...
  const useItem = (itemId: string, quality: Quality = "normal"): boolean => {
    ...
    if (isPill && gameStore.isPastBedtime) {
    ...
    if (itemId === "stamina_pill") {
      const now = Date.now();
      const lastUse = Number(
        localStorage.getItem(staminaPillLastUseKey()) || 0,
      );
      if (now - lastUse < STAMINA_PILL_COOLDOWN_MS) {
        addLog("体力丹药力尚未化开，请稍后再服用。");
        return false;
      }
      const usageKey = staminaPillUsageKey();
      const usedToday = Number(localStorage.getItem(usageKey) || 0);
      if (usedToday >= STAMINA_PILL_REAL_DAY_LIMIT) {
        addLog(
          `今日（现实时间）体力丹最多服用${STAMINA_PILL_REAL_DAY_LIMIT}枚，过量服用已无效。`,
        );
        return false;
      }
      if (playerStore.stamina >= playerStore.maxStamina + 500) {
        addLog("体力已经达到体力丹可提升的上限。");
        return false;
      }
      ...
      addLog(
        `服用了体力丹，体力+${gained}（今日现实${usedToday + 1}/${STAMINA_PILL_REAL_DAY_LIMIT}，最多可临时超过上限500点），炼化药力也耗去片刻。${time.message ? ` ${time.message}` : ""}`,
      );
      return true;
    }
    if (itemId === "time_stasis_pill") {
      ...
      const usedToday = Number(localStorage.getItem(usageKey) || 0);
      if (usedToday >= TIME_STASIS_PILL_REAL_DAY_LIMIT) {
        addLog(
          `今日（现实时间）时间禁锢丹最多服用${TIME_STASIS_PILL_REAL_DAY_LIMIT}枚。`,
        );
        return false;
      }
      if (!inventoryStore.removeItem(itemId, 1, quality)) return false;
      ...
      localStorage.setItem(usageKey, String(usedToday + 1));
      addLog(
        `服用了时间禁锢丹，游戏时间将在现实时间30分钟内暂停流逝（今日现实${usedToday + 1}/${TIME_STASIS_PILL_REAL_DAY_LIMIT}）。`,
      );
      return true;
    }
    ...
    if (itemId === "storage_talisman") {
      ...
      addLog(`使用了纳物符，纳戒永久扩容至${inventoryStore.capacity}格！`);
      return true;
    }
    if (itemId === "cosmos_bag") {
      if (!inventoryStore.removeItem(itemId, 1, quality)) return false;
      for (let i = 0; i < 4; i++) inventoryStore.expandCapacityExtra();
      addLog(`使用了乾坤袋，纳戒永久扩容至${inventoryStore.capacity}格！`);
      return true;
    }
    if (
