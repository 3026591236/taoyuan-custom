...
export const applyCropBlessing = (quality) => {
...
/** 处理地块点击：翻耕/种植/浇水/收获 */
export const handlePlotClick = (plotId) => {
...
/** 从商店购买种子 */
export const handleBuySeed = (seedId) => {
...
/** 通过商店出售物品 */
export const handleSellItem = (itemId, quality) => {
    const shopStore = useShopStore();
...
/** 出售指定物品的全部数量 */
export const handleSellItemAll = (itemId, quantity, quality) => {
    const shopStore = useShopStore();
    const itemDef = getItemById(itemId);
...
/** 一键出售背包中所有可出售物品 */
export const handleSellAll = (filterCategories) => {
    const shopStore = useShopStore();
    const inventoryStore = useInventoryStore();
    let totalEarned = 0;
    let totalCount = 0;
...
/** 一键种植（在所有空耕地上种植指定作物） */
export const handleBatchPlant = (cropId) => {
    const gameStore = useGameStore();
    const playerStore = usePlayerStore();
    const farmStore = useFarmStore();
    const inventoryStore = useInventoryStore();
...
export const handleBatchFertilize = (fertilizerType) => {
    const gameStore = useGameStore();
    const farmStore = useFarmStore();
    const inventoryStore = useInventoryStore();
    if (gameStore.isPastBedtime) {
    ...
    let applied = 0;
    for (const plot of targets) {
        if (!inventoryStore.hasItem(fertilizerType))
            break;
        if (!inventoryStore.removeItem(fertilizerType))
            break;
...
/** 铲除单块作物 */
export const handleRemoveCrop = (plotId) => {
    const gameStore = useGameStore();
    const playerStore = usePlayerStore();
    const farmStore = useFarmStore();
    const skillStore = useSkillStore();
    ...
    const ringFarmReduction = inventoryStore.getRingEffectValue('farming_stamina');
    const ringGlobalReduction = inventoryStore.getRingEffectValue('stamina_reduction');
    const cost = Math.max(1, Math.floor(2 * (1 - skillStore.getStaminaReduction('farming')) * (1 - farmingBuff) * (1 - ringFarmReduction) * (1 - ringGlobalReduction)));
    if (!playerStore.consumeStamina(cost)) {
        addLog('体力不足，无法铲除。');
        return;
    }
...
/** 除虫（单块） */
export const handleCurePest = (plotId) => {
    const gameStore = useGameStore();
    const playerStore = usePlayerStore();
    const farmStore = useFarmStore();
    const skillStore = useSkillStore();
...
export const handleBatchCurePest = () => {
    ...
    for (const plot of targets) {
        const farmingBuff = cookingStore.activeBuff?.type === 'farming' ? cookingStore.activeBuff.value / 100 : 0;
        const cost = Math.max(1, Math.floor(2 *
            (1 - skillStore.getStaminaReduction('farming')) *
            (1 - farmingBuff) *
            (1 - batchRingFarmReduction) *
            (1 - batchRingGlobalReduction)));
...
/** 除草（单块） */
export const handleClearWeed = (plotId) => {
    const gameStore = useGameStore();
    const playerStore = usePlayerStore();
    const farmStore = useFarmStore();
    const skillStore = useSkillStore();
