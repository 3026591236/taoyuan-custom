...
const pendingHeartEvent = ref(null);
const currentFestival = ref(null);
const pendingPerk = ref(null);
/** 宠物领养弹窗 */
...
/** 处理天赋选择对话框 */
export const handlePerkSelect = (perk) => {
    if (!pendingPerk.value)
        return;
    const skillStore = useSkillStore();
    const { skillType, level } = pendingPerk.value;
    if (level === 5) {
        skillStore.setPerk5(skillType, perk);
    }
    else {
        skillStore.setPerk10(skillType, perk);
    }
    addLog('习得了新专精！');
    pendingPerk.value = null;
};
/** 判断是否为隐藏NPC */
const isHiddenNpcId = (npcId) => HIDDEN_NPCS.some(n => n.id === npcId);
/** 触发心事件（由 NpcView / HiddenNpcModal 调用） */
export const triggerHeartEvent = (event) => {
...
/** 关闭心事件对话框并应用友好度变化 */
export const closeHeartEvent = (changes) => {
...
export const triggerWeddingEvent = (npcId) => {
    const event = { ...WEDDING_EVENT, npcId };
    pendingHeartEvent.value = event;
};
/** 显示季节事件对话框 */
...
/** 晨间选项事件弹窗 */
const pendingFarmEvent = ref(null);
/** 显示晨间选项事件 */
export const showFarmEvent = (event) => {
...
/** 仙灵发现场景弹窗队列 */
const pendingDiscoveryScenes = ref([]);
/** 当前显示的发现场景（队列头部） */
const pendingDiscoveryScene = ref(null);
/** 添加仙灵发现场景到队列 */
export const showDiscoveryScene = (npcId, step) => {
