...
const webdavFetch = async (url, method, headers, body) => {
...
const getPathHint = (serverUrl) => {
    try {
        const host = new URL(serverUrl).hostname.toLowerCase();
        if (host.includes('jianguoyun'))
            return '坚果云请在「存储路径」中填写已有的文件夹名，如"我的坚果云"。';
        if (host.includes('nextcloud') || serverUrl.includes('/remote.php/dav'))
            return 'Nextcloud 请在「存储路径」中填写目标文件夹名。';
        if (host.includes('owncloud') || serverUrl.includes('/remote.php/webdav'))
            return 'ownCloud 请在「存储路径」中填写目标文件夹名。';
    }
    catch {
        /* ignore */
    }
...
export const useWebdav = () => {
    ...
    /** 远程文件路径 */
    const remoteFilePath = (slot) => fullDirUrl() + `taoyuan_save_${slot}.tyx`;
    /** 根据状态码设置测试错误提示 */
    const setTestErrorByStatus = (status, phase) => {
    ...
    /** 上传存档到 WebDAV */
    const uploadSave = async (slot) => {
        pushTrace(`上传开始 slot=${slot}`);
        const raw = localStorage.getItem(`${SAVE_KEY_PREFIX}${slot}`);
        if (!raw)
            return { success: false, message: '本地存档不存在。' };
        try {
            let res = await webdavFetch(remoteFilePath(slot), 'PUT', {
                ...authHeaders(),
                'Content-Type': 'application/octet-stream'
            }, raw);
            ...
            if (res.status === 404 && !Capacitor.isNativePlatform()) {
                ...
                res = await webdavFetch(remoteFilePath(slot), 'PUT', {
                    ...authHeaders(),
                    'Content-Type': 'application/octet-stream'
                }, raw);
            }
