...
const playSfx = (freq, duration = 0.1, type = 'square', vol = sfxVolume) => {
    if (!sfxEnabled.value || !T || document.hidden)
        return;
    try {
...
export const sfxGameActionLight = () => {
    const now = Date.now();
    if (now - lastLightActionTime < LIGHT_ACTION_INTERVAL)
        return;
...
let currentFestivalOverride = null;
// ---- 春季 BGM（明快上行，晨光田园） ----
const SPRING_MELODY = [
...
const SPRING_BASS = [131, 147, 165, 196, 220, 196, 165, 131, 147, 165, 196, 131, 220, 196, 165, 131];
// ---- 夏季 BGM（活泼高音域，蝉鸣荷塘） ----
const SUMMER_MELODY = [
...
const SUMMER_BASS = [165, 196, 220, 196, 131, 165, 196, 220, 196, 165, 131, 147, 165, 196, 220, 131];
// ---- 秋季 BGM（缓慢下行，落叶忧伤） ----
const AUTUMN_MELODY = [
...
const AUTUMN_BASS = [220, 196, 165, 131, 147, 165, 196, 165, 131, 147, 165, 196, 220, 196, 165, 131];
// ---- 冬季 BGM（稀疏空灵，初雪炉火） ----
const WINTER_MELODY = [
...
const WINTER_BASS = [131, 0, 165, 0, 196, 0, 131, 0, 147, 0, 165, 131, 196, 165, 131, 0];
...
const BATTLE_BASS = [131, 196, 131, 196, 220, 196, 165, 196, 131, 220, 196, 165, 131, 165, 196, 220];
// ---- 节日 BGM ----
const FESTIVAL_SPRING_MELODY = [523, 587, 659, 784, 659, 587, 523, 440, 523, 659, 784, 880, 784, 659, 523, 0];
const FESTIVAL_SPRING_BASS = [131, 196, 165, 220];
const FESTIVAL_SUMMER_MELODY = [440, 523, 587, 523, 440, 392, 440, 523, 587, 659, 587, 523, 440, 392, 330, 0];
const FESTIVAL_SUMMER_BASS = [220, 196, 165, 196];
const FESTIVAL_AUTUMN_MELODY = [392, 440, 523, 440, 392, 330, 392, 440, 523, 587, 523, 440, 523, 440, 392, 0];
const FESTIVAL_AUTUMN_BASS = [196, 220, 165, 131];
const FESTIVAL_WINTER_MELODY = [262, 330, 392, 440, 523, 440, 392, 330, 440, 523, 587, 659, 587, 523, 440, 0];
const FESTIVAL_WINTER_BASS = [131, 165, 196, 220];
// ---- 小游戏 BGM ----
// 钓鱼大赛：流水般起伏，耐心等待的感觉
const MINIGAME_FISHING_MELODY = [
...
const MINIGAME_FISHING_BASS = [131, 196, 165, 220, 131, 196, 165, 131];
// 端午赛龙舟：快节奏驱动，鼓点般的急促
const MINIGAME_DRAGON_MELODY = [
...
const MINIGAME_DRAGON_BASS = [131, 196, 131, 220, 165, 196, 131, 196];
// 七夕猜灯谜：灯影摇曳，神秘沉思
const MINIGAME_RIDDLE_MELODY = [
...
const MINIGAME_RIDDLE_BASS = [131, 0, 165, 0, 196, 0, 131, 0];
// 斗茶大会：文雅精致，古琴意韵
const MINIGAME_TEA_MELODY = [
...
const MINIGAME_TEA_BASS = [220, 196, 165, 131, 220, 196, 165, 131];
// 丰收宴·农展会：喜庆欢快，五谷丰登
const MINIGAME_HARVEST_MELODY = [
...
const MINIGAME_HARVEST_BASS = [131, 165, 196, 220, 131, 196, 165, 131];
// 重阳投壶：沉着聚焦，武道精神
const MINIGAME_POT_MELODY = [
...
const MINIGAME_POT_BASS = [147, 196, 131, 165, 147, 196, 165, 131];
// 秋风筝会：轻盈高远，翱翔天际
const MINIGAME_KITE_MELODY = [
...
const MINIGAME_KITE_BASS = [196, 220, 196, 165, 131, 196, 165, 196];
// 冬至包饺子：温暖热闹，家常节奏
const MINIGAME_DUMPLING_MELODY = [
...
const MINIGAME_DUMPLING_BASS = [131, 165, 196, 131, 165, 131, 196, 165];
// 年末烟花会：华彩绚烂，跌宕起伏
const MINIGAME_FIREWORK_MELODY = [
...
const MINIGAME_FIREWORK_BASS = [131, 196, 220, 196, 131, 165, 196, 220];
// ---- 瀚海 BGM（西域丝路，神秘赌场） ----
const HANHAI_MELODY = [
    // Phase 1: 神秘开场，低音域徘徊
...
const HANHAI_BASS = [131, 165, 196, 165, 131, 196, 220, 196, 131, 165, 196, 220, 165, 131, 196, 165];
const BGM_CONFIG = {
...
/** BGM 播放循环 */
const playBgmLoop = async (type = 'spring', weather = 'sunny') => {
    ...
    const playNext = () => {
        ...
        try {
            ...
            if (detune !== 0)
                melodySynth.detune.value = detune;
            ...
            if (noteIndex % bassInterval === 0) {
                ...
                if (bassFreq > 0) {
                    ...
                    if (detune !== 0)
                        bassSynth.detune.value = detune * 0.5;
...
/** 解析当前应播放的 BGM 类型和天气 */
const resolveCurrentBgm = () => {
    const gameStore = useGameStore();
    if (currentFestivalOverride) {
        return { type: currentFestivalOverride, weather: 'sunny' };
    }
    return { type: gameStore.season, weather: gameStore.weather };
};
...
export const useAudio = () => {
    ...
    /** 开始播放节日 BGM */
    const startFestivalBgm = (season) => {
        if (!bgmEnabled.value)
            return;
        const festivalType = `festival_${season}`;
        currentFestivalOverride = festivalType;
        stopBgm();
        void playBgmLoop(festivalType, 'sunny');
    };
    /** 开始播放小游戏专属 BGM */
    const startMinigameBgm = (festivalType) => {
        if (!bgmEnabled.value)
            return;
        const MINIGAME_BGM_MAP = {
