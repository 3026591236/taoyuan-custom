...
const meetsLevel = (
  current: string,
  required: "acquaintance" | "friendly" | "bestFriend",
): boolean => {
  return (
    LEVEL_ORDER.indexOf(current as (typeof LEVEL_ORDER)[number]) >=
    LEVEL_ORDER.indexOf(required)
  );
};
...
const checkAchievementRecipes = () => {
  ...
  const checks: { condition: boolean; recipeId: string; message: string }[] = [
    ...
    {
      condition: LEGENDARY_FISH_IDS.some((id) =>
        achievementStore.isDiscovered(id),
      ),
  ...
  for (const check of checks) {
    if (check.condition) {
      if (cookingStore.unlockRecipe(check.recipeId)) {
        addLog(`【功业食谱】${check.message}解锁了新食谱！`);
      }
    }
  }
};
...
const applyMorningEffect = (effect?: MorningEffect) => {
  ...
  switch (effect.type) {
    case "loseCrop": {
      const growing = farmStore.plots.filter(
        (p) => p.state === "growing" || p.state === "harvestable",
      );
      if (growing.length > 0) {
        const target = growing[Math.floor(Math.random() * growing.length)]!;
        const cropName = getCropById(target.cropId ?? "")?.name ?? "灵植";
        target.state = "tilled";
        target.cropId = null;
        target.growthDays = 0;
...
const rollMorningEvent = ():
  ...
  if (roll < 0.002) {
    const egg =
      MORNING_EASTER_EGGS[
        Math.floor(Math.random() * MORNING_EASTER_EGGS.length)
      ]!;
  ...
  if (roll < 0.01) {
    const event =
      MORNING_CHOICE_EVENTS[
        Math.floor(Math.random() * MORNING_CHOICE_EVENTS.length)
      ]!;
  ...
  // 4% 小偷/动物旁白
  const farmStore = useFarmStore();
  const hasCrops = farmStore.plots.some(
    (p) => p.state === "growing" || p.state === "harvestable",
  );
...
export const handleEndDay = () => {
  ...
  // 玄矿幽脉强制退出：无论玩家是否在探索中，睡觉/晕厥后都重置玄矿幽脉状态
  ...
  // 戒指效果：灵植生长加速
  const ringGrowthBonus =
    inventoryStore.getRingEffectValue("crop_growth_bonus");
      ...
      if (
        ...
        if (crop) {
          ...
          const effectiveDays = Math.max(
            1,
            Math.floor(crop.growthDays * (1 - speedup)),
          );
  ...
  if (upgradeResult?.completed) {
    addLog(
      `小满完成了${TOOL_NAMES[upgradeResult.toolType]}的升级！现在是${TIER_NAMES[upgradeResult.targetTier]}级。`,
    );
  }
  ...
  if (crowResult.attacked) {
    addLog(
      `乌鸦袭击了你的灵田，一株${crowResult.cropName}被吃掉了！放个稻草人保护灵植吧。`,
    );
  }
  ...
  if (pestResult.pestDeaths > 0) {
    addLog(
      `${pestResult.pestDeaths}株灵植因虫害持续太久而枯死了！及时除虫可以拯救灵植。`,
    );
  }
  ...
  if (pestResult.weedDeaths > 0) {
    addLog(
      `${pestResult.weedDeaths}株灵植被杂草覆盖窒息而死！及时除草可以拯救灵植。`,
    );
  }
  ...
  for (const gc of giantCrops) {
    addLog(`巨型${gc.cropName}出现了！3×3的灵植合体成了巨型灵植！`);
  }
    ...
    switch (zhiji.npcId) {
      case "a_shi":
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(
            `${zhijiName}送来了${qty}个${getItemById(ore)?.name ?? "矿石"}。`,
          );
        }
        break;
      case "dan_qing":
      ...
      case "yun_fei":
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(
            `${zhijiName}从山里带回了${getItemById(item2)?.name ?? "东西"}。`,
          );
        }
        break;
      case "da_niu":
  ...
  if (museumIncome > 0) addLog(`藏珍阁今日人气收入 ${museumIncome} 文。`);
  ...
  if (shippingIncome > 0) {
    ...
    addLog(`出货灵匣结算：收入${shippingIncome}文。`);
  }
  ...
  if (
    hanhaiStore.unlocked &&
    (gameStore.day % 7 === 1 || hanhaiStore.weeklyRotatingStock.length === 0)
  ) {
  ...
  // 晨间工作：雇工浇水/收获/除草
  const helperMorningResult = npcStore.processDailyHelpers([
    "water",
    "harvest",
    "weed",
  ]);
  ...
  if (spouse) {
    ...
    // 收获：30%（好感>=3000），最多3块，纳戒满时不收
    if (
      spouse.friendship >= 3000 &&
      !inventoryStore.isFull &&
      Math.random() < 0.3 + bonusChance
    ) {
      ...
      if (harvested > 0)
        addLog(`${spouseName}一早帮你收了${harvested}块地的庄稼。`);
    }
  }
  ...
  if (incubatorResult.hatched) {
    addLog(`灵禽舍孵化器中的蛋孵出了一只${incubatorResult.hatched.name}！`);
  }
  ...
  if (fishPondStore.pond.built) {
    ...
    if (pondResult.products.length > 0) {
      addLog(`灵泉鱼池产出了${pondResult.products.length}件水产品。`);
    }
    ...
    if (pondResult.bred) {
      addLog(`灵泉鱼池繁殖成功，新的${pondResult.bred}出生了！`);
    }
  ...
  for (const p of caveProducts) {
    ...
    addLog(
      `山洞中发现了${qtyText}${itemDef?.name ?? p.itemId}${qualityLabel}。`,
    );
  }
  ...
  if (wildTreeResult.products.length > 0) {
    addLog(
      `采脂器收获了${wildTreeResult.products.map((p) => p.productName).join("、")}。`,
    );
  }
  ...
  if (homeStore.farmhouseLevel >= 3) {
    ...
    for (const r of cellarResult.upgraded) {
      ...
      addLog(
        `酒窖中的${name}价值提升了+${homeStore.cellarValuePerCycle}文（共+${r.addedValue}文）`,
      );
  ...
  if (pregResult.stageChanged) {
    ...
    addLog(
      `孕期进入${stageLabels[pregResult.stageChanged.to]}。记得多多照顾配偶。`,
    );
  }
  ...
  const { moneyLost, recoveryPct } = playerStore.dailyReset(
    recoveryMode,
    bedHour,
  );
  ...
  // 换季处理
  if (seasonChanged) {
    const { witheredCount, reclaimedCount } = farmStore.onSeasonChange(
      gameStore.season,
    );
    addLog(
      `—— 季节更替：${SEASON_NAMES[oldSeason]}→${SEASON_NAMES[gameStore.season]} ——`,
    );
    if (witheredCount > 0) {
      addLog(`${witheredCount}株不适应新季节的灵植枯萎了……`);
    }
    if (reclaimedCount > 0) {
      addLog(`${reclaimedCount}块荒废的耕地被杂草覆盖了。`);
    }
    if (oldSeason === "winter" && gameStore.season === "spring") {
      addLog("新的一年开始了！灵田经过一冬有些荒废，需要重新开垦。");
    }
  ...
  if (gameStore.weather === "stormy") {
    ...
    if (strike.absorbed) {
      ...
      addLog(
        `雷暴中一道闪电击中了你的灵田，一株${strike.cropName}被毁了！建造避雷针可以防护。`,
      );
    }
  }
  ...
  if (!seasonChanged && gameStore.day >= 25 && gameStore.day <= 27) {
    ...
    if (cropAtRisk > 0) {
      addLog(
        `距离换季还有${daysLeft}天，${cropAtRisk}株灵植不适应${SEASON_NAMES[nextSeason]}季，届时将会枯萎。`,
      );
      showFloat(`换季倒计时${daysLeft}天！${cropAtRisk}株灵植将枯萎`, "danger");
    } else {
  ...
  // 今日行情
  const marketInfo = getDailyMarketInfo(
    gameStore.year,
  ...
  if (booms.length > 0) {
    addLog(
      `今日行情：${booms.map((b) => MARKET_CATEGORY_NAMES[b.category]).join("、")}价格大涨！`,
    );
  }
  if (crashes.length > 0) {
    addLog(
      `今日行情：${crashes.map((c) => MARKET_CATEGORY_NAMES[c.category]).join("、")}价格暴跌。`,
    );
  }
  ...
  if (tutorialStore.enabled && gameStore.year === 1) {
    const conditions: Record<string, () => boolean> = {
      ...
      allWasteland: () =>
        farmStore.plots.every((p) => p.state === "wasteland") &&
        gameStore.day > 2,
      tilledNoPlanted: () =>
      ...
      hasHarvestable: () =>
        farmStore.plots.some((p) => p.state === "harvestable"),
      harvestedNeverSold: () =>
        achievementStore.stats.totalCropsHarvested > 0 &&
        achievementStore.stats.totalMoneyEarned === 0,
      ...
      neverFished: () =>
        achievementStore.stats.totalFishCaught === 0 && gameStore.day >= 4,
      neverMined: () =>
        achievementStore.stats.highestMineFloor === 0 && gameStore.day >= 6,
      neverTalkedNpc: () =>
        npcStore.npcStates.every((n) => n.friendship === 0) &&
        gameStore.day >= 3,
      neverCheckedQuests: () =>
        !tutorialStore.hasPanelVisited("quest") && gameStore.day >= 5,
      neverCooked: () =>
        achievementStore.stats.totalRecipesCooked === 0 && gameStore.day >= 8,
      firstRainyDay: () =>
        gameStore.isRainy && !tutorialStore.getFlag("seenRain"),
      justChangedSeason: () => tutorialStore.getFlag("justChangedSeason"),
      hasCropNoSprinkler: () =>
        farmStore.plots.some((p) => p.state === "growing") &&
        farmStore.sprinklers.length === 0 &&
        gameStore.day >= 11,
      neverHadAnimal: () =>
        animalStore.animals.length === 0 && gameStore.day >= 15,
    };
    for (const tip of MORNING_TIPS) {
      ...
      if (check && check()) {
        ...
        if (tip.conditionKey === "firstRainyDay")
          tutorialStore.setFlag("seenRain");
        if (tip.conditionKey === "justChangedSeason")
          tutorialStore.setFlag("seenSeasonChange");
        break; // 每天只显示一条
      }
    }
    // 清除临时标记
  ...
  if (event) {
    ...
    const yearStr =
      gameStore.year <= 10
        ? ORDINALS[gameStore.year - 1]!
        : String(gameStore.year);
    const resolved = {
      ...event,
  ...
  for (const a of newAchievements) {
    addLog(
      `【功业达成】${a.name}！${a.reward.money ? `获得${a.reward.money}文` : ""}`,
    );
    showFloat(`功业: ${a.name}`, "accent");
  }
  ...
  // 洞穴解锁检查
  if (
    !homeStore.caveUnlocked &&
    achievementStore.stats.totalMoneyEarned >= CAVE_UNLOCK_EARNINGS
  ) {
    homeStore.unlockCave();
    addLog(
      "你的累计收入引起了注意……集后的山洞已为你开放！去设施面板选择山洞用途吧。",
    );
  }

  // ===== 灵田洞天特殊效果 =====

  // 荒野灵田洞天：被动矿石（增强）+ 夜间野兽遭遇
  if (gameStore.farmMapType === "wilderness") {
    ...
    if (Math.random() < 0.25) {
      ...
      if (Math.random() < winRate) {
        ...
        addLog(
          `夜间有野兽入侵！你奋力击退了它，缴获了${lootQty}个${lootName}。`,
        );
      } else {
        const damage = 5 + Math.floor(Math.random() * 11); // 5-15
        playerStore.takeDamage(damage);
        const crops = farmStore.plots.filter(
          (p) => p.state === "growing" || p.state === "harvestable",
        );
        if (crops.length > 0) {
  ...
  if (gameStore.farmMapType === "riverland") {
    const seasonFish = FISH.filter(
      (f) =>
        (f.location ?? "creek") === "creek" &&
        f.season.includes(gameStore.season as any),
    );
    if (seasonFish.length > 0) {
      const isRainy = gameStore.isRainy;
      const catchCount = isRainy
      ...
      for (let i = 0; i < catchCount; i++) {
        ...
        const quality: "normal" | "fine" =
          Math.random() < 0.15 ? "fine" : "normal";
        catches.push({ fishId: fish.id, quality });
      }
      const MAX_CREEK_CATCH = 10;
      const merged = [...gameStore.creekCatch, ...catches].slice(
        0,
        MAX_CREEK_CATCH,
      );
      gameStore.creekCatch = merged;
      addLog(`溪流中有鱼儿在跳跃，去灵田界面收取鱼获吧。`);
    }
  }
