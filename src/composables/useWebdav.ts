...
export const useWebdav = () => {
  ...
  const webdavReady = computed(
    () => config.value.enabled && testStatus.value === "success",
  );
  ...
  const authHeaders = (): Record<string, string> => ({
    Authorization:
      "Basic " + btoa(config.value.username + ":" + config.value.password),
  });

  /** 远程文件路径 */
  const remoteFilePath = (slot: number): string =>
    fullDirUrl() + `taoyuan_save_${slot}.tyx`;
  ...
  const setTestErrorByStatus = (status: number, phase?: string) => {
    ...
    if (status === 401 || status === 403) {
      ...
      testError.value =
        prefix + "路径不存在。" + getPathHint(config.value.serverUrl);
  ...
  const probeNativeWritable = async (
    dirUrl: string,
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          message:
            "上传路径无效。Android 端不支持自动建目录，请先在网盘创建该文件夹。" +
            getPathHint(config.value.serverUrl),
        };
      }
      if (res.status === 404) {
        return {
  ...
  /** 列出远程存档是否存在 */
  const listRemoteSaves = async (): Promise<
