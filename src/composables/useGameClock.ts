...
const timeFreezeUntil = ref(
  Number(localStorage.getItem(TIME_FREEZE_UNTIL_KEY) || 0) || 0,
);
...
const tick = () => {
  ...
  if (
    !gameStore.midnightWarned &&
    prevHour < MIDNIGHT_HOUR &&
    newHour >= MIDNIGHT_HOUR
  ) {
...
export const useGameClock = () => {
  ...
  const timeFreezeRemainingMs = computed(() =>
    Math.max(0, timeFreezeUntil.value - Date.now()),
  );

  const freezeGameTime = (realHours = 3) => {
    const until =
      Math.max(timeFreezeUntil.value, Date.now()) +
      Math.max(0, realHours) * 3600000;
  ...
  const adjustSpeed = (direction: "slower" | "faster") => {
    const index = Math.max(
      0,
      GAME_SPEED_OPTIONS.findIndex((s) => s === gameSpeed.value),
    );
    const nextIndex =
      direction === "slower"
        ? Math.max(0, index - 1)
  ...
  const cycleSpeed = () => {
    ...
    const next =
      GAME_SPEED_OPTIONS[(index + 1) % GAME_SPEED_OPTIONS.length] ?? 1;
    setSpeed(next);
  };
