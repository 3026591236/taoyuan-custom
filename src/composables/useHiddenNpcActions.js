...
/** 执行供奉 */
export const doOffering = (npcId, itemId, quality) => {
...
/** 执行独特互动 */
export const doSpecialInteraction = (npcId) => {
...
/** 获取供奉偏好标签 */
export const getOfferingPreference = (npcId, itemId) => {
    const def = getHiddenNpcById(npcId);
    if (!def)
        return 'neutral';
    if (def.resonantOfferings.includes(itemId))
        return 'resonant';
    if (def.pleasedOfferings.includes(itemId))
        return 'pleased';
    if (def.repelledOfferings.includes(itemId))
        return 'repelled';
    return 'neutral';
};
export const OFFERING_PREF_LABELS = {
    resonant: '灵犀',
    pleased: '合意',
    repelled: '排斥',
    neutral: ''
};
export const OFFERING_PREF_CLASS = {
    resonant: 'text-accent',
    pleased: 'text-success',
    repelled: 'text-danger',
    neutral: ''
};
export const OFFERING_PREF_ORDER = {
    resonant: 0,
    pleased: 1,
    neutral: 2,
    repelled: 3
