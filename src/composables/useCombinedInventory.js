...
/** 合计背包 + 仓库所有箱子中某物品数量 */
export const getCombinedItemCount = (itemId, quality) => {
...
/** 背包+仓库所有箱子是否合计拥有足够数量 */
export const hasCombinedItem = (itemId, quantity = 1) => getCombinedItemCount(itemId) >= quantity;
/** 优先从背包消耗，不足部分从仓库箱子消耗（虚空原料箱优先） */
export const removeCombinedItem = (itemId, quantity = 1, quality) => {
    const inv = useInventoryStore();
    const wh = useWarehouseStore();
    // 统计总数
    ...
    if (invCount + warehouseTotal < quantity)
        return false;
...
export const getLowestCombinedQuality = (itemId) => {
    const inv = useInventoryStore();
    const wh = useWarehouseStore();
    const order = ['normal', 'fine', 'excellent', 'supreme'];
    for (const q of order) {
        if (inv.getItemCount(itemId, q) > 0)
            return q;
        if (wh.unlocked) {
            for (const chest of wh.chests) {
                if (wh.getChestItemCount(chest.id, itemId, q) > 0)
                    return q;
            }
        }
    }
