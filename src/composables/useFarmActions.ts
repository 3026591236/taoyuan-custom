...
/** 仙缘结缘：灵植祝福（crop_blessing）概率品质+1 */
...
export const handlePlotClick = (plotId: number) => {
  ...
  if (plot.state === "wasteland") {
    ...
    if (!inventoryStore.hasItem(cropDef.seedId)) {
      addLog(`没有${cropDef.name}灵种了。`);
      return;
    }
    const plantQuality = selectedSeed.value.quality;
    const cropFarmingBuff =
      cookingStore.activeBuff?.type === "farming"
        ? cookingStore.activeBuff.value / 100
        : 0;
    const cropRingFarmReduction =
      inventoryStore.getRingEffectValue("farming_stamina");
    const cropRingGlobalReduction =
      inventoryStore.getRingEffectValue("stamina_reduction");
    const cost = Math.max(
      1,
      Math.floor(
    ...
    if (cropDef.growthDays > daysLeft) {
      ...
      if (!cropDef.season.includes(nextSeason)) {
        showFloat(
          `${cropDef.name}需${cropDef.growthDays}天，本季仅剩${daysLeft}天！`,
          "danger",
        );
        addLog(
          `注意：${cropDef.name}需要${cropDef.growthDays}天成熟，但本季仅剩${daysLeft}天，换季后将枯萎。`,
        );
      }
    }
    ...
    const farmingBuff =
      cookingStore.activeBuff?.type === "farming"
        ? cookingStore.activeBuff.value / 100
        : 0;
    const waterRingFarmReduction =
      inventoryStore.getRingEffectValue("farming_stamina");
    const waterRingGlobalReduction =
      inventoryStore.getRingEffectValue("stamina_reduction");
    const cost = Math.max(
      1,
      Math.floor(
    ...
    if (cropId) {
      ...
      const ringCropQualityBonus =
        inventoryStore.getRingEffectValue("crop_quality_bonus");
      const allSkillsBuff =
        cookingStore.activeBuff?.type === "all_skills"
          ? cookingStore.activeBuff.value
          : 0;
      let quality = skillStore.rollCropQualityWithBonus(
        (fertDef?.qualityBonus ?? 0) + ringCropQualityBonus,
        allSkillsBuff,
      );
      quality = applyCropBlessing(quality);
      // 精耕细作天赋：20% 概率双倍收获
      const intensiveDouble =
        skillStore.getSkill("farming").perk10 === "intensive" &&
        Math.random() < 0.2;
      ...
      const standardDouble =
        !intensiveDouble &&
        !yieldDouble &&
        gameStore.farmMapType === "standard" &&
        Math.random() < 0.15;
      const harvestQty =
        intensiveDouble || yieldDouble || standardDouble ? 2 : 1;
      ...
      sfxHarvest();
      const qtyLabel =
        intensiveDouble || yieldDouble || standardDouble ? "×2" : "";
      showFloat(
...
export const handleBuySeed = (seedId: string) => {
  ...
  if (shopStore.buySeed(seedId)) {
    ...
    addLog(`购买了${seed.cropName}灵种。(-${actualPrice}文)`);
  } else {
    addLog("铜钱不足或纳戒已满。");
  }
};
...
export const handleSellItem = (itemId: string, quality: Quality) => {
  ...
  if (earned > 0) {
    ...
    reportEconomyEvent({
      ...
      quality,
      source: "shop_sell",
      detail: { itemName: itemDef.name },
    });
  }
};

/** 出售指定物品的全部数量 */
export const handleSellItemAll = (
  itemId: string,
  quantity: number,
  quality: Quality,
) => {
  ...
  if (earned > 0) {
    ...
    reportEconomyEvent({
      eventType: "sell_item_all",
      amount: earned,
      itemId,
      quantity,
      quality,
      source: "shop_sell_all",
      detail: { itemName: itemDef.name },
    });
  }
};
...
export const handleSellAll = (filterCategories?: ItemCategory[]) => {
  ...
  const sellable = inventoryStore.items
    .filter((inv) => {
      ...
      return (
        ...
        def.category !== "seed" &&
        !inv.locked &&
        (!allowed || allowed.has(def.category))
      );
    })
    .map((inv) => ({
      itemId: inv.itemId,
  ...
  if (totalEarned > 0) {
    ...
    reportEconomyEvent({
      ...
      amount: totalEarned,
      quantity: totalCount,
      source: "shop_sell_all_bulk",
      detail: { filterCategories },
    });
  }
};
...
export const handleBatchWater = () => {
  ...
  const targets = farmStore.plots.filter(
  ...
  let watered = 0;
  const batchRingFarmReduction =
    inventoryStore.getRingEffectValue("farming_stamina");
  const batchRingGlobalReduction =
    inventoryStore.getRingEffectValue("stamina_reduction");
...
export const handleBatchTill = () => {
  ...
  let tilled = 0;
  const tillRingFarmReduction =
    inventoryStore.getRingEffectValue("farming_stamina");
  const tillRingGlobalReduction =
    inventoryStore.getRingEffectValue("stamina_reduction");
  for (const plot of targets) {
    const farmingBuff =
      cookingStore.activeBuff?.type === "farming"
...
export const handleBatchHarvest = () => {
  ...
  for (const groupId of giantGroups) {
    const groupPlot = farmStore.plots.find(
      (p) => p.giantCropGroup === groupId && p.state === "harvestable",
    );
    ...
    if (result) {
      ...
      const attributeUps = playerStore.addAttributeExpBatch({
        physique: result.quantity * 4,
        perception: result.quantity * 2,
      });
      skillStore.addExp("farming", 10);
      harvested++;
      if (attributeUps.length > 0) harvestedCrops.push(attributeUps.join("，"));
      harvestedCrops.push(
        `巨型${cropDef?.name ?? result.cropId}x${result.quantity}`,
      );
    }
  }
  ...
  // 再收获普通灵植
  const targets = farmStore.plots.filter(
    (p) => p.state === "harvestable" && p.giantCropGroup === null,
  );
  let seedsReturned = 0;
  ...
  for (const plot of targets) {
    ...
    if (cropId) {
      const cropDef = getCropById(cropId);
      const fertDef = plotFertilizer ? getFertilizerById(plotFertilizer) : null;
      const batchRingCropQuality =
        inventoryStore.getRingEffectValue("crop_quality_bonus");
      const batchAllSkillsBuff =
        cookingStore.activeBuff?.type === "all_skills"
          ? cookingStore.activeBuff.value
          : 0;
      let quality = skillStore.rollCropQualityWithBonus(
  ...
  if (harvested > 0) {
    ...
    const cropSummary = Array.from(cropCounts.entries())
      ...
      .join("、");
    addLog(`一键收获了${harvested}株灵植：${cropSummary}。`);
    const tr = gameStore.advanceTime(
      ACTION_TIME_COSTS.batchHarvest *
        inventoryStore.getToolStaminaMultiplier("scythe"),
    );
    if (tr.message) addLog(tr.message);
    if (tr.passedOut) handleEndDay();
  } else {
...
export const handleBatchPlant = (cropId: string) => {
  ...
  if (planted > 0) {
    ...
    const tr = gameStore.advanceTime(
      ACTION_TIME_COSTS.plant * Math.min(planted, 3),
    );
    if (tr.message) addLog(tr.message);
    if (tr.passedOut) handleEndDay();
  } else {
...
export const handleBatchFertilize = (fertilizerType: FertilizerType) => {
  ...
  const targets = farmStore.plots.filter(
    (p) => p.state !== "wasteland" && !p.fertilizer,
  );
  if (targets.length === 0) {
    addLog("没有可施肥的地块。");
    return;
  ...
  if (applied > 0) {
    ...
    const tr = gameStore.advanceTime(
      ACTION_TIME_COSTS.plant * Math.min(applied, 3),
    );
    if (tr.message) addLog(tr.message);
    if (tr.passedOut) handleEndDay();
  } else {
