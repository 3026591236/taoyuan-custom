...
/** 蘑菇层奖励 */
export const getMushroomRewards = (
  floor: number,
...
/** 宝箱层奖励 */
export const getTreasureRewards = (
  floor: number,
): { items: { itemId: string; quantity: number }[]; money: number } => {
  ...
  const rewardTable: Record<
    string,
    { qty: number; minMoney: number; maxMoney: number }
  > = {
  ...
  return {
    items,
    money:
      r.minMoney + Math.floor(Math.random() * (r.maxMoney - r.minMoney + 1)),
  };
};
...
/** 感染层清除奖励 */
export const getInfestedClearRewards = (
  floor: number,
): { items: { itemId: string; quantity: number }[]; money: number } => {
  const zoneIndex = Math.floor((floor - 1) / 20);
  const zones: MineFloorDef["zone"][] = [
...
/** BOSS 额外掉落矿石 */
export const BOSS_ORE_REWARDS: Record<
  number,
  { itemId: string; quantity: number }[]
> = {
...
export const getSkullCavernBoss = (floor: number): MonsterDef | undefined => {
  ...
  const randomFloor =
    bossFloors[Math.floor(Math.random() * bossFloors.length)]!;
...
/** 获取楼层格子分布配置 */
export const getFloorDistribution = (
  specialType: FloorSpecialType,
): FloorTileDistribution => {
  switch (specialType) {
    case "mushroom":
      return {
...
export const generateFloorGrid = (
  ...
  for (let i = 0; i < GRID_TOTAL; i++) {
    if (
      i !== entryIndex &&
      manhattanDistance(entryIndex, i) >= MIN_STAIRS_DISTANCE
    ) {
  ...
  const bossCount = dist.bossCount
    ? randInt(dist.bossCount[0], dist.bossCount[1])
    : 0;
  let totalMonsters = 0;
  if (bossCount > 0) {
    // 中心 2×2 区域（行2-3，列2-3）
    ...
    if (centerCandidates.length > 0) {
      ...
      if (isSkullCavern) {
        ...
        bossMonster = isFirstKill
          ? BOSS_MONSTERS[floorNum]
          : getWeakenedBoss(floorNum);
      }
  ...
  // 暗河层使用特殊矿石池
  const orePool =
    floorData.specialType === "dark" ? getDarkFloorOres() : floorData.ores;
