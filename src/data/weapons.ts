...
export const rollRandomEnchantment = (): string | null => {
  ...
  return RANDOM_ENCHANT_IDS[
    Math.floor(Math.random() * RANDOM_ENCHANT_IDS.length)
  ]!;
};
...
export const SHOP_WEAPONS: WeaponDef[] = Object.values(WEAPONS).filter(
  (w) => w.shopPrice !== null,
);

/** 各区域怪物可掉落的武器 ID */
export const MONSTER_DROP_WEAPONS: Record<
  string,
...
/** 计算武器卖出价格 */
export const getWeaponSellPrice = (
  defId: string,
  enchantmentId: string | null,
): number => {
  const def = WEAPONS[defId];
  if (!def) return 0;
  const base = def.shopPrice
    ? Math.floor(def.shopPrice * 0.5)
    : def.attack * 15;
  // 附魔额外加价
  if (enchantmentId) {
