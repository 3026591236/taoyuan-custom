...
/** 收获育种作物时按品质返还种子的概率 */
export const SEED_RETURN_CHANCE = {
    normal: 0.5,
    fine: 0.7,
    excellent: 0.9,
    supreme: 1.0
...
/** 判断收获时是否返还育种种子 */
export const shouldReturnBreedingSeed = (quality) => {
...
/** 生成唯一基因ID */
export const generateGeneticsId = () => {
    return `gen_${Date.now()}_${_nextGeneticsId++}`;
};
/** 限制属性值在 0-100 */
export const clampStat = (value) => {
    return Math.max(0, Math.min(STAT_CAP, Math.round(value)));
};
/** 限制变异率在 1-50 */
export const clampMutationRate = (value) => {
    return Math.max(1, Math.min(50, Math.round(value)));
};
/** 根据作物ID计算默认基因属性 */
export const getDefaultGenetics = (cropId) => {
...
/** 计算总属性 */
export const getTotalStats = (g) => {
    return g.sweetness + g.yield + g.resistance;
};
/** 获取星级评分 */
export const getStarRating = (g) => {
...
for (let t = 0; t < TIER_COUNTS.length; t++) {
    for (let i = 0; i < TIER_COUNTS[t]; i++) {
        ...
        if (def)
            _tierMap.set(def.id, t + 1);
    }
    _offset += TIER_COUNTS[t];
}
/** 获取杂交品种所属阶层 (1-10) */
export const getHybridTier = (hybridId) => _tierMap.get(hybridId) ?? 1;
/** 查找可能的杂交配方 */
export const findPossibleHybrid = (cropIdA, cropIdB) => {
    return (HYBRID_DEFS.find(h => (h.parentCropA === cropIdA && h.parentCropB === cropIdB) || (h.parentCropA === cropIdB && h.parentCropB === cropIdA)) ?? null);
};
/** 根据杂交种ID查找配方 */
export const findPossibleHybridById = (hybridId) => {
    return HYBRID_DEFS.find(h => h.id === hybridId) ?? null;
};
/** 种子制造机产出育种种子的概率 */
export const getSeedMakerGeneticChance = (farmingLevel) => {
