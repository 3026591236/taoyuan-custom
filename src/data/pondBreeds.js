...
/** 全部品种定义（400条） */
export const POND_BREEDS = buildAllBreeds();
/** 根据品种ID查找 */
export const getBreedById = (breedId) => POND_BREEDS.find(b => b.breedId === breedId);
/** 获取指定代数的所有品种 */
export const getBreedsByGeneration = (gen) => POND_BREEDS.filter(b => b.generation === gen);
/** 获取指定鱼种的所有品种 */
export const getBreedsBySpecies = (baseFishId) => POND_BREEDS.filter(b => b.baseFishId === baseFishId);
/** 获取指定鱼种的 Gen1 品种列表 */
export const getGen1BreedsForFish = (fishId) => POND_BREEDS.filter(b => b.generation === 1 && b.baseFishId === fishId);
/** 根据亲本品种ID查找子代品种（顺序无关） */
export const findBreedByParents = (breedIdA, breedIdB) => POND_BREEDS.find(b => (b.parentBreedA === breedIdA && b.parentBreedB === breedIdB) || (b.parentBreedA === breedIdB && b.parentBreedB === breedIdA));
/** 各代品种数量 */
export const BREED_COUNTS = {
    1: 200,
    2: 100,
    3: 50,
    4: 30,
