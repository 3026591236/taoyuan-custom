/** 瀚海驿站固定商品（每周都有） */
export const HANHAI_FIXED_ITEMS = [
...
/** 瀚海驿站轮换商品池（每周随机抽4个上架） */
export const HANHAI_ROTATING_POOL = [
...
/** 兼容旧代码：合并固定+轮换池的全部商品 */
export const HANHAI_SHOP_ITEMS = [...HANHAI_FIXED_ITEMS, ...HANHAI_ROTATING_POOL];
/** 根据年份+季节+周数生成本周轮换商品（确定性伪随机） */
export const getWeeklyRotatingItems = (year, seasonIndex, day) => {
...
/** 轮盘投注档位 */
export const ROULETTE_BET_TIERS = [100, 500, 1000];
/** 骰子投注金额 */
...
/** 根据ID获取场次配置 */
export const getTexasTier = (id) => TEXAS_TIERS.find(t => t.id === id);
/** 花色显示符号 */
...
/** 庄家AI决策 */
export const texasDealerAI = (dealerHole, community, street, pot, dealerStack, playerBet, dealerBet, playerAllIn, blind) => {
    ...
    if (visibleCards.length >= 5) {
        ...
        if (paired || (highCard && suited))
            strength = 2;
        else if (highCard || suited)
            strength = 1;
    }
    // 玩家已全押 → 只能跟或弃
    if (playerAllIn) {
...
/** 生成弹仓（打乱顺序） */
export const loadShotgun = () => {
    const shells = [];
    for (let i = 0; i < BUCKSHOT_LIVE_COUNT; i++)
        shells.push('live');
    for (let i = 0; i < BUCKSHOT_BLANK_COUNT; i++)
        shells.push('blank');
    // Fisher-Yates
...
/** 庄家AI决策 */
export const dealerDecide = (shells, currentIndex, knowsCurrent) => {
    if (knowsCurrent) {
...
// === 通商系统 ===
/** 通商店铺升级定义 */
export const TRADE_SHOP_UPGRADES = [
...
/** 积分兑换商品 */
export const TRADE_EXCHANGE_ITEMS = [
...
/** 积分计算：物品售价转通商积分 */
export const TRADE_QUALITY_MULTIPLIER = {
    normal: 1.0,
    fine: 1.25,
    excellent: 1.5,
    supreme: 2.0
};
export const calcTradePoints = (sellPrice, quality) => {
