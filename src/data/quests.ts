...
const SPECIAL_ORDER_TEMPLATES: SpecialOrderTemplate[] = [
  ...
  },
  {
    ...
    orderTag: "宗门灵植",
    description:
      "丹房炼回灵丹缺少凝露草，完成后可获得灵石和青丘旧林常见的木灵珠。",
  },
  {
    name: "筑基丹主材",
  ...
  },
  {
    ...
    orderTag: "高阶资源",
    description:
      "登仙塔镇塔器出现裂纹，需要星陨铁、法宝碎片和雷精修复；这是高价值材料回收订单。",
  },
  {
    name: "护山灵阵补材",
    ...
    orderTag: "高阶资源",
    description:
      "护山灵阵入夜灵压不稳，需仙露、灵骨与日炎晶补阵；奖励大量灵石与炼器材料。",
  },
];
...
export const generateQuest = (
  ...
  // 按季节过滤目标
  const validTargets = template.targets.filter(
  ...
  // 从候选池随机选择 NPC
  const npcId =
    template.npcPool[Math.floor(Math.random() * template.npcPool.length)]!;
  ...
  const quantity =
    target.minQty +
    Math.floor(Math.random() * (target.maxQty - target.minQty + 1));

  // 奖励计算
  const moneyReward = Math.floor(
    target.unitPrice * quantity * template.rewardMultiplier,
  );
