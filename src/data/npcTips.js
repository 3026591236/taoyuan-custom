...
/** 根据 dailyLuck 获取运势台词 */
export const getFortuneTip = (luck) => {
...
/** 获取当天的生活提示 */
export const getLivingTip = (day, year) => {
    const index = ((year - 1) * 112 + day - 1) % LIVING_TIPS.length;
    return LIVING_TIPS[index];
};
/** 王大婶 - 食谱推荐台词模板 */
export const getRecipeTipMessage = (recipeName, ingredientNames) => {
    return `今天教你做${recipeName}，需要${ingredientNames.join('、')}。`;
};
/** 王大婶 - 无可推荐食谱时的通用台词 */
export const NO_RECIPE_TIP = '好好学做饭，以后日子还长着呢。';
/** 有每日提示功能的NPC ID列表 */
export const TIP_NPC_IDS = ['li_yu', 'zhou_xiucai', 'wang_dashen', 'liu_cunzhang'];
/** NPC提示标签 */
export const TIP_NPC_LABELS = {
    li_yu: '天气预报',
    zhou_xiucai: '今日运势',
    wang_dashen: '食谱推荐',
    liu_cunzhang: '生活提示'
