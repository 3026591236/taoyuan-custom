...
/** 根据等级获取山洞升级定义 */
export const getCaveUpgrade = (level) => {
    return CAVE_UPGRADES.find(u => u.level === level);
};
/** 根据天数获取山洞品质 */
export const getCaveQuality = (daysActive) => {
    let result = 'normal';
    for (const t of CAVE_QUALITY_THRESHOLDS) {
        if (daysActive >= t.days)
            result = t.quality;
    }
    return result;
};
...
export const getCellarUpgrade = (level) => CELLAR_UPGRADES.find(u => u.level === level);
export const getFarmhouseUpgrade = (level) => {
