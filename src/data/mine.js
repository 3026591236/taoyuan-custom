...
/** 蘑菇层奖励 */
export const getMushroomRewards = (floor) => {
    if (floor <= 20) {
        return [
    ...
    if (floor <= 40) {
        ...
        if (Math.random() < 0.1)
            rewards.push({ itemId: 'ginseng', quantity: 1 });
        return rewards;
    }
    if (floor <= 60) {
        return [
...
/** 宝箱层奖励 */
export const getTreasureRewards = (floor) => {
...
/** 暗河层陷阱伤害（随深度增加） */
export const getDarkFloorTrapDamage = (floor) => {
    const zoneIndex = Math.floor((floor - 1) / 20);
...
/** 暗河层隐藏宝藏铜钱 */
export const getDarkFloorTreasureMoney = (floor) => {
    const zoneIndex = Math.floor((floor - 1) / 20);
...
/** 感染层清除奖励 */
export const getInfestedClearRewards = (floor) => {
    const zoneIndex = Math.floor((floor - 1) / 20);
    const zones = ['shallow', 'frost', 'lava', 'crystal', 'shadow', 'abyss'];
    const zone = zones[zoneIndex] ?? 'abyss';
    const orePool = ZONE_ORES[zone];
...
/** 获取奖励物品名称列表 */
export const getRewardNames = (items) => {
    return items
        .map(r => {
...
/** 骷髅矿穴专属怪物 */
export const SKULL_CAVERN_MONSTERS = {
    iridium_golem: {
        id: 'iridium_golem',
        name: '铱金魔像',
        hp: 400,
...
/** BOSS 额外掉落铜钱 */
export const BOSS_MONEY_REWARDS = {
    20: 100,
    40: 200,
    60: 500,
    80: 800,
...
export const getWeakenedBoss = (floor) => {
...
/** 生成骷髅矿穴楼层 */
export const generateSkullCavernFloor = (floor) => {
    ...
    // 怪物池
    const monsterPool = Object.values(SKULL_CAVERN_MONSTERS);
    if (floor > 10) {
...
/** 缩放怪物属性（骷髅矿穴用） */
export const scaleMonster = (monster, scaleFactor) => {
    return {
        ...monster,
...
export const getSkullCavernBoss = (floor) => {
    if (floor % 25 !== 0)
        return undefined;
...
export const getAdjacentIndices = (index) => {
    ...
    const adj = [];
    if (row > 0)
        adj.push((row - 1) * GRID_SIZE + col);
    if (row < GRID_SIZE - 1)
        adj.push((row + 1) * GRID_SIZE + col);
    if (col > 0)
        adj.push(row * GRID_SIZE + col - 1);
    if (col < GRID_SIZE - 1)
        adj.push(row * GRID_SIZE + col + 1);
    return adj;
};
...
/** 获取炸弹范围内的索引 */
export const getBombIndices = (center, bombId) => {
    ...
    if (bombId === 'cherry_bomb') {
        ...
        // 曼哈顿距离 ≤ 2（十字+菱形）
        for (let i = 0; i < GRID_TOTAL; i++) {
            if (manhattanDistance(center, i) <= 2)
                indices.push(i);
        }
    }
...
/** 获取楼层格子分布配置 */
export const getFloorDistribution = (specialType) => {
    switch (specialType) {
        case 'mushroom':
            return {
                oreCount: [1, 2],
...
/** 生成楼层 6×6 格子 */
export const generateFloorGrid = (floorData, floorNum, isSkullCavern, scaleFactor) => {
    ...
    // 已占用位置集合
    const used = new Set([entryIndex, stairsIndex]);
    /** 在随机空闲位置放置格子 */
    const placeRandom = (type, data) => {
        const candidates = [];
        for (let i = 0; i < GRID_TOTAL; i++) {
            if (!used.has(i))
                candidates.push(i);
    ...
    for (let i = 0; i < monsterCount; i++) {
        if (floorData.monsters.length === 0)
            break;
    ...
    if (dist.mushroomCount) {
        ...
        for (let i = 0; i < mushCount; i++) {
            ...
            if (floorNum <= 40) {
                ...
                if (Math.random() < 0.3)
                    items.push({ itemId: 'herb', quantity: 1 });
            }
