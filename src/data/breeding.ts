...
export const getDefaultGenetics = (
  ...
  const baseResistance = clampStat(
    10 + Math.round((priceScore + growthScore) * 15),
  );
...
/** 获取杂交品种所属阶层 (1-10) */
export const getHybridTier = (hybridId: string): number =>
  _tierMap.get(hybridId) ?? 1;

/** 查找可能的杂交配方 */
export const findPossibleHybrid = (
  cropIdA: string,
  cropIdB: string,
): HybridDef | null => {
  return (
    HYBRID_DEFS.find(
      (h) =>
        (h.parentCropA === cropIdA && h.parentCropB === cropIdB) ||
        (h.parentCropA === cropIdB && h.parentCropB === cropIdA),
    ) ?? null
  );
};
