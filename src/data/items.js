...
const SEED_ITEMS = CROPS.filter(crop => crop.seedId !== 'ancient_seed' && crop.seedId !== 'hanhai_cactus_seed' && crop.seedId !== 'hanhai_date_seed').map(crop => ({
    id: crop.seedId,
    name: `${crop.name}种子`,
    category: 'seed',
...
/** 从作物定义自动生成收获物品 */
const CROP_ITEMS = CROPS.map(crop => ({
    id: crop.id,
    name: crop.name,
    category: 'crop',
...
/** 从鱼定义自动生成鱼物品 */
const FISH_ITEMS = FISH.map(fish => ({
    id: fish.id,
    name: fish.name,
    category: 'fish',
...
/** 从食谱定义自动生成烹饪物品 */
const _preFoodItems = [...SEED_ITEMS, ...CROP_ITEMS, ...ORE_ITEMS, ...MISC_ITEMS, ...FISH_ITEMS];
const FOOD_ITEMS = RECIPES.map(recipe => {
...
/** 机器物品 */
const MACHINE_ITEMS = PROCESSING_MACHINES.map(m => ({
    id: `machine_${m.id}`,
    name: m.name,
    category: 'machine',
...
/** 洒水器物品 */
const SPRINKLER_ITEMS = SPRINKLERS.map(s => ({
    id: s.id,
    name: s.name,
    category: 'sprinkler',
...
/** 肥料物品 */
const FERTILIZER_ITEMS = FERTILIZERS.map(f => ({
    id: f.id,
    name: f.name,
    category: 'fertilizer',
...
/** 浮漂物品 */
const TACKLE_ITEMS = TACKLES.map(t => ({
    id: t.id,
    name: t.name,
    category: 'tackle',
...
/** 果树水果 */
const FRUIT_TREE_ITEMS = FRUIT_TREE_DEFS.map(t => ({
    id: t.fruitId,
    name: t.fruitName,
    category: 'fruit',
...
/** 树苗 */
const SAPLING_ITEMS = FRUIT_TREE_DEFS.map(t => ({
    id: t.saplingId,
    name: `${t.name}苗`,
    category: 'sapling',
...
/** 武器图鉴物品 */
const WEAPON_ITEMS = Object.values(WEAPONS).map(w => ({
    id: w.id,
    name: w.name,
    category: 'weapon',
...
/** 获取物品来源描述 */
export const getItemSource = (itemId) => {
...
/** 箱子阶梯顺序 */
export const CHEST_TIER_ORDER = ['wood', 'copper', 'iron', 'gold', 'void'];
