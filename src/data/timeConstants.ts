...
const isNpcBirthday = (npcId: string, season: Season, day: number): boolean => {
  ...
  return (
    !!npc?.birthday &&
    npc.birthday.season === season &&
    npc.birthday.day === day
  );
};

export const isNpcAvailable = (
  npcId: string,
  day: number,
  hour: number,
  season?: Season,
  ...
  if (season && isNpcBirthday(npcId, season, day)) {
    return (
      hour >= schedule.availableHours.from && hour < schedule.availableHours.to
    );
  }
  const weekday = getWeekday(day);
  if (
    schedule.availableDays !== "all" &&
    !schedule.availableDays.includes(weekday)
  )
    return false;
  return (
    hour >= schedule.availableHours.from && hour < schedule.availableHours.to
...
export const getNpcUnavailableReason = (
  npcId: string,
  day: number,
  hour: number,
  season?: Season,
