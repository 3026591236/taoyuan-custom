...
export const formatHour = (hour) => {
    ...
    if (hour >= 6 && hour < 12)
        return `上午 ${realHour}:00`;
    if (hour === 12)
        return `中午 12:00`;
    if (hour > 12 && hour < 18)
        return `下午 ${realHour}:00`;
    if (hour >= 18 && hour < 24)
        return `晚上 ${realHour}:00`;
...
// === 地点分组映射 ===
export const TAB_TO_LOCATION_GROUP = {
    farm: 'farm',
    animal: 'farm',
    home: 'farm',
    cottage: 'farm',
...
export const getLocationGroupName = (group) => {
...
export const isShopOpen = (tabKey, day, hour) => {
...
/** 检查今天是否为该NPC的生日 */
const isNpcBirthday = (npcId, season, day) => {
    const npc = getNpcById(npcId);
    return !!npc?.birthday && npc.birthday.season === season && npc.birthday.day === day;
};
export const isNpcAvailable = (npcId, day, hour, season) => {
...
export const getNpcUnavailableReason = (npcId, day, hour, season) => {
    const schedule = NPC_SCHEDULES.find(s => s.npcId === npcId);
    if (!schedule)
        return null;
    // 生日当天一定在村里
    ...
    if (hour < schedule.availableHours.from)
        return '还没出门';
    if (hour >= schedule.availableHours.to)
        return '已经回家了';
    return null;
};
// === 深夜惩罚 ===
