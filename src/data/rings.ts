...
/** 所有可合成的戒指 */
export const CRAFTABLE_RINGS: RingDef[] = RINGS.filter(
  (r) => r.recipe !== null,
);

/** 各区域怪物可掉落的戒指 */
export const MONSTER_DROP_RINGS: Record<
  string,
  { ringId: string; chance: number }[]
> = {
...
/** 宝箱层可掉落的戒指（按区域） */
export const TREASURE_DROP_RINGS: Record<
  string,
  { ringId: string; chance: number }[]
> = {
