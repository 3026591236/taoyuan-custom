...
/** 根据ID获取帽子定义 */
export const getHatById = (id) => {
    return HATS.find(h => h.id === id);
};
/** 绸缎庄可购买的帽子 */
export const SHOP_HATS = HATS.filter(h => h.shopPrice !== null);
/** 铁匠铺可合成的帽子 */
export const CRAFTABLE_HATS = HATS.filter(h => h.recipe !== null);
/** 怪物掉落帽子（按矿洞区域） */
export const MONSTER_DROP_HATS = {
    shallow: [],
...
/** 宝箱掉落帽子（按矿洞区域） */
export const TREASURE_DROP_HATS = {
