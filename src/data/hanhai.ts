...
export const getWeeklyRotatingItems = (
  year: number,
  seasonIndex: number,
  day: number,
): HanhaiShopItemDef[] => {
...
/** 根据ID获取场次配置 */
export const getTexasTier = (id: TexasTierId): TexasTierDef =>
  TEXAS_TIERS.find((t) => t.id === id)!;
...
export const evaluateHand = (cards: PokerCard[]): PokerHandResult => {
  ...
  // 按频率降序、同频率按rank降序排列
  const groupedRanks = [...freq.entries()]
