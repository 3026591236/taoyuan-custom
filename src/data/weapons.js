...
/** 随机获取一个附魔（30% 概率触发） */
export const rollRandomEnchantment = () => {
    if (Math.random() >= 0.3)
        return null;
...
export const SHOP_WEAPONS = Object.values(WEAPONS).filter(w => w.shopPrice !== null);
/** 各区域怪物可掉落的武器 ID */
export const MONSTER_DROP_WEAPONS = {
    shallow: [
...
/** BOSS 掉落武器映射 */
export const BOSS_DROP_WEAPONS = {
    20: 'mud_king_fang',
...
/** 根据 ID 获取附魔定义 */
export const getEnchantmentById = (id) => {
    return ENCHANTMENTS[id];
};
/** 计算武器卖出价格 */
export const getWeaponSellPrice = (defId, enchantmentId) => {
    ...
    if (enchantmentId) {
        ...
        if (enchant)
            return base + 100 + enchant.attackBonus * 20;
    }
    return base;
};
/** 获取附魔武器的显示名称 */
export const getWeaponDisplayName = (defId, enchantmentId) => {
...
/** 宝箱掉落武器（按矿洞区域） */
export const TREASURE_DROP_WEAPONS = {
    shallow: [],
