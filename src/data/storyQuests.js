...
/** 根据ID获取主线任务 */
export const getStoryQuestById = (id) => {
    return STORY_QUESTS.find(q => q.id === id);
};
/** 根据章节和序号获取主线任务 */
export const getStoryQuestByOrder = (chapter, order) => {
    return STORY_QUESTS.find(q => q.chapter === chapter && q.order === order);
};
/** 获取下一个主线任务 */
export const getNextStoryQuest = (currentId) => {
    const idx = STORY_QUESTS.findIndex(q => q.id === currentId);
    if (idx === -1 || idx >= STORY_QUESTS.length - 1)
        return undefined;
    return STORY_QUESTS[idx + 1];
};
/** 获取某章的所有主线任务 */
export const getChapterQuests = (chapter) => {
    return STORY_QUESTS.filter(q => q.chapter === chapter);
};
/** 获取第一个主线任务 */
export const getFirstStoryQuest = () => {
