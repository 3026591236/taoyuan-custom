  ...
  <div class="game-panel max-w-sm w-full">
    ...
    <div class="flex items-center justify-center space-x-2 mb-2">
      <span class="text-xs text-muted"
        >第 {{ currentRound }}/{{ tier.rounds }} 手</span
      >
    ...
    <!-- 操作按钮 -->
    <div
      v-if="!handOver && isPlayerTurn && !animating"
    ...
    >
      ...
      <template v-else>
        <Button class="flex-1 justify-center" @click="doCall"
          >跟注{{ toCall }}</Button
        >
        <Button
          v-if="!dealerAllIn && playerStack > toCall"
          class="flex-1 justify-center"
          @click="doRaise(dealerBetRound + tier.blind * 2)"
        >
          加注{{ toCall + tier.blind * 2 }}
        </Button>
      </template>
      <Button v-if="!dealerAllIn" class="flex-1 justify-center" @click="doAllIn"
        >全押</Button
      >
      <Button class="flex-1 justify-center text-danger" @click="doFold"
        >弃牌</Button
      >
    </div>
    ...
    <template v-if="sessionOver && finalResult">
      <div class="border border-accent/10 rounded-xs p-3 text-center mb-2">
        ...
        <p
          ...
          :class="finalResult.netProfit >= 0 ? 'text-success' : 'text-danger'"
        >
          {{
            finalResult.netProfit >= 0
              ? "+" + finalResult.netProfit + "文"
              : finalResult.netProfit + "文"
          }}
        </p>
        <p class="text-xs text-muted mt-0.5">荷官小费 {{ tier.rake }}文</p>
        <p class="text-xs text-muted mt-0.5">
          共 {{ currentRound }} 手 · 最终筹码 {{ playerStack }}
        </p>
      </div>
      <Button
        class="w-full justify-center"
        @click="emit('complete', playerStack, tier.name)"
        >确定</Button
      >
    </template>
  </div>
...
const toCall = computed(() =>
  Math.max(0, dealerBetRound.value - playerBetRound.value),
);
...
const checkRoundEnd = (playerActed: boolean) => {
  ...
  const settled =
    pBet === dBet ||
    (pBet < dBet && playerAllIn.value) ||
    (dBet < pBet && dealerAllIn.value);
