  ...
  <div class="game-panel max-w-sm w-full">
    ...
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">
        三条龙舟蓄势待发！疯狂点击「划桨」按钮让龙舟前进，10秒内看谁划得最远！
      </p>
    ...
    <div v-else-if="phase === 'racing'">
      <div class="flex items-center justify-between mb-2">
        ...
        <p
          ...
          :class="timeLeft <= 3 ? 'text-danger time-pulse' : 'text-accent'"
        >
          {{ timeLeft }}s
        </p>
      </div>
      ...
      <div class="border border-accent/20 p-2 mb-3">
        <div v-for="(boat, i) in boats" :key="i" class="mb-2 last:mb-0">
          <div class="flex items-center space-x-2 mb-0.5">
            ...
            <span class="text-xs text-muted flex-1 text-right"
              >{{ boat.progress }}m</span
            >
          </div>
          <!-- 赛道条 -->
          <div
      ...
      <div class="border border-accent/20 mb-3">
        ...
        >
          <div class="flex items-center space-x-2">
            ...
            <span :class="{ 'text-accent': entry.name === '你' }">{{
              entry.name
            }}</span>
          </div>
