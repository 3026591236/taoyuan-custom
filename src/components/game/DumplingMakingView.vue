  ...
  <div class="game-panel max-w-sm w-full">
    ...
    <div v-else-if="phase === 'making'">
      <div class="flex items-center justify-between mb-2">
        ...
        <p
          class="text-xs"
          :class="timeLeft <= 5 ? 'text-danger time-pulse' : 'text-accent'"
        >
      ...
      <!-- 饺子计数器 -->
      <div
        v-if="dumplingCount > 0"
        class="flex flex-wrap space-x-1 mb-2 justify-center"
      >
        <Cookie
          v-for="i in Math.min(dumplingCount, 10)"
          :key="i"
          :size="14"
          class="text-accent dumpling-icon"
        />
        <span v-if="dumplingCount > 10" class="text-xs text-muted"
          >+{{ dumplingCount - 10 }}</span
        >
      </div>
      ...
      <div class="border border-accent/20 p-3 mb-3 text-center">
        ...
        <!-- 错误提示 -->
        <p v-if="showError" class="text-danger text-xs mt-1 wrong-shake">
          顺序不对！重来！
        </p>
    ...
    <div v-else>
      ...
      >
        <Cookie
          ...
          class="text-accent"
        />
        <span v-if="dumplingCount > 10" class="text-xs text-muted self-center"
          >+{{ dumplingCount - 10 }}</span
        >
      </div>
