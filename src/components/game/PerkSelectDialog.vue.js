...
const options = computed(() => {
    if (props.level === 5)
        return PERK5_OPTIONS[props.skillType];
...
(__VLS_ctx.SKILL_NAMES[__VLS_ctx.skillType]);
(__VLS_ctx.level);
...
for (const [option] of __VLS_vFor((__VLS_ctx.options))) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
        ...{ onClick: (...[$event]) => {
                return __VLS_ctx.handleSelect(option.id);
                // @ts-ignore
                [SKILL_NAMES, skillType, level, options, handleSelect,];
            } },
        key: (option.id),
