  ...
  <div class="game-panel max-w-xs w-full">
    ...
    <div class="border border-accent/20 rounded-xs p-2 mb-3">
      <div class="flex items-center justify-between text-xs">
        ...
        <span>
          <span v-for="i in liveRemaining" :key="'l' + i" class="text-danger"
            >&bull;</span
          >
          <span v-for="i in blankRemaining" :key="'b' + i" class="text-muted"
            >&bull;</span
          >
          <span class="text-muted ml-1"
            >({{ liveRemaining }}实{{ blankRemaining }}空)</span
          >
        </span>
      </div>
    ...
    <div class="flex items-center space-x-3 mb-3">
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between text-xs mb-0.5">
          <span
            :class="isPlayerTurn && !gameOver ? 'text-accent' : 'text-muted'"
            >你</span
          >
        ...
        <div
          ...
          :class="{ 'buckshot-flash-red': playerHit }"
        >
          <div
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between text-xs mb-0.5">
          <span
            :class="!isPlayerTurn && !gameOver ? 'text-danger' : 'text-muted'"
            >庄家</span
          >
        ...
        <div
          class="h-1.5 bg-panel rounded-full overflow-hidden"
          :class="{ 'buckshot-flash-red': dealerHit }"
        >
          <div
            class="h-full transition-all duration-300"
            :class="
    ...
    <template v-if="gameOver">
      <div class="border border-accent/10 rounded-xs p-3 text-center mb-3">
        <p
          class="text-sm"
          :class="won ? 'text-success' : draw ? 'text-accent' : 'text-danger'"
        >
          {{ won ? "你赢了！" : draw ? "平局" : "你输了…" }}
        </p>
        <p
      ...
      <Button class="w-full justify-center" @click="emit('complete', won, draw)"
        >确定</Button
      >
    </template>
  </div>
