<template>
  <div class="game-panel max-w-sm w-full">
    ...
    <div v-else-if="phase === 'tension'">
      <p class="text-xs text-center mb-2 text-accent bite-flash">
        鱼上钩了！注意张力！
      </p>
      ...
      <div class="flex space-x-3 items-stretch mb-3">
        ...
        >
          ...
          <span
            ...
            style="bottom: 65%; font-size: 8px; color: var(--color-success)"
            >佳</span
          >
          <span
            class="absolute text-center w-full"
            style="bottom: 88%; font-size: 8px; color: var(--color-danger)"
            >断</span
          >
        </div>
        ...
        >
          ...
          <div
            ...
            :style="{ top: `${fishVisualY}%`, left: `${fishVisualX}%` }"
          >
          ...
          <span
            ...
            style="font-size: 9px"
            >第{{ currentRound }}轮</span
          >
        </div>
      </div>

      <Button class="w-full py-2.5" :icon="ArrowUp" @click="pullRod"
        >收竿！</Button
      >
      ...
      <div :class="resultAnimClass">
        ...
        <div v-if="lastGrade !== 'escaped'" class="mt-2">
          ...
          <p class="text-xs text-muted">
            {{ catches[catches.length - 1]?.weight }}斤 · +{{
              catches[catches.length - 1]?.score
            }}分
          </p>
        </div>
        ...
        >
          <div>
            <span
              ...
              >第{{ i + 1 }}名</span
            >
            <span :class="{ 'text-success': entry.name === '你' }">{{
              entry.name
            }}</span>
          </div>
...
type Phase =
  "ready" | "casting" | "waiting" | "tension" | "round_result" | "finished";
...
const playerTotal = computed(() =>
  catches.value.reduce((sum, c) => sum + c.score, 0),
);
...
const randomFish = (grade: "perfect" | "good" | "poor"): CatchRecord => {
  ...
  const weight = +(fish.minW + Math.random() * (fish.maxW - fish.minW)).toFixed(
    1,
  );
  // 权重乘数封顶1.8，避免极端高分
...
const startTension = () => {
  ...
  fishMoveTimer = setInterval(() => {
    fishVisualX.value = Math.max(
      5,
      Math.min(75, fishVisualX.value + (Math.random() - 0.5) * 15),
    );
    fishVisualY.value = Math.max(
      5,
      Math.min(75, fishVisualY.value + (Math.random() - 0.5) * 15),
    );
  }, 200);
};
...
const pullRod = () => {
  ...
  if (grade !== "escaped") {
    const fishGrade =
      grade === "perfect" ? "perfect" : grade === "good" ? "good" : "poor";
...
const finishContest = () => {
  ...
  const npcScores = npcProfiles.map(({ name, perfectRate, goodRate }) => {
    ...
    for (let i = 0; i < 3; i++) {
      const r = Math.random();
      const grade =
        r < perfectRate ? "perfect" : r < goodRate ? "good" : ("poor" as const);
