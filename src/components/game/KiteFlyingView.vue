  ...
  <div class="game-panel max-w-sm w-full">
    ...
    <div v-else-if="phase === 'playing'">
      ...
      <div class="text-center mb-2">
        <p
          class="text-xs"
          :class="windDirection > 0 ? 'text-success' : 'text-danger'"
        >
      ...
      <div class="relative h-10 bg-bg border border-accent/20 mb-3">
        ...
        >
          <span
            class="text-lg"
            :class="inSafeZone ? 'kite-float' : 'kite-shake'"
            >🪁</span
          >
        </div>
        <!-- 区域标签 -->
      ...
      <div class="flex space-x-2">
        <Button class="flex-1 py-2" :icon="ArrowLeft" @click="pullLeft"
          >拉左</Button
        >
...
const inSafeZone = computed(
  () => kitePosition.value >= 32 && kitePosition.value <= 68,
);
