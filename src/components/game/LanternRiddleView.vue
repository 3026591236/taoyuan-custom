  ...
  <div class="game-panel max-w-sm w-full">
    ...
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">
        广场上挂满了灯笼，每个灯笼下都有一条灯谜。共5题，每题限时，答对有奖！
      </p>
    ...
    <div v-else-if="phase === 'answering'">
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <p
          class="text-xs"
          :class="countdown <= 3 ? 'text-danger time-pulse' : 'text-accent'"
        >
    ...
    <div v-else-if="phase === 'result'" class="text-center">
      ...
      <div
        :class="lastCorrect ? 'correct-flash' : 'wrong-shake'"
        class="mb-3 py-3 border border-accent/20"
      >
        <p
          class="text-sm mb-1"
          :class="lastCorrect ? 'text-success' : 'text-danger'"
        >
        ...
        <p class="text-xs text-muted mt-1">
          ...
          <span class="text-accent">{{
            currentRiddle.options[currentRiddle.answer]
          }}</span>
        </p>
      </div>
    ...
    <div v-else>
      ...
      <div class="border border-accent/20 p-3 mb-3 text-center">
        ...
        <p class="text-xs">
          ...
          <span class="text-accent">{{ score }}</span>
          文
          <span v-if="correctCount === 5" class="text-accent finish-flash"
            >（全对+300文！）</span
          >
        </p>
      </div>
...
const RIDDLE_POOL: Riddle[] = [
  ...
  {
    question:
      "圆圆脸儿像苹果，又酸又甜营养多，既能做菜吃，又能当水果。（打一蔬果）",
    options: ["番茄", "苹果", "桃子", "杏子"],
    answer: 0,
  },
  {
  ...
  {
    question:
      "一根竹管二尺长，开了七个小圆窗，对准一个窗口吹，悠悠乐声传四方。（打一乐器）",
    options: ["箫", "笛子", "埙", "琵琶"],
    answer: 1,
  },
  {
  ...
  {
    question:
      "四四方方一座城，城里住着十万兵，派出将军去攻打，万马奔腾杀敌人。（打一物品）",
    options: ["棋盘", "算盘", "印章", "砚台"],
    answer: 0,
  },
];
