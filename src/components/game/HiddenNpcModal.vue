    ...
    <div class="game-panel max-w-lg w-full max-h-[80vh] overflow-y-auto">
      ...
      <div class="flex justify-between items-start mb-2">
        <div>
          ...
          <p v-if="showTrueName" class="text-[10px] text-accent/60 mt-0.5">
            真名：{{ npcDef.trueName }}
          </p>
        </div>
      ...
      <div class="border border-accent/10 rounded-xs p-2 mb-2">
        <div class="flex items-center justify-between mb-1">
          <div class="flex items-center space-x-px">
            <Diamond
              ...
              class="flex-shrink-0"
              :class="
                state.affinity >= d * 250 ? 'text-accent' : 'text-muted/20'
              "
        ...
        <div class="flex items-center space-x-1.5 flex-wrap">
          ...
          <span
            v-if="hiddenNpcStore.isManifestationDay(npcId)"
      ...
      <div class="mb-3">
        <Button
          class="w-full"
          :disabled="
            state.interactedToday || state.specialInteractionCooldown > 0
          "
          @click="handleInteraction"
        >
          {{ INTERACTION_NAMES[npcDef.interactionType] }}
          <span
            v-if="state.specialInteractionCooldown > 0"
            class="text-muted ml-1"
            >(冷却{{ state.specialInteractionCooldown }}天)</span
          >
        </Button>
      </div>
        ...
        <template v-else-if="state.courting">
          ...
          <div class="flex flex-col space-y-0.5 mb-1.5">
            <span
              ...
              :class="
                state.affinity >= npcDef.bondThreshold
                  ? 'text-success'
                  : 'text-muted/50'
              "
            >
              <CircleCheck
                v-if="state.affinity >= npcDef.bondThreshold"
                :size="10"
              />
            ...
            <span
              class="text-[10px] flex items-center space-x-0.5"
              :class="
                inventoryStore.hasItem(npcDef.bondItemId)
                  ? 'text-success'
                  : 'text-muted/50'
              "
            >
              <CircleCheck
                v-if="inventoryStore.hasItem(npcDef.bondItemId)"
                :size="10"
              />
              <Circle v-else :size="10" />
              持有{{
                getItemById(npcDef.bondItemId)?.name ?? npcDef.bondItemId
              }}
            </span>
          </div>
          <!-- 结缘物品制作 -->
          ...
          >
            <p class="text-[10px] text-muted/60 mb-1">
              制作{{ getItemById(npcDef.bondItemId)?.name }}：
            </p>
            <div class="flex flex-col space-y-0.5 mb-1">
              <span
                ...
                class="text-[10px]"
                :class="
                  inventoryStore.getItemCount(cost.itemId) >= cost.quantity
                    ? 'text-success'
                    : 'text-muted/50'
                "
              >
                {{ getItemById(cost.itemId)?.name ?? cost.itemId }} ×{{
                  cost.quantity
                }}
                <span class="text-muted/30"
                  >（持有{{ inventoryStore.getItemCount(cost.itemId) }}）</span
                >
              </span>
            </div>
            <Button
              class="w-full"
              :disabled="!canCraftBond"
              @click="handleCraft('bond')"
              >制作</Button
            >
          </div>
          <Button
        ...
        <template v-else>
          <div class="flex flex-col space-y-0.5 mb-1.5">
            <span
              class="text-[10px] flex items-center space-x-0.5"
              :class="
                state.affinity >= npcDef.courtshipThreshold
                  ? 'text-success'
                  : 'text-muted/50'
              "
            >
              <CircleCheck
                v-if="state.affinity >= npcDef.courtshipThreshold"
                :size="10"
              />
              <Circle v-else :size="10" />
            ...
            <span
              class="text-[10px] flex items-center space-x-0.5"
              :class="
                inventoryStore.hasItem(npcDef.courtshipItemId)
                  ? 'text-success'
                  : 'text-muted/50'
              "
            >
              <CircleCheck
                v-if="inventoryStore.hasItem(npcDef.courtshipItemId)"
                :size="10"
              />
              <Circle v-else :size="10" />
              持有{{
                getItemById(npcDef.courtshipItemId)?.name ??
                npcDef.courtshipItemId
              }}
            </span>
          </div>
          <!-- 求缘物品制作 -->
          ...
          >
            <p class="text-[10px] text-muted/60 mb-1">
              制作{{ getItemById(npcDef.courtshipItemId)?.name }}：
            </p>
            <div class="flex flex-col space-y-0.5 mb-1">
              <span
            ...
            <Button
              class="w-full"
              :disabled="!canCraftCourtship"
              @click="handleCraft('courtship')"
              >制作</Button
            >
          </div>
          <Button
      ...
      <div v-if="npcDef.abilities.length > 0" class="mb-3">
        ...
        <div class="flex flex-col space-y-0.5">
          <div
            ...
            :class="
              state.unlockedAbilities.includes(ability.id)
                ? 'text-accent'
                : 'text-muted/40'
            "
          >
            ...
            <span
              v-if="state.unlockedAbilities.includes(ability.id)"
              class="text-success"
              >已激活</span
            >
            ...
            >
              <span class="flex items-center space-x-1">
                ...
                <span
                  ...
                  :class="
                    OFFERING_PREF_CLASS[
                      getOfferingPreference(npcId, item.itemId)
                    ]
                  "
                >
                  {{
...
const canCourt = computed(() => {
  ...
  const existingBond = hiddenNpcStore.hiddenNpcStates.find(
    (st) => st.bonded || st.courting,
  );
...
const canCraftCourtship = computed(() => {
  const d = npcDef.value;
  return d.courtshipCraftCost.every(
    (c) => inventoryStore.getItemCount(c.itemId) >= c.quantity,
  );
});

const canCraftBond = computed(() => {
  const d = npcDef.value;
  return d.bondCraftCost.every(
    (c) => inventoryStore.getItemCount(c.itemId) >= c.quantity,
  );
});
...
const handleInteraction = () => {
  ...
  if (success) {
    ...
    dialogueText.value =
      dialogues[Math.floor(Math.random() * dialogues.length)] ?? null;
