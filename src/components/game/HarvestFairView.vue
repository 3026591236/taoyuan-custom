    ...
    <div v-if="!submitted">
      ...
      <div class="mb-3">
        ...
        <div v-else class="flex flex-col space-y-1.5">
          ...
          >
            <span class="truncate" :class="qualityClass(sel.quality)">{{
              getItemById(sel.itemId)?.name
            }}</span>
            <span class="flex items-center space-x-2 shrink-0 ml-2">
              <span class="text-muted"
                >{{ getItemById(sel.itemId)?.sellPrice }}文</span
              >
      ...
      <div class="mb-3">
        ...
        >
          ...
          >
            <span class="truncate" :class="qualityClass(item.quality)">{{
              getItemById(item.itemId)?.name
            }}</span>
            <span class="flex items-center space-x-2 shrink-0 ml-2">
              <span class="text-muted">×{{ item.quantity }}</span>
              <span class="text-muted"
                >{{ getItemById(item.itemId)?.sellPrice }}文</span
              >
            </span>
          </button>
        </div>
      ...
      <div class="flex space-x-2">
        <Button
          ...
          >参展！</Button
        >
        <Button class="flex-1 opacity-60 hover:opacity-100" @click="handleQuit"
          >放弃参赛</Button
        >
      </div>
    </div>
    ...
    <div v-else>
      ...
      <div class="border border-accent/20 p-2 mb-3">
        ...
        <div
          ...
          class="flex items-center justify-between text-xs py-0.5 border-b border-accent/10 last:border-0"
        >
          <span :class="qualityClass(d.quality) || 'text-accent'">{{
            d.name
          }}</span>
          <span class="text-muted"
            >{{ d.basePrice }}文 × {{ d.multiplier }} = {{ d.score }}分</span
          >
        </div>
        <div
...
const handleSubmit = () => {
  ...
  const npcs: Participant[] = [
    { name: "沈听澜", score: Math.round(600 + Math.random() * 600) },
